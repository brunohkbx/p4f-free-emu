...
BOOL CQuery::Connect(const char * lpszDNS, const char * lpszUser, const char * lpszPassword)
{
	...
	if ( Result != SQL_SUCCESS && Result != SQL_SUCCESS_WITH_INFO )
	{
		...
		if ( SQLGetDiagRec(SQL_HANDLE_DBC, this->m_hConnection, sRecord,
			SqlState, &NativeError, SQLMsgError, sizeof(SQLMsgError), &MsgLen) != SQL_NO_DATA )
		{
			Log.LogAdd("SQLSTATE:%s, Diagnosis:%s", SqlState, SQLMsgError);
		}
...
BOOL CQuery::ExecQuery(const char* str)
{
	Log.LogAdd(str);
	Exec++;
	SQLRETURN Result = SQLExecDirect(this->m_hStmt, (SQLTCHAR *)str, SQL_NTS);
	...
	{
		Log.LogAdd("[CQuery] ColCount >= MAX_COLUMNS-1");
		return FALSE;
	}
...
void CQuery::GetAsString(LPTSTR ColName, LPTSTR pOutBuffer,int len)
{
	int iIndex = this->FindIndex(ColName);

	if ( iIndex != -1 )
	{
		strncpy(pOutBuffer, this->m_SQLData[iIndex],len);
	}
	else
	{
		pOutBuffer[0] = 0;
...
void CQuery::Diagnosis()
{
	...
	if ( SQLGetDiagRec(SQL_HANDLE_STMT, this->m_hStmt, sRecord,
		SqlState, &NativeError, SQLMsgError, sizeof(SQLMsgError), &MsgLen) != SQL_NO_DATA )
	{
		Log.LogAdd("SQLSTATE:%s, Diagnosis:%s", SqlState, SQLMsgError);
...
int CQuery::GetAsBinary(LPTSTR lpszStatement, LPBYTE OUT lpszReturnBuffer,int MaxLen)
{
	SQLCHAR * pSQLBuf;
	SQLINTEGER BufLen;
	...
	while ( true )
	{
		...
		while ( true )
		{
			...
			if (lOfs+BufLen > MaxLen)
			{
				int len = MaxLen-lOfs;
				memcpy(pSQLBuf, SQLBinary, len);
				pSQLBuf = &pSQLBuf[lOfs+len];
				break;
			}
