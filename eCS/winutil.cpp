...
LONG TopLevelFilter(struct _EXCEPTION_POINTERS * pExceptionInfo)
{
	...
	if ( hDll == NULL )
	{
		szResult        = "DBGHELP.DLL не найдена";
		// ----
	...
	// ----
	sprintf(szDumpPath, "%s\\JoinServer_Dump_%02d-%02d-%02d_%02d.%02d.%04d.dmp",
		lpCurDir,
	...
	{
		sprintf(szScratch,
			"Ошибка создания диагностического файла '%s' (код %d)",
	...
	if ( bOK != 0 )
	{
		sprintf(szScratch, "Файл сохранен в: '%s'", szDumpPath);
		// ----
		szResult        = szScratch;
		// ----
...
//---------------------------------------------------------------------------
/*
std::wstring MD5(std::string Str)
{
	//DWORD dwStatus = 0;
	BOOL bResult = FALSE;
	HCRYPTPROV hProv = 0;
...
{
	std::wstring dest = GetCommandLineW();
	dest.erase(0,1);
	//-----------
#ifdef X64
	size_t num = dest.find_last_of(L"\\");
#else
	int num = dest.find_last_of(L"\\");
#endif
	//-----------
	dest.erase(num+1,dest.length());
	return dest;
}
...
{
	std::string dest = GetCommandLineA();
	dest.erase(0,1);
#ifdef X64
	size_t num = dest.find_last_of("\\");
#else
	int num = dest.find_last_of("\\");
#endif
	dest.erase(num+1,dest.length());
	return dest;
}
