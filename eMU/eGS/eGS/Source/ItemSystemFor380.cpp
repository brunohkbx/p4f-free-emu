...
{
	if ( !this->Is380Item(pItem) && !isJOH380ForAll)
		return FALSE;
...
BOOL CItemSystemFor380::ChaosMix380ItemOption(LPOBJ lpObj)
{
	...
	{
		GCServerMsgStringSend("380 Items are unable to use the combination attribute system.", lpObj->m_Index, 1);
	...
	{
		if ( (this->Is380Item(&lpObj->pChaosBox[n]) == TRUE || isJOH380ForAll) &&
	...
	LogAdd("[380Item][Item Mix] - Mix Start");
	...
	if ( iRate < iRateSuccess )
	{
		...
		LogAdd("[380Item][ItemMix] Mix Success [%s][%s], Money(%d-%d) Rate(%d/%d) Option(%d,%d) OptionValue(%d,%d)",
	...
	else
	{
		...
		LogAdd("[380Item][ItemMix] Mix Fail [%s][%s], Money(%d-%d) Rate(%d/%d)",
			lpObj->AccountID, lpObj->Name, lpObj->Money,
			iMixPrice, iRate, iRateSuccess);
