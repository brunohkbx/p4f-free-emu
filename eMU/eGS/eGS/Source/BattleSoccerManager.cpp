...
void gSetBattleTeamMaster(int ground, int team, std::string szname, _GUILD_INFO_STRUCT * lpGuild)
{
...
void gBattleGetTeamPosition(int ground, int team, short & X, short & Y)
{
	...
	X = short(gBSGround[ground]->m_rect.left + Random(0,w));
	Y = short(gBSGround[ground]->m_rect.top + Random(0,h));
}


std::string GetBattleTeamName(int ground, int team)
{
	if ( BATTLE_SOCCER_GROUND_RANGE(ground)  == FALSE )
		return NULL;

	return gBSGround[ground]->GetTeamName(team).c_str();
}
