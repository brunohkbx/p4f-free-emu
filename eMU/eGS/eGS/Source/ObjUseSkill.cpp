...
//	GS-CS	1.00.18	JPN	0x004B7770 - Completed
...
int  CObjUseSkill::GetTargetLinePath(int sx, int sy, int tx, int ty, int * PathX, int * PathY, int distance)
{
	...
	dis = (int)sqrt((double)(delta_x * delta_x + delta_y * delta_y));
	dis = distance - dis;
...
BOOL CObjUseSkill::SpeedHackCheck(int aIndex)
{
		...
		if(lpObj->m_DetectCount > gHackCheckCount)
		{
			...
			if(gIsKickDetecHackCountLimit != 0 && lpObj->m_DetectedHackKickCount > gDetectedHackKickCount)
			{
				LogAdd("[%s][%s] Kick DetecHackCountLimit Over User (%d)",
					lpObj->AccountID,lpObj->Name,lpObj->m_DetectedHackKickCount);
				CloseClient(aIndex);
				return false;
			}

			lpObj->m_SpeedHackPenalty = gSpeedHackPenalty;
			LogAdd("[%s][%s] Attack Speed Is Wrong MagicSkill (%d)(%d) Penalty %d",
				lpObj->AccountID,lpObj->Name,lpObj->m_DetectSpeedHackTime,lpObj->m_SumLastAttackTime / lpObj->m_DetectCount,lpObj->m_SpeedHackPenalty);
		}
	}
	else
	{
	...
	if(bIsIgnorePacketSpeedHackDetect != 0 && lpObj->m_SpeedHackPenalty > 0)
	{
		...
		LogAdd("[%s][%s] Apply Attack Speed Penalty (%d left)",
			lpObj->AccountID,lpObj->Name,lpObj->m_SpeedHackPenalty);
		return false;
	}
	return true;
}
...
void CObjUseSkill::UseSkill(int aIndex, CMagicInf * lpMagic, BYTE x, BYTE y, BYTE dir, BYTE TargetPos, int aTargetIndex)
{
	...
	if(usemana >= 0)
	{
		...
		{
			LogAdd("[%] ½ºÅ³ µô·¹ÀÌ½Ã°£ ¿À¹ö",gObj[aIndex].Name);
			return;
		}
...
BOOL CObjUseSkill::RunningSkill(int aIndex, int aTargetIndex, CMagicInf * lpMagic, BOOL bCombo)
{
	...
	case AT_SKILL_CALLMON1:	return this->SkillMonsterCall(lpObj->m_Index,ElfSummon1,lpObj->X-1,lpObj->Y+1);
	case AT_SKILL_CALLMON2:	return this->SkillMonsterCall(lpObj->m_Index,ElfSummon2,lpObj->X-1,lpObj->Y+1);
	case AT_SKILL_CALLMON3:	return this->SkillMonsterCall(lpObj->m_Index,ElfSummon3,lpObj->X-1,lpObj->Y+1);
	case AT_SKILL_CALLMON4:	return this->SkillMonsterCall(lpObj->m_Index,ElfSummon4,lpObj->X-1,lpObj->Y+1);
	case AT_SKILL_CALLMON5:	return this->SkillMonsterCall(lpObj->m_Index,ElfSummon5,lpObj->X-1,lpObj->Y+1);
	case AT_SKILL_CALLMON6:	return this->SkillMonsterCall(lpObj->m_Index,ElfSummon6,lpObj->X-1,lpObj->Y+1);
	case AT_SKILL_CALLMON7:	return this->SkillMonsterCall(lpObj->m_Index,ElfSummon7,lpObj->X-1,lpObj->Y+1);
...
void CObjUseSkill::MaGumSkillDefenseDown(int aIndex, int aTargetIndex, int skill_level)
{
	...
	LogAdd("¸¶°Ë»ç ¸ó½ºÅÍ¹æ¾î·Â°¨¼Ò : %d",10); // !!! Need Translation
	...
	else
	{
		for(int n = 0; n < MAX_USER_IN_PARTY; n++)
		{
			if(ApplyPartyIndex[n]!= -1)
			{
				...
				GCReFillSend(lpPartyObj->m_Index,WORD(lpPartyObj->MaxLife + lpPartyObj->AddLife),0xFE,0,lpPartyObj->iMaxShield + lpPartyObj->iAddShield);
...
void CObjUseSkill::WizardMagicDefense(int aIndex, int aTargetIndex, int skill_level)
{
	...
	if (lpTargetObj->m_WizardSkillDefense > MaxMSPercent)
	{
		lpTargetObj->m_WizardSkillDefense=MaxMSPercent;
	}

	lpTargetObj->m_WizardSkillDefenseTime = 60 + (lpObj->Energy + lpObj->AddEnergy) / 40;
	if (lpTargetObj->m_WizardSkillDefenseTime > MaxMSDuration )
	{
		lpTargetObj->m_WizardSkillDefenseTime=MaxMSDuration;
	}
...
void CObjUseSkill::SkillDefense(int aIndex, int aTargetIndex, int skill_level)
{
	...
	double skill_defense = double(skill_level + 1)*2 + (lpObj->Energy + lpObj->AddEnergy) / 8;
...
void CObjUseSkill::SkillAttack(int aIndex, int aTargetIndex, int skill_level)
{
	...
	double skill_attack = double((skill_level + 1) * 3) + (lpObj->Energy + lpObj->AddEnergy) / 7.0f;
...
BOOL CObjUseSkill::SkillHellFire2(int aIndex, int aTargetIndex, CMagicInf * lpMagic)
{
	...
	{
		LogAdd("[%s][%s] ÇïÆÄÀÌ¾î2 ¹öÆ° ´­·¯ÁöÁöµµ ¾Ê¾Ò´Âµ¥ ¿Ï·á°¡ µÇ¾ú´Ù",lpObj->AccountID,lpObj->Name);
		return false;
	}
...
BOOL CObjUseSkill::SkillHellFire2Start(int aIndex, CMagicInf * lpMagic)
{
	...
	if(lpObj->SkillHellFire2State != 0)
	{
		LogAdd("[%s][%s] ÇïÆÄÀÌ¾î2 ¹öÆ° ´­·¯Á³´Âµ¥ ¶Ç ´­·¯Á³´Ù´Â°Å ¿Ô´Ù",lpObj->AccountID,lpObj->Name);
		return false;
	}
...
BOOL CObjUseSkill::SkillKnightBlow(int aIndex, int aTargetIndex, CMagicInf* lpMagic, BOOL isCombo)
{
	...
	double fangle = this->GetAngle(lpObj->X,lpObj->Y,gObj[aTargetIndex].X,gObj[aTargetIndex].Y);
...
void CObjUseSkill::SkillFrustrum(int aIndex, double fangle, double tx, double ty)
{
	LPOBJ lpObj = &gObj[aIndex];
...
void CObjUseSkill::SkillFrustrum2(int aIndex, double fangle, double tx, double ty)
{
	LPOBJ lpObj = &gObj[aIndex];
			...
			if(tObjNum >= 0)
			{
				...
				if(EnableAttack != 0)
				{
					if(gObjCalDistance(lpObj,&gObj[tObjNum]) < 5)
					{
						...
						if(HitCount >= 10)
						{
							if( Random(0,1) )//#error ìîæåò äîëæíî áûòü íå ==1 à == 0
							{
								bAttack = 1;
							}
						}
...
void CObjUseSkill::SkillRecallParty(int aIndex, int skill_level)
{
	...
	{
		MsgOutput(aIndex,"Skill is being used by a party member.");
		return;
	}
	...
	{
		char msg[255];
		wsprintf(msg,"You cannot use your siege battle skills in this location.");
	...
	{
		char msg[255];
		wsprintf(msg,"Please close the window or Personal Shop that's currently open.");
		GCServerMsgStringSend(msg,lpObj->m_Index,1);
		return;
	}
			...
			if(number >= 0)
			{
				...
				if(lpPartyObj->m_Index != lpObj->m_Index)
				{
						...
						&& bCheckInterfaceToMove != false)
					{
						...
						while(count--)
						{
							addx = lpObj->X + Random(0,8) - 4;
							addy = lpObj->Y + Random(0,8) - 4;
						...
						wsprintf(msg,"You have exceeded the amount of Zen you can possess.");
	...
	{
		char msg[128];
		wsprintf(msg,"%d players will be summoned.",recallcount);
		GCServerMsgStringSend(msg,lpObj->m_Index,1);
	}
}
...
void CObjUseSkill::SkillAddCriticalDamage(int aIndex, int skill_level)
{
	...
	if(partynum == -1)
	{
		...
		MsgOutput(aIndex,"Magic skill cast will remain for %d minute(s).",SkillTime);
		...
		if(number >= 0)
		{
			...
			if(lpObj->MapNumber == lpPartyObj->MapNumber && lpObj->m_Index != number)
			{
				...
				if(dis < 10)
				{
					...
					int declife = int(lpPartyObj->Life * 20.0 / 100.0);
					int decmana = int(lpPartyObj->Mana * 5.0 / 100.0);
...
BOOL CObjUseSkill::SkillSummon(int aIndex, int aTargetIndex, CMagicInf * lpMagic)
{
	...
	{
	case 161:
		iSummonMonterType = Random(0,1)+ 147;//#info ñóììîíû =)))
		bDoSummon = Random(0,14) ? false : true;
		break;
	case 181:
		iSummonMonterType = Random(0,1)+ 177;
		bDoSummon = Random(0,14) ? false : true;
		break;
	case 189:
		iSummonMonterType = Random(0,1)+ 185;
		bDoSummon = Random(0,14) ? false : true;
		break;
	case 197:
		iSummonMonterType = Random(0,1)+ 193;
		bDoSummon = Random(0,14) ? false : true;
		break;
	case 267:
		iSummonMonterType = Random(0,1)+ 263;
		bDoSummon = Random(0,14) ? false : true;
		break;
	case 275:
		iSummonMonterType = Random(0,1)+ 271;
		bDoSummon = Random(0,14) ? false : true;
		break;
	}
	...
	{
		BYTE cSX = (BYTE)lpObj->X;
		BYTE cSY = (BYTE)lpObj->Y;
...
BOOL CObjUseSkill::SkillCloaking(int aIndex, int aTargetIndex, CMagicInf * lpMagic)
{
	...
	if(lpTargetObj->Type != OBJ_USER)
	{
		LogAdd("[ANTI-HACK][Cloaking Skill] [%s][%s] Invalid Targeting  TargetType [%d] TargetClass [%d]",
...
BOOL CObjUseSkill::SkillInfinityArrow(int aIndex,int aTargetIndex,CMagicInf * lpMagic)
{
	...
	LogAdd("[%s][%s] Use Infinity Arrow Skill (Time:%d)(Character Level : %d)(ChangeUp: %d)",
...
BOOL CObjUseSkill::FireScreamExplosionAttack(LPOBJ lpObj, LPOBJ lpTargetObj, int iAttackDamage)
{
	...
	if( Random(0,10000) > g_SkillAdditionInfo.GetFireScreamExplosionRate())
	{
		return false;
	}
