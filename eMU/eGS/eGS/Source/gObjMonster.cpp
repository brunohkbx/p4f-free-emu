...
#include "..\Header\MonsterSetBase.h"
...
//	GS-CS	1.00.18	JPN	0x00XXXXXX	-	Completed
...
BOOL gObjMonsterRegen(LPOBJ lpObj)
{
	...
	if ( lpObj->Class == 43 )
	{
		lpObj->MapNumber = map[ Random(0,2) ];
	...
	else if ( gMSetBase.GetPosition(lpObj->m_PosNum, lpObj->MapNumber, lpObj->X, lpObj->Y) == FALSE )
	{
		...
		{
			BYTE btX = (BYTE)lpObj->X,btY = (BYTE)lpObj->Y;
...
int	 gObjMonsterLastHitDamageUser(LPOBJ lpMonObj, int & hitindex)
{
	...
	for ( int n=0;n<MAX_ST_HIT_DAMAGE;n++)
	{
		if ( lpMonObj->sHD[n].number >= 0 )
		{
			if ( (int)lpMonObj->sHD[n].LastHitTime > LastHitTime )
			{
...
BOOL gObjMonsterMoveCheck(LPOBJ lpObj, int tx, int ty)
{
	...
	if ( lpObj->MapNumber == MAP_INDEX_KANTURU_BOSS && lpObj->m_PosNum < 0)
	{
		...
		if ( dis > 30 )
		{
			LogAdd("[ KANTURU ][ Debug - m_PosNum ] Fail %s(Index:%d) X%d-Y%d -> X%d-Y%d(%d)",
...
void gObjMonsterStateProc(LPOBJ lpObj, int aMsgCode, int aIndex, int aMsgSubCode)
{
	...
	switch ( aMsgCode )
	{
		case 0:
			...
			{
				if ( Random(0,1) && lpObj->PathStartEnd == 0)
				{
		...
		case 1:
			...
			if ( KALIMA_MAP_RANGE(lpObj->MapNumber)  )
			{
				...
				{
					g_KalimaGate.CreateKalimaGate2(aIndex, lpObj->MapNumber, (BYTE)lpObj->X, (BYTE)lpObj->Y);
				}
			}
			...
			if (  BC_MAP_RANGE(lpObj->MapNumber) != FALSE && lpObj->Type >= OBJ_MONSTER)
			{
				...
				if ( g_BloodCastle.CheckMonsterKillCount(lpObj->MapNumber-MAP_INDEX_BLOODCASTLE1) != false )
				{
					if (g_BloodCastle.m_BridgeData[lpObj->MapNumber-MAP_INDEX_BLOODCASTLE1].m_bBC_MONSTER_KILL_COMPLETE == false )
					{
						...
						LogAdd("[Blood Castle] (%d) All of the Monster Terminated -> %d", lpObj->MapNumber-MAP_INDEX_BLOODCASTLE1+1,
							g_BloodCastle.m_BridgeData[lpObj->MapNumber-MAP_INDEX_BLOODCASTLE1].m_iBC_MONSTER_KILL_COUNT);
					...
					if (g_BloodCastle.m_BridgeData[lpObj->MapNumber-MAP_INDEX_BLOODCASTLE1].m_iBC_MONSTER_SUCCESS_MSG_COUNT < 1 )
					{
						...
						g_BloodCastle.SendNoticeMessage(lpObj->MapNumber-MAP_INDEX_BLOODCASTLE1, "Monsters eliminated!! Destroy the castle Gates!!"); 
					}
				...
				if ( g_BloodCastle.m_BridgeData[lpObj->MapNumber-MAP_INDEX_BLOODCASTLE1].m_bBC_MONSTER_KILL_COMPLETE != false )
				{
					if ( g_BloodCastle.CheckBossKillCount(lpObj->MapNumber-MAP_INDEX_BLOODCASTLE1) != false )
					{
						if ( g_BloodCastle.m_BridgeData[lpObj->MapNumber-MAP_INDEX_BLOODCASTLE1].m_bBC_BOSS_MONSTER_KILL_COMPLETE == false )
						{
							...
							LogAdd("[Blood Castle] (%d) All of the Boss Monster Terminated -> %d",
						...
						if ( g_BloodCastle.m_BridgeData[lpObj->MapNumber-MAP_INDEX_BLOODCASTLE1].m_iBC_BOSS_MONSTER_SUCCESS_MSG_COUNT < 1 )
						{
							...
							g_BloodCastle.SendNoticeMessage(lpObj->MapNumber-MAP_INDEX_BLOODCASTLE1, "Kundun's minions have been subdued! Destroy the Crystal Statue!");
						}
					}
				}
			}
...
void gObjMonsterProcess(LPOBJ lpObj)
{
	...
	if ( lpObj->m_Attribute == 100 )
	{
		if ( lpObj->m_RecallMon >= 0 && lpObj->m_RecallMon < OBJMAX )
		{
			...
			if ( Success == TRUE )
			{
				gObjTeleportMagicUse(lpObj->m_Index, (BYTE)lpCallMonObj->X+1, (BYTE)lpCallMonObj->Y);
	...
	if ( lpObj->m_ActState.Attack == 1 )
	{
		...
		else
		{
			...
			if ( lc6 != FALSE || lpObj->m_AttackType == 50)
			{
				if ( lpObj->TargetNumber >= 0 )
				{
					if ( gObj[lpObj->TargetNumber].Connected == PLAYER_PLAYING && gObj[lpObj->TargetNumber].CloseCount == -1 )
					{
...
void gObjMonsterAttack(LPOBJ lpObj, LPOBJ lpTargetObj)
{
		...
		lpObj->Class == 197 || lpObj->Class == 267 || lpObj->Class == 275  )
	{
		...
		pAttackMsg.MagicNumber = Random(1,6);
		pAttackMsg.NumberH = SET_NUMBERH(lpObj->TargetNumber);
		pAttackMsg.NumberL = SET_NUMBERL(lpObj->TargetNumber);
		pAttackMsg.Dis = 0;
		...
		lpObj->Class == 195 || lpObj->Class == 265 || lpObj->Class == 273  )
	{
		...
		pAttackMsg.MagicNumber = Random(1,2);
		pAttackMsg.NumberH = SET_NUMBERH(lpObj->TargetNumber);
		pAttackMsg.NumberL = SET_NUMBERL(lpObj->TargetNumber);
		pAttackMsg.Dis = 0;
...
void gObjMonsterMoveAction(LPOBJ lpObj)
{
	...
	int maxmoverange = lpObj->m_MoveRange*2;
	int searchc=10;
	lpObj->NextActionTime = 1000;
	BYTE tpx;
	BYTE tpy;
	...
	while ( searchc-- != 0 )
	{
		__try
		{
			tpx = (lpObj->X - lpObj->m_MoveRange) + (Random(0,maxmoverange));
			tpy = (lpObj->Y - lpObj->m_MoveRange) + (Random(0,maxmoverange));
		}
...
void gObjMonsterBaseAct(LPOBJ lpObj)
{
		...
		if ( lpObj->m_Attribute )
		{
			...
			else if ( lpObj->m_MoveRange > 0 && !lpObj->m_SkillHarden && !lpObj->m_iSkillStunTime )
			{
				...
				else
				{
					...
					if ( lpObj->m_RecallMon >= 0 )
					{
						if ( gObj[lpObj->m_RecallMon].Connected == PLAYER_PLAYING )
						{
	...
	else if ( lpObj->m_ActState.Emotion == 1 )
	{
		...
		if ( lpObj->TargetNumber >= 0 && lpObj->PathStartEnd == 0)
		{
			...
			{
				int iRAND_CHANGE_TARGET = Random(0,9);
				...
				if ( iRAND_CHANGE_TARGET == 1 )
				{
					...
					{
						if ( gObj[lpObj->LastAttackerID].Connected == PLAYER_PLAYING &&
...
void gObjRefillMonsterHP(LPOBJ lpMonsterObj, int iRefillHPSec)
{
	...
	LPOBJ lpObj;
		...
		{
			iRefillHP = Random(0,((giKundunRefillHP > 0)? giKundunRefillHP : 1)) ;
		...
		{
			LogAdd("[KUNDUN][Status Log] HP:%d RefillHP:%d Map:%d X:%d Y:%d",
			...
			if(gObjIsConnected(lpMonsterObj->LastAttackerID))
			{
				...
				LogAdd("[KUNDUN][Attacker Info] [%s][%s] HP:%d MP:%d X:%d Y:%d",
						  ...
						  BOOL bMustHaveAdditionalOption)
{
	...
	{
		BYTE cDropX = (BYTE)lpObj->X;
		BYTE cDropY = (BYTE)lpObj->Y;
		...
		if ( Random(0,10000) < iExtDropPer )//#info
		{
		...
		else if ( Random(0,10000) < iCommonDropPer )//#info
		{
...
void gObjMonsterDieGiveItem(LPOBJ lpObj, LPOBJ lpTargetObj)
{
	...
	{
		if ( g_bCrywolfMonsterDarkElfItemDrop )//#info
		{
			int ItemDropRate = Random(0,10000);

			if ( ItemDropRate <= g_iCrywolfMonsterDarkElfItemDropRate )
			{
				int iMaxHitUser = gObjMonsterTopHitDamageUser(lpObj);

				LogAdd("[ Crywolf ][Reward] Monster Dark Elf ItemDrop MaxHitUser [%s][%s]",
	...
	if ( lpObj->Class == 349 )	// Crywolf Boss Monster
	{
		if ( g_bCrywolfBossMonsterItemDrop )
		{
			...
			if ( ItemDropRate <= g_iCrywolfBossMonsterItemDropRate )
			{
				...
				LogAdd("[ Crywolf ][Reward] Boss Monster ItemDrop MaxHitUser [%s][%s]",
					lpTargetObj->AccountID, lpTargetObj->Name);
	...
	if ( lpObj->Class == 295 )
	{
		...
		for ( int i=0; i<iMaxCount ; i++ )
		{
			...
			LogAdd("[Castle HuntZone] Boss Monster ItemDrop MaxHitUser [%d][%s][%s]",
	...
	if ( lpObj->Class == 362 || lpObj->Class == 363 )	// Maya Hand
	{
		if ( g_bKanturuMayaHandItemDrop )
		{
			...
			if ( ItemDropRate <= g_iKanturuMayaHandItemDropRate )
			{
				...
				LogAdd("[ KANTURU ][ Reward ] MayaHand ItemDrop MaxHitUser [%s][%s]",
					lpTargetObj->AccountID, lpTargetObj->Name);
	...
	if ( lpObj->Class == 361 )	// NightMare
	{
		if ( g_bKanturuNightmareItemDrop )
		{
			...
			if ( ItemDropRate <= g_iKanturuNightmareItemDropRate )
			{
				...
				LogAdd("[ KANTURU ][ Reward ] Nightmare ItemDrop MaxHitUser [%s][%s]",
					lpTargetObj->AccountID, lpTargetObj->Name);
	...
	{
		LogAdd("[★★Kundun EVENT] Kundun die, Killer [%s][%s]",
			lpTargetObj->AccountID, lpTargetObj->Name);
		...
		for ( int i=0;i<iMaxNumOfRewardItem;i++)
		{
			...
			if ( OBJMAX_RANGE(MaxHitUser) )
			{
				...
				LogAdd("[★Kundun EVENT] In KALIMA(7), ItemDrop MaxHitUser [%d][%s][%s]",
			...
			if ( OBJMAX_RANGE(MaxHitUser ))
			{
				LogAdd("[○Kundun EVENT] Drop Item [%d][%s][%s]",
					i, gObj[MaxHitUser].AccountID, gObj[MaxHitUser].Name);
	...
	for (int i=0; i < (int)IDR_MobMinLvl.size(); i ++ )
	{
		if (lpObj->Level >= IDR_MobMinLvl[i] )
		{
			if ( IDR_MobMaxLvl[i] <= lpObj->Level || IDR_MobMaxLvl[i]==-1)
			{
				if (IDR_DropMap[i] == lpObj->MapNumber || IDR_DropMap[i]==-1)
				{
					if (Random(0,99999) < IDR_DropRate[i])
					{
						...
						x = lpObj->X;
						y = lpObj->Y;
						type = ItemGetNumberMake(IDR_Type[i], IDR_Index[i]);
						ItemSerialCreateSend(lpObj->m_Index, lpObj->MapNumber, x, y, type, IDR_Level[i], IDR_Dur[i],
							IDR_Skill[i], IDR_Luck[i], IDR_Opt[i], MaxHitUser, IDR_Exc[i], IDR_Anc[i]);
	...
	ItemDropPer = int(ItemDropPer * (lpTargetObj->m_wItemDropRate / 100.0));
	...
	if ( ExtDropPer == 1 )
	{
		...
		else
		{
			...
			if ( foundChangeupitem )
				ExtDropPer = Random(0,99);

			if ( ExtDropPer )
				if ( Random(0,itemrate-1) < ItemDropPer )
					item_drop=TRUE;
		}
	}
	else
	...
	if ( item_drop )
	{
		...
		if ( g_CrywolfSync.GetOccupationState() == 1 && g_iCrywolfApplyMvpPenalty )
		{
			...
			{
				if ( Random(0,99) > g_CrywolfSync.GetGemDropPenaltiyRate() )
				{
					item_drop = FALSE;
				}
	...
	if ( item_drop)
	{
		...
		if ( ExtDropPer == TRUE )
		{
			option1rand=g_ExcItemDropSkillRate;
			option2rand=g_ExcItemDropLuckRate;
			option3rand=Random(0,99);
			optionc=Random(0,2);
			NOption = NewOptionRand(lpObj->Level);
			level = 0;
		...
		{
			option1rand=g_ItemDropSkillRate;
			option2rand=g_ItemDropLuckRate;
			option3rand=Random(0,99);
			optionc=Random(0,2);
			NOption = 0;
		}
	...
	else if ( Random(0,moneyrate-1) < 10 )//#info
	{
		int x = lpObj->X;
		int y = lpObj->Y;
	...
	{
		if ( Random(0,gEvent1ItemDropTodayPercent-1) == 0 )//#info
		{
			if ( gEvent1ItemDropTodayCount < gEvent1ItemDropTodayMax )
			{
				...
				ItemSerialCreateSend(lpObj->m_Index, lpObj->MapNumber, x, y,
					type, level, (BYTE)dur, 0, 0, 0, -1, 0, 0);
			}
		}
	}
...
BOOL gEventMonsterItemDrop(LPOBJ lpObj, LPOBJ lpTargetObj)
{
	...
	if ( g_bFenrirStuffItemDrop )
	{
		...
		if ( lpObj->Level >= g_iFenrirStuff_01_DropLv_Min && lpObj->Level <= g_iFenrirStuff_01_DropLv_Max &&
			...
			bDropStuff == FALSE )
		{
			if ( Random(0,10000) < g_iFenrirStuff_01_DropRate ) //#info
			{
				bDropStuff = TRUE;
		...
		if ( lpObj->Level >= g_iFenrirStuff_02_DropLv_Min && lpObj->Level <= g_iFenrirStuff_02_DropLv_Max &&
			...
			bDropStuff == FALSE )
		{
			if ( Random(0,10000) < g_iFenrirStuff_02_DropRate ) 
			{
				bDropStuff = TRUE;
		...
		if ( lpObj->Level >= g_iFenrirStuff_03_DropLv_Min && lpObj->Level <= g_iFenrirStuff_03_DropLv_Max &&
			...
			bDropStuff == FALSE )
		{
			if ( Random(0,10000) < g_iFenrirStuff_03_DropRate ) 
			{
				bDropStuff = TRUE;
	...
	if ( g_CompoundPotionDropOn == TRUE )
	{
		...
		if ( lpObj->Level >= g_iCompoundPotionLv1DropLevel && Random(0,10000) <= g_iCompoundPotionLv1DropRate &&
			bDropStuff == FALSE )
		{
			bDropStuff = TRUE;
		...
		if ( bDropStuff == FALSE && lpObj->Level >= g_iCompoundPotionLv2DropLevel && Random(0,10000) <= g_iCompoundPotionLv2DropRate  )
		{
			bDropStuff = TRUE;
		...
		if ( !bDropStuff )
		{
			if ( lpObj->Level >= g_iCompoundPotionLv3DropLevel && Random(0,10000) <= g_iCompoundPotionLv3DropRate )
			{
				bDropStuff = TRUE;
		...
		if ( bDropStuff == 1 )
		{
			...
			ItemSerialCreateSend(lpObj->m_Index, lpObj->MapNumber, (BYTE)lpObj->X, (BYTE)lpObj->Y, iType, level, dur,
	...
	{
		if ( Random(0,10000) < gItemDropRingOfTransform )//#info
		{
		...
		if ( gLanguage == 0 )	// If Korea
		{
			if ( lpObj->MapNumber < 7 )
			{
				...
				if ( lpattr )
				{
					...
					{
						if ( Random(0,10000) < gFireCrackerDropRate )//#info
						{
							dur = 255.0f;
							x = lpObj->X;
							y = lpObj->Y;
	...
	if ( gHeartOfLoveEvent )
	{
		if ( gLanguage == 2 )
		{
			...
			if ( lpattr )
			{
				...
				{
					if ( Random(0,10000) < gHeartOfLoveDropRate )
					{
						dur = 255.0f;
						x = lpObj->X;
						y = lpObj->Y;
	...
	if ( gMedalEvent )
	{
		...
		{
			if ( Random(0,10000) < gSilverMedalDropRate )
			{
				dur = 255.0f;
				x = lpObj->X;
				y = lpObj->Y;
		...
		{
			if ( Random(0,10000) < gGoldMedalDropRate )
			{
				dur = 255.0f;
				x = lpObj->X;
				y = lpObj->Y;
	...
	if ( gEventChipEvent )
	{
		if ( Random(0,10000) < gBoxOfGoldDropRate )
		{
			dur = 255.0f;
			x = lpObj->X;
			y = lpObj->Y;
	...
	if ( Random(0,10000) < g_iKundunMarkDropRate )
	{
		Option1=0;
		Option2=0;
		Option3=0;
	...
	if ( Random(0,10000) < g_iMarkOfTheLord )
	{
		Option1=0;
		Option2=0;
		Option3=0;
	...
	if ( Random(0,10000) < g_iJapan1StAnivItemDropRate )
	{
		if ( lpTargetObj->PartyNumber >= 0 )
		{
			...
			x = lpObj->X;
			y = lpObj->Y;
			type = ItemGetNumberMake(14,20);	// Branch of Remedy of Love
	...
	if ( gIsDropDarkLordItem )
	{
		...
		{
			if ( Random(0,10000) < gSleeveOfLordDropRate)
			{
		...
		{
			if ( Random(0,10000) < gSoulOfDarkHorseDropRate)
			{
		...
		{
			if ( Random(0,10000) < gSoulOfDarkSpiritDropRate )
			{
				type = ItemGetNumberMake(13,31);
				level = 1;
				x = lpObj->X;
	...
	if ( gIsDropGemOfDefend )
	{
		...
		else
		{
			...
			{
				if ( Random(0,10000) < gGemOfDefendDropRate )
				{
	...
	if ( gIsDropSetItemInCastleHuntZone )
	{
		...
		else
		{
			...
			{
				if ( Random(0,10000) < gSetItemInCastleHuntZoneDropRate )
				{
					int MaxHitUser = gObjMonsterTopHitDamageUser(lpObj);

					if ( CHECK_LIMIT(MaxHitUser, OBJMAX) )
					{
						LogAdd("[Castle HuntZone] Drop SetItem [%s][%s] ",
							gObj[MaxHitUser].AccountID, gObj[MaxHitUser].Name);
			
						MakeRewardSetItem(MaxHitUser, (BYTE)lpObj->X, (BYTE)lpObj->Y, 0, lpObj->MapNumber);
						return TRUE;
					}
				}
			}
	...
	if ( g_bKanturuSpecialItemDropOn )
	{
		...
		{
			if ( Random(0,10000) < g_iKanturuMoonStoneDropRate )
			{
		...
		{
			if ( Random(0,10000) < g_iKanturuJewelOfHarmonyDropRate )
			{
	...
	if ( lpObj->MapNumber == MAP_INDEX_AIDA || lpObj->MapNumber == MAP_INDEX_CRYWOLF_FIRSTZONE)
	{
		...
		switch ( lpObj->Class )
		{
			...
			case 307:	case 308:	case 309:
				if ( Random(0,10000) < g_iMysteriousBeadDropRate1 )
					bDropMysteriousBead = true;
				break;

			case 310:	case 311:	case 312:	case 313:
				if ( Random(0,10000) < g_iMysteriousBeadDropRate2 )
					bDropMysteriousBead = true;
				break;
		}
	...
	if ( g_bBloodCastle )
	{
		if ( !BC_MAP_RANGE(lpObj->MapNumber ) )
		{
			...
			if ( Random(0,10000) < g_iAngelKingsPaperDropRate )
			{
				dur = 128.0f;
				x = lpObj->X;
				y = lpObj->Y;
			...
			if ( Random(0,10000) < g_iBloodBoneDropRate )
			{
				dur = 128.0f;
				x = lpObj->X;
				y = lpObj->Y;
	...
	if ( Random(0,10000) < g_iDarkLordHeartDropRate )
	{
		int iDropRate = 0;
	...
	if ( g_bRibbonBoxEvent )
	{
		...
		{
			if ( Random(0,10000) < g_iRedRibbonBoxDropRate )
			{
				...
				ItemSerialCreateSend(lpTargetObj->m_Index, lpTargetObj->MapNumber, x, y, type, level, (BYTE)dur,
					Option1,Option2,Option3, MaxHitUser, 0, 0);
		...
		{
			if ( Random(0,10000) < g_iGreenRibbonBoxDropRate )
			{
		...
		{
			if ( Random(0,10000) < g_iBlueRibbonBoxDropRate )
			{
	...
	if ( g_bChocolateBoxEvent )
	{
		...
		{
			if ( Random(0,10000) < g_iPinkChocolateBoxDropRate )
			{
		...
		{
			if ( Random(0,10000) < g_iRedChocolateBoxDropRate )
			{
		...
		{
			if ( Random(0,10000) < g_iBlueChocolateBoxDropRate )
			{
	...
	if ( g_bCandyBoxEvent )
	{
		...
		{
			if ( Random(0,10000) < g_iLightPurpleCandyBoxDropRate )
			{
				type = ItemGetNumberMake(14,32);
				level = 1;
				x = lpTargetObj->X;
		...
		{
			if ( Random(0,10000) < g_iVermilionCandyBoxDropRate )
			{
				type = ItemGetNumberMake(14,33);
				level = 1;
				x = lpTargetObj->X;
		...
		{
			if ( Random(0,10000) < g_iDeepBlueCandyBoxDropRate )
			{
				type = ItemGetNumberMake(14,34);
				level = 1;
				x = lpTargetObj->X;
	...
	if ( g_bHallowinDayEventOn )
	{
		...
		if ( !bIsBossMonster )
		{
			if ( Random(0,10000) < g_iHallowinDayEventItemDropRate  )
			{
				...
				ItemSerialCreateSend(lpTargetObj->m_Index, lpObj->MapNumber, x, y, type, level, (BYTE)dur,
    ...
    if ( g_bSantaPolymorphRingDropOn )
    {
            ...
            if ( !bIsBossMonster )
            {
                    if ( Random(0,10000) < g_iSantaPolymorphRingDropRate  )
                    {
    ...
    {
            if ( Random(0,10000) < g_iCondorFlameDropRate )
            {
...
void CQeustNpcTeleport::Run(int aIndex)
{
	...
	if ( this->TimeCount > gQuestNPCTeleportTime )
	{
		...
		while ( true )
		{
			tableindex = Random(0,MAX_QUEST_TELEPORT-1);
