...
#include "..\Header\DevilSquareGround.h"
...
BOOL CChaosCastle::LoadFinalGift(char* filename )
{
	int Token;
	...
	{
		MsgBox("Chaos Castle data load error %s", filename);
		return FALSE;
	}
...
void CChaosCastle::CheckSync(int iChaosCastleIndex)
{
	...
	{
		LogAdd( "[Chaos Castle] (%d) No Schedule Time Data - Chaos Castle Terminated (m_vtChaosCastleOpenTime.empty())", iChaosCastleIndex);
	...
	switch ( bTIME_CHANGED )
	{
		...
		default:
			LogAdd( "[Chaos Castle] (%d) No Schedule Time Data - Chaos Castle Terminated (bTIME_CHANGED=%d)",
	...
	LogAdd("[Chaos Castle] (%d) Sync Start Time. [%d] min remain (START HOUR:%d, MIN:%d)",
		...
		if ( g_bChaosCastle != FALSE )
		{
			...
			if ( this->m_stChaosCastleData[iChaosCastleIndex].m_iCC_REMAIN_MSEC <= ( this->m_iCC_TIME_MIN_OPEN * 60 * 1000 ) && this->m_stChaosCastleData[iChaosCastleIndex].m_iCC_REMAIN_MSEC > 0 && (this->m_stChaosCastleData[iChaosCastleIndex].m_iCC_REMAIN_MSEC/60000) !=this->m_stChaosCastleData[iChaosCastleIndex].m_iCC_NOTIFY_COUNT)
			{
				...
				if ( iChaosCastleIndex == 0 )
				{
					...
					TNotice::MakeNoticeMsgEx(&pNotice, 0, "%d minute(s) left before Chaos Castle opens.", this->m_stChaosCastleData[iChaosCastleIndex].m_iCC_NOTIFY_COUNT+1);
...
void CChaosCastle::ProcState_Playing(int iChaosCastleIndex)
{
	...
	if ( iTICK_MSEC >= 1000 )
	{
			...
			this->m_stChaosCastleData[iChaosCastleIndex].m_bCC_PLAY_START == false )
		{
			...
			TNotice::MakeNoticeMsgEx((TNotice*)&pNotice, 0, "Chaos Castle %d has already commenced!", iChaosCastleIndex+1);
			...
			LogAdd("[Chaos Castle] (%d) Chaos Castle Quest Start (UserCount:%d)",
			 ...
			 this->m_stChaosCastleData[iChaosCastleIndex].m_bCC_CAN_ENTER == false )
		{
			...
			{
				this->SetState(iChaosCastleIndex, CC_STATE_CLOSED);
				LogAdd("[Chaos Castle] (%d) Chaos Castle Quest Closed - No User",
					iChaosCastleIndex+1);
		...
		if ( this->m_stChaosCastleData[iChaosCastleIndex].m_bCC_PLAY_START == true )
		{
			...
			if ( bPlayEnded == TRUE )
			{
				...
				if ( iMonsterCount <= 0 )
				{
					...
					if ( iWinnerIndex != -1 )
					{
						LogAdd("[Chaos Castle] (%d) [%s][%s] is Winner. [CharInfoSave : Class=%d, Level=%d, LvPoint=%d, Exp=%d, Str=%d, Dex=%d, Vit=%d, Energy=%d, LeaderShip=%d, Map=%d, Pk=%d]",
					...
					{
						LogAdd("[Chaos Castle] (%d) Has No Winner.", iChaosCastleIndex+1);
					}
				}
				else
				{
					LogAdd("[Chaos Castle] (%d) Has No Winner : Monster Left (%d)",
	...
	if ( this->m_stChaosCastleData[iChaosCastleIndex].m_iCC_REMAIN_MSEC <= 0 )
	{
		LogAdd("[Chaos Castle] (%d) is Over : TIME-OUT (Left User:%d, Monster:%d)",
...
void CChaosCastle::SetState_Closed(int iChaosCastleIndex)
{
	...
	LogAdd("[Chaos Castle] (%d) SetState CLOSED", iChaosCastleIndex+1);
}
...
void CChaosCastle::SetState_Playing(int iChaosCastleIndex)
{
	...
	if ( this->CheckCanStartPlay(iChaosCastleIndex) == false )
	{
		...
		LogAdd("[Chaos Castle] (%d) Failed to Start Chaos Castle : Lack of User", iChaosCastleIndex +1 );
		this->SetState_Closed(iChaosCastleIndex);
	}
	else
	{
		...
		LogAdd("[Chaos Castle] (%d) SetState PLAYING", iChaosCastleIndex + 1 );
	}
}
...
void CChaosCastle::SetState_PlayEnd(int iChaosCastleIndex)
{
	...
	LogAdd("[Chaos Castle] (%d) SetState PLAYEND", iChaosCastleIndex+1);
...
void CChaosCastle::ClearMonster(int iChaosCastleIndex)
{
	...
	int CCIndex=iChaosCastleIndex+MAP_INDEX_CHAOSCASTLE1;
...
int  CChaosCastle::SetMonster(int iChaosCastleIndex)
{
	...
	for ( int n=0;n<gMSetBase.m_Count;n++)
	{
		...
		{
			BYTE btMonsterIndex = (BYTE)gMSetBase.m_Mp[n].m_Type;
...
void CChaosCastle::DelMonsterList(int iChaosCastleIndex, int iMonsterIndex)
{
	...
	LogAdd("[Chaos Castle][Bug Tracer] MONSTER DEL (%d) Count(%d) : ENTER - Index(%d)",
	...
	LogAdd("[Chaos Castle][Bug Tracer] MONSTER DEL (%d) Count(%d) : OUT - Index(%d)",
		iChaosCastleIndex+1, this->m_stChaosCastleData[iChaosCastleIndex].m_lCC_CURRENT_MONSTER_COUNT, iMonsterIndex);
}
...
int  CChaosCastle::GetMonsterListCount(int iChaosCastleIndex)
{
	...
	#if (MP==1)
	#pragma omp parallel for reduction(+:iAliveMonsterCount)
	#endif
...
void CChaosCastle::SetItemsToMonster(int iChaosCastleIndex)
{
	...
	for ( int iA = 0;iA < MAX_CC_MONSTER_DROP_ITEM ; iA++)
	{
		for ( int iB=0; iB< ::g_iChaosCastle_MonsterItems[iChaosCastleIndex][iA][1] ; iB++)
		{
			...
			{
				int iMONSTER_INDEX = this->m_stChaosCastleData[iChaosCastleIndex].m_lCC_MONSTER_COUNT[Random(0,iMAX_MONSTER-1)];
...
void CChaosCastle::SearchNDropMonsterItem(int iChaosCastleIndex, int iMonsterIndex, int iMaxHitUserIndex)
{
	...
	::ItemSerialCreateSend(gObj[iMaxHitUserIndex].m_Index, gObj[iMaxHitUserIndex].MapNumber, (BYTE)gObj[iMaxHitUserIndex].X,
		(BYTE)gObj[iMaxHitUserIndex].Y, iType, iLevel, 0, 0, 0, 0, iMaxHitUserIndex, 0, 0);

	if ( OBJMAX_RANGE(iMaxHitUserIndex) != FALSE )
	{
		LogAdd("[Chaos Castle] (%d) Monster Dropped ChaosCastle Item To [%s][%s] (Item:%s)",
...
void CChaosCastle::SendWinMessage(int iChaosCastleIndex, int iWinnerIndex)
{
	...
	::GCServerMsgStringSend("The Spirits of the Guardsmen have been exorcised. Quest complete!", iWinnerIndex, 1);
		...
		if ( this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex != -1 )
		{
			if ( this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex != iWinnerIndex )
			{
				if ( this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_bSendQuitMsg == false )
				{
					...
					::GCServerMsgStringSend("You have failed to complete the Quest. Summon more courage and try again.", this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex, 1);
...
int  CChaosCastle::LeaveUserChaosCastle(int iChaosCastleIndex, int iUserIndex)
{
	...
	for ( int i=0;i<MAX_CHAOSCASTLE_USER;i++)
	{
		if ( this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex == iUserIndex )
		{
			iRET_VAL = i;	// #error Instead of iUserIndex put i
...
void CChaosCastle::ProcessFallUser(int iChaosCastleIndex)
{
	...
	int CCIndex=iChaosCastleIndex+18;
	...
	for ( int iFALL=0;iFALL<MAX_CHAOSCASTLE_USER;iFALL++)
	{
		if ( this->m_stChaosCastleData[iChaosCastleIndex].m_lFallUser[iFALL] != -1 )
		{
			...
			if ( (gObj[iFALL_INDEX].MapNumber) == CCIndex &&
...
BOOL CChaosCastle::LevelUp(int iUserIndex, int iAddExp, int iEventType)
{
	...
	{
		::GCServerMsgStringSend("You have reached a maximum level", gObj[iUserIndex].m_Index, 1);
		return 0;
	}
	...
	else
	{
		...
		{
			gObj[iUserIndex].LevelUpPoint += g_LvlUpPointDLMG;
		}
		else
		{
			gObj[iUserIndex].LevelUpPoint += g_LvlUpPointNormal;
		}
		...
		LogAdd("Level Up [%s][%s] (%d)", gObj[iUserIndex].AccountID, gObj[iUserIndex].Name, gObj[iUserIndex].Level);
	}
...
bool CChaosCastle::CheckCanStartPlay(int iChaosCastleIndex)
{
	...
	{
		LogAdd("[Chaos Castle] (%d) GetCurPlayUser() FAILED (UserCount:%d)", iChaosCastleIndex+1, iEnteredUser);
...
BOOL CChaosCastle::ObjSetPosition(int iIndex, int iX, int iY)
{
	...
	PHeadSetB((LPBYTE)&pMove2, P_DAMAGE, sizeof(pMove2));
...
void CChaosCastle::SearchNBlowObjs(int iMapNumber, int iX, int iY)
{
	...
	for ( int i=0;i<MAX_CHAOSCASTLE_USER;i++)
	{
		...
		if ( gObj[iIndex].MapNumber == iMapNumber && gObj[iIndex].Connected == PLAYER_PLAYING )
		{
				 ...
				 gObj[iIndex].Y >= iMIN_Y && gObj[iIndex].Y <= iMAX_Y )
			{
				...
				if ( btMapAttr == 8 )
				{
					...
					LogAdd("[Chaos Castle] (%d) [%s][%s] User Dead In Chaos Castle : Fall from Castle (X:%d, Y:%d)",
...
BOOL CChaosCastle::BlowObjsFromPoint(int iIndex, int iMapNumber, int& iX, int& iY)
{
	...
	for ( int iBLOWOUT_COUNT = 0;iBLOWOUT_COUNT < 5 ;iBLOWOUT_COUNT++)
	{
		...
		int iBLOW_X = Random(iBLOW_MIN,iBLOW_MAX); //#warning было iBLOW_MIN+1
		int iBLOW_Y = Random(iBLOW_MIN,iBLOW_MAX); //#warning было iBLOW_MIN+1
		int iRND = Random(0,1);
		...
		if ( iRND )
		{
			...
			{
				iBLOW_X = iBLOW_MAX;
				iBLOW_Y = iBLOW_MIN + Random(0,1)-1;//#warning искренне не понимаю... БЛОУ может уменьшится на 1...
		...
		{
			iBLOW_Y = iBLOW_MAX;
			iBLOW_X = iBLOW_MIN + Random(0,1)-1;
...
int  CChaosCastle::CalDistance(int iX1, int iY1, int iX2, int iY2)
{
	...
	double fTX = (double)(iX1 - iX2);
	double fTY = (double)(iY1 - iY2);
...
void CChaosCastle::CheckUserInDieTile(int iChaosCastleIndex)
{
	...
	for ( int i=0;i<MAX_CHAOSCASTLE_USER;i++)
	{
		...
		{
			if ( gObjIsConnected(this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex) ==TRUE &&
				 gObj[this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex].MapNumber == CCIndex )
			{
				if ( gObj[this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex].Life > 0 )
				{
					int iSX = gObj[this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex].X;
					int iSY = gObj[this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex].Y;
					BYTE btMapAttr = MapC[CCIndex].m_attrbuf[iSY * 256 + iSX] & 0x08;
	...
	for ( int i=0;i<MAX_CHAOSCASTLE_USER;i++)
	{
		if ( this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex != -1 )
		{
				 ...
				 gObj[this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex].MapNumber == (iChaosCastleIndex + MAP_INDEX_CHAOSCASTLE1) )
			{
				...
				TNotice::MakeNoticeMsgEx(&Notice, 1, "Not enough warriors to start the Chaos Castle Quest.");

				DataSend(this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex, (LPBYTE)&Notice, Notice.h.size);

				LogAdd("[Chaos Castle] (%d) [%s][%s] Payback to User ChaosCastle Money (FIRST:%d, PAYBACK:%d, FINAL:%d)",
					iChaosCastleIndex+1, gObj[this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex].AccountID,
					gObj[this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex].Name,
...
void CChaosCastle::RewardUserEXP(int iChaosCastleIndex, int iChaosCastleSubIndex, BOOL bWinner)
{
	...
	if ( this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[iChaosCastleSubIndex].m_iIndex != -1 )
	{
		if ( gObjIsConnected(this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[iChaosCastleSubIndex].m_iIndex) ==TRUE )
		{
			...
			LogAdd("[Chaos Castle] (%d) [%s][%s] Reward User EXP (USER_KILL:%d, MON_KILL:%d, TOT_EXP:%d, TOT_REWARD_EXP:%d)",
	...
	for (int i=0; i < (int)GType.size(); i++)
	{
		if (GLevel[i]==iChaosCastleIndex)
		{
			...
			if (tmp<DropRate)
			{
				...
				ItemSerialCreateSend(gObj[iWinnerIndex].m_Index, gObj[iWinnerIndex].MapNumber, (BYTE)gObj[iWinnerIndex].X,
					(BYTE)gObj[iWinnerIndex].Y, iType, ItemLevel, Dur, Opt1, Opt2, Opt3, iWinnerIndex, Excellent, Ancient);
			}
		}
	}
...
int CChaosCastle::RewardZettoMoney(int iChaosCastleIndex)
{
	...
	if ( iZettoMoney <= 0 )
	{
		LogAdd("[Chaos Castle] (%d) ERROR : REWARD ZEN <= 0 (%d)",
	...
	int iZettoWinnerIndex = vtZettoWinner[Random(0,iPlayUser-1)];
	...
	if ( gObj[iZettoWinnerIndex].Money > (int)MAX_ZEN )
		gObj[iZettoWinnerIndex].Money = MAX_ZEN;

	GCMoneySend(iZettoWinnerIndex, gObj[iZettoWinnerIndex].Money);

	PMSG_NOTICE pNotice;
	TNotice::MakeNoticeMsgEx(&pNotice, 0, "Chaos Castle %d: %s has been granted entry for a fee of %d Zen.", iChaosCastleIndex+1, szZettoWinnerName, iZettoMoney);
	...
	for(int i=0;i<MAX_CHAOSCASTLE_USER;i++)
	{
		if ( this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex != -1 )
		{
			if ( gObj[this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex].Connected == PLAYER_PLAYING )
			{
				...
				LogAdd("[Chaos Castle][Bug Tracer] (%d) [%s][%s] Reword ZettoMoney(Map:%d) Send Message",
					iChaosCastleIndex+1, gObj[this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex].AccountID,
					gObj[this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex].Name,
	...
	{
		LogAdd("[Chaos Castle] (%d) [%s][%s] Win Zen Lotto (RewardZen:%d, UserZen:%d)",
