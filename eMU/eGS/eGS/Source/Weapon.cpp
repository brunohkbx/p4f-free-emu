...
BOOL CWeapon::GetTargetPointXY(int iObjClass, int iTargetPointIndex, BYTE &btX, BYTE &btY, BOOL bRandomPos)
{
	if ( iObjClass == 221 )
	{
		if ( iTargetPointIndex >= 0 && iTargetPointIndex < MAX_ST_CS_WEAPON )
		{
			...
			if ( bRandomPos != FALSE )
			{
				btX = WEAPON_CS_ATTACKER[iTargetPointIndex].btXBegin + Random(0,tx-1);
				btY = WEAPON_CS_ATTACKER[iTargetPointIndex].btYBegin + Random(0,ty-1);
			}
			else
			{
	...
	if ( iObjClass == 222 )
	{
		if ( iTargetPointIndex >= 0 && iTargetPointIndex < MAX_ST_CS_WEAPON )
		{
			...
			if ( bRandomPos != FALSE )
			{
				btX = WEAPON_CS_DEFENSER[iTargetPointIndex].btXBegin + Random(0,tx-1);
				btY = WEAPON_CS_DEFENSER[iTargetPointIndex].btYBegin + Random(0,ty-1);
			}
			else
			{
...
BOOL CWeapon::Attack(LPOBJ lpObj, LPOBJ lpTargetObj, CMagicInf * lpMagic, int iCriticalDamage, int iActionType)
{
	...
	if ( gObjAngelSprite(lpTargetObj) == TRUE )
	{
		if ( iAttackDamage > 1 )
		{
			double lc13 = (double)(iAttackDamage * 8 ) / 10.0f;
	...
	if ( gObjWingSprite(lpTargetObj) == TRUE )
	{
		...
		if ( iAttackDamage > 1 )
		{
            ...
            {
                    float lc15 = float((int)iAttackDamage *  (int)(61 - Wing->m_Level * 2) ) / 100.0f;
                    iAttackDamage = (int )lc15;
            }
            else if ( Wing->m_Type == ITEMGET(12, 40) ) //Third Wings
            {
                    float lc15 = float((int)iAttackDamage *  (int)(76 - Wing->m_Level * 2) ) / 100.0f;
                    iAttackDamage = (int )lc15;
            }
            else
            {
	...
	if ( ManaChange != FALSE )
	{
		GCManaSend(lpTargetObj->m_Index, (short)lpTargetObj->Mana, (BYTE)-1, 0, lpTargetObj->BP);
	}
...
void CWeapon::WeaponAttackProc()
{
	for ( int i=0;i<MAX_WEAPON_DAMAGED_TARGET_INFO;i++)
	{
		...
		{
			if ( this->m_WeaponDamagedTargetInfo[i].m_iDamageDealingTime > (int)GetTickCount() )
			{
				continue;
			}
...
void CWeapon::WeaponAct(int iIndex)
{
	for ( int i=0;i<MAX_WEAPON_CAL_DAMAGER_INFO;i++)
	{
		if ( this->m_WeaponCalDamageInfo[i].m_IsUsed == TRUE )
		{
			...
			{
				if ( this->m_WeaponCalDamageInfo[i].m_iLimitTime < (int)GetTickCount() )
				{
					this->m_WeaponCalDamageInfo[i].RESET();
				}
			}
