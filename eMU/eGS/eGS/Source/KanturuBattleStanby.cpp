...
#include "..\Header\KanturuBattleStanby.h"
...
BOOL CKanturuBattleStanby::LoadData(LPSTR lpszFileName)
{
	...
	try
	{
		...
		LogAdd( "[ KANTURU ][ BattleStanby ] %s file is Loaded",
			lpszFileName);
...
{
	LogAdd( "[ KANTURU ][ BattleStanby ] State(%d) -> NONE",
...
{
	LogAdd( "[ KANTURU ][ BattleStanby ] State(%d) -> START",
		this->m_iBattleStanbyState);
...
{
	LogAdd( "[ KANTURU ][ BattleStanby ] State(%d) -> NOTIFY",
		this->m_iBattleStanbyState);

	this->SetBattleStanbyState(KANTURU_STANBY_NOTIFY);
	KANTURU_UTIL.SendMsgKauturuMapUser("1 minute(s) left before Kanturu Gate Core opens.");
}


void CKanturuBattleStanby::SetState_END()
{
	LogAdd( "[ KANTURU ][ BattleStanby ] State(%d) -> END",
		this->m_iBattleStanbyState);
