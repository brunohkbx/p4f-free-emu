...
/* Disorder with same effect in CBloodCastle::LevelUp
		GCLevelUpMsgSend(gObj[iIndex].Index, gObj[iIndex].Level, gObj[iIndex].LevelUpPoint, 
			(int)((double)gObj[iIndex].unk104 + gObj[iIndex].fMaxLife), (int)((double)gObj[iIndex].unk108 + gObj[iIndex].fMaxMana),
...
// GS-CS	1.00.18	0x005303F0	-	Completed
...
CBloodCastle::CBloodCastle()
{
	...
	this->m_iBC_NORMAL_ITEM_DROP = g_BloodCastleNormalItemDrop;
	this->m_iBC_EXCEL_ITEM_DROP = g_BloodCastleExcelItemDrop;
...
void CBloodCastle::Init(bool bEVENT_ENABLE)
{
	...
	{
		this->m_BridgeData[i].m_iAngelKingPosNum = -1;
		memset(&this->m_BridgeData[i].m_nBossMonsterPosNum,-1,sizeof(this->m_BridgeData[i].m_nBossMonsterPosNum));
	...
	for ( int n =0; n<gMSetBase.m_Count;n++)
	{
		...
		{
			BYTE btMonsterType = (BYTE)gMSetBase.m_Mp[n].m_Type;
	...
	g_iBC_ChoasMixSuccessRate[0] = g_BC_MixRate1;
	g_iBC_ChoasMixSuccessRate[1] = g_BC_MixRate2;
	g_iBC_ChoasMixSuccessRate[2] = g_BC_MixRate3;
	g_iBC_ChoasMixSuccessRate[3] = g_BC_MixRate4;
	g_iBC_ChoasMixSuccessRate[4] = g_BC_MixRate5;
	g_iBC_ChoasMixSuccessRate[5] = g_BC_MixRate6;
	g_iBC_ChoasMixSuccessRate[6] = g_BC_MixRate7;

	g_iBC_ChoasMixMoney[0] = g_BC_MixMoney1;
	g_iBC_ChoasMixMoney[1] = g_BC_MixMoney2;
	g_iBC_ChoasMixMoney[2] = g_BC_MixMoney3;
	g_iBC_ChoasMixMoney[3] = g_BC_MixMoney4;
	g_iBC_ChoasMixMoney[4] = g_BC_MixMoney5;
	g_iBC_ChoasMixMoney[5] = g_BC_MixMoney6;
	g_iBC_ChoasMixMoney[6] = g_BC_MixMoney7;

}

BOOL CBloodCastle::LoadFinalGift(std::string filename )
{
	int Token;
	...
	{
		MsgBox("Blood Castle data load error %s", filename);
		return FALSE;
	}
	...
	while ( true )
	{
		...
		if ( Token == 1 )
		{
			GLevel.push_back((int)TokenNumber);

			Token = GetToken();
			GType.push_back((int)TokenNumber);

			Token = GetToken();
			GIndex.push_back((int)TokenNumber);

			Token = GetToken();
			GMinLvl.push_back((int)TokenNumber);

			Token = GetToken();
			GMaxLvl.push_back((int)TokenNumber);

			Token = GetToken();
			GDur.push_back((int)TokenNumber);

			Token = GetToken();
			GOpt1.push_back((int)TokenNumber);

			Token = GetToken();
			GOpt2.push_back((int)TokenNumber);

			Token = GetToken();
			GMinOpt3.push_back((int)TokenNumber);

			Token = GetToken();
			GMaxOpt3.push_back((int)TokenNumber);

			Token = GetToken();
			GMinExcel.push_back((int)TokenNumber);

			Token = GetToken();
			GMaxExcel.push_back((int)TokenNumber);

			Token = GetToken();
			GAncient.push_back((int)TokenNumber);

			Token = GetToken();
			GDropRate.push_back((int)TokenNumber);
...
void CBloodCastle::Load(std::string filename)
{
	int Token;
	int type;
	int BridgeCount;
...
void CBloodCastle::CheckSync(int iBridgeIndex)
{
	...
	switch ( g_iBloodCastle_StartHour )
	{
		case 2:
			...
			if ( (today.wHour%2) == 0 )
			{
				if ( ( today.wMinute *60 + today.wSecond ) >= 1800 )
				{
					this->m_BridgeData[iBridgeIndex].m_iBC_REMAIN_MSEC = (9000 - (today.wMinute * 60 + today.wSecond)) * 1000;
				}
				else
				{
					this->m_BridgeData[iBridgeIndex].m_iBC_REMAIN_MSEC = (1800 - (today.wMinute * 60 + today.wSecond)) * 1000;
				}
			}
			else
			{
				this->m_BridgeData[iBridgeIndex].m_iBC_REMAIN_MSEC = (5400 - (today.wMinute * 60 + today.wSecond)) * 1000;
			}
	...
	LogAdd("[Blood Castle] (%d) Sync Open Time. [%d] min remain",
		...
		if ( g_bBloodCastle != FALSE )
		{
			...
			if ( this->m_BridgeData[iBridgeIndex].m_iBC_REMAIN_MSEC <= ( this->m_iBC_TIME_MIN_OPEN * 60 * 1000 ) && this->m_BridgeData[iBridgeIndex].m_iBC_REMAIN_MSEC > 0 && (this->m_BridgeData[iBridgeIndex].m_iBC_REMAIN_MSEC/60000) !=this->m_BridgeData[iBridgeIndex].m_iBC_NOTIFY_COUNT)
			{
				...
				if ( iBridgeIndex == 0 )
				{
					...
					TNotice::MakeNoticeMsgEx(&pNotice, 0, "%d minute(s) left before entry into Blood Castle.", this->m_BridgeData[iBridgeIndex].m_iBC_NOTIFY_COUNT+1);
...
void CBloodCastle::ProcState_Playing(int iBridgeIndex)
{
	...
	if ( iTICK_MSEC >= 1000 )
	{
		...
		if ( this->m_BridgeData[iBridgeIndex].m_bBC_MONSTER_KILL_COMPLETE == true	)//якщо всі монстри вбиті необхідно відкрити ворота
		{
			if ( this->m_BridgeData[iBridgeIndex].m_dwBC_TICK_DOOR_OPEN != -1 )//якщо -1 це значить що ворота ще не відкриті
			{
				if ( GetTickCount() > this->m_BridgeData[iBridgeIndex].m_dwBC_TICK_DOOR_OPEN ) //коли вбивають всіх монстрів тоді в m_dwBC_TICK_DOOR_OPEN записується 
					//GetTickCount + 3000 , теоритично ворота відкриються через 3 сек після вбивства всіх монстрів
				{
					this->ReleaseCastleBridge(iBridgeIndex);
					this->SendCastleBridgeBlockInfo(iBridgeIndex, 0);
					LogAdd("[Blood Castle] (%d) Bridge Change Bridge Attribute -> Open", iBridgeIndex+1);
					this->m_BridgeData[iBridgeIndex].m_dwBC_TICK_DOOR_OPEN = -1; // ставимо що ворота вже відкриті
				}
			}
		}
		...
		if ( this->m_BridgeData[iBridgeIndex].m_iBC_REMAIN_MSEC <= ((this->m_iBC_TIME_MIN_PLAY*60-60)*1000) && this->m_BridgeData[iBridgeIndex].m_bBC_PLAY_START == false )
		{
			...
			this->m_BridgeData[iBridgeIndex].m_bBC_CAN_PARTY = false;
			TNotice::MakeNoticeMsgEx((TNotice*)&pNotice, 0, "The Blood Castle Quest %d has started!", iBridgeIndex+1);
			...
			LogAdd("[Blood Castle] (%d) Blood Castle Quest Start", iBridgeIndex+1);
		}
		//повідомлення за 30 сек до закінчення БЦ
		...
		if ( this->CheckEveryUserDie(iBridgeIndex) != false )
		{
			...
			TNotice::MakeNoticeMsg(&pNotice, 0, "Infiltration into Blood Castle has failed!");
			this->SendBridgeAnyMsg( (LPBYTE)&pNotice, pNotice.h.size, iBridgeIndex);

			LogAdd("[Blood Castle] (%d) Blood Castle Quest Fail Result -> Destroy Castle Door [%s][%s]",
			...
			LogAdd("[Blood Castle] (%d) Blood Castle Quest Fail Result -> Destroy Saint Status [%s][%s]",
				iBridgeIndex+1, this->m_BridgeData[iBridgeIndex].m_szKill_Door_AccountID,
				this->m_BridgeData[iBridgeIndex].m_szKill_Status_CharName);
			this->GiveReward_Fail(iBridgeIndex);
			this->SetState(iBridgeIndex, BC_STATE_CLOSED);
			LogAdd("[Blood Castle] (%d) Blood Castle Quest Failed -> Every User Out", iBridgeIndex+1);
		}
		else
		{
		...
		//даємо нагороду за пройдений БЦ
		if ( this->CheckWinnerExist(iBridgeIndex) == true )
		{
			if ( this->CheckWinnerValid(iBridgeIndex) == true )
			{
				//якщо не потрібно команди для проходження БЦ
				if (!PartyNeedToFinishBC)
				{
					...
					LogAdd("[Blood Castle] (%d) CheckWinnerPartyComplete(iBridgeIndex) == true",
						iBridgeIndex+1);
				...
				{
					//якщо потрібна команда, але її немає то БЦ буде не пройдене, і приз не буде виданий
					...
					else
					{
						...
						LogAdd("[Blood Castle] (%d) CheckWinnerValid(iBridgeIndex) == false",
							iBridgeIndex+1);
	...
	if ( this->m_BridgeData[iBridgeIndex].m_iBC_REMAIN_MSEC <= 0 )
	{
		if ( this->m_BridgeData[iBridgeIndex].m_bBC_REWARDED == false )
		{
			...
			LogAdd("[Blood Castle] (%d) Blood Castle Quest Failed -> Time Out", iBridgeIndex+1);
...
void CBloodCastle::SetState_Closed(int iBridgeIndex)
{
	...
	LogAdd("[Blood Castle] (%d) SetState CLOSED", iBridgeIndex+1);
}
...
void CBloodCastle::SetState_Playing(int iBridgeIndex)
{
	...
	TNotice::MakeNoticeMsgEx(&pNotice, 1, "The Blood Castle Quest %d starts in %d second(s).", iBridgeIndex+1, 60);
	this->SendBridgeAnyMsg( (LPBYTE)&pNotice, pNotice.h.size, iBridgeIndex);
	...
	LogAdd("[Blood Castle] (%d) SetState PLAYING", iBridgeIndex+1);
}
...
void CBloodCastle::SetState_PlayEnd(int iBridgeIndex)
{
	...
	LogAdd("[Blood Castle] (%d) SetState PLAYEND", iBridgeIndex+1);
...
int  CBloodCastle::CheckEnterLevel(int iIndex, int iLevel)	// RET : [2:Error][1:OverLevel][0:InLevel - Success][-1:UnderLevel]
{
	...
	if ( gObj[iIndex].Type != OBJ_USER || gObj[iIndex].Connected != PLAYER_PLAYING )
	{
		return 2;
	}
...
bool CBloodCastle::BloodCastleChaosMix(int iIndex, int iLEVEL)
{
	...
	LogAdd("[Blood Castle] ЕхёнёБЕд Mix Chaos Mix Start (Account:%s, Name:%s, Level:%d)",
		gObj[iIndex].AccountID, gObj[iIndex].Name, iLEVEL);
	LogChaosItem(&gObj[iIndex], "ЕхёнёБЕд Mix");
	...
	if ( iMIX_SUCCESS_RATE < 0 || iMIX_SUCCESS_RATE > 100 )
	{
		...
		LogAdd("[Blood Castle] ЕхёнёБЕд Mix Chaos Mix Failed - MixRate Out of Bound (Account:%s, Name:%s, Level:%d)",
			gObj[iIndex].AccountID, gObj[iIndex].Name, iLEVEL);
	...
	if ( iMIX_SUCCESS_RATE > g_BC_MixRateMax )
	{
		iMIX_SUCCESS_RATE = g_BC_MixRateMax;
	}
	...
	{
		DataSend(iIndex, (UCHAR*)&pMsg, pMsg.h.size);
		LogAdd("[Blood Castle] ЕхёнёБЕд Mix Chaos Mix Failed - MixMoney < 0 (Account:%s, Name:%s, Level:%d)",
			gObj[iIndex].AccountID, gObj[iIndex].Name, iLEVEL);
	...
	if ( (gObj[iIndex].Money - iMIX_NEED_MONEY) < 0 )	// Not enoght zen
	{
		...
		DataSend(iIndex, (UCHAR*)&pMsg, pMsg.h.size);
		LogAdd("[Blood Castle] ЕхёнёБЕд Mix Chaos Mix Failed - Not Enough Money (Account:%s, Name:%s, Level:%d)",
			gObj[iIndex].AccountID, gObj[iIndex].Name, iLEVEL);
	...
	else	// Failure
	{
		...
		DataSend(iIndex, (UCHAR*)&pMsg, pMsg.h.size);
		LogAdd("[ЕхёнёБЕд Mix] [%s][%s] CBMix Fail %d Money : %d-%d",
			gObj[iIndex].AccountID, gObj[iIndex].Name, iMIX_SUCCESS_RATE, gObj[iIndex].Money, iMIX_NEED_MONEY);
	...
	else
	{
		...
		else
		{
			if ( (today.wHour % 2) == 0 )
			{
				if ( (today.wMinute * 60 + today.wSecond) >= 1800 )
				{
					iREMAIN_MINUTE = (9000 - (today.wMinute * 60 + today.wSecond)) / 60;
				}
				else
				{
					iREMAIN_MINUTE = (1800 - (today.wMinute * 60 + today.wSecond)) / 60;
				}
			}
			else
			{
		...
		if ( today.wMinute * 60 + today.wSecond >= 1800 )
		{
			iREMAIN_MINUTE = (5400 - (today.wMinute * 60 + today.wSecond)) / 60;
		}
		else
		{
			iREMAIN_MINUTE = (1800 - (today.wMinute * 60 + today.wSecond)) / 60;
...
void CBloodCastle::ClearMonster(int iBridgeIndex, bool bClearCastleDoor)
{
	...
	int BCIndex=iBridgeIndex+MAP_INDEX_BLOODCASTLE1;
	...
	{
		if ( gObj[n].MapNumber == BCIndex)
		{
			if (bClearCastleDoor == true && gObj[n].Class!= 131 && gObj[n].Class != 232 )
			{
				gObjDel(n);
			}
		}
...
void CBloodCastle::SetMonster(int iBridgeIndex)
{
	...
	BYTE btMonsterType = (BYTE)gMSetBase.m_Mp[nCastleDoorPosNum].m_Type;
	...
	for ( int n=0;n<gMSetBase.m_Count;n++)
	{
		if ( BC_MAP_RANGE(gMSetBase.m_Mp[n].m_MapNumber) != FALSE  )
		{
			BYTE btIndex = (BYTE)gMSetBase.m_Mp[n].m_Type;
			...
			if ( result >= 0 )
			{
				...
				gObj[result].Dir = (BYTE)gMSetBase.m_Mp[n].m_Dir;
...
void CBloodCastle::SetBossMonster(int iBridgeIndex)
{
	...
	for ( int x=0;x<MAX_BLOOD_CASTLE_BOSS_MONSTER;x++)
	{
		if ( this->m_BridgeData[iBridgeIndex].m_nBossMonsterPosNum[x] != -1 )
		{
			...
			BYTE btMonsterType = (BYTE)gMSetBase.m_Mp[nBossMonsterPosNum].m_Type;
...
void CBloodCastle::SetSaintStatue(int iBridgeIndex)
{
	...
	BYTE btMonsterType = (BYTE)gMSetBase.m_Mp[nSaintStatuePosNum].m_Type;
	...
	if ( nSaintStatuePosNum != -1 )
	{
		if ( BC_STATUE_RANGE(btMonsterType-132) != FALSE )
		{
			...
			if ( result >= 0 )
			{
				...
				::gObjSetMonster(result, btMonsterType);
				gObj[result].Class = Random(0,2) + 132;//
				gObj[result].m_cBloodCastleIndex = btBloodCastleIndex;
				gObj[result].m_ItemRate = this->m_iBC_NORMAL_ITEM_DROP;
				gObj[result].Dir = 1;
		...
		{
			LogAdd("[Blood Castle][Bug Tracer] (%d) It's not Saint Statue( Map:%d, BloodCastleIndex:%d, Type:%d )",
...
int  CBloodCastle::LevelUp(int iIndex, int iAddExp, int iEventType)
{
	...
	{
		GCServerMsgStringSend("You have reached a maximum level", gObj[iIndex].m_Index, 1);
		return 0;
	}
	...
	else
	{
		...
		if ( gObj[iIndex].Class == CLASS_DARKLORD || gObj[iIndex].Class == CLASS_MAGUMSA )
		{
			gObj[iIndex].LevelUpPoint += g_LvlUpPointDLMG;
		}
		else
		{
			gObj[iIndex].LevelUpPoint += g_LvlUpPointNormal;
		}
		...
		//	(int)((double)gObj[iIndex].AddLife + gObj[iIndex].MaxLife), (int)((double)gObj[iIndex].AddMana + gObj[iIndex].MaxMana),
		//	gObj[iIndex].MaxBP + gObj[iIndex].AddBP, AddPoint, MaxAddPoint);
		gObjCalcMaxLifePower(gObj[iIndex].m_Index);
		LogAdd("Level Up [%s][%s] (%d)", gObj[iIndex].AccountID, gObj[iIndex].Name, gObj[iIndex].Level);
	}
...
void CBloodCastle::ReleaseCastleBridge(int iBridgeIndex)
{
	...
	for ( int i= ::g_btCastleBridgeMapXY[iBridgeIndex].btStartX;i<= ::g_btCastleBridgeMapXY[iBridgeIndex].btEndX;i++)
	{
		for ( int j= ::g_btCastleBridgeMapXY[iBridgeIndex].btStartY;j<= ::g_btCastleBridgeMapXY[iBridgeIndex].btEndY;j++)
		{
			...
			LogAdd("[Blood Castle][Bug Tracer] ReleaseCastleBridge-> %d", iBridgeIndex+1);
		}
	}
}
...
void CBloodCastle::SendCastleEntranceBlockInfo(int iBridgeIndex, bool bLive)
{
	...
	/*PMSG_SETMAPATTR_COUNT Msg;
	Msg.h.headcode = 0x46;
	Msg.h.c = 0xC1;
	Msg.h.size = sizeof(PMSG_SETMAPATTR_COUNT);

	Msg.btType = 0;
	Msg.btCount = 1;
	Msg.btMapAttr = 4;
	(bLive)?(Msg.btMapSetType=0) :( Msg.btMapSetType=1);

	PMSG_SETMAPATTR MsgBody[2];

	MsgBody[0].btX = ::g_btCastleEntranceMapXY[iBridgeIndex].btStartX;
	MsgBody[0].btY = ::g_btCastleEntranceMapXY[iBridgeIndex].btStartY;
	MsgBody[1].btX   = ::g_btCastleEntranceMapXY[iBridgeIndex].btEndX;
	MsgBody[1].btY   = ::g_btCastleEntranceMapXY[iBridgeIndex].btEndY;
	...
	for (int i=OBJ_MAXMONSTER;i<OBJMAX;i++)
	{
		if ( gObj[i].MapNumber == BCIndex )
		{
			if ( gObj[i].Connected == PLAYER_PLAYING )
			{
				DataSend(i, (unsigned char *)&Msg, Msg.h.size);
				DataSend(i, (unsigned char *)&MsgBody[0], sizeof(MsgBody[0]));
				DataSend(i, (unsigned char *)&MsgBody[1], sizeof(MsgBody[1]));
			}
		}
	}*/
	...
	for ( int i=OBJ_MAXMONSTER;i<OBJMAX;i++)
	{
		if ( gObj[i].MapNumber == BCIndex )
		{
			if ( gObj[i].Connected == PLAYER_PLAYING )
			{
				...
				LogAdd("[Blood Castle][Bug Tracer] SendCastleBridgeBlockInfo-> Bridge:%d/Live:%d, (%d,%d,%d,%d)",
	...
	for (int i=OBJ_MAXMONSTER;i<OBJMAX;i++)
	{
		if ( gObj[i].MapNumber == BCIndex )
		{
			if ( gObj[i].Connected == PLAYER_PLAYING )
			{
				...
				LogAdd("[Blood Castle][Bug Tracer] SendCastleBridgeBlockInfo-> Bridge:%d/Live:%d, (%d,%d,%d,%d)",
					iBridgeIndex+1, bLive, MsgBody[0].btX, MsgBody[0].btY, MsgBody[1].btX, MsgBody[1].btY);
			}
		}
	}*/
}
...
void CBloodCastle::SendCastleDoorBlockInfo(int iBridgeIndex, bool bLive)
{
	...
	PMSG_SETMAPATTR MsgBody[6];

	MsgBody[0].btX = ::g_btCastleDoorMapXY[iBridgeIndex][0].btStartX;
	MsgBody[0].btY = ::g_btCastleDoorMapXY[iBridgeIndex][0].btStartY;
	MsgBody[1].btX   = ::g_btCastleDoorMapXY[iBridgeIndex][0].btEndX;
	MsgBody[1].btY   = ::g_btCastleDoorMapXY[iBridgeIndex][0].btEndY;

	MsgBody[2].btX = ::g_btCastleDoorMapXY[iBridgeIndex][1].btStartX;
	MsgBody[2].btY = ::g_btCastleDoorMapXY[iBridgeIndex][1].btStartY;
	MsgBody[3].btX   = ::g_btCastleDoorMapXY[iBridgeIndex][1].btEndX;
	MsgBody[3].btY   = ::g_btCastleDoorMapXY[iBridgeIndex][1].btEndY;

	MsgBody[4].btX = ::g_btCastleDoorMapXY[iBridgeIndex][2].btStartX;
	MsgBody[4].btY = ::g_btCastleDoorMapXY[iBridgeIndex][2].btStartY;
	MsgBody[5].btX   = ::g_btCastleDoorMapXY[iBridgeIndex][2].btEndX;
	MsgBody[5].btY   = ::g_btCastleDoorMapXY[iBridgeIndex][2].btEndY;
	...
	for (int i=OBJ_MAXMONSTER;i<OBJMAX;i++)
	{
		if ( gObj[i].MapNumber == BCIndex )
		{
			if ( gObj[i].Connected == PLAYER_PLAYING )
			{
				...
				DataSend(i, (unsigned char *)&MsgBody[0], sizeof(MsgBody[0]));
				DataSend(i, (unsigned char *)&MsgBody[1], sizeof(MsgBody[1]));
				DataSend(i, (unsigned char *)&MsgBody[2], sizeof(MsgBody[2]));
				DataSend(i, (unsigned char *)&MsgBody[3], sizeof(MsgBody[3]));
				DataSend(i, (unsigned char *)&MsgBody[4], sizeof(MsgBody[4]));
				DataSend(i, (unsigned char *)&MsgBody[5], sizeof(MsgBody[5]));
...
void CBloodCastle::SendNoticeMessage(int iBridgeIndex, std::string MSG)
{
	if ( BC_BRIDGE_RANGE(iBridgeIndex) == FALSE )
	{
		return;
	...
	TNotice::MakeNoticeMsg( &pNotice, 0, MSG.c_str());
		...
		if ( this->m_BridgeData[iBridgeIndex].m_UserData[i].m_iIndex != -1 )
		{
			if ( gObj[this->m_BridgeData[iBridgeIndex].m_UserData[i].m_iIndex].Connected == PLAYER_PLAYING )
			{
				if ( gObj[this->m_BridgeData[iBridgeIndex].m_UserData[i].m_iIndex].m_cBloodCastleIndex != -1 )
				{
					if ( gObj[this->m_BridgeData[iBridgeIndex].m_UserData[i].m_iIndex].m_cBloodCastleSubIndex != -1 )
					{
						TNotice::MakeNoticeMsgEx(&pNotice, 0, "The Blood Castle Quest %d has gained you a total of %d EXP.", iBridgeIndex+1, this->m_BridgeData[iBridgeIndex].m_UserData[i].m_iEXP);
						DataSend(this->m_BridgeData[iBridgeIndex].m_UserData[i].m_iIndex, (UCHAR*)&pNotice, pNotice.h.size);
					}
				}
			}
		...
		if ( gObj[iIndex].pInventory[x].IsItem() == TRUE )
		{
			if ( gObj[iIndex].pInventory[x].m_Type == ITEMGET(13,19) )
			{
				...
				if ( BC_WEAPON_LEVEL_RANGE(iLEVEL) != FALSE )
				{
					...
					LogAdd("[Blood Castle] Force (Account:%s, Name:%s) to Delete Angel King's Weapon (%d) [Serial:%d]",
			...
			if ( gObj[iIndex].pInventory[x].m_Type == ITEMGET(13,19) )
			{
				...
				if ( BC_WEAPON_LEVEL_RANGE(iLEVEL) != FALSE )
				{
					...
					if ( BC_MAP_RANGE(gObj[iIndex].MapNumber) != FALSE )
					{
						...
						LogAdd("[Blood Castle] Force (Account:%s, Name:%s) to Drop Angel King's Weapon (%d) [Serial:%d-%d]",
					...
					{
						LogAdd("[Blood Castle] Force (Account:%s, Name:%s) to Drop Angel King's Weapon (%d) [Serial:%d]",
							gObj[iIndex].AccountID, gObj[iIndex].Name, iLEVEL,
							gObj[iIndex].pInventory[x].m_Number);
					}
				}
...
void CBloodCastle::GiveReward_Win(int iIndex, int iBridgeIndex)
{
	...
	LogAdd("[Blood Castle] (%d) Winner Party Point - Alive Party Member : %d, Alive Party Point : %d",
	...
	if ( gObj[iIndex].Connected == PLAYER_PLAYING )
	{
		...
		wsprintf(szNOTIFY_MSG, "The Blood Castle Quest has been successfully completed by %s!", gObj[iIndex].Name);
	}
	else
	{
		wsprintf(szNOTIFY_MSG, "The Blood Castle quest has been successfully concluded.");
	}

	if ( this->m_BridgeData[iBridgeIndex].m_iMISSION_SUCCESS != -1 )
	{
		int iTOTAL_EXP = this->GetAliveUserTotalEXP(iBridgeIndex);
		LogAdd("[Blood Castle] (%d) Quest Complete - All Alive User's Total EXP:%d",
		...
		for ( int i=0;i<MAX_BLOOD_CASTLE_SUB_BRIDGE;i++)
		{
			...
			switch ( this->m_BridgeData[iBridgeIndex].m_UserData[i].m_iUserState  )
			{
				case 0:
					...
					LogAdd("[Blood Castle] (%d) Quest Complete Rewarded (Account:%s, Name:%s [State:%d], Exp:%d, ExtExp:%d, Zen:%d, Score:%d, LeftTime:%d, MapNumber:%d)",
				...
				case 2:
					...
					if ( BC_MAP_RANGE(gObj[this->m_BridgeData[iBridgeIndex].m_UserData[i].m_iIndex].MapNumber ) != FALSE )
					{
						this->DropChaosGem(this->m_BridgeData[iBridgeIndex].m_UserData[i].m_iIndex,iBridgeIndex);
					}
...
void CBloodCastle::GiveReward_Fail(int iBridgeIndex)
{
	...
	if ( iUserWhoGotUltimateWeapon != -1 )
	{
		...
		{
			LogAdd("[Blood Castle] (%d) (Account:%s, Name:%s) got Ultimate Weapon (%d) but Failed - Time Out",
	...
	for ( int i=0;i<MAX_BLOOD_CASTLE_SUB_BRIDGE;i++)
	{
		...
		LogAdd("[Blood Castle] (%d) Quest Fail Rewarded (Account:%s, Name:%s, Score:%d, ExtEXP:%d, LeftTime:%d)",
			iBridgeIndex+1, gObj[this->m_BridgeData[iBridgeIndex].m_UserData[i].m_iIndex].AccountID,
			gObj[this->m_BridgeData[iBridgeIndex].m_UserData[i].m_iIndex].Name,
		...
		if ( gObj[this->m_BridgeData[iBridgeIndex].m_UserData[i].m_iIndex].m_cBloodCastleIndex != -1 && gObj[this->m_BridgeData[iBridgeIndex].m_UserData[i].m_iIndex].m_cBloodCastleSubIndex != -1 )
		{
			...
			LogAdd("[Blood Castle] (%d) Send User Quest Fail Message (Account:%s, Name:%s [State:%d], MapNumber:%d)",
				iBridgeIndex+1, gObj[this->m_BridgeData[iBridgeIndex].m_UserData[i].m_iIndex].AccountID,
				gObj[this->m_BridgeData[iBridgeIndex].m_UserData[i].m_iIndex].Name, 
...
void CBloodCastle::DropChaosGem(int iIndex,int iBridgeIndex)
{
	if ( OBJMAX_RANGE(iIndex) == FALSE )
	{
		return;
	...
	int Opt3=0;
	int tmp=0;
	for (int i=0; i < (int)GType.size(); i++)
	{
		if (GLevel[i]==iBridgeIndex)
		{

			DropRate=GDropRate[i];
			...
			if (tmp<DropRate)
			{
				...
				MinOpt3=GMinOpt3[i];
				MaxOpt3=GMaxOpt3[i];
				MinExcel=GMinExcel[i];
				MaxExcel=GMaxExcel[i];
				Ancient=GAncient[i];

				ItemLevel=Random(MinLvl,MaxLvl);
				...
				Opt3=Random(MinOpt3,MaxOpt3);

				Excellent=Random(MinExcel,MaxExcel);

				iType=ItemGetNumberMake(Type, Index);

				ItemSerialCreateSend(gObj[iIndex].m_Index, gObj[iIndex].MapNumber + 227, (BYTE)gObj[iIndex].X,
					(BYTE)gObj[iIndex].Y, iType, ItemLevel, Dur, Opt1, Opt2, Opt3, iMaxHitUser, Excellent, Ancient);
			}
		}
	}
}
...
void CBloodCastle::SendRewardScore(int iIndex, int iSCORE, int iLeftTime, int iAlivePartyCount)
{
	...
	if( ::IsDataServerConnected == FALSE )
	{
		if ( !GMDataServerConnect(DataServerIp) )
		{
			::IsDataServerConnected = FALSE;
			LogAdd("Can not connect Data Server");
...
void CBloodCastle::SetMonsterKillCount(int iBridgeIndex)
{
	...
	LogAdd("[Blood Castle] (%d) Monster Kill Count Set - Monster:%d, Current Monster:%d, USER TOT:%d, LIVE:%d, DEAD:%d",
...
bool CBloodCastle::CheckMonsterKillCount(int iBridgeIndex)
{
	...
	{
		LogAdd("[Blood Castle][Bug Tracer] (%d) All Monster Kill Success(Max Kill Count:%d / Kill Count:%d)",
			iBridgeIndex+1, this->m_BridgeData[iBridgeIndex].m_iBC_MONSTER_MAX_COUNT,
			this->m_BridgeData[iBridgeIndex].m_iBC_MONSTER_KILL_COUNT);
...
bool CBloodCastle::CheckBossKillCount(int iBridgeIndex)
{
	...
	{
		LogAdd("[Blood Castle][Bug Tracer] (%d) All Boss Monster Kill Success(Max Boss Kill Count:%d / Boss Kill Count:%d)",
...
bool CBloodCastle::CheckEveryUserDie(int iBridgeIndex)
{
	...
	for ( int i=0;i<MAX_BLOOD_CASTLE_SUB_BRIDGE;i++)
	{
		if ( this->m_BridgeData[iBridgeIndex].m_UserData[i].m_iIndex != -1 )
		{
			...
			{
				LogAdd("error-L3 : [Blood Castle] (%d) User Out of Bound UserIndex:%d, SubIndex:%d",
			...
			if ( gObj[this->m_BridgeData[iBridgeIndex].m_UserData[i].m_iIndex].MapNumber == (iBridgeIndex + MAP_INDEX_BLOODCASTLE1) )
			{
				...
				{
					LogAdd("error-L3 : [Blood Castle] (%d) Connection Closed UserIndex:%d, SubIndex:%d",
						iBridgeIndex+1, this->m_BridgeData[iBridgeIndex].m_UserData[i].m_iIndex, i);
				}
			}
			else
...
bool CBloodCastle::CheckAngelKingExist(int iBridgeIndex)
{
	...
	{
		LogAdd("[Blood Castle] (%d) Angel King's Monster Position Doesn't Exist",
			iBridgeIndex+1);
	...
	if ( bRET_VAL == false )
	{
		...
		{
			LogAdd("[Blood Castle] (%d) Angel King - gObjAdd() failed (no space to add monster, result:%d)",
...
BOOL CBloodCastle::DropItemDirectly(int iBridgeIndex, int iIndex, int iItemType, int iItemPos)
{
	...
	{
		LogAdd("[Blood Castle] [%s][%s] DropItemDirectly() failed (iBridgeIndex wrong - %d)",
	...
	double dur = gObj[iIndex].pInventory[iItemPos].m_Durability;
	...
	if ( MapC[map_num].ItemDrop(type, level, dur, gObj[iIndex].X, gObj[iIndex].Y,Option1, Option2, Option3, NOption, SOption, item_number, aAntiLootIndex, PetLevel, PetExp, ItemEffectEx) == TRUE )
	{
		...
		LogAdd("***0479*** [%s][%s] (%d)%d/%d Throw away Serial:%d (%s)(%d)(%d)(%d)(%d)(%d) Dur: (%d) Ex:(%d,%d,%d,%d,%d,%d,%d) Set : (%d)", 
			...
			gObj[iIndex].X, gObj[iIndex].Y, s_num, "Item", type, level, Option1, Option2, Option3, (int)dur, NewOption[0],
...
bool CBloodCastle::CheckWinnerValid(int iBridgeIndex)
{
	if ( !BC_BRIDGE_RANGE(iBridgeIndex))
	{
		LogAdd("[Blood Castle] (%d) CBloodCastle::CheckWinnerValid() - !CHECK_LIMIT(iBridgeIndex, MAX_BLOODCASTLE_BRIDGE_COUNT)",
			iBridgeIndex+1);
	...
	if ( this->m_BridgeData[iBridgeIndex].m_iBC_COMPLETE_USER_INDEX == -1 )
	{
		LogAdd("[Blood Castle] (%d) CBloodCastle::CheckWinnerValid() - m_BridgeData[iBridgeIndex].m_iBC_COMPLETE_USER_INDEX == -1",
			iBridgeIndex+1);
	...
	{
		LogAdd("[Blood Castle] (%d) CBloodCastle::CheckWinnerValid() - !gObjIsConnected(m_BridgeData[iBridgeIndex].m_iBC_COMPLETE_USER_INDEX)",
			iBridgeIndex+1);
	...
	{
		LogAdd("[Blood Castle] (%d) CBloodCastle::CheckWinnerValid() - (gObj[m_BridgeData[iBridgeIndex].m_iBC_COMPLETE_USER_INDEX].m_cBloodCastleIndex == -1) ...",
			iBridgeIndex+1);
	...
	{
		LogAdd("[Blood Castle] (%d) CBloodCastle::CheckWinnerValid() - !CHECK_BLOODCASTLE(gObj[m_BridgeData[iBridgeIndex].m_iBC_COMPLETE_USER_INDEX].MapNumber)",
			iBridgeIndex+1);
...
void CBloodCastle::FixUsersPlayStateWin(int iBridgeIndex)
{
	...
	if ( this->m_BridgeData[iBridgeIndex].m_iMISSION_SUCCESS != -1 )
	{
		for ( int i=0;i<MAX_BLOOD_CASTLE_SUB_BRIDGE;i++)
		{
			...
			LogAdd("[Blood Castle] (%d) [%s][%s] FixUsersPlayStateWin() - State : %d",
...
void CBloodCastle::FixUsersPlayStateFail(int iBridgeIndex)
{
	...
	for ( int i=0;i<MAX_BLOOD_CASTLE_SUB_BRIDGE;i++)
	{
		...
		LogAdd("[Blood Castle] (%d) [%s][%s] FixUsersPlayStateFail() - State : %d",
