...
void CBUPR_ItemRequestRecv( CB_PMSG_REQ_PRESENT_RESULT * lpMsg)
{
	...
	if ( lpMsg->present != -1 )
	{
		...
		wsprintf(szTemp, "[%s] has won the [%s]. Congratulations!", szName.c_str(), szPresentName.c_str());
...
void DarkSpiritChaosMix(LPOBJ lpObj)
{
	...
	lpObj->ChaosSuccessRate = g_DarkRavenMixSuccessRate;
	lpObj->ChaosSuccessRate += iCharmOfLuckCount;
...
void BlessPotionChaosMix(LPOBJ lpObj)
{
	...
	{
		MsgOutput(lpObj->m_Index, "Test 8");
		DataSend(lpObj->m_Index, (LPBYTE)&pMsg, pMsg.h.size);
		lpObj->ChaosLock = FALSE;
