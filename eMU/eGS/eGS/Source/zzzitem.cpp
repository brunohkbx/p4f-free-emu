...
#include "..\Header\ItemSystemFor380.h"
...
void CItem::Convert(int type, BYTE Option1, BYTE Option2, BYTE Option3,  BYTE Attribute2, BYTE SetOption, BYTE ItemEffectEx,  BYTE DbVersion)
{
	...
	{
		LogAdd( "error-L1 : ItemIndex error %d", _type);
	}
	...
	if ( ((_type >= ITEMGET(12,3) ) && (_type <= ITEMGET(12,6) ) ) || ((_type >= ITEMGET(12,36) ) && (_type <= ITEMGET(12,40) ) ) || (_type == ITEMGET(13,30) )) //Third Wings Fix
	{
		Attribute2=0;
	}
	...
	{
		this->m_BaseDurability = ItemGetDurability(this->m_Type, this->m_Level, Attribute2 & 0x7F, this->m_SetOption );
	}
	else
	{
	...
	if ( (Attribute2 & 0x3F) > 0 )
	{
		...
		{
			if ( _type >= ITEMGET(12,36) && _type <= ITEMGET(12,40) ) //Third Wings Fix Required Level
			{
				this->m_RequireLevel;
			}
			else if ( _type < ITEMGET(0,0) || _type >= ITEMGET(12,0) && !(_type >= ITEMGET(12,36) && _type <= ITEMGET(12,40)) ) //Third Wings Fix Anti-Hack
			{
				this->m_RequireLevel += 20;
			}
		}
	...
	if ( p->Defense > 0 )	
	{
		...
		else 
		{
			...
			else if ( _type >= ITEMGET(12,36) && _type <= ITEMGET(12,40) ) //Third Wings Defense * 4
			{
			...
			if ( this->m_Level >= 10 && !(_type >= ITEMGET(12,36) && _type <= ITEMGET(12,40)) ) //Third Wings Anti-Hack, Prevent add Normal Wings Defense
			{
	...
	if ( _type == ITEMGET(13,30) )
	{
		...
		if ( this->m_Level >= 10 )
		{
			this->m_Defense += (this->m_Level - 9) * (this->m_Level - 8) / 2; //[?, 10~13 ??? +4,+5,+6,+7? ???? ??]
		}
	}

	if ( _type >= ITEMGET(12,36) && _type <= ITEMGET(12,40) ) //Third Wings Add when higher than +10 to +13
	{
		if ( this->m_Level >= 10 )
		{
			this->m_Defense += (this->m_Level - 8) * (this->m_Level - 7) / 2; //[10~13 ??? +5 ~ +8? ??]
		}
	}
	...
	if ( Option3 != 0 )
	{
		...
		else if ( _type == ITEMGET(12,40) ) // Third Wings "Mantle of Monarch"
		{
			this->m_Option3 = Option3;
			this->m_RequireStrength +=  Option3 * 4;
	...
	if ( ((_type >= ITEMGET(12,3)) && (_type <= ITEMGET(12,6))) || ((_type >= ITEMGET(12,36)) && (_type <= ITEMGET(12,40))) || (_type ==ITEMGET(13,30)) ) //Third Wings Applied
	{
		if ( ((this->m_NewOption)&1) != 0 )
		{
...
void CItem::Value()
{
	...
	else if ( this->m_Type == ITEMGET(13,14) ) //Loch Feather
	{
		if ( this->m_Level == 1 )
		{
			Gold = g_ChrestOfMonarchPrice;	// Crest of Monarch
		}
		else
		{
			Gold = g_BlueFeatherPrice;	// Blue Feather
		}
	}
	...
	{
		if ( this->m_Level == 1 ) //#error maybe unusefull
		{
			Gold = g_LifeStonePrice;
		}
	}
	...
	else if ( this->m_Type == ITEMGET(13,35) ) // Piece of Horn
	{
		Gold = g_POHPrice;
	}
	...
	{
		Gold = g_DinorantPrice;

		for ( int k=0;k<3;k++)
		{
			if ( (this->m_Option3 & (1<<k)) != 0 )
			{
				Gold += g_DinorantOptPrice;
			}
		}
	}
	...
	else if ( this->m_Type == ITEMGET(14,19) ) // Devil's Invitation
	{
		...
		{
			case 1:
				Gold = g_DSInvitation1Price;
				break;
			case 2:
				Gold = g_DSInvitation2Price;
				break;
			case 3:
				Gold = g_DSInvitation3Price;
				break;
			case 4:
				Gold = g_DSInvitation4Price;
				break;
			case 5:
				Gold = g_DSInvitation5Price;
				break;
			case 6:
				Gold = g_DSInvitation6Price;
				break;
			default:
				Gold = g_DSInvitation3Price;
				break;
	...
	else if ( this->m_Type == ITEMGET(13,18) ) // Invisibility Cloak
	{
		...
		{
			case 1:
				Gold = g_InvisCloack1Price;
				break;
			case 2:
				Gold = g_InvisCloack2Price;
				break;
			case 3:
				Gold = g_InvisCloack3Price;
				break;
			case 4:
				Gold = g_InvisCloack4Price;
				break;
			case 5:
				Gold = g_InvisCloack5Price;
				break;
			case 6:
				Gold = g_InvisCloack6Price;
				break;
			case 7:
				Gold = g_InvisCloack7Price;
				break;
			default:
				Gold = g_InvisCloack3Price;
				break;
	...
	else if ( this->m_Type == ITEMGET(13,29) ) // Armor of Guardsman
	{
		Gold = g_ArmorOfGuardmenPrice;
	}
	...
	else if ( this->m_Type == ITEMGET(13,31) ) // Spirit
	{
		...
		{
			case 0:
				Gold = g_HorseSpiritPrice;
				break;

			case 1:
				Gold = g_RavenSpiritPrice;
		}
	}
	...
	{
		Gold = __int64(( 10000.0 * this->m_Durability) * 3.0);
	}
	...
	{
		Gold = __int64(( 50.0 * this->m_Durability) * 3.0);
	}
	...
	else if ( p->Value > 0 )
	{
		...
		if ( this->m_Type >= ITEMGET(14,0) && this->m_Type <= ITEMGET(14,8) )	// Potions + Antidote
		{
			...
			this->m_SellMoney = (DWORD)Gold/3;
			this->m_SellMoney = this->m_SellMoney/10*10;
			return;
		}
	}
	else if ( (Type == 12 && (this->m_Type > ITEMGET(12,6) && this->m_Type < ITEMGET(12,36)) ) || Type == 13 || Type == 15 ) // Third Wings Fix for Zen Sell
	{
	...
	else
	{
		...
		if ( Type == 12 && (this->m_Type <= ITEMGET(12,6) || this->m_Type >= ITEMGET(12,36)) ) // Third Wings Fix Selling Price
		{
		...
		this->m_Type != ITEMGET(14,50) )
	{
		...
		if ( this->m_Durability==0.0 || this->m_BaseDurability==0.0 )
			lc15=0;
		else
			lc15 = 1.0 - (this->m_Durability / this->m_BaseDurability);
...
void CItem::SetItemPlusSpecialStat(unsigned short * Value, int Special)
{
	if ( this->m_Type < 0 )
	{
		return;
...
BOOL CItem::AddPetItemExp(int petexp)
{
	...
	if ( this->m_Type == ITEMGET(13,4) )
	{
		if ( this->m_PetItem_Level >= MAX_HORSE_LEVEL )
		{
			return FALSE;
		}

		BOOL ret = FALSE;
		DWORD maxexp = gPetItemExp.m_DarkHorseExpTable[1+this->m_PetItem_Level];
		this->m_PetItem_Exp += petexp;
	...
	{
		if ( this->m_PetItem_Level >= MAX_RAVEN_LEVEL )
		{
			return FALSE;
		}

		BOOL ret = FALSE;
		DWORD maxexp = gPetItemExp.m_DarkSpiritExpTable[1+this->m_PetItem_Level];
		this->m_PetItem_Exp += petexp;
...
BOOL CItem::PetItemLevelDown(int exp)
{
	...
	if ( this->m_Type == ITEMGET(13,4) )
	{
		...
		DWORD petexp = gPetItemExp.m_DarkHorseExpTable[petlevel];
	...
	{
		int petlevel = this->m_PetItem_Level;
		DWORD petexp = gPetItemExp.m_DarkSpiritExpTable[petlevel];
...
{
	//---
	this->m_DarkSpiritExpTable.push_back(0);
	this->m_DarkSpiritExpTable.push_back(0);
	//---
	this->m_DarkHorseExpTable.push_back(0);
	this->m_DarkHorseExpTable.push_back(0);
	//---

	for ( DWORD i = 2 ; i < MAX_RAVEN_LEVEL+2 ; i++ )
	{
		this->m_DarkSpiritExpTable.push_back((i+10) * i * i * i/* * 100*/);
	}

	for (DWORD i = 2 ; i < MAX_HORSE_LEVEL+2 ; i++ )
	{
		this->m_DarkHorseExpTable.push_back((i+10) * i * i * i/* * 100*/);
	}
}
...
double GetRepairItemRate(int index)
{
	return ItemAttribute[index].RepaireMoneyRate;
}
// ItemAttribute Begin:9439368 END 9446B68 Array[512]

double GetAllRepairItemRate(int index)
{
...
{
	p->RepaireMoneyRate = (double)0.1;
	p->AllRepaireMoneyRate = (double)0.4;
}
	...
	while ( true )
	{
		...
		if ( Token == 1 )
		{
			...
			while ( true )
			{
				...
				Token = GetToken();
				strcpy_s(p->Name, sizeof(p->Name), TokenString );
				...
				if ( ItemType >= 0 && ItemType <= 5 )
				{
					...
					if ( p->Width  >= 2)
					{
						p->TwoHand = TRUE; // #warning Review this later
					}
...
BOOL OpenItemScript(char* Buffer, int iSize) // Save Item(kor) Values Into ItemAttribute
{
	...
	while ( true )
	{
		...
		if ( Token == 1 )
		{
			...
			ItemType = (int)WzMemScript.GetNumber();
			loop_count = 0;
			while ( true )
			{
				int ItemSubType;
				...
				Token = WzMemScript.GetToken();
				ItemSubType = (int)WzMemScript.GetNumber();
				...
				Token = WzMemScript.GetToken();
				p->ItemSlot = (int)WzMemScript.GetNumber();

				Token = WzMemScript.GetToken();
				p->SkillType = (int)WzMemScript.GetNumber();

				Token = WzMemScript.GetToken();
				p->Width  = (BYTE)WzMemScript.GetNumber();

				Token = WzMemScript.GetToken();
				p->Height  = (BYTE)WzMemScript.GetNumber();

				Token = WzMemScript.GetToken();
				p->Serial = (char)WzMemScript.GetNumber();

				Token = WzMemScript.GetToken();
				p->OptionFlag = (BYTE)WzMemScript.GetNumber();

				Token = WzMemScript.GetToken();
				p->MondownFlag = (BYTE)WzMemScript.GetNumber();

				Token = WzMemScript.GetToken();
				strcpy_s(p->Name, sizeof(p->Name), WzMemScript.GetString() );

				if ( ItemType >= 0 && ItemType <= 5 )
				{
					Token = WzMemScript.GetToken();
					p->Level = (BYTE)WzMemScript.GetNumber();

					Token = WzMemScript.GetToken();
					p->DamageMin = (BYTE)WzMemScript.GetNumber();

					Token = WzMemScript.GetToken();
					p->DamageMax = (BYTE)WzMemScript.GetNumber();

					Token = WzMemScript.GetToken();
					p->AttackSpeed = (BYTE)WzMemScript.GetNumber();

					Token = WzMemScript.GetToken();
					p->Durability = (BYTE)WzMemScript.GetNumber();

					Token = WzMemScript.GetToken();
					p->MagicDurability = (BYTE)WzMemScript.GetNumber();

					Token = WzMemScript.GetToken();
					p->MagicPW = (int)WzMemScript.GetNumber();

					Token = WzMemScript.GetToken();
					p->RequireLevel = (WORD)WzMemScript.GetNumber();

					Token = WzMemScript.GetToken();
					p->RequireStrength = (WORD)WzMemScript.GetNumber();

					Token = WzMemScript.GetToken();
					p->RequireDexterity = (WORD)WzMemScript.GetNumber();

					Token = WzMemScript.GetToken();
					p->RequireEnergy = (WORD)WzMemScript.GetNumber();

					Token = WzMemScript.GetToken();
					p->RequireVitality = (WORD)WzMemScript.GetNumber();

					Token = WzMemScript.GetToken();
					p->RequireLeadership = (int)WzMemScript.GetNumber();
				...
				if ( ItemType >= 6 && ItemType <= 11 )
				{
					...
					if ( ItemType == 6 )
					{
						...
						Token = WzMemScript.GetToken();
						p->Defense = (BYTE)WzMemScript.GetNumber();

						Token = WzMemScript.GetToken();
						p->SuccessfulBlocking  = (BYTE)WzMemScript.GetNumber();
					...
					else if ( ItemType >= 7 && ItemType <= 9 )
					{
						...
						Token = WzMemScript.GetToken();
						p->MagicDefense = (BYTE)WzMemScript.GetNumber();
					}
					else if ( ItemType == 10 )
					{
						Token = WzMemScript.GetToken();
					...
					else if ( ItemType == 11 )
					{
						...
						Token = WzMemScript.GetToken();
						p->WalkSpeed = (BYTE)WzMemScript.GetNumber();
					}
				...
				if ( ItemType == 13 )
				{
					...
					Token = WzMemScript.GetToken();
					p->Resistance[0] = (BYTE)WzMemScript.GetNumber();

					Token = WzMemScript.GetToken();
					p->Resistance[1] = (BYTE)WzMemScript.GetNumber();

					Token = WzMemScript.GetToken();
					p->Resistance[2] = (BYTE)WzMemScript.GetNumber();

					Token = WzMemScript.GetToken();
					p->Resistance[3] = (BYTE)WzMemScript.GetNumber();

					Token = WzMemScript.GetToken();
					p->Resistance[4] = (BYTE)WzMemScript.GetNumber();

					Token = WzMemScript.GetToken();
					p->Resistance[5] = (BYTE)WzMemScript.GetNumber();

					Token = WzMemScript.GetToken();
					p->Resistance[6] = (BYTE)WzMemScript.GetNumber();
				...
				{
					Token = WzMemScript.GetToken();
					p->Value = (WORD)WzMemScript.GetNumber();
				...
				if ( ItemType == 12 )
				{
					...
					Token = WzMemScript.GetToken();
					p->BuyMoney = (int)WzMemScript.GetNumber();
				...
				p->SetAttr = (BYTE)WzMemScript.GetNumber();

				if ( ItemType <= 13 || ItemType == 15 )
				{

					Token = WzMemScript.GetToken();
					p->RequireClass[0]  = (BYTE)WzMemScript.GetNumber();

					Token = WzMemScript.GetToken();
					p->RequireClass[1]  = (BYTE)WzMemScript.GetNumber();

					Token = WzMemScript.GetToken();
					p->RequireClass[2]  = (BYTE)WzMemScript.GetNumber();

					Token = WzMemScript.GetToken();
					p->RequireClass[3]  = (BYTE)WzMemScript.GetNumber();

					Token = WzMemScript.GetToken();
					p->RequireClass[4]  = (BYTE)WzMemScript.GetNumber();
				}
...
int GetLevelItem(int type, int index, int level)
{
	...
	if ( itemlevel >= level - 18 && itemlevel <= level)
	{
		...
		if ( type == 13 )
		{
			if ( index == 8 || index == 9 || index == 12 || index == 13 || index >= 20 && index <= 28)
			{
				if ( itemlevel > 4 )
				{
					itemlevel=4;
...
int GetSerialItem(int type)
{
	...
	if ( ItemAttribute[type].Level == 0xFF )
	{
		return -1;
	}
	if ( ItemAttribute[type].Serial  == 0 )
	{
		return 0;
	}
	return 1;
