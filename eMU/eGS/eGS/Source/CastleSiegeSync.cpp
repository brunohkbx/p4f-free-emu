...
void CCastleSiegeSync::SetCastleOwnerGuild(std::string GuildName)
{
	if ( GuildName.empty() )
	{
		return;
	...
	this->m_szCastleOwnerGuild = GuildName;
}
...
void CCastleSiegeSync::AddTributeMoney(int iMoney)
{
	...
	if ( (this->m_lCastleTributeMoney + iMoney) > (int)MAX_TRIBUTE_MONEY )
	{
		return;
	}
...
BOOL CCastleSiegeSync::CheckCastleOwnerMember(int iIndex)
{
	...
	if ( this->m_szCastleOwnerGuild.empty() )
	{
		return FALSE;
	}

	if ( this->m_szCastleOwnerGuild.compare(gObj[iIndex].GuildName ) != 0)
	{
		return FALSE;
	}
...
BOOL CCastleSiegeSync::CheckCastleOwnerUnionMember(int iIndex)
{
	...
	if ( this->m_szCastleOwnerGuild.compare(pUnionInfo->m_szMasterGuild) == 0 )
	{
		return TRUE;
	}
