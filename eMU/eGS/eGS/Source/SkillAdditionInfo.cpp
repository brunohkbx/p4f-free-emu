...
BOOL CSkillAdditionInfo::Load(LPSTR lpszFileName)
{
	...
	{
		LogAdd("[Skill Addition Info] - File load error : File Name Error");
		return FALSE;
	}
	...
	LogAdd("[Skill Addition Info] [Infinity Arrow] Skill Time[%d], UseLevel[%d], Arrow MP Consumption +0[%d] +1[%d] +2[%d]",
	...
	LogAdd("[Skill Addition Info] [Fire Scream] Use(%d) : Explosion Distance(%d) - Rate(%d), MaxAttack_SameSerial(%d), SerialInitCount(%d), Check Serial(%d)",
