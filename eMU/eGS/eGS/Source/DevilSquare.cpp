...
void DataSendRank(char* pMsg, int size)
{
	...
	if ( IsDataServerConnected == FALSE )
	{
		wsDataServerCli.DataSend(pMsg, size);
	}
}
...
{
	this->m_bQuit = TRUE;
	WaitForSingleObject((HANDLE)this->m_hThread , INFINITE);
	CloseHandle((HANDLE)this->m_hThread);
}
...
void CDevilSquare::CheckSync()
{
	...
	LogAdd("[DevilSquare] Sync Open Time. [%d]min remain", this->m_iRemainTime);
}
	...
	if ( this->m_iremainTimeSec != -1 )
	{
		...
		if ( lc2 != 0 )
		{
			...
			if (this->m_iremainTimeSec < 1 )
			{
				...
				TNotice::MakeNoticeMsg((TNotice *)&pNotice, 0, "[Devil Square] The gates of Devil Square are open!");
	...
	else if ( this->m_iTime < GetTickCount() )
	{
		...
		if ( this->m_iRemainTime <= 15 )
		{
			if ( (this->m_iRemainTime%5)== 0 )
			{
				...
				TNotice::MakeNoticeMsgEx((TNotice *)&pNotice, 0, "%d minute(s) left before Devil Square opens!", this->m_iRemainTime);
	...
	else
	{
		if ( this->m_iTime < GetTickCount() )
		{
			...
			if ( this->m_iRemainTime > 0 )
			{
				...
				TNotice::MakeNoticeMsgEx(&pNotice, 0, "[Devil Square] will start after %d minute(s)", this->m_iRemainTime);
		...
		if ( this->m_iTime < GetTickCount() )
		{
			...
			if ( this->m_iRemainTime > 0 )
			{
				if ( (this->m_iRemainTime%5) == 0 )
				{
					...
					TNotice::MakeNoticeMsgEx((TNotice *)&pNotice, 0, "[Devil Square] will close after %d minute(s)", this->m_iRemainTime);
					LogAdd((char*)pNotice.Notice);
...
void CDevilSquare::SetMonster()
{
	...
	for ( int n=0;n<gMSetBase.m_Count;n++)
	{
		if ( DS_MAP_RANGE(gMSetBase.m_Mp[n].m_MapNumber) != FALSE )
		{
			...
			if ( btDSIndex == 0xFF )
			{
				LogAdd("[DevilSquare] [%d] Invalid MonterType", wMonIndex);
				continue;
			}
...
void CDevilSquare::gDevilSquareMonsterRegen(LPOBJ lpObj)
{
	...
	{
		LogAdd("[DevilSquare] Invalid DevilSquareIndex [%d]", devilsquareindex);
		return;
	}
	...
	{
		LogAdd("[DevilSquare] [%d] Invalid MonterType", monstertype);
		return;
	}
	...
	LogAdd("[DevilSquare] Monter Regen [%d][%d][%d,%d]",
...
void CDevilSquare::SendEventStartMsg()
{
	...
	TNotice::MakeNoticeMsg(&pToEventer, 0, "[Devil Square] has started.");
	TNotice::MakeNoticeMsg(&pWithOutEvneter, 0, "[Devil Square] is closed. Try again next time.");
	...
	LogAdd("[DevilSquare] Start Event");
}
	...
	for ( int n=OBJ_STARTUSERINDEX;n<OBJMAX;n++)
	{
		if ( gObj[n].Connected == PLAYER_PLAYING )
		{
			if ( DS_MAP_RANGE(gObj[n].MapNumber) != FALSE )
			{
				...
				{
					LogAdd( "[DevilSquare] [%s][%s] Found user in DevilSquare [State:Close]",
