	...
	while ( true )
	{
		...
		if ( Token == NUMBER )
		{
			...
			while ( true )
			{
				...
				Token = (SMDToken)GetToken();
				strcpy_s(p->szOptionName, sizeof(p->szOptionName), TokenString);
	...
	while ( true )
	{
		...
		if ( Token == NUMBER )
		{
			...
			while ( true )
			{
				...
				Token = (SMDToken)GetToken();//имя в файле пропускается, оно чисто в познавательных целях
...
void CJewelOfHarmonySystem::SetEnableToUsePuritySystem(BOOL bEnable)
{
	...
	LogAdd("[JewelOfHarmony][PuritySystem] Enable %d", bEnable);
}
...
void CJewelOfHarmonySystem::PurityJewelOfHarmony(LPOBJ lpObj)
{
	...
	{
		GCServerMsgStringSend("The refinement system is unable to use.", lpObj->m_Index, 1);
		return;
	}
	...
	{
		GCServerMsgStringSend("The refinement system is unable to use.", lpObj->m_Index, 1);
		LogAdd("[JewelOfHarmony][%s][%s] Not Purity Time ", lpObj->AccountID, lpObj->Name);
		return;
	}
	...
	// Chaos Lock was Enabled
		...
		LogAdd("[JewelOfHarmony][Purity] - Mix Start");
		...
		if ( iRate < this->m_iRatePuritySuccess )
		{
			...
			LogAdd("[JewelOfHarmony][Purity] Purity Success [%s][%s] Rate %d/%d",
		...
		else
		{
			...
			LogAdd("[JewelOfHarmony][Purity] Purity Fail [%s][%s] Rate %d/%d",
				lpObj->AccountID, lpObj->Name, iRate, this->m_iRatePuritySuccess);
		}
...
BOOL CJewelOfHarmonySystem::StrengthenItemByJewelOfHarmony(LPOBJ lpObj, int source, int target)
{
	...
	{
		GCServerMsgStringSend("The strengthened system is unable to use.", lpObj->m_Index, 1);
		return FALSE;
	}
	...
	{
		LogAdd("[JewelOfHarmony][Strengten Item] Already Strengtened [%s][%s]",
	...
	{
		GCServerMsgStringSend("The coverall goods are unable to carry on the strengthening.", lpObj->m_Index, 1);
		LogAdd("[JewelOfHarmony][Strengthen Item] SetItem not enable to Strengthened [%s][%s]",
			lpObj->AccountID, lpObj->Name);
	...
	{
		LogAdd("[JewelOfHarmony][Strengthen Item] Strengthen Fail [%s][%s] Name[%s] Type[%d] Serial[%d] Invalid ItemType[%d]",
	...
	{
		LogAdd("[JewelOfHarmony][Strengthen Item] Strengthen Fail - NOT OPTION [%s][%s] Name[%s] Type[%d] Serial[%d] ItemType[%d]",
			lpObj->AccountID, lpObj->Name, pTarget->GetName(), pTarget->m_Type,
			pTarget->m_Number, iItemType);
	...
	int iSuccessRate = Random(0,99);

	if ( iSuccessRate >= this->m_iRateStrengthenSuccess )
	{
		LogAdd("[JewelOfHarmony][Strengthen Item] Strengthen Fail [%s][%s] Name[%s] Type[%d] Serial[%d]  Rate (%d/%d)",
			lpObj->AccountID, lpObj->Name, pTarget->GetName(), pTarget->m_Type,
			pTarget->m_Number, iSuccessRate, this->m_iRateStrengthenSuccess);
		GCServerMsgStringSend("Regeneration gem strengthening defeat.", lpObj->m_Index, 1);
		return TRUE;
	}
	...
	LogAdd("[JewelOfHarmony][Strengthen Item] Strengthen Success [%s][%s] Name[%s] Type[%d] Serial[%d] Rate (%d/%d) Option %d OptionLevel %d",
		lpObj->AccountID, lpObj->Name, pTarget->GetName(), pTarget->m_Type,
		pTarget->m_Number, iSuccessRate, this->m_iRateStrengthenSuccess,
		iItemOption, iItemOptionLevel);

	GCServerMsgStringSend("The regeneration gem strengthens successfully.", lpObj->m_Index, 1);

	gObjMakePreviewCharSet(lpObj->m_Index);

	double levelitemdur = ItemGetDurability(lpObj->pInventory[target].m_Type,
...
BYTE CJewelOfHarmonySystem::ShowStrengthenOption(CItem *pItem)
{
	...
	char buf[256];
	sprintf_s(buf,sizeof(buf), "Иї°ъ : %20s    type:%d, option:%d optionLevel %d value %d \n", 
...
BOOL CJewelOfHarmonySystem::MakeSmeltingStoneItem(LPOBJ lpObj)
{
	...
	{
		GCServerMsgStringSend("The smelting system is unable to use.", lpObj->m_Index, 1);
		return TRUE;
	}
	...
	LogAdd("[JewelOfHarmony][Smelt Item Mix] - Mix Start");
	...
	if ( iRate < JEWEL_OF_HARMONY_MAKE_SMELTINGSTONE_RATE )
	{
		...
		LogAdd("[JewelOfHarmony][Smelt Item Mix] Smelting Stone Normal[%d] Mix Success [%s][%s], Money(%d-%d) Rate(%d/%d)",
	...
	else
	{
		...
		LogAdd("[JewelOfHarmony][Smelt Item Mix] Smelting Stone Normal[%d] Mix Fail [%s][%s], Money : %d-%d Rate(%d/%d)",
			bIsItemNormal, lpObj->AccountID, lpObj->Name,
			lpObj->Money, iMakeSmeltingStoneMixPrice, iRate,
			JEWEL_OF_HARMONY_MAKE_SMELTINGSTONE_RATE);
	}
...
BOOL CJewelOfHarmonySystem::SmeltItemBySmeltingStone(LPOBJ lpObj, int source, int target)
{
	...
	{
		GCServerMsgStringSend("The evolution system is unable to use.", lpObj->m_Index, 1);
		return FALSE;
	}
	...
	{
		LogAdd("[JewelOfHarmony][Smelt Item] Not Strengthen Item [%s][%s]",
			lpObj->AccountID, lpObj->Name);
	...
	if ( iItemOptionLevel >= pTarget->m_Level )
	{
		...
		{
			GCServerMsgStringSend("Already strengthened for the fiercest value, was unable to continue to strengthen.", lpObj->m_Index, 1);
		}
		else
		{
			GCServerMsgStringSend("Because the goods rank too is low, is unable to continue to strengthen.", lpObj->m_Index, 1);
		}

		LogAdd("[JewelOfHarmony][Smelt Item] Already Have Max OptionLevel [%s][%s] OptionLevel [%d] ItemLevel [%d]",
	...
	if ( iSuccessRate >= iRateSmeltingSuccess )
	{
		...
		LogAdd("[JewelOfHarmony][Smelt Item] Smelt Item Fail by Normal[%d] [%s][%s] Name[%s] Type [%d] Serial [%d] Rate(%d/%d) Level(%d->%d)",
		...
		GCServerMsgStringSend("Strengthened evolution gem defeat.", lpObj->m_Index, 1);
	}
	else
	{
		...
		LogAdd("[JewelOfHarmony][Smelt Item] Smelt Item Success by Normal[%d] [%s][%s] Name[%s] Type [%d] Serial [%d] Rate(%d/%d) Level(%d->%d)",
			bIsNormalSmeltingStone, lpObj->AccountID, lpObj->Name, pTarget->GetName(), pTarget->m_Type,
			pTarget->m_Number, iSuccessRate, iRateSmeltingSuccess, iItemOptionLevel, iItemOptionNewLevel & 0x0F);
		GCServerMsgStringSend("The strengthened evolution gem is successful.", lpObj->m_Index, 1);
	}
...
BOOL CJewelOfHarmonySystem::RestoreStrengthenItem(LPOBJ lpObj)
{
	...
	{
		GCServerMsgStringSend("The Items return to original state system is unable to use.", lpObj->m_Index, 1);
		return TRUE;
	}
	...
	LogAdd("[JewelOfHarmony][Restore Item] - Restore Start");

	LogAdd("[JewelOfHarmony][Restore Item] Restore Strengthened Item [%s][%s] Name[%s] ItemType[%d] Serial[%d] OptionType[%d] OptionLevel [%d] Money %d-%d",
...
BOOL CJewelOfHarmonySystem::NpcJewelOfHarmony(LPOBJ lpNpc, LPOBJ lpObj)
{
	...
	if ( lpNpc->Class == 368 )
	{
		...
		{
			GCServerMsgStringSend("The refinement system is unable to use.", lpObj->m_Index, 1);
			LogAdd("[JewelOfHarmony][%s][%s] Not Purity Time ",
				lpObj->AccountID, lpObj->Name);
	...
	else  if ( lpNpc->Class == 369 )
	{
		...
		if ( this->m_bSystemMixSmeltingStone != TRUE )
		{
			ChatTargetSend(lpNpc, "The smelting system is unable to use.", lpObj->m_Index);
			return TRUE;
		}
	}
	else if ( lpNpc->Class == 370 )
	{
		...
		if ( this->m_bSystemRestoreStrengthen != TRUE )
		{
			ChatTargetSend(lpNpc, "The Items return to original state system is unable to use.", lpObj->m_Index);
			return TRUE;
		}
	}
	else
	{
		LogAdd("[JewelOfHarmony][%s][%s] Open Chaos Box Failed NpcType [%d]",
	...
	if ( bCanChaosBox == TRUE )
	{
		...
		LogAdd("[JewelOfHarmony][%s][%s] Open Chaos Box",
			lpObj->AccountID, lpObj->Name);
