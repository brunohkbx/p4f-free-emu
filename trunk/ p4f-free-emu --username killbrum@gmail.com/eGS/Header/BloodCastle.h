...
typedef struct _BLOODCASTLE_BRIDGE
{
	...
	double m_iCastleStatueHealth;	// 168
	double m_iCastleDoorHealth;	// 16C
...
class CBloodCastle
{
	...
	std::vector<int> GMinLvl;
	std::vector<int> GMaxLvl;
	std::vector<int> GDur;
	std::vector<int> GOpt1;
	std::vector<int> GOpt2;
	std::vector<int> GMinOpt3;
	std::vector<int> GMaxOpt3;
	std::vector<int> GMinExcel;
	std::vector<int> GMaxExcel;
	std::vector<int> GAncient;
	std::vector<int> GDropRate;
...
public:
	...
	void DropChaosGem(int iIndex, int iBridgeIndex);
	...
	void SendNoticeMessage(int iBridgeIndex, std::string MSG);
	...
	BOOL LoadFinalGift(std::string filename );
...
public:
	...
	int g_iBC_ChoasMixSuccessRate[MAX_BLOOD_CASTLE_LEVEL];
	int g_iBC_ChoasMixMoney[MAX_BLOOD_CASTLE_LEVEL];
