		...
		else
		{
			...
			if ( mana > lpObj->Mana )
			{
				...
				while ( true )
				{
					...
					{
						LogAdd("ЎЪ[CHECK_LOG_INFINITY] gObjIsConnected() error %s %d", __FILE__, __LINE__);
						break;
					}

					if ( count > 100 )
					{
						LogAdd("ЎЪ[CHECK_LOG_INFINITY] ( _count > 100 ) error %s %d", __FILE__, __LINE__);
						break;
					}
...
bool gObjBotInit(GOBJ_BOT_INIT bot)
{
	...
	gObj[bot.aIndex].m_RecallMon = -1;//это респаун для мобов
	//---
	gObj[bot.aIndex].bot=true;
	gObj[bot.aIndex].m_Change=0; //для корректного телепорта
	gObj[bot.aIndex].Dir=1; // для перемещения
	//---
	gObj[bot.aIndex].Connected = PLAYER_LOGGED;//чтобы вызвать gObjSetCharacter надо чтобы игрок логинился
	//---
	...
	gObj[bot.aIndex].RegenMapX = (BYTE)gObj[bot.aIndex].X;//координаты появления персонажа после смерти
	gObj[bot.aIndex].RegenMapY = (BYTE)gObj[bot.aIndex].Y;
	gObj[bot.aIndex].RegenOk=0;
	gObj[bot.aIndex].ChatLimitTime=0;//лиимт на написание в чат. Каждая еденица значит бан в чате на 1 минуту
	//---
...
void gObjBotChat(char* msg,int aIndex)
{
	...
	Chat.h.size=14+strlen(msg);//14 видимо длинна аккаунта... короче первый символ начинается с 15... 
	Chat.h.c=193;//при чате он всегда равен 193
	Chat.h.headcode=0;//аналогично этот равен 0
...
int gObjSetCharacter(SDHP_DBCHAR_INFORESULT *lpMsg, int aIndex)
{
	...
	{
		LogAdd( lMsg.Get(MSGGET(1, 242)), gObj[aIndex].AccountID, gObj[aIndex].Name);
		return FALSE;
	}
	...
	lpObj->m_State = 1;//юзверь в любом случае уже находится в городе так что нет смысла проверять что либо...

	if ( (lpMsg->CtlCode & 8) == 8 )
	{
		lpObj->Authority = 2;
		LogAdd( lMsg.Get(MSGGET(1, 245)), lpObj->AccountID, lpObj->Name);
	...
	LogAdd(lMsg.Get(MSGGET(1, 246)), lpObj->m_Index, lpObj->AccountID, lpObj->Name);
	...
	{
		PMSG_NOTICE pNotice;
		TNotice::MakeNoticeMsgEx((TNotice *)&pNotice, 1, "ѕшґВ ѕЖАМЕЫ №Я°Я !!!");	// Deathway need Translation
...
void gObjMagicTextSave(LPOBJ lpObj)
{
	...
	for ( int n=0;n<MAGIC_SIZE;n++)
	{
		...
		{
			LogAdd(lMsg.Get(MSGGET(1, 247)), lpObj->AccountID, lpObj->Name, n, lpObj->Magic[n].m_Skill);
			total++;
		}
	}
}
...
void gObjItemTextSave(LPOBJ lpObj)
{
	...
	for ( n=0;n<INVENTORY_SIZE;n++)
	{
		if ( lpObj->pInventory[n].IsItem() == TRUE )
		{
			if ( lpObj->pInventory[n].m_serial != 0 )
			{
				...
				LogAdd(lMsg.Get(MSGGET(1, 248)), lpObj->AccountID, lpObj->Name, n, lpObj->pInventory[n].GetName(),
	...
	LogAdd(lMsg.Get(MSGGET(1, 249)), lpObj->AccountID, lpObj->Name, lpObj->Money);
}
...
void gObjWarehouseTextSave(LPOBJ lpObj)
{
	...
	for ( n=0;n<WAREHOUSE_SIZE;n++)
	{
		if ( lpObj->pWarehouse[n].IsItem() == TRUE )
		{
			if ( lpObj->pWarehouse[n].m_serial != 0 )
			{
				...
				LogAdd(lMsg.Get(MSGGET(1, 250)), lpObj->AccountID, lpObj->Name, n, lpObj->pWarehouse[n].GetName(),
	...
	LogAdd(lMsg.Get(MSGGET(1, 251)), lpObj->AccountID, lpObj->Name, lpObj->WarehouseMoney);
}
...
int gObjAddCallMon()//вертає позицію в яку можна добавити монстра(і ставить цій позиції PLAYER_CONNECTED)
{
	int count;
	int totalcount = 0;
...
BOOL gObjGameClose(int aIndex)
{
	...
	if ( lpObj->Type != OBJ_USER )
	{
		LogAdd(lMsg.Get(MSGGET(2, 3)), lpObj->AccountID, lpObj->Name);
		return FALSE;
	}
	...
	gObjViewportClose(lpObj);
	LogAdd(lMsg.Get(MSGGET(2, 4)), lpObj->m_Index, lpObj->AccountID, lpObj->Name);
...
BOOL gObjCheckXYMapTile(LPOBJ lpObj, int iDbgName)
{
	...
	//#info Fix what elf mast start in noria
// 	if ( lpObj->Class == 2 && lpObj->Level < 10 )
// 	{
// 		if ( lpObj->MapNumber != MAP_INDEX_NORIA )
// 		{
// 			LogAdd(lMsg.Get(MSGGET(2, 5)), lpObj->Name, lpObj->MapNumber, lpObj->X, lpObj->Y);
// 			PMSG_TELEPORT pMsg;
// 			pMsg.MoveNumber = 27;
...
BOOL gObjSetAccountLogin(int aIndex, char * szId, int aUserNumber, int aDBNumber, char* JoominNumber)
{
	...
	if ( gObj[aIndex].Connected != PLAYER_CONNECTED )
	{
		LogAdd(lMsg.Get(MSGGET(2, 6)), aIndex, szId, aDBNumber, __FILE__, __LINE__);
		return 0;
	}
...
bool gObjLevelUp(LPOBJ lpObj, int addexp, int iMonsterType, int iEventType)
{
	...
	LogAdd(lMsg.Get(MSGGET(2, 8)), lpObj->AccountID, lpObj->Name, lpObj->Level);
...
BOOL gObjLevelUpPointAdd(BYTE type, LPOBJ lpObj)	// type : [1:Strength][2:Agility][3:Vitality][4:Energy][5:Leadership]
{
	...
	{
		LogAdd( lMsg.Get(MSGGET(2, 9)), lpObj->AccountID, lpObj->Name, __FILE__, __LINE__);
		return FALSE;
	}
...
void gObjPlayerKiller(LPOBJ lpObj, LPOBJ lpTargetObj)
{
	...
	if ( gObjGetRelationShip(lpObj, lpTargetObj) == 2 )
	{
		...
		{
			LogAdd("[U.System][Rival][Player Kill][ЎЪERROR : Can't find GuildInfo] (  [%s][%s] ) vs ( [%s][%s] )",
...
void gObjUserDie(LPOBJ lpObj, LPOBJ lpTargetObj)
{
	...
	if(lpObj->Money > 0)
	{
		...
		LogAdd(lMsg.Get(574),lpObj->AccountID,lpObj->Name,lpObj->Money,submoney);
	}
	...
	if(lpObj->WarehouseMoney > 0)
	{
		...
		LogAdd(lMsg.Get(575),lpObj->AccountID,lpObj->Name,lpObj->WarehouseMoney,submoney);
	}
...
void gObjSpriteDamage(LPOBJ lpObj, int damage)
{
	...
	else if ( sprite->m_Type == ITEMGET(13,4) )
	{
		...
		if ( sprite->m_DurabilitySmall > 1500 )
		{
			...
			if ( sprite->m_Durability < 1.0f )
			{
				...
				LogAdd("[%s][%s] ґЩЕ©ИЈЅєАЗ»Ф Item is Broken because durability is exhausted [%d]",
	...
	if ( send_dur != FALSE )
	{
		...
		if ( sprite->m_Durability < 1.0f )
		{
			...
			LogAdd(lMsg.Get(MSGGET(2, 10)), lpObj->AccountID, lpObj->Name,
...
void gObjChangeDurProc(LPOBJ lpObj)
{
	...
	if(lpObj->pInventory[10].IsItem() == 1 && lpObj->pInventory[10].m_Type == ITEMGET(13,10))
	{
		...
		if(m_Durability > (int)lpObj->pInventory[10].m_Durability)
		{
			...
			if(dur == 0)
			{
				...
				LogAdd(lMsg.Get(544),lpObj->AccountID,lpObj->Name,lpObj->pInventory[10].GetName(),lpObj->pInventory[10].m_Level,dur);
	...
	else if(lpObj->pInventory[11].IsItem() == 1 && lpObj->pInventory[11].m_Type == ITEMGET(13,10))
	{
		...
		if(m_Durability > (int)lpObj->pInventory[11].m_Durability)
		{
			...
			{
				gObjUseSkill.SkillChangeUse(lpObj->m_Index);
				LogAdd(lMsg.Get(544),lpObj->AccountID,lpObj->Name,lpObj->pInventory[10].GetName(),lpObj->pInventory[11].m_Level,dur);
...
BOOL gObjInventorySearchSerialNumber(LPOBJ lpObj, DWORD serial)
{
	...
	for(int n = 0; n < INVENTORY_SIZE; n++)
	{
		...
		if(s_num != 0 && s_num == serial)
		{
			LogAdd(lMsg.Get(529),lpObj->AccountID,lpObj->Name,lpObj->pInventory[n].GetName(),n,s_num);
...
int s_num;
	...
	for(int n = 0; n < WAREHOUSE_SIZE; n++)
	{
		...
		if(s_num != 0 && s_num == sirial)
		{
			LogAdd(lMsg.Get(530),lpObj->AccountID,lpObj->Name,lpObj->pWarehouse[n].GetName(),n,s_num);
			GCServerMsgStringSend(lMsg.Get(1102),lpObj->m_Index,1);
			gObjUserKill(lpObj->m_Index);
		}
	}
...
BYTE gObjInventoryMoveItem(int aIndex, unsigned char source, unsigned char target, int& durSsend, int& durTsend, unsigned char sFlag, unsigned char tFlag, unsigned char* siteminfo)
{
	...
	__try
	{
		...
		{
			LogAdd(lMsg.Get(533),type,level,op1,op2,op3,sitem->m_Type,sitem->m_Level,sitem->m_Option1,sitem->m_Option2,sitem->m_Option3);
			return -1;
		}
...
void gObjTradeCancel(int aIndex)
{
	...
	LogAdd(lMsg.Get(514),gObj[aIndex].AccountID,gObj[aIndex].Name);
}
...
void gObjTradeOkButton(int aIndex)
{
	...
	if(result == 1)
	{
		...
		LogAdd(lMsg.Get(540),gObj[aIndex].AccountID,gObj[aIndex].Name,gObj[number].AccountID,gObj[number].Name,gObj[aIndex].TradeMoney,gObj[number].TradeMoney);
			...
			lpObj->Connected == PLAYER_PLAYING)
		{
			if(lpObj->Type == OBJ_USER)
			{
				if(lpObj->Connected >= PLAYER_LOGGED)
				{
					if(GetTickCount() - lpObj->ConnectCheckTime > 60000)
					{
						if (!gObj[n].bot)
						{
							...
							LogAdd(lMsg.Get(542),lpObj->m_Index,lpObj->AccountID,lpObj->Name,lpObj->Ip_addr);
						}
				...
				else
				{
					...
					{
						ResponErrorCloseClient(n);
						LogAdd(lMsg.Get(543),lpObj->m_Index,lpObj->AccountID,lpObj->Name,lpObj->Ip_addr);
					}
				}
			}
		}
			...
			if(lpObj->VpPlayer[n].state == 1)
			{
				...
				if(tObjNum >= 0)
				{
					...
					case 1:
						...
						else//#error раньше было if (false)
						{
...
BOOL gObjItemLevelUp(LPOBJ lpObj, int source, int target)
{
	...
	LogAdd(lMsg.Get(556),lpObj->AccountID,lpObj->Name,lpObj->pInventory[target].GetName(),lpObj->pInventory[target].m_Number,lpObj->pInventory[target].m_Level);
	...
	LogAdd(lMsg.Get(557),lpObj->AccountID,lpObj->Name,lpObj->pInventory[source].m_Number,
...
BOOL gObjItemRandomLevelUp(LPOBJ lpObj, int source, int target)
{
	...
	LogAdd(lMsg.Get(559),lpObj->AccountID,lpObj->Name,lpObj->pInventory[source].m_Number,
			lpObj->pInventory[target].GetName(),lpObj->pInventory[target].m_Number,lpObj->pInventory[target].m_Level);
...
BOOL gObjItemRandomOption3Up(LPOBJ lpObj, int source, int target)
{
	...
	LogAdd(lMsg.Get(577),lpObj->AccountID,lpObj->Name,lpObj->pInventory[source].m_Number,
...
BOOL gObjGuildWarCheck(LPOBJ lpObj, LPOBJ lpTargetObj)
{
	...
	int score = 1;//1 поінт за просте вбивство

	if(strcmp(lpTargetObj->Name,lpTargetObj->lpGuild->Names[0])==0)
	{
		score = 2;// 2 поінта за вбивство ворожого ГМа
	}
...
{
	if(lpObj->GuildNumber < 1)//перевірка чи взагалі існує гільдія
	{
		return false;
	}

	if(strcmp(lpObj->lpGuild->Names[0],lpObj->Name))//перевірка чи це ГМ
	{
		return false;
	}
...
BOOL gObjMonsterCall(int aIndex, int MonsterType, int x, int y)
{
	...
	if(result >= 0)
	{
		...
		gObjSetMonster(result,MonsterType);//створюємо монстра
...
int  gObjGetItemCountInEquipment(int aIndex, int itemtype, int itemindex, int itemlevel) //повертає к-сть екземплярів вказаної речі на персонажі
{
	if(!OBJMAX_RANGE(aIndex))
	{
		return 0;
...
int  gObjGetItemCountInIventory(int aIndex, int itemtype, int itemindex, int itemlevel)//повертає к-сть екземплярів вказаної речі в інвентарі
{
	if(!OBJMAX_RANGE(aIndex))
	{
		return 0;
...
int gObjGetItemCountInIventory(int aIndex, int itemnum)//повертає к-сть екземплярів вказаної речі в інвентарі
{
	if(!OBJMAX_RANGE(aIndex))
	{
		return 0;
...
int  gObjGetManaItemPos(int aIndex)	//Знаходження в інвентарі бутилок манни повертає номер позиції
{
	if(!OBJMAX_RANGE(aIndex))
	{
		return -1;
...
void gObjDelteItemCountInInventory(int aIndex, int itemtype, int itemindex, int count)//видалення речей з інвентаря
{
	if(!OBJMAX_RANGE(aIndex))
	{
		return;
...
void gObjGetStatPointState(int aIndex, short& AddPoint, short& MaxAddPoint, short& MinusPoint, short& MaxMinusPoint)//находження поінтів для фруктів
{
	if(!OBJMAX_RANGE(aIndex))
	{
		return;
...
BOOL gObjCheckStatPointUp(int aIndex)//перевірка чи можна додати "стати" до персонажу
{
	if(!OBJMAX_RANGE(aIndex))
	{
		return false;
...
BOOL gObjCheckStatPointDown(int aIndex)//перевірка чи можна відняти "стати" від персонажа
{
	if(!OBJMAX_RANGE(aIndex))
	{
		return false;
...
void gObjUsePlusStatFruit(int aIndex,int pos)//використання фрукта на плюс для статів
{


	//загальна перевірка на валідність гравця
	if(!OBJMAX_RANGE(aIndex))
	{
		return;
	}
	...
	if(lpObj->Level < FruitPlusMinUseLevel || iItemLevel > 4)//перевірка на мінімальний рівень користувача або на "заточку" самого фрукта
	{
		pResult.result = 5;
	...
	if(iItemLevel == 4 && lpObj->Class != 4)//якщо фрукт на комманд але класс персонажа не ДЛ
	{
		pResult.result = 5;
		DataSend(aIndex,(unsigned char *)&pResult,pResult.h.size);
		return;
	...
	/*for(int iItemIndex = 0; iItemIndex < INVETORY_WEAR_SIZE; iItemIndex++)//бред... шукаємо любу шмотку на персонажі, і якщо є шмотка то не юзаєм фрукт
	{
...
void gObjUseMinusStatFruit(int aIndex, int pos)//використання фрукта для мінусування статів
{
	//загальна перевірка на валідність гравця
	if(!OBJMAX_RANGE(aIndex))
	{
	...
	if(lpObj->Level < FruitMinusMinUseLevel || iItemLevel > 4)//перевірка на мінімальний рівень користувача або на "заточку" самого фрукта
	{
		pResult.result = 5;
		DataSend(aIndex,(unsigned char *)&pResult,pResult.h.size);
		return;
...
void gObjDelayLifeCheck(int aIndex)//перевірка життя гравця
{
	LPOBJ lpObj = &gObj[aIndex];
	...
	if(lpObj->m_CheckLifeTime > 0)
	{
		...
		if(lpObj->m_CheckLifeTime <= 0)
		{
			...
			if(lpObj->lpAttackObj != 0)
			{
				gObjLifeCheck(lpObj,lpObj->lpAttackObj,0,1,0,0,0,0);//якщо гравця хтось атакує перевіряємо життя (чи не вмер)
			}
		}
	}
}
...
BOOL gObjDuelCheck(LPOBJ lpObj)//перевірка чи данний гравець находиться в дуелі
{
	BOOL bRetVal = FALSE;
...
BOOL gObjDuelCheck(LPOBJ lpObj, LPOBJ lpTargetObj)//перевірка чи 2 гравця находяться в дуелі
{
	BOOL bRetVal = FALSE;
...
void gObjResetDuel(LPOBJ lpObj)//занулення параметрів для дуелі
{
	...
	if ( OBJMAX_RANGE(iDuelIndex))
	{
		lpObj->m_iDuelUser = -1; //встановлення дефолтного індексу користувача
		lpObj->m_iDuelUserReserved = -1;
		lpObj->m_btDuelScore = 0;//очки
		lpObj->m_iDuelTickCount = 0;//час останнього вбивства
		gDarkSpirit[lpObj->m_Index].ReSetTarget(iDuelIndex);//занулення ворони
		gDarkSpirit[iDuelIndex].ReSetTarget(lpObj->m_Index);//ті ж самі параметри і для іншого "дуелянта"
	...
	for(int i = OBJ_STARTUSERINDEX; i < OBJMAX; i++)
	{
		if(gObj[i].Connected == PLAYER_PLAYING)
		{
			if(gObj[i].Type == OBJ_USER)
			{
				...
				{
					if(GetTickCount() - gObj[i].m_iDuelTickCount > 60000) //якщо на протязі 1 хв ніхто нікого не вбив то дуель завершується
					{
...
void gObjSetExpPetItem(int aIndex, int exp)
{
	...
	if(lpObj->pInventory[8].m_Type == ITEMGET(13,4) && lpObj->pInventory[1].m_Type == ITEMGET(13,5)) //если надет конь и ворон то они получают в 2 раза больше экспы
	{
		int addexp = exp / 10;
...
void gObjWeaponDurDownInCastle(LPOBJ lpObj, LPOBJ lpTargetObj, int iDecValue)
{
	...
	case 3:
	case 4:
		if(Right->m_Type >= ITEMGET(0,0) && Right->m_Type < ITEMGET(4,0) //всё оружие до луков
	...
	if(Left->m_Type >= ITEMGET(4,0) && Left->m_Type < ITEMGET(4,7)//все луки
	...
	else if(Right->m_Type >= 0 && Right->m_Type < ITEMGET(6,0))//вообще любое оружие в правой руке
	{
		bIsRightDurDown = true;
	}
...
void Refresh(std::string &TableName, std::string &TableTime,char* Name)
{
	...
	SYSTEMTIME m_time;//мінімальний час
	SYSTEMTIME t;//ще одна змінна для перевірки часу
	std::vector<int> i_time;//змінна для зчитування часу
	int element=0;//який елемент необхідно замінити (тобто найменьший час)
	...
	for (int i=0; i < (int)time.size(); i++)
	{
		...
		if (i_time.size()!=6) //обовязково має бути 6 елементів інакше зрівняння буде неможливим
			return;
		t.wDay=i_time[0];
		t.wMonth=i_time[1];
		t.wYear=i_time[2];
		...
		if ( DataEcval(m_time,t) == 1)//1 - перший більший, значить 2ий меньший
		{
			m_time=t;
			element=i;
		}
	...
	GetLocalTime(&t);//t-більше не потрібен можем використовувати в інших цілях
