...
void CKanturuEntranceNPC::NotifyResultEnterKanturuBossMap(int iIndex)
{
	...
	if ( iResult == 0 )
	{
		...
		if ( iCurrentState == KANTURU_STATE_BATTLE_OF_MAYA )
		{
			...
			if ( bMoveGateSuccess ==TRUE )
			{
				...
				LogAdd("[ KANTURU ][ Battle User ] Add User [%s][%s] Current Battle User:%d State:[%d-%d] (CharInfoSave : Class=%d, Level=%d, LvPoint=%d, Exp=%u, Str=%d, Dex=%d, Vit=%d, Energy=%d, LeaderShip=%d, Map=%d, Pk=%d",
		...
		{
			LogAdd("[ KANTURU ][ BossMapMove ] Error - State(%d) Character[%s][%s]",
	...
	else
	{
		...
		LogAdd("[ KANTURU ][ BossMapMove ] Fail - Result(%d) Character[%s][%s]",
