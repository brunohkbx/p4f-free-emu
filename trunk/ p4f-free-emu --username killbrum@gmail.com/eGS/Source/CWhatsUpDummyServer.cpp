...
CWhatsUpDummyServer::~CWhatsUpDummyServer()
{
	...
	{
		shutdown(this->m_sckDUMMY, SD_BOTH);
...
int CWhatsUpDummyServer::Start(HWND hWnd, WORD wPort)
{
	...
	if ( (this->m_sckDUMMY=socket(AF_INET, SOCK_STREAM, IPPROTO_IP))==INVALID_SOCKET)
	{
		return 0;
	}
	...
	if (bind(this->m_sckDUMMY , (sockaddr*)&sockinfo,sizeof(sockinfo))!=0)
	{
		closesocket(this->m_sckDUMMY);
	...
	if (listen(this->m_sckDUMMY, SOMAXCONN )!=0)
	{
		closesocket(this->m_sckDUMMY);
		this->m_sckDUMMY = INVALID_SOCKET;
		return 0;
	...
	if (WSAAsyncSelect(this->m_sckDUMMY , hWnd, WM_GM_CONNECT_SERVER_MSG, FD_ACCEPT)!=0)
	{
		closesocket(this->m_sckDUMMY);
		this->m_sckDUMMY = INVALID_SOCKET;
		return 0;
				...
				case FD_ACCEPT:
					{
						...
						if ( (sckTemp = accept(wParam & 0xFFFF & 0xFFFF, (sockaddr *)&sockinfo, &isockinfolen))==INVALID_SOCKET)
						{
							return FALSE;
						}

						if (WSAAsyncSelect(sckTemp, hWnd, WM_GM_CONNECT_SERVER_MSG, FD_READ|FD_CLOSE)!=0)
						{
							closesocket(sckTemp);
				...
				case FD_READ:
					{
						...
						if ( recv(wParam & 0xFFFF & 0xFFFF, cBUFFER, sizeof(cBUFFER), 0) <= 0 )
						{
