...
BOOL TMonsterAIGroup::LoadData(LPSTR lpszFileName)
{
	...
	try
	{
		...
		LogAdd( "[Monster AI Group ] - %s file is Loaded", lpszFileName);
...
void TMonsterAIGroup::ChangeAIOrder(int iGroupNumber, int iAIOrder)
{
	...
	{
		LogAdd("[Monster AI Group] ChangeAIOrder() Error - (GroupNumber=%d)", iGroupNumber);
		return;
	}
...
int TMonsterAIGroup::FindGroupLeader(int iGroupNumber)
{
	if ( iGroupNumber < 0 || iGroupNumber >= MAX_MONSTER_AI_GROUP )
	{
		LogAdd("[Monster AI Group] FindGroupLeader() Error - (GroupNumber=%d)", iGroupNumber);
		return -1;	// #error Change this to -1
	}
...
int TMonsterAIGroup::FindGroupMemberObjectIndex(int iGroupNumber, int iGuid)
{
	if ( iGroupNumber < 0 || iGroupNumber >= MAX_MONSTER_AI_GROUP )
	{
		LogAdd("[Monster AI Group] FindGroupMemberObjectIndex() Error - (GroupNumber=%d Guid=%d)", iGroupNumber, iGuid);
		return -1;	// #error Change this to -1
	}
...
TMonsterAIGroupMember * TMonsterAIGroup::FindGroupMember(int iGroupNumber, int iGuid)
{
	if ( iGroupNumber < 0 || iGroupNumber >= MAX_MONSTER_AI_GROUP )
	{
		LogAdd("[Monster AI Group] FindGroupMember() Error - (GroupNumber=%d)(Guid=%d) ", iGroupNumber, iGuid);
		return NULL;
	}
