...
void JGPAccountRequest(SDHP_IDPASSRESULT * lpMsg)
{
	...
	switch ( lpMsg->result  )
	{
		...
		case 15:
			...
			{	
				LogAdd(lMsg.Get(MSGGET(1, 211)), gObj[aIndex].Ip_addr, szId, lpMsg->UserNumber , lpMsg->DBNumber );
			}

			if ( lpMsg->UserNumber  == 0 && lpMsg->DBNumber  == 0)
			{
				LogAdd(lMsg.Get(MSGGET(1, 212)), szId, lpMsg->UserNumber , lpMsg->DBNumber );
			}
	}
...
void GJPAccountFail(short number, LPSTR Id, int DBNumber, int UserNumber)
{
	...
	strcpy_s(pMsg.Id , sizeof(pMsg.Id), Id);
...
void AllNoticeRecv( SDHP_NOTICE * lpMsg)
{
	...
	LogAdd("[Mu Manager][Notice Log][Type:ALL] %s", szNotice);
...
void UserNoticeRecv( SDHP_USER_NOTICE * lpMsg)
{
	...
	LogAdd("[Mu Manager][Notice Log][Type:USER] (%s) %s", szId, szNotice);
...
void GJReqMapSvrMove(int iIndex, WORD wDesMapSvrCode, WORD wMapNumber, BYTE btX, BYTE btY)
{
	...
	{
		LogAdd( "[MapServerMng] Packet Error GJ [0x7A] - User not Connected : %d", iIndex);
	}
	else
	{
...
void JGAnsMapSvrMove(PMSG_ANS_MAPSVRMOVE * lpMsg)
{
	...
	{
		LogAdd( "[MapServerMng] Packet Error JG [0x7A] - User not Connected : %d", lpMsg->iIndex);
		return;
	}
	...
	{
		LogAdd( "[MapServerMng] Packet Error JG [0x7A] - User Info Mismatch [%s][%s]-[%s][%s] (%d)", lpMsg->szAccountID, lpMsg->szCharName, lpObj->AccountID, lpObj->Name, lpObj->m_Index);
		return;
	}
	...
	if ( lpMsg->iResult == 0	 )
	{
		...
		if ( bRESULT == FALSE )
		{
			LogAdd( "[MapServerMng] Packet Error JG [0x7A] - Failed to find Map Info [%s]-[%s] (%d)",
		...
		LogAdd("[MapServerMng] [%s][%s] JG [0x7A] - Move Map Request OK : MAPSVR_D:%d",
			lpObj->AccountID, lpObj->Name, lpMsg->wDstMapSvrCode);
	}
	else
	{
		...
		LogAdd( "[MapServerMng] Packet Error JG [0x7A] - Failed to Authorize User [%s][%s] Result:%d",
...
void JGAnsMapSvrAuth(PMSG_ANS_MAPSVRAUTH * lpMsg)
{
	...
	switch ( lpMsg->iResult )
	{
		case 0:
			...
			{
				LogAdd(lMsg.Get(MSGGET(1, 211)), gObj[iIndex].Ip_addr, szId, lpMsg->iUserNumber, lpMsg->iDBNumber);
				iSendResult = 1;
				bAuthOK = TRUE;
			}
			...
			{
				LogAdd(lMsg.Get(MSGGET(1, 212)), szId, lpMsg->iUserNumber, lpMsg->iDBNumber);
			}
