...
void CRingMonsterHerd::SendEventGiftWinner(int iIndex, int iGiftKind)
{
	...
	LogAdd("[Ring Event] [%s][%s] Request to Register Gift - Gift Kind (%d)",
...
void CRingAttackEvent::StartEvent()
{
	...
	for ( int i=0;i<MAX_RINGMONSTER_MAP;i++)
	{
		...
		while ( iCount-- != 0 )
		{
			iRandX = Random(g_RingMapPosition[i][0],g_RingMapPosition[i][2]);
			iRandY = Random(g_RingMapPosition[i][1],g_RingMapPosition[i][3]);
		...
		else
		{
			...
			{
				LogAdd("[Ring Event] - Error : No Monster Data Exist");
				continue;
			}

			LogAdd("[Ring Event] - Monster Start Position MapNumber:%d, X:%d, Y:%d",
...
void CRingAttackEvent::CheckSync()
{
	...
	{
		LogAdd( "[Ring Event] No Schedule Time Data");
		this->SetState(0);
	...
	LogAdd("[Ring Event] Sync Start Time. [%d] min remain (START HOUR:%d, MIN:%d)",
...
void CRingAttackEvent::ProcState_Playing()
{
	...
	if ( ((this->m_iTIME_MSEC_REMAIN / 1000 ) % 30 ) == 0 && ((this->m_iTIME_MSEC_REMAIN / 1000) / 30) != s_iTIME_REMAIN  )
	{
		...
		for ( int i=0;i<MAX_RINGMONSTER_MAP;i++)
		{
			...
			LogAdd("[Ring Event] - per 30'S Inform MapNumber:%d, X:%d, Y:%d, Boss Live:%d",
	...
	if ( this->m_iTIME_MSEC_REMAIN <= (this->m_iTIME_MONSTER_TO_DEST * 60 * 1000) && this->m_bMonsterToDest == FALSE )
	{
		...
		for ( int i=0;i<MAX_RINGMONSTER_MAP;i++)
		{
			...
			{
				LogAdd("[Ring Event] - Monster Herd Move MapNumber:%d, X:%d, Y:%d",
					g_RingEventMapNum[i], iRandX, iRandY);
			}
		}
...
void CRingAttackEvent::Move()
{
	if ( this->m_bDoEvent )
	{
		...
		for ( int i=0;i<MAX_RINGMONSTER_MAP;i++)
		{
			...
			while ( iCount-- != 0 )
			{
				iRandX = Random(0,this->m_iMOVE_RAND_SIZE-1) * ( (Random(0,2) -1) * -1 ) + iCX;
				iRandY = Random(0,this->m_iMOVE_RAND_SIZE-1) * ( (Random(0,2) -1) * -1 ) + iCY;
			...
			this->m_RingMonsterHerd[i].SetRadius(Random(this->m_iRADIUS_MIN,this->m_iRADIUS_MAX));
		}
	}
}
...
void CRingAttackEvent::Start_Menual()
{
	...
	LogAdd("¡Û¡Ü[Event Management] [Start] RingAttack Event!");
