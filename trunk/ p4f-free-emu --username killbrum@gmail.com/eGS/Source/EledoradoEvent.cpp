	...
	for (int n=0;n<OBJ_MAXMONSTER;n++)
	{
		if ( gObj[n].Class == 79 )	// Gold Dercon
		{
			...
			LogAdd("Make GoldDercon : %d, %d,%d",
				...
				{
					case 0:
						strcat_s(szTemp, lMsg.Get(MSGGET(7, 208)));
						break;
					case 2:
						strcat_s(szTemp, lMsg.Get(MSGGET(7, 210)));
						break;
					case 3:
						strcat_s(szTemp, lMsg.Get(MSGGET(7, 211)));
						break;
				}

				strcat_s(szTemp, lMsg.Get(MSGGET(4, 134)));
