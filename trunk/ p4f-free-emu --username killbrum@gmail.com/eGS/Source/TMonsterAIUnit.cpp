...
BOOL TMonsterAIUnit::LoadData(LPSTR lpszFileName)
{
	...
	try
	{
		...
		LogAdd( "[Monster AI Unit] - %s file is Loaded", lpszFileName);
...
TMonsterAIUnit * TMonsterAIUnit::FindAIUnit(int iUnitNumber)
{
	if ( iUnitNumber < 0 || iUnitNumber >= MAX_MONSTER_AI_UNIT )
	{
		LogAdd("[Monster AI Unit] FindAIUnit() Error - (UnitNumber=%d) ",
			iUnitNumber);
...
BOOL TMonsterAIUnit::RunAIUnit(int iIndex)
{
	...
	if ( int(GetTickCount() - lpObj->m_iLastAutomataRuntime) < lpObj->m_iLastAutomataDelay )
		return FALSE;
