...
static SMDToken GetToken()
{
	...
	case '9':	//9
		...
		while (((ch=(char)getc(SMDFile)) != -1) && ((ch == 0x2E) || (isdigit(ch) != 0) || (ch == 0x2D)))  // 2e '.'  2D '-'
		{
	...
	case '\"':	// "	String Case
		...
		while (((ch=(char)getc(SMDFile)) != -1 ) && (ch != 0x22))	// nice
		{
			*p = ch;
			p++;
		}
	...
	default:	// Others
		if (isalpha(ch))
		{
			...
			*p=ch;
			...
			while ( ((ch=(char)getc(SMDFile)) != -1) && ( (ch == 0x2E) || (ch == 0x5F) || (isalnum(ch) != 0) ) )
			{
				*p=ch;
				p++;
