...
//const int MAX_PET_LEVEL=GetPrivateProfileInt("GameServerInfo","MaxPetLevel", 100, "NewConfigs.ini");
const int MAX_HORSE_LEVEL=GetPrivateProfileInt("GameServerInfo", "MaxHorseLevel", 50, "..\\data\\commonserver.cfg");
const int MAX_RAVEN_LEVEL=GetPrivateProfileInt("GameServerInfo", "MaxRavenLevel", 50, "..\\data\\commonserver.cfg");
...
typedef struct
{
	...
	int BuyMoney; // 4C
	int MagicPW; // 50
	double RepaireMoneyRate;	// 54
	double AllRepaireMoneyRate; // 58
	bool QuestItem;	// 5C
	BYTE SetAttr; // 5D
...
public:
	...
	void SetItemPlusSpecialStat(unsigned short* Value, int Special);
	int GetWeaponType();
	...
	WORD m_Magic;	// 20
	double m_Durability; // 24
	WORD m_DurabilitySmall; // 28
	double m_BaseDurability;	// 2C
	...
	double m_DurabilityState[4];	// 7C
	double m_CurrentDurabilityState;	// 8C
...
public:
	...
	std::vector<DWORD> m_DarkSpiritExpTable;
	std::vector<DWORD> m_DarkHorseExpTable;
