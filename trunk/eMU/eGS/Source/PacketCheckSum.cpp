...
{
	if ( gObj[aIndex].m_InWebzen == true )	// Only for Local Test and Test Mains
	{
		return;
	}
	...
	for ( int i=0 ; i<MAX_PACKET_CHECKSUM_FUNCTION_INDEX ; i++)
	{
		if ( this->PaketChecksum[aIndex].Check[i] == 0 )
		{
			...
			LogAdd("PacketCheckSum Error [%d][%s][%s] %d",
	...
	LogAdd("Check PacketCheckSum [%d][%s][%s]", aIndex, gObj[aIndex].AccountID, gObj[aIndex].Name);
	this->ClearCheckSum(aIndex);
}
...
{
	if ( gObj[aIndex].m_InWebzen == true )
	{
		return true;
	}
	...
	else
	{
		...
		LogAdd("PacketCheckSum : Compare Fail : [%d][%s][%s] (%d,%d)", aIndex,
...
void CPacketCheckSum::AddCheckSum(int aIndex, int funcindex, DWORD checksum)
{
	...
	{
		LogAdd("PacketCheckSum : Invalid FuncIndex : %d, %d", aIndex, funcindex);
		return;
	}
	...
	LogAdd("Insert PacketCheckSum %d, %d,%d", funcindex, this->m_ChecksumTableAVGCount[funcindex], checksum);
...
void CPacketCheckSum::SetClearChecksumFunc(int funcindex)
{
	...
	if ( iCount >= MAX_CHECKSUM_TABLE_AVG-1 )
	{
		...
		{
			LogAdd("Complete Init Checksum");

			for ( i=0;i<MAX_PACKET_CHECKSUM_FUNCTION_INDEX;i++)
			{
				this->m_ChecksumTable[i] = this->m_ChecksumTableAVG[i][0];
				LogAdd("%d, %d", i, this->m_ChecksumTable[i]);
			}
		}
	}
}