...
#include "..\Header\WzMultiCastSock.h"
	...
	switch ( lpHostent->h_addrtype )
	{
		case 2:
			while ( lpaddr = (in_addr *)(*lpHostent->h_addr_list++)  )
			{
				...
				strcpy_s(this->m_stIPINFO_LIST[this->m_iIPINFO_COUNT++].m_szIPADDR, sizeof(this->m_stIPINFO_LIST[this->m_iIPINFO_COUNT++].m_szIPADDR), inet_ntoa(*lpaddr));
...
void CWzMultiCastSock::SavePrivateErrorMessage(LPSTR lpszErrorMessage)
{
	...
	strcpy_s(this->m_szLastErrorMessage, sizeof(this->m_szLastErrorMessage), lpszErrorMessage);
