...
void CCrywolf::DelAllData()
{
	...
	for ( int j=0 ; j < m_StartTimeInfo.size();j++)
	{
	...
	memset(this->m_iMVPScoreTable, 0, sizeof(this->m_iMVPScoreTable));	
	memset(this->m_iMVPRankScoreTable, 0, sizeof(this->m_iMVPRankScoreTable));	
	memset(this->m_iMVPRankExpTable, 0, sizeof(this->m_iMVPRankExpTable));	
...
void CCrywolf::SetCrywolfMapAttr(int iOccupationState)
{
	...
	{
		LogAdd( "[ Crywolf ] Fail - Chang Map Attribute (State:%d) ", iOccupationState);
		return;
	}
...
int CCrywolf::CheckStateTimeSync()
{
	...
	for ( int i=0;i<m_StartTimeInfo.size();i++)
	{
		if ( this->m_StartTimeInfo[i].CheckScheduleTime() == TRUE )
		{
			LogAdd("[ Crywolf ] Start Crywolf");
...
{
	LogAdd( "[ Crywolf ] State (%d) -> NONE", this->m_iCrywolfState);
...
{
	LogAdd( "[ Crywolf ] State (%d) -> ENDCYCLE", this->m_iCrywolfState);
...
void CCrywolf::ProcState_START()
{
		 ...
		 !this->m_bTurnUpBoss )
	{
		...
		LogAdd("[ Crywolf ][MVP] Barlgars TurnUp !!!");
	}
...
void CCrywolf::ApplyCrywolfDBInfo(int iCrywolfState, int iOccupationState)
{
	...
	LogAdd("[ Crywolf ][ ApplyDBInfo] OccupationState : %d",
		iOccupationState);
}
			...
			if ( lpMonObj->m_iCurrentAI != 0 )
			{
				if ( lpMonObj->Class == 349 )
				{
					if ( bExistKiller )
					{
						...
						LogAdd("[ Crywolf ][MVP] [Balgars Dead] by [%s][%s]",
					...
					else
					{
						...
						LogAdd("[ Crywolf ][MVP] [Balgars Dead] by Unknown User");
					}
				}
...
void CCrywolf::MakeRewardForAltarElf(int iAltarUserIndex)
{
	...
	ItemSerialCreateSend(gObj[iAltarUserIndex].m_Index,
						 ...
						 (BYTE)gObj[iAltarUserIndex].X,
						 (BYTE)gObj[iAltarUserIndex].Y,
						 iItemNumber, 
						 0, 0, 0, 0, 0,
						 iAltarUserIndex,
						 0, 0);

	LogAdd("[ Crywolf ][Reward] [Altar Elf] [%s][%s] Success Reward - Gem Of Bless",
