...
int WzUdp::RecvSet(int port)
{
	...
	this->m_Recvbuf=new BYTE[this->m_dwLength];//(unsigned char*)HeapAlloc(GetProcessHeap(), HEAP_ZERO_MEMORY, this->m_dwLength);
...
void WzUdp::RecvThread()
{
	...
	ULONG tim = 2000;
	if ( setsockopt(this->m_Socket, SOL_SOCKET, SO_RCVTIMEO,(char *)&tim, sizeof(tim))==SOCKET_ERROR)
	{
		closesocket(this->m_Socket);
		return;
	}
	...
	while ( !gCloseAllThread )
	{
		FD_ZERO(&ReadSet);
		FD_ZERO(&WriteSet);
		FD_SET(this->m_Socket, &ReadSet);            
		ret = select(NULL, &ReadSet, &WriteSet, NULL, &tv);
		...
		if(ret > 0) 
		{
			if(FD_ISSET(this->m_Socket, &ReadSet)) {
