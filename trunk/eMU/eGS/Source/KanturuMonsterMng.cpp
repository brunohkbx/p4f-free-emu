...
BOOL CKanturuMonsterMng::LoadData(LPSTR lpszFileName)
{
	...
	try
	{
		...
		fclose(SMDFile);
		LogAdd( "[ KANTURU ][ MonsterSetBase ] - %s file is Loaded",
			lpszFileName);
...
int CKanturuMonsterMng::SetKanturuMonster(int iGroupNumber)
{
	...
	for ( int iCount=0;iCount<MAX_KANTURU_MONSTER;iCount++)
	{
			 ...
			 this->m_SetBaseInfo[iCount].wType != 0 )
		{
			...
			if ( iResult < 0 )
			{
				LogAdd("[ KANTURU ][ SetBossMapMonster ] Fail - Type:%d Map[%d]-[%d][%d]",
			...
			else
			{
				...
				if ( this->m_SetBaseInfo[iCount].wType == 364 )	/// Maya Hand
				{
					...
					LogAdd("[ KANTURU ][ SetBossMapMonster ] %s(Index:%d / ObjIndex:%d) Map:%d-[%d][%d]",
				...
				if ( gObj[iResult].Type == OBJ_MONSTER )
				{
					...
					LogAdd("[ KANTURU ][ SetBossMapMonster ] Count:%d %s(Index:%d / ObjIndex:%d) Map:%d-[%d][%d]",
