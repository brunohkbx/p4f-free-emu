...
void CWzMemScript::UnGetC(int ch)
{
	...
	else
	{
		...
		this->m_pBuffer [ this->m_iBufferCount ] = (char)ch;
	}
}
...
enum SMDToken CWzMemScript::GetToken()
{
	...
	do
	{
		...
		{	
			if (ch=='/')		// this is /
			{
				if((ch=(char)this->GetC ()) == '/')
				{
					while ( ((ch=(char)this->GetC()) != ('\n')) )	// End of Line (EOL)
					{
						continue;
					}
				}
	...
	case 0x39:	//9
		...
		while (((ch=(char)this->GetC()) != -1) && ((ch == 0x2E) || (isdigit(ch) != 0) || (ch == 0x2D))) 
		{
			*p = ch;
			p++;
		}		
	...
	case 0x22:	// "	
		...
		while (((ch=(char)this->GetC() ) != -1 ) && (ch != 0x22))	// nice
		{
			*p = ch;
			p++;
		}
	...
	default:	
		if (isalpha(ch))
		{
			...
			*p=ch;
			...
			while ( ((ch=(char)this->GetC() ) != -1) && ( (ch == 0x2E) || (ch == 0x5F) || (isalnum(ch) != 0) ) )
			{
				*p=ch;
				p++;
