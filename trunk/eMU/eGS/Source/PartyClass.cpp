...
{
	if ( party_number < 0 || party_number > OBJMAX-1 )	
	{
		return FALSE;
	}
...
void PartyClass::Paint(int party_number)
{
	...
	for ( int i = 0; i< MAX_USER_IN_PARTY ; i++ )
	{
		if ( (this->m_PartyS[party_number].Number[i] != 0 ) && (this->m_PartyS[party_number].DbNumber[i] != 0 ) )
		{
			...
			strcat_s(this->m_szTempPaint, "[");
			strcat_s(this->m_szTempPaint, gObj[usern].AccountID);
			strcat_s(this->m_szTempPaint, "][");
			strcat_s(this->m_szTempPaint, gObj[usern].Name);
			strcat_s(this->m_szTempPaint, "], ");
		}
	}

	LogAdd(this->m_szTempPaint);
}
