...
void CDarkSpirit::ModeAttackRandom()
{
		...
		if ( lpObj->VpPlayer2[count].state != 0 )
		{
			if ( lpObj->VpPlayer2[count].type == OBJ_MONSTER )
			{
				...
				if ( tObjNum >= 0 )
				{
					...
					if ( gObj[tObjNum].Life > 0.0 && (gObj[tObjNum].Class < 100 || gObj[tObjNum].Class  > 110 ) )
					{
	...
	if ( FindObjCount != 0 )
	{
		if ( criticaldamage != 0 )
		{
			int target = FindObj[Random(0,FindObjCount-1)];
...
void CDarkSpirit::ModeAttackWithMaster()
{
	...
	{
		if ( gObj[this->m_iTargetIndex].Life > 0.0 )
		{
...
BOOL CDarkSpirit::Attack(LPOBJ lpObj, LPOBJ lpTargetObj, CMagicInf * lpMagic, int criticaldamage, int iActionType)
{
	...
	{
		AttackDamage = int(double(AttackDamage) / 1.5 );
	}
	...
	lpObj->pInventory[1].m_DurabilitySmall += AttackDamage * 2 / Div1RavenDurability;

	if ( lpObj->pInventory[1].m_DurabilitySmall >= Div2RavenDurability )
	{
		...
		if ( lpObj->pInventory[1].m_Durability < 1.0f )
		{
			...
			LogAdd("[%s][%s] ´ÙÅ©½ºÇÇ¸´ÀÇ¹ßÅé Item is Broken because durability is exhausted [%d]",
	...
	if ( gObjWingSprite(lpTargetObj) == TRUE )
	{
		...
		if ( AttackDamage > 1 )
		{
            ...
            {
                    double damage = (AttackDamage * (75 - (Wing->m_Level*2))) / 100;
                    AttackDamage = (int)(damage);   //  #formula
            }
            else if ( Wing->m_Type > ITEMGET(12, 35) && Wing->m_Type < ITEMGET(12, 40) ) //Third Wings Damage Absortion
            {
                    double damage = (AttackDamage * (61 - (Wing->m_Level*2))) / 100;
                    AttackDamage = (int)(damage);   //  #formula
            }
            else if ( Wing->m_Type == ITEMGET(12, 40) ) //Third Wings Damage Absortion
            {
                    double damage = (AttackDamage * (76 - (Wing->m_Level*2))) / 100;
                    AttackDamage = (int)(damage);   //  #formula
            }
            else
            {
                    double damage = (AttackDamage * (88 - (Wing->m_Level*2))) / 100;
                    AttackDamage = (int)(damage);   //  #formula
            }
		}
	}
	...
	if ( AttackDamage > 0 )
	{
		int atd_reflect = (int)((double)AttackDamage * lpTargetObj->DamageReflect / 100.0);
...
int  CDarkSpirit::GetAttackDamage(LPOBJ lpObj, int targetDefense, BOOL bIsOnDuel, int crititcaldamage)
{
	...
	{
		AttackDamage = this->m_AttackDamageMin + Random(0,sub+1);
	}
...
BOOL CDarkSpirit::MissCheck(LPOBJ lpObj, LPOBJ lpTargetObj, int skill,  int skillSuccess, BOOL& bAllMiss)
{
	...
	{
		if ( Random(0,this->m_SuccessAttackRate) < TargetSuccessfulBlocking)
		{
			GCDamageSend(lpObj->m_Index, lpTargetObj->m_Index, 0, 0, MsgDamage, 0);
			return FALSE;
		}
...
BOOL CDarkSpirit::MissCheckPvP(LPOBJ lpObj, LPOBJ lpTargetObj, int skill,  int skillSuccess, BOOL& bAllMiss)
{
	...
	if ( iAttackRate <= 0 || iDefenseRate <= 0 || lpObj->Level <= 0 || (DWORD)lpTargetObj <= 0 )	// #error lpTargetObj
		return FALSE;
	...
	double iExpressionA = ( iAttackRate * 10000.0 ) / ( iAttackRate + iDefenseRate );	// #formula
	double iExpressionB = ( lpObj->Level * 10000 ) / ( lpObj->Level + lpTargetObj->Level );	// #formula

	iExpressionA /= 10000.0;
	iExpressionB /= 10000.0;

	iAttackSuccessRate = int(100.0 * iExpressionA * g_fSuccessAttackRateOption * iExpressionB);
