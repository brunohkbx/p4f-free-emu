...
BOOL CKanturuBattleUserMng::AddUserData(int iIndex)
{
	...
	{
		LogAdd("[ KANTURU ][ BattleUser ] Add User Fail - Disconnect User [%s][%s]",
	...
	{
		LogAdd("[ KANTURU ][ BattleUser ] Add User Fail - Over Max User [%s][%s]",
			gObj[iIndex].AccountID, gObj[iIndex].Name);
...
BOOL CKanturuBattleUserMng::DeleteUserData(int iIndex)
{
	...
	{
		LogAdd("[ KANTURU ][ BattleUser ] Delete User Fail - Unvalid Index:%d",
			iIndex);
	...
	for ( int iCount=0;iCount<MAX_KANTURU_BATTLE_USER;iCount++)
	{
		if ( this->m_BattleUser[iCount].IsUseData() == TRUE )
		{
			if ( this->m_BattleUser[iCount].GetIndex() == iIndex )
			{
				...
				LogAdd("[ KANTURU ][ BattleUser ] Delete User - [%s][%s] Current Battle User:%d",
	...
	for ( int iCount=0;iCount<MAX_KANTURU_BATTLE_USER;iCount++)
	{
		if ( this->m_BattleUser[iCount].IsUseData() == TRUE )
		{
			...
			if ( !gObjIsConnected(iIndex))
			{
				...
				LogAdd("[ KANTURU ][ BattleUser ] Delete User - Disconnect [%s][%s]",
					gObj[iIndex].AccountID, gObj[iIndex].Name);
			}
				 ...
				 gObj[iIndex].Live == 1 )
			{
				...
				LogAdd("[ KANTURU ][ BattleUser ] Delete User - Map Move [%s][%s]",
					gObj[iIndex].AccountID, gObj[iIndex].Name);
			}
		}
	}
...
BOOL CKanturuBattleUserMng::MoveAllUser(int iGateNumber)
{
	...
	for ( int iCount=0;iCount<MAX_KANTURU_BATTLE_USER;iCount++)
	{
		...
		if ( this->m_BattleUser[iCount].IsUseData()  )
		{
			...
			{
				LogAdd("[ KANTURU ][ BattleUser ] [%s][%s] MoveGate(%d)",
			...
			else
			{
				...
				LogAdd( "[ KANTURU ][ BattleUser ] [%s][%s] MoveGate Fail (%d)",
					gObj[iIndex].AccountID, gObj[iIndex].Name, iGateNumber);
...
void CKanturuBattleUserMng::SetMaxUser(int iMaxUser)
{
	...
	if ( iMaxUser > MAX_KANTURU_BATTLE_USER )
	{
		...
		LogAdd("[ KANTURU ][ BattleUser ] Set Max User:%d",
			iMaxUser);
	}
}
...
{
	int iIndex = -1;
	double fTime = iElapsedTime / 1000.0f;
	...
	for ( int iCount=0;iCount<MAX_KANTURU_BATTLE_USER;iCount++)
	{
		if ( this->m_BattleUser[iCount].IsUseData() == TRUE )
		{
			...
			LogAdd("[ KANTURU ][ BATTLE WINNER ] [%d/ElapsedTime:%0.3f] [%s][%s] MapNumber[%d]-(%d/%d)",
