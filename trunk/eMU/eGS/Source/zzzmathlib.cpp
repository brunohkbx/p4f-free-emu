...
double VectorLength(vec3_t v)	// Good
{
	int		i;
	double	length;
...
void VectorMA (vec3_t va, double scale, vec3_t vb, vec3_t vc)	// Good
{
...
void AngleMatrix (const double *angles, double (*matrix)[4] )	// Good - Strange this0.0174532925199432955f
{
	double		angle;
	double		sr, sp, sy, cr, cp, cy;
...
void AngleIMatrix (const vec3_t angles, double matrix[3][4] ) // GoodStrange this 0.0174532925199432955f
{
	double angle;
	double sr, sp, sy, cr, cp, cy;
	
	angle = angles[ROLL] * (double)(0.0174532925199432955f);
	sy = sin(angle);
	cy = cos(angle);
	angle = angles[YAW] * (double)(0.0174532925199432955f);
	sp = sin(angle);
	cp = cos(angle);
	angle = angles[PITCH] * (double)(0.0174532925199432955f);	// M_PI*2 /360
	sr = sin(angle);
	cr = cos(angle);
...
void R_ConcatTransforms (const double in1[3*4], const double in2[3*4], double out[3*4]) // Good
{
...
void VectorRotate (const double in1[3*4],const double in2[3*4],double out[3*4])	// Good
{
...
void VectorIRotate (const double in1[3*4],const double in2[3*4],double out[3*4])	// Good
{
...
void VectorTranslate(const double in1[3*4],const double in2[3*4],double out[3*4])	// Good
{
...
void VectorTransform(const double in1[3*4],const double in2[3*4],double out[3*4])	// Good
{
...
void AngleQuaternion(const vec3_t angles, double * quaternion)	// Good
{
	double angle;
	double sr, sp, sy, cr, cp, cy;
...
void QuaternionMatrix(const double * quaternion, double (*matrix)[4])
{
...
void QuaternionSlerp(const double * p, double * q, double t, double * qt)
{
	int i;
	double omega;
	double cosom;
...
/*
void VectorPolar(vec3_t v, double radius, double theta, double phi)
{
 	v[0]=double(radius * cos(theta) * cos(phi));
	v[1]=double(radius * sin(theta) * cos(phi));
	v[2]=double(radius * sin(phi));
}
...
/*void VectorToAngles( vec3_t vec, vec3_t angles )
   ...
   if ( ( vec[ 0 ] == 0 ) && ( vec[ 1 ] == 0 ) )
      ...
      {
      yaw = (double)( atan2( vec[ 1 ], vec[ 0 ] ) * 180 / M_PI );
      if ( yaw < 0 )
         {
         yaw += 360;
         }

      forward = ( double )sqrt( vec[ 0 ] * vec[ 0 ] + vec[ 1 ] * vec[ 1 ] );
      pitch = (double) ( atan2( vec[ 2 ], forward ) * 180 / M_PI );
      if ( pitch < 0 )
         {
         pitch += 360;
         }
...
void R_ConcatTransforms(const double In1[3][4], const double In2[3][4], double Out[3][4])
{
...
void VectorIRotate(const vec3_t In1, const double In2[3][4], vec3_t Out)
{
	Out[0]	= In1[0] * In2[0][0] + In1[1] * In2[1][0] + In1[2] * In2[2][0];
	Out[1]	= In1[0] * In2[0][1] + In1[1] * In2[1][1] + In1[2] * In2[2][1];
	Out[2]	= In1[0] * In2[0][2] + In1[1] * In2[1][2] + In1[2] * In2[2][2];
}
void VectorTransform(const vec3_t In1, const double In2[3][4], vec3_t Out)
{
...
{
	double	Angle	= Angles[0] * 0.5f;
	double	Sin0	= (double)sin(Angle);
	double	Cos0	= (double)cos(Angle);

	Angle			= Angles[1] * 0.5f;
	double	Sin1	= (double)sin(Angle);
	double	Cos1	= (double)cos(Angle);

	Angle			= Angles[2] * 0.5f;
	double	Sin2	= (double)sin(Angle);
	double	Cos2	= (double)cos(Angle);
...
void QuaternionMatrix(const vec4_t Quaternion, double (*Matrix)[4])
{
...
void QuaternionSlerp(const vec4_t In1, vec4_t In2, double Time, vec4_t Out)
{
	double	Slerp1;
	double	Slerp2;
	...
	double	CosAngle	= In1[0] * In2[0] + In1[1] * In2[1] + In1[2] * In2[2] + In1[3] * In2[3];

	if ((1.0f + CosAngle) > 0.00000001f)
	{
		if ((1.0f - CosAngle) > 0.00000001f)
		{
			double	Omega		= (double)acos(CosAngle);
			double	SinAngle	= (double)sin(Omega);
			
			Slerp1				= (double)sin((1.0f - Time) * Omega) / SinAngle;
			Slerp2				= (double)sin(Time * Omega) / SinAngle;
		}
	...
	else
	{
		...
		Out[2]	= -In1[3];
		Out[3]	= In1[2];
		Slerp1	= (double)sin((1.0f - Time) * 0.5f * Q_PI);
		Slerp2	= (double)sin(Time * 0.5f * Q_PI);
