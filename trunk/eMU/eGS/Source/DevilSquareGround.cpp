		...
		if ( this->m_DevilSquareBossMonsterInfo[i].m_Type != (WORD)-1 )
		{
			if ( currtime == this->m_DevilSquareBossMonsterInfo[i].m_RegenStartTime )
			{
				...
				if ( iMonIndex >= 0 )
				{
					...
					LogAdd("[DevilSquare] Boss %d %d %d create ", map, x, y);
					...
					gObj[iMonIndex].StartX = (BYTE)gObj[iMonIndex].X;
					gObj[iMonIndex].StartY = (BYTE)gObj[iMonIndex].Y;
...
WORD CDevilSquareGround::GetMonsterType(int currtime)
{
	...
	return sTemp[Random(0,icount-1)];
}
