...
short CMapServerManager::CheckMoveMapSvr(int iIndex, int iMAP_NUM, short sSVR_CODE_BEFORE)
{
	...
	{
		LogAdd( "[MapServerMng] CheckMoveMapSvr() - Map Index doesn't exist [%s][%s] : %d",
	...
	if ( lpMapSvrData == NULL )
	{
		LogAdd( "[MapServerMng] CheckMoveMapSvr() - m_lpThisMapSvrData == NULL [%s][%s] : %d",
			gObj[iIndex].AccountID, gObj[iIndex].Name, iMAP_NUM);
	...
	if ( lpMapSvrData->m_bIN_USE == FALSE )
	{
		LogAdd( "[MapServerMng] CheckMoveMapSvr() - lpMapSvrData->m_bIN_USE == FALSE [%s][%s] : %d",
			gObj[iIndex].AccountID, gObj[iIndex].Name, iMAP_NUM);
	...
	switch ( sMAP_MOVE_INFO )
	{
		case -1:
			{
				...
				{
					sDestServerCode = vtMapSvrData[Random(0,vtMapSvrData.size()-1)]->m_sSVR_CODE;
				}

				if ( sDestServerCode != -1 )
				{
					LogAdd("[MapServerMng] CheckMoveMapSvr() - MapServer Check OK [%s][%s] : MAP-%d / SVR-%d(State Map:%d X:%d Y:%d)",
		...
		default:
			...
			{
				LogAdd( "[MapServerMng] CheckMoveMapSvr() - Unknown MapMove Info [%s][%s] : MAP-%d / INFO-%d",
...
BOOL CMapServerManager::GetSvrCodeData(WORD wServerCode, char* lpszIpAddress, WORD * lpwPort)
{
	...
	strcpy_s(lpszIpAddress, sizeof(lpszIpAddress), lpMapSvrData->m_cIPADDR);
