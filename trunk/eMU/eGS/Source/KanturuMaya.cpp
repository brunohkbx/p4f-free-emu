...
{
	if ( Random(0,10000) > iSkillUsingRate )
		return;

	if ( this->m_iMayaObjIndex < 0 || this->m_iMayaObjIndex > OBJMAX-1 )	
		return;
	...
	LogAdd("[ KANTURU ][ IceStorm ] Skill Using(%d) : Index(%d) %s",
...
void CKanturuMaya::KanturuMayaAct_Hands()
{
	...
	for ( int iCount=OBJ_STARTUSERINDEX;iCount < OBJMAX;iCount++)
	{
			 ...
			 gObj[iCount].MapNumber == MAP_INDEX_KANTURU_BOSS )
		{
			...
			LogAdd("[ KANTURU ][ BrokenShower ] [%s][%s] User Dying cause NOT wearing MoonStone Pandent",
				gObj[iCount].AccountID, gObj[iCount].Name);
		}
	}

	LogAdd("[ KANTURU ][ BrokenShower ] Skill Using : Index(%d) %s",
