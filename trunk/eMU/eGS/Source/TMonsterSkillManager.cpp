...
BOOL TMonsterSkillManager::LoadData(LPSTR lpszFileName)
{
	...
	try
	{
		...
		while ( true )
		{
			...
			while ( true )
			{
				if ( iType == 0 )
				{
					...
					if ( bVerified ==FALSE )
					{
						LogAdd( "[Monster Manager] - Invalid SkillInfo : MIndex(%s/%d)",
		...
		LogAdd( "[Monster Skill Manager] - %s file is Loaded", lpszFileName);
...
{
	if ( iMonsterClass < 0 || iMonsterClass > MAX_MONSTER_SKILL_INFO_ARRAY-1 )	// #error Change to MAX_MONSTER_SKILL_INFO_ARRAY-1
		return FALSE;
...
BOOL TMonsterSkillManager::AddMonsterSkillDelayInfo(int iIndex, int iTargetIndex, int iDelayTime, TMonsterSkillUnit * lpMonsterSkillUnit)
{
	...
	LogAdd( "[Monster Skill Manager] - (MonsterSkillDelayInfo) Slot Full!! ");
	return FALSE;
}
...
TMonsterSkillUnit * TMonsterSkillManager::FindMonsterSkillUnit(int iIndex, int iMonsterSkillUnitType)
{
	...
	{
		int iSlot = iFoundSkillArray[Random(0,iFoundSkillCount)];
...
void TMonsterSkillManager::UseMonsterSkill(int iIndex, int iTargetIndex, int iMonsterSkillUnitType)
{
	...
	{
		LogAdd( "[Monster Skill Manager] SkillUnit is NULL ");
		return;
	}
