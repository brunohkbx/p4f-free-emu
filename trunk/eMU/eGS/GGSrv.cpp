...
DWORD  InitGameguardAuth(LPSTR szGGAuthName, DWORD nNumberOfUser)
{
	...
	if ( szGGAuthName != NULL && szGGAuthName[0] != '\0')
	{
		...
		{
			sprintf_s(ggGlobal.szGGAuthName,"%s%c", szGGAuthName, '\\');
		}
	}
...
{
	if ( ggGlobal.lpCurrentAuthProtocol != NULL )//#error maybe
	{
...
DWORD  LoadAuthProtocol(LPSTR lpszAuthDll, BOOL Flag)	// Perfect
{
	...
	_GG_AUTH_PROTOCOL * pProtocol = new _GG_AUTH_PROTOCOL;
	...
	sprintf_s(szTemp, "%s%s", ggGlobal.szGGAuthName, lpszAuthDll);
