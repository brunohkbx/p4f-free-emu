...
#define OBJMAX 15600			//GS-CS 4000 Normal 9600 def:7400
#define OBJMAXUSER 1000		//GS-CS 1500 Normal 1000 def:1000
#define OBJ_MAXMONSTER 14000	//GS-CS 2500 Normal 8000 def:5800
...
enum PROTO_CLASS_CODES {
	...
	PR_SUMMONER			= 0x9a,
	PR_BLOODY_SUMMONER	= 0xaa,
	PR_DIMENSION_MASTER = 0xb2,
...
struct securepackets
{
	DWORD PacketsCountInOneSec;
	DWORD OldPacketTime;
	DWORD MaxPaketsPerSec;
	DWORD MoveHackCount;
	DWORD OldMovePacketTime;
};
...
struct OBJECTSTRUCT
{
	...
	WORD Leadership;	// DC
#endif
	unsigned short AddLeadership;	// DE
	WORD  ChatLimitTime;	// E0
	BYTE  ChatLimitTimeSec;	// E2
	BYTE  FillLifeCount;	// E3
	unsigned short AddStrength;	// E4
	unsigned short AddDexterity;	// E6
	unsigned short AddVitality;	// E8
	unsigned short AddEnergy;	// EA
	int   BP;	// EC
	int   MaxBP;	// F0
	int	  AddBP;	// F4
	double VitalityToLife;	// F8
	double EnergyToMana;	// FC
	char  m_PK_Count;	// 100
	char  m_PK_Level;	// 101
	int   m_PK_Time;	// 104
	short X; // 108
	...
	int m_RecallMon;//аІндекс визваного монстра 
	int m_Change;	// 2F0
	...
	short m_Attribute;	// если == 100 значит обект будет искать моба для убийства, если 0 - игрока
...
int gObjLevelUpPointAdd(unsigned char type, LPOBJ lpObj, int count);
...
int gObjCheckStatPointUp(int aIndex);//перевірка чи можна додати "стати" до персонажу
int gObjCheckStatPointDown(int aIndex);//перевірка чи можна відняти "стати" від персонажу
void gObjUsePlusStatFruit(int aIndex, int pos);
void gObjUseMinusStatFruit(int aIndex, int pos);
void gObjCalcMaxLifePower(int aIndex);//використання фрукта на плюс для статів
void gObjDelayLifeCheck(int aIndex);//перевірка життя гравця
int gObjDuelCheck(LPOBJ lpObj, LPOBJ lpTargetObj);//перевірка чи данний гравець находиться в дуелі
int gObjDuelCheck(LPOBJ lpObj);//перевірка чи 2 гравця находяться в дуелі
