...
int CMagicInf::Set(BYTE aType, BYTE aIndex, BYTE aLevel)
{
	if ( this->m_Skill != 0xFF )
	{
		LogAdd("No space to add. %s %d", __FILE__, __LINE__);
		return -1;
	}
...
int CMagicInf::GetDamage()
{
	...
	if ( this->m_Skill == 0xFF )
	{
		LogAdd("Damage value doesn't exist.(%s %d)", __FILE__, __LINE__);
		return 0;
	}


	return Random(this->m_DamageMin,this->m_DamageMax);
