...
BOOL TMonsterAIAutomata::LoadData(LPSTR lpszFileName)
{
	...
	try
	{
		...
		while ( true )
		{
			...
			while ( true )
			{
				if ( iType == 0 )
				{
					...
					LogAdd("[Monster AI Automata] Loaded Transition Desc . [ %s ]", AIState->m_szStateTransitionDesc);
		...
		fclose(SMDFile);
		LogAdd("[Monster AI Automata] - %s file is Loaded", lpszFileName);
...
TMonsterAIAutomata * TMonsterAIAutomata::FindAutomata(int iAutomataNumber)
{
	if ( iAutomataNumber < 0 || iAutomataNumber >= MAX_MONSTER_AI_AUTOMATA )
	{
		LogAdd("[Monster AI Automata] FindAutomata() Error - (AutomataNumber=%d) ", iAutomataNumber);
		return NULL;
	}
...
TMonsterAIState * TMonsterAIAutomata::RunAutomata(int iIndex)
{
	...
	for ( int iPriority=0;iPriority<iMaxStateTransCount;iPriority++)
	{
			...
			case MAI_STATE_TRANS_DEC_HP_PER:
				{
					...
					int iLife = int( ((double)lpObj->AddLife + lpObj->MaxLife) * (double)AIState->m_iTransitionValue / 100.0 );
		...
		if ( Random(0,99) < AIState->m_iTransitionRate )
		{
			bRateSuccess = TRUE;
