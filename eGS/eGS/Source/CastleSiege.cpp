...
// GS-CS 1.00.18  0x00XXXXXX	-	Completed
...
int CCastleSiege::LoadData(std::string FileName)
{
	using namespace std;
	...
	{
		LogAdd("[CastleSiege] CCastleSiege::LoadData() - m_iCastleDataLoadState != CASTLESIEGE_DATALOAD_1 (%d)",m_iCastleDataLoadState);
		return false;
	}
	...
	{
		MsgBox("[CastleSiege] file load error - fopen() : %s",FileName.c_str());
		return false;
	}
	...
	while( true )
	{
		...
		while ( true )
		{
			if(type == 0)
			{
				...
				{
					LogAdd("[CastleSiege] CCastleSiege::LoadData() - iCastleSiegeCycle <= 0 : %d",iCastleSiegeCycle);
					return false;
				}
				else
				{
				...
				if(!vtScheduleData.empty())
				{
					for(std::vector<_CS_SCHEDULE_DATA>::iterator it = vtScheduleData.begin(); it != vtScheduleData.end(); it++)
					{
						...
						if(pScheduleData.m_bIN_USE != 0)
						{
							...
							{
								LogAdd("[CastleSiege] CCastleSiege::LoadData() - Same State Exist : %d",iSTATE);
								return false;
							}
							...
							if(pScheduleData.m_iSTATE > iSTATE)
							{
								...
								{
									LogAdd("[CastleSiege] CCastleSiege::LoadData() - Date Order is wrong : %d",iSTATE2_TIME);
									return false;
								}
							}
							else
	...
	if(!m_vtScheduleData.empty())
	{
		for(int iIDX = 0; iIDX < m_vtScheduleData.size(); iIDX++)
		{
			if(iIDX+1 < m_vtScheduleData.size())
			{
				...
				if(iIDX_RESULT < 0)
				{
					LogAdd("[CastleSiege] CCastleSiege::LoadData() - Date Order is wrong (sort fail) : %d-%d",
...
int CCastleSiege::DataRequest()
{
	...
	{
		LogAdd("[CastleSiege] CCastleSiege::DataRequest() - m_bFileDataLoadOK == FALSE");
		return false;
	}

	if(m_iMapSvrGroup < 0)
	{
		LogAdd("[CastleSiege] CCastleSiege::DataRequest() - m_iMapSvrGroup < 0");
		return false;
	}

	if(m_iCastleDataLoadState != CASTLESIEGE_DATALOAD_2)
	{
		LogAdd("[CastleSiege] CCastleSiege::DataRequest() - m_iCastleDataLoadState != CASTLESIEGE_DATALOAD_2 (%d)",m_iCastleDataLoadState);
		return false;
	}
	...
	if(m_dwDbDataLoadTickCount == 0)
	{
		...
		LogAdd("[CastleSiege] CCastleSiege::DataRequest() - REQUEST DATA FIRST");
	}
	else if(GetTickCount() - m_dwDbDataLoadTickCount > 3000)
	{
		bDbRequestData = TRUE;
		m_dwDbDataLoadTickCount = GetTickCount();
		LogAdd("[CastleSiege] CCastleSiege::DataRequest() - REQUEST DATA AGAIN");
	}
...
int CCastleSiege::Init()
{
	...
	{
		LogAdd("[CastleSiege] CCastleSiege::Init() - m_iCastleDataLoadState != CASTLESIEGE_DATALOAD_4 (%d)",m_iCastleDataLoadState);
	...
	{
		LogAdd("[CastleSiege] CCastleSiege::Init() - Data Load Fail (FILE:%d, DB:%d)",
	...
	if(CheckSync() == FALSE)
	{
		LogAdd("[CastleSiege] CCastleSiege::Init() - CheckSync() == FALSE");
		...
		for(it = m_vtNpcData.begin(); it != m_vtNpcData.end(); it++)
		{
			...
			if(pNpcData.m_iNPC_NUM == 277)
			{
				...
				else
				{
					...
					LogAdd("[CastleSiege] CCastleSiege::CreateAllCsGateLever() ERROR - Failed to Create Gate Lever : GateIndex:%d",pNpcData.m_iNPC_INDEX);
				}
			}
		}
	}
...
int CCastleSiege::CheckSync()
{
	if(m_bFileDataLoadOK == FALSE)
	{
		LogAdd("[CastleSiege] CCastleSiege::CheckSync() - m_bFileDataLoadOK == FALSE");
		return false;
	}
				...
				if(pNpcData.m_iNPC_NUM == 277)
				{
					...
					if(bCreateLever != FALSE)
					{
						...
						if(OBJMAX_RANGE(iLeverIndex))
						{
							...
							if(bRETVAL == FALSE)
							{
								LogAdd("[CastleSiege] CCastleSiege::CreateDbNPC() ERROR - Failed to Create Gate Lever : GateIndex:%d",pNpcData.m_iNPC_INDEX);
							}
						}
						else
						{
...
void CCastleSiege::SetState_STARTSIEGE(int bSetRemainMsec)
{
	...
	if(CheckAttackGuildExist() == FALSE)
	{
		...
		LogAdd("[CastleSiege] CCastleSiege::SetState_STARTSIEGE() - CheckAttackGuildExist() == FALSE");
	}
	else
	{
		StoreDbNpc();
...
void CCastleSiege::SetState_ENDSIEGE(int bSetRemainMsec)
{
	...
	if(bSetRemainMsec != FALSE)
	{
		...
		dResultSecond = difftime(ttETime,ttSTime);
		m_iCS_REMAIN_MSEC = int(dResultSecond * (double)1000.0);
	}

	ClearNonDbNPC();
	ResetAllUserCsJoinSide();
	SendAllUserAnyMsg("Siege Warfare has been concluded.",1);
}
...
void CCastleSiege::SetState_ENDCYCLE(int bSetRemainMsec)
{
	...
	GS_GDReqSiegeDateChange(m_iMapSvrGroup,-1,m_tmStartDate.wYear,(BYTE)m_tmStartDate.wMonth,(BYTE)m_tmStartDate.wDay,m_tmEndDate.wYear,(BYTE)m_tmEndDate.wMonth,(BYTE)m_tmEndDate.wDay);
...
int CCastleSiege::CheckCastleSiegeResult()
{
	...
	if(m_btIsCastleOccupied == TRUE && strcmp(m_szCastleOwnerGuild,"") != FALSE)
	{
		if(strcmp(m_szMiddleWinnerGuild,"") == FALSE || strcmp(m_szMiddleWinnerGuild,m_szCastleOwnerGuild) == FALSE)
		{
			...
			wsprintf(szMsg,"%s guild has succeeded in defending the castle.",m_szCastleOwnerGuild);
			bRETVAL = FALSE;
		}
		else
		{
			...
			wsprintf(szMsg,"%s guild became a new castle lord.",m_szCastleOwnerGuild);
			bRETVAL = TRUE;
		}
	}
	else
	{
		if(strcmp(m_szMiddleWinnerGuild,"") == FALSE)
		{
			...
			wsprintf(szMsg,"The castle lord has not been assigned.");
			bRETVAL = FALSE;
		}
		else
		{
...
void CCastleSiege::StoreDbNpc()
{
	...
	wsDataServerCli.DataSend((char *)lpMsg,iCOUNT * sizeof(CSP_NPCUPDATEDATA) + sizeof(CSP_REQ_NPCUPDATEDATA));
	...
	for(int iUSER = OBJ_STARTUSERINDEX; iUSER < OBJMAX; iUSER++)
	{
		...
		case 1:
			if(bRefreshOwnerUser != FALSE)
			{
				...
				GCTeleportSend(&gObj[iUSER],0xFF,gObj[iUSER].MapNumber,(BYTE)gObj[iUSER].X,(BYTE)gObj[iUSER].Y,gObj[iUSER].Dir);
		...
		case 2:
			{
				...
				{
					iTX = Random(g_iCsUserReSpawnArea[2][0],g_iCsUserReSpawnArea[2][2]);
					iTY = Random(g_iCsUserReSpawnArea[2][1],g_iCsUserReSpawnArea[2][3]);
		...
		case 0:
			{
				for(int iCNT = 0; iCNT < 100; iCNT++)
				{
					iTX = Random(g_iCsUserReSpawnArea[0][0],g_iCsUserReSpawnArea[0][2]);
					iTY = Random(g_iCsUserReSpawnArea[0][1],g_iCsUserReSpawnArea[0][3]);
		...
		if(bMoveOK != FALSE)
		{
			...
			gObjClearViewport(&gObj[iUSER]);
			GCTeleportSend(&gObj[iUSER],0xFF,MAP_INDEX_CASTLESIEGE,(BYTE)gObj[iUSER].X,(BYTE)gObj[iUSER].Y,gObj[iUSER].Dir);
...
void CCastleSiege::SendAllUserAnyMsg(std::string lpszMsg, int iType)
{
	PMSG_NOTICE pNotice;
...
void CCastleSiege::SendMapServerGroupMsg(std::string Msg)
{
	GS_GDReqMapSvrMsgMultiCast(this->m_iMapSvrGroup, (char*)Msg.c_str());
}
				...
				if( NpcData.m_iNPC_NUM == 277 )
				{
					...
					if( iGateLevelerIndex != 0 )
					{
						...
						if( OBJMAX_RANGE(iGateLevelerIndex) )
						{
							...
							if( iLinkIndex == 0 )
							{
								::LogAdd( "[CastleSiege] CCastleSiege::CreateDbNPC() ERROR - Failed to Create Gate Lever : GateIndex:%d",
									NpcData.m_iNPC_INDEX);
							}
						}
						else
...
BOOL CCastleSiege::UpgradeDbNPC(int iIndex, int iNpcType, int iNpcIndex, int iNpcUpType, int iNpcUpValue, int iNpcUpIndex)
{
	...
	{
		LogAdd( "[CastleSiege] CCastleSiege::UpgradeDbNPC() ERROR - lpMsg->iNpcUpIndex is Out of Bound : %d", iNpcUpIndex);
		return FALSE;
	}
	...
	if( bIsLive == FALSE )
	{
		LogAdd( "[CastleSiege] CCastleSiege::UpgradeDbNPC() ERROR - bIsLive == FALSE", iNpcUpIndex);
		return FALSE;
	}
...
BOOL CCastleSiege::FirstCreateDbNPC()
{
	...
	{
		LogAdd( "[CastleSiege] CCastleSiege::FirstCreateDbNPC() - m_iCastleDataLoadState != CASTLESIEGE_DATALOAD_4");
		return FALSE;
	}
	...
	wsDataServerCli.DataSend((PCHAR)lpMsg, (iCOUNT * sizeof(CSP_NPCSAVEDATA)) + sizeof(CSP_REQ_NPCSAVEDATA));
...
BOOL CCastleSiege::SetCastleNpcData(CSP_CSINITDATA* lpMsg, int iCOUNT)
{
	...
	{
		LogAdd( "[CastleSiege] CCastleSiege::SetCastleInitData() - m_iCastleDataLoadState != CASTLESIEGE_DATALOAD_3");
		return FALSE;
	}
...
*/
	...
	{
		LogAdd( "[CastleSiege] CCastleSiege::SetCastleInitData() - iSTART_DATE_NUM > iEND_DATE_NUM");
		return FALSE;
	}
	return TRUE;
}
...
//	sort(vtRegGuildScore.begin(), vtRegGuildScore.end(), CCastleSiege::RegGuildScoreCompFunc);
	...
	wsDataServerCli.DataSend((PCHAR)lpMsgSend, (lpMsgSend->iCount * sizeof(CSP_CSGUILDUNIONINFO))+ sizeof(CSP_REQ_CSGUILDUNIONINFO));
...
void CCastleSiege::MakeCsTotalGuildInfo(CSP_CSGUILDUNIONINFO* lpMsg, int iCOUNT)
{
	...
	{
		LogAdd( "[CastleSiege] CCastleSiege::MakeCsTotalGuildInfo() ERROR - m_bCsBasicGuildInfoLoadOK == FALSE");
		return;
	}
	if( iCOUNT <= 0 )
	{
		LogAdd( "[CastleSiege] CCastleSiege::MakeCsTotalGuildInfo() ERROR - iCOUNT <= 0 : %d", iCOUNT);
		return;
	}
...
void CCastleSiege::SetCsTotalGuildInfo(CSP_CSLOADTOTALGUILDINFO* lpMsg, int iCOUNT)
{
	...
	if( iCOUNT <= 0 )
	{
		LogAdd( "[CastleSiege] CCastleSiege::SetCsTotalGuildInfo() ERROR - iCOUNT <= 0 : %d",
			iCOUNT);
...
void CCastleSiege::Run()
{
	...
	if( (GetTickCount() - g_iLastSiegeUpdate) > 3000 )
	{
		...
		if( iLeftSiegeDate >= 0 )
		{
			...
			LogAdd( "LEFT : SEC(%d), SIEGE-DAY(%04d-%02d-%02d %02d:%02d:%02d)",
		...
		else
		{
			/*switch( iLeftSiegeDate )
			{
				case CASTLESIEGE_LEFTTIME_ONSIGE:
					LogAdd( "[CastleSiege] LEFT-TIME : CASTLESIEGE_LEFTTIME_ONSIGE");
					break;
				case CASTLESIEGE_LEFTTIME_ENDSIEGE:
					LogAdd( "[CastleSiege] LEFT-TIME : CASTLESIEGE_LEFTTIME_ENDSIEGE");
					break;
				case CASTLESIEGE_LEFTTIME_NOTRUNNING:
					LogAdd( "[CastleSiege] LEFT-TIME : CASTLESIEGE_LEFTTIME_NOTRUNNING");
					break;
			}*/
		}
	}
...
int CCastleSiege::CalcCastleLeftSiegeDate()
{
	...
	if( iGAP_SEC >= 0 )
	{
		...
		if( iGAP_SEC == 0 )
		{
			LogAdd( "[CastleSiege] CCastleSiege::CalcCastleLeftSiegeDate() - iGAP_SEC == 0");
		}
...
void CCastleSiege::ProcState_IDLE_1()
{
	...
	if( iTICK_MSEC >= 1000 )
	{
		...
		LogAdd( "RUN ProcState_IDLE_1()\tLEFT-MSEC:%d", this->m_iCS_REMAIN_MSEC);
	}
...
void CCastleSiege::ProcState_REGSIEGE()
{
	...
	if( iTICK_MSEC >= 1000 )
	{
		...
		{
			this->SendAllUserAnyMsg("Siege Warfare registration period commences.", 1);
			this->m_dwEVENT_MSG_TICK_COUNT = GetTickCount();
		}
		LogAdd( "RUN ProcState_REGSIEGE()\tLEFT-MSEC:%d", this->m_iCS_REMAIN_MSEC);
	}
...
void CCastleSiege::ProcState_IDLE_2()
{
	...
	if( iTICK_MSEC >= 1000 )
	{
		this->m_iCS_REMAIN_MSEC -= iTICK_MSEC;
		this->m_iCS_TICK_COUNT = GetTickCount();
		LogAdd( "RUN ProcState_IDLE_2()\tLEFT-MSEC:%d", this->m_iCS_REMAIN_MSEC);
	}
...
void CCastleSiege::ProcState_REGMARK()
{
	...
	if( iTICK_MSEC >= 1000 )
	{
		...
		if( (GetTickCount() - this->m_dwEVENT_MSG_TICK_COUNT) > 1800000 )
		{
			this->SendAllUserAnyMsg("All participating guilds for Siege Warfare: please register your Emblem of Lord.", 1);
			this->m_dwEVENT_MSG_TICK_COUNT = GetTickCount();
		}
		LogAdd( "RUN ProcState_REGMARK()\tLEFT-MSEC:%d", this->m_iCS_REMAIN_MSEC);
	}
...
void CCastleSiege::ProcState_IDLE_3()
{
	...
	if( iTICK_MSEC >= 1000 )
	{
		...
		{
			this->SendAllUserAnyMsg("The Guild selection period for Siege Warfare commences.", 1);
			this->m_dwEVENT_MSG_TICK_COUNT = GetTickCount();
		}

		LogAdd( "RUN ProcState_IDLE_3()\tLEFT-MSEC:%d", this->m_iCS_REMAIN_MSEC);
	}
...
void CCastleSiege::ProcState_NOTIFY()
{
	...
	if( iTICK_MSEC >= 1000 )
	{
		...
		if( (GetTickCount() - this->m_dwEVENT_MSG_TICK_COUNT) > 1800000 )
		{
			this->SendAllUserAnyMsg("Guilds have now been selected for  Siege Warfare.", 1);
			this->m_dwEVENT_MSG_TICK_COUNT = GetTickCount();
		}
		LogAdd( "RUN ProcState_NOTIFY()\tLEFT-MSEC:%d", this->m_iCS_REMAIN_MSEC);
	}
...
void CCastleSiege::ProcState_READYSIEGE()
{
	...
	if( iTICK_MSEC >= 1000 )
	{
		...
		if( (GetTickCount() - this->m_dwEVENT_MSG_TICK_COUNT) > 1800000 )
		{
			this->SendAllUserAnyMsg("The Siege Warfare preparation period begins! All the guild members, please get ready.", 1);
			this->m_dwEVENT_MSG_TICK_COUNT = GetTickCount();
		}
		if( this->m_iCS_REMAIN_MSEC <= 300000 )
		{
			...
			{
				char szBuff[256]={0};
				wsprintf(szBuff, "Siege Warfare will start in %d minute(s).", (this->m_iCS_REMAIN_MSEC/60000)+1);
				this->SendAllUserAnyMsg(szBuff, 2);
				this->m_dwCS_STARTTIME_TICK_COUNT = GetTickCount();
			}
		}
		LogAdd( "RUN ProcState_READYSIEGE()\tLEFT-MSEC:%d", this->m_iCS_REMAIN_MSEC);
	}
...
void CCastleSiege::ProcState_STARTSIEGE()
{
	...
	if( iTICK_MSEC >= 1000 )
	{
		...
		if( (GetTickCount() - this->m_dwEVENT_MSG_TICK_COUNT) > 1800000 )
		{
			this->SendAllUserAnyMsg("Siege Warfare is being waged!", 1);
			this->m_dwEVENT_MSG_TICK_COUNT = GetTickCount();
		}


		LogAdd( "RUN ProcState_STARTSIEGE()\tLEFT-MSEC:%d", this->m_iCS_REMAIN_MSEC);
	}
...
void CCastleSiege::ProcState_ENDSIEGE()
{
	...
	if( iTICK_MSEC >= 1000 )
	{
		...
		LogAdd( "RUN ProcState_ENDSIEGE()\tLEFT-MSEC:%d", this->m_iCS_REMAIN_MSEC);
	}
...
void CCastleSiege::ProcState_ENDCYCLE()
{
	...
	if( iTICK_MSEC >= 1000 )
	{
		...
		LogAdd( "RUN ProcState_ENDCYCLE()\tLEFT-MSEC:%d", this->m_iCS_REMAIN_MSEC);
	}
...
BOOL CCastleSiege::CheckGuardianStatueExist()
{
	...
	for( int iIDX = 0; iIDX < OBJ_MAXMONSTER; iIDX++ )
	{
		...
		{
			if( gObj[iIDX].Connected == PLAYER_PLAYING && gObj[iIDX].Live == 1 )
			{
				if( gObj[iIDX].MapNumber == MAP_INDEX_CASTLESIEGE )
				{
					count++;
...
int CCastleSiege::CheckOverlapCsMarks(int iIndex)
{
	for( int x = INVETORY_WEAR_SIZE; x < MAIN_INVENTORY_SIZE; x++ )
	{
		if( gObj[iIndex].pInventory[x].IsItem() == TRUE )
		{
			...
			{
				int dur = (int)gObj[iIndex].pInventory[x].m_Durability;
...
void CCastleSiege::ReSpawnAllUser()
{
	...
	for( int iUSER = OBJ_STARTUSERINDEX; iUSER < OBJMAX; iUSER++ )
	{
		...
		case 1:
			{
				for(int iCNT = 0; iCNT < 100; iCNT++)
				{
					iTX = Random(g_iCsUserReSpawnArea[1][0],g_iCsUserReSpawnArea[1][2]);
					iTY = Random(g_iCsUserReSpawnArea[1][1],g_iCsUserReSpawnArea[1][3]);
		...
		if( bMoveOK )
		{
			...
			gObj[iUSER].Y = iTY;
			gObj[iUSER].TX = iTX;
			gObj[iUSER].TY = iTY;	// #error Deathway error here!!!!
			gObj[iUSER].MapNumber = MAP_INDEX_CASTLESIEGE;
			gObj[iUSER].PathCount = 0;
			gObj[iUSER].Teleport = 0;
...
void CCastleSiege::SaveCsTotalGuildInfo()
{
	if( m_bCsTotalGuildInfoLoadOK == FALSE )
	{
		LogAdd( "[CastleSiege] CCastleSiege::SaveCsTotalGuildInfo() ERROR - m_bCsTotalGuildInfoLoadOK == FALSE");
		return;
	}
	char cBUFFER[2684];
	...
	wsDataServerCli.DataSend(cBUFFER, ( sizeof(CSP_CSSAVETOTALGUILDINFO) * lpMsgSend->iCount) + sizeof(CSP_REQ_CSSAVETOTALGUILDINFO) );
...
void CCastleSiege::SendCsUserAnyMsg(std::string zMsg)
{
	PMSG_NOTICE pNotice;
	...
	TNotice::MakeNoticeMsg(&pNotice, 0, zMsg);
...
void CCastleSiege::NotifyCrownSwitchInfo(int iIndex) /*00578550*/
{
	if( gObjIsConnected(iIndex) == FALSE )
	{
		LogAdd( "[CastleSiege] CCastleSiege::NotifyCrownSwitchInfo() - iCrownSwitchIndex is Invalid");
		...
		case 0:
			{
				...
				else
				{
					...
					{
						strcpy_s(this->m_szCastleOwnerGuild,sizeof(this->m_szCastleOwnerGuild), "");
...
void CCastleSiege::NotifyCrownState(BYTE btState)
{
	...
	{
		LogAdd( "[CastleSiege] CCastleSiege::NotifyCrownState() - m_iCrownIndex isn't Valid");
		return;
	}
