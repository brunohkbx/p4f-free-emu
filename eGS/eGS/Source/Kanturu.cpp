...
void CKanturu::SetKanturuMapAttr(BYTE btLevel)
{
	...
	LogAdd( "[ KANTURU ][ Map Attr Change ] Map(%d) State(%d) DetailState(%d)",
...
{
	LogAdd( "[ KANTURU ] State(%d) -> STANDBY", this->m_iKanturuState);
...
{
	LogAdd( "[ KANTURU ] State(%d) -> TOWER_OF_REFINEMENT", this->m_iKanturuState);
...
void CKanturu::SetKanturuTimeAttackEventInfo()
{
	...
	LogAdd("[ KANTURU ][ TimeAttackEvent ] Date:%d, Counter:%d",
