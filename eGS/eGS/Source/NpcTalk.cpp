...
BOOL NpcTalk(LPOBJ lpNpc, LPOBJ lpObj)
{
	...
	switch ( npcnum )
	{
		...
		case 380:
			if ( NpcStoneStatue ( lpNpc, lpObj ) == TRUE)
			{
				return TRUE;
			}
			break;
		...
		case 383:
		case 384:
			if ( NpcITStorage ( lpNpc, lpObj ) == TRUE)
			{
				return TRUE;
			}
			break;
...
BOOL NpcStoneStatue(LPOBJ lpNpc, LPOBJ lpObj)
{
	ILAPI_GiftUserTake(lpObj->m_Index);
	return TRUE;
}

BOOL NpcITStorage(LPOBJ lpNpc, LPOBJ lpObj)
{
	ILAPI_GiftUserGive(lpObj->m_Index);
	return TRUE;
}
	...
	if ( lpObj->m_PK_Level > 4 )
	{
		...
		if ( lpNpc->TargetNumber != lpObj->m_Index )
		{
			if ( gObjGetPkTime(lpObj, hour, min) == TRUE )
			{
				...
				wsprintf(szTemp, "You have %d hour(s) %02d second(s) to get rid of the outlaw status.", hour, min);
		...
		{
			int numbertext = Random(0,2);

			switch ( numbertext )
			{
				case 0:
					ChatTargetSend(lpNpc, "I will not sell to criminals like you!!!", lpObj->m_Index);
					break;

				case 1:
					ChatTargetSend(lpNpc, "More business with me?", lpObj->m_Index);
					break;

				case 2:
					ChatTargetSend(lpNpc, "Look for it elsewhere!", lpObj->m_Index);
					break;
			}
...
BOOL NpcGuildMasterTalk(LPOBJ lpNpc, LPOBJ lpObj)
{
	...
	{
		ChatTargetSend(lpNpc, "Create guild features is disabled by Administrator", lpObj->m_Index);
		return TRUE;
	}

	if ( lpObj->GuildNumber > 0 )
	{
		ChatTargetSend(lpNpc, "You already belong to a guild", lpObj->m_Index);
		return TRUE;
	}
	...
	if ( capacity == 0 )
	{
		ChatTargetSend(lpNpc, "Your level should be at least level 100", lpObj->m_Index);
	}
	else if ( capacity == 2 )
	{
		ChatTargetSend(lpNpc, "All items you're wearing should be at least +7 level.", lpObj->m_Index);
	}
	else
	{
...
BOOL NpcChaosCardMaster(LPOBJ lpNpc, LPOBJ lpObj)
{
	...
	if ( bCanChaosBox == TRUE )
	{
		...
		LogAdd("[JewelMix] [%s][%s] Jewel Mix Dealer Start",
			lpObj->AccountID, lpObj->Name);
...
BOOL NpcMummy(LPOBJ lpNpc, LPOBJ lpObj)
{

	int TempleLvl = ILAPI_GetTempleLevel(lpObj->Level);
	...
	DataSend(lpObj->m_Index, MummyMsg, 6);
...
BOOL NpcEventChipNPC(LPOBJ lpNpc, LPOBJ lpObj)
{
	...
	if ( lpNpc->MapNumber == 0 )
	{
		...
		strcpy_s(pMsgTha.szUID, sizeof(pMsgTha.szUID), lpObj->AccountID);
		pEventObj = lpNpc;

		//DataSendEventChip((char*)&pMsgTha, sizeof(pMsgTha)); #error
...
BOOL NpcAngelKing(LPOBJ lpNpc, LPOBJ lpObj)
{
	...
	LogAdd("[Blood Castle] (%d) [%s][%s] Talk to Angel King",
	...
	if ( iITEM_LEVEL = iITEM_LEVEL = CHECK_LIMIT( g_BloodCastle.CheckQuestItem(lpObj->m_Index), 3)  )
	{
		...
		LogAdd("[Blood Castle] (%d) Blood Castle Quest Succeed Result -> Destroy Castle Door [%s][%s]",
		...
		LogAdd("[Blood Castle] (%d) Blood Castle Quest Succeed Result -> Destroy Saint Status [%s][%s]",
		...
		LogAdd("[Blood Castle] (%d) Blood Castle Quest Succeed Result -> Win Quest [%s][%s]",
		...
		if ( g_BloodCastle.SetBridgeWinner(lpObj->m_cBloodCastleIndex, lpObj->m_Index) == TRUE )
		{
			...
			if ( g_BloodCastle.CheckPartyExist(lpObj->m_Index) == FALSE )
			{
				...
				LogAdd("[Blood Castle] (%d) (Account:%s, Name:%s) Succeed Blood Castle Quest",
					lpNpc->MapNumber-10, lpObj->AccountID, lpObj->Name);
			}
			else
			{
				GCServerCmd(lpObj->m_Index, 1, 0x17, 0);
				LogAdd("[Blood Castle] (%d) (Account:%s, Name:%s) Succeed Blood Castle Quest - More Party to register",
					lpNpc->MapNumber-10, lpObj->AccountID, lpObj->Name);
			}
		}
		else
		{
			LogAdd("[Blood Castle] (%d) (Account:%s, Name:%s) Winner Already Exist !!",
				lpNpc->MapNumber-10, lpObj->AccountID, lpObj->Name);
...
BOOL NpcJewelMixDealer(LPOBJ lpNpc, LPOBJ lpObj)
{
	if ( bCanChaosBox == TRUE )
	{
		...
		LogAdd("[JewelMix] [%s][%s] Jewel Mix Dealer Start", lpObj->AccountID, lpObj->Name);
...
BOOL NpcElderCircle(LPOBJ lpNpc, LPOBJ lpObj) //GS-CS Decompiled 100%
{
        ...
        {
                ChatTargetSend(lpNpc,"It is not the designated time to man the castle.", lpObj->m_Index);
                return TRUE;
        }
        ...
        {
                ChatTargetSend(lpNpc,"Only the castle Lord can issue commands for the castle.", lpObj->m_Index);
                return TRUE;
        }
...
BOOL NpcCastleGuard(LPOBJ lpNpc, LPOBJ lpObj)
{
    ...
    {
            ChatTargetSend(lpNpc,"Castle information is unavailable at the moment.", lpObj->m_Index);
            return TRUE;
    }
...
BOOL NpcCastleCrown(LPOBJ lpNpc, LPOBJ lpObj) //GS-CS Decompiled 100%
{
        ...
        if(gObjIsConnected(iUserIndex) == FALSE )
        {
                ...
                {
                        LogAdd("[CastleSiege] [%s][%s] Failed to Register Castle Crown (GUILD:%s)",lpObj->AccountID,lpObj->Name,lpObj->GuildName);
                        return TRUE;
                }
                ...
                if(lpObj->m_btCsJoinSide != gObj[iSwitchIndex1].m_btCsJoinSide || lpObj->m_btCsJoinSide != gObj[iSwitchIndex2].m_btCsJoinSide)
                {
                        ...
                        LogAdd("[CastleSiege] [%s][%s] Failed to Register Castle Crown (GUILD:%s) (S1:%s/%s)(S2:%s/%s)",lpObj->AccountID,lpObj->Name,lpObj->GuildName,gObj[iSwitchIndex1].Name,gObj[iSwitchIndex1].GuildName,gObj[iSwitchIndex2].Name,gObj[iSwitchIndex2].GuildName);
                        return TRUE;
                }
                else
                {
                        ...
                        g_CastleSiege.SetCrownAccessUserX((BYTE)lpObj->X);
                        g_CastleSiege.SetCrownAccessUserY((BYTE)lpObj->Y);
                        g_CastleSiege.SetCrownAccessTickCount();
                        g_CastleSiege.NotifyAllUserCsProgState(0,lpObj->GuildName);
                        LogAdd("[CastleSiege] [%s][%s] Start to Register Castle Crown (GUILD:%s)",lpObj->AccountID,lpObj->Name,lpObj->GuildName);
                }
        }
...
BOOL NpcCastleSwitch(LPOBJ lpNpc, LPOBJ lpObj) //GS-CS Decompiled 100%
{
        ...
        {
                MsgOutput(lpObj->m_Index,"Test 7");
                return TRUE;
        }
        ...
        if(!gObjIsConnected(iUserIndex))
        {
                ...
                LogAdd("[CastleSiege] [%s][%s] Start to Push Castle Crown Switch (GUILD:%s) - CS X:%d/Y:%d",lpObj->AccountID,lpObj->Name,lpObj->GuildName,lpNpc->X,lpNpc->Y);
        }
        else if(lpObj->m_Index != iUserIndex)
        {
