...
BOOL CJewelMixSystem::MixJewel( int iIndex, int iJewelType, int iMixType)
{
    ...
    {
            LogAdd("[ANTI-HACK][Trade Jewel Duplication] [Mix Jewel] (%s)(%s)",
	...
	{
		MsgOutput(iIndex, "Unable to use");
		GCAnsJewelMix(iIndex, 4);

		LogAdd("[ANTI-HACK][Serial 0 Item] [Mix Jewel] (%s)(%s)",
			gObj[iIndex].AccountID, gObj[iIndex].Name);
	...
	{
		LogAdd("[JewelMix] [%s][%s] Chaos Mix is already working",
			gObj[iIndex].AccountID, gObj[iIndex].Name);
	...
	{
		LogAdd("[JewelMix] [%s][%s] Mix iJewelType is out of bound : %d",
	...
	{
		LogAdd("[JewelMix] [%s][%s] iMixType is out of bound : %d",
	...
	{
		LogAdd("[JewelMix] [%s][%s] lack of jewel to mix : %d / %d",
			gObj[iIndex].AccountID, gObj[iIndex].Name,
	...
	{
		LogAdd("[JewelMix] [%s][%s] lack of money to mix : %d / %d",
			gObj[iIndex].AccountID, gObj[iIndex].Name,
	...
	for ( int x= INVETORY_WEAR_SIZE ; x< MAIN_INVENTORY_SIZE; x++)
	{
		if ( gObj[iIndex].pInventory[x].IsItem() == TRUE )
		{
			...
			{
				LogAdd("[JewelMix] [%s][%s] Mix - Delete Jewel, Type:%d, Level:%d, Serial:%d",
					gObj[iIndex].AccountID, gObj[iIndex].Name,
	...
	if ( bItemDelOK == FALSE )
	{
		LogAdd("[JewelMix] [%s][%s] lack of jewel to mix (in deleting) : %d / %d",
			gObj[iIndex].AccountID, gObj[iIndex].Name,
	...
	LogAdd("[JewelMix] [%s][%s] jewel mix succeed : ItemType:%d, JewelCount:%d",
		gObj[iIndex].AccountID, gObj[iIndex].Name,
...
BOOL CJewelMixSystem::UnMixJewel(int iIndex, int iJewelType, int iJewelLevel, int iInventoryPos)
{
    ...
    if ( gObj[iIndex].m_IfState.use > 0 && gObj[iIndex].m_IfState.type != 12)
    {
            LogAdd("[ANTI-HACK][Trade Jewel Duplication] [UnMix Jewel] (%s)(%s)",
                    gObj[iIndex].AccountID, gObj[iIndex].Name);
            return FALSE;
    }
	...
	if ( !gObjCheckInventorySerial0Item(&gObj[iIndex]))
	{
		...
		LogAdd("[ANTI-HACK][Serial 0 Item] [UnMix Jewel] (%s)(%s)",
			gObj[iIndex].AccountID, gObj[iIndex].Name);
	...
	if ( !CHECK_LIMIT(iJewelType, 2) )
	{
		LogAdd("[JewelMix] [%s][%s] UnMix iJewelType is out of bound : %d",
			gObj[iIndex].AccountID, gObj[iIndex].Name, iJewelType);
	...
	{
		LogAdd("[JewelMix] [%s][%s] iInventoryPos is out of bound : %d",
	...
	if ( gObj[iIndex].pInventory[iInventoryPos].IsItem() == TRUE )
	{
		...
		{
			LogAdd("[JewelMix] [%s][%s] iJewelLevel is different from request : %d / %d",
				gObj[iIndex].AccountID, gObj[iIndex].Name,
	...
	{
		LogAdd("[JewelMix] [%s][%s] Item to unmix is not exist",
			gObj[iIndex].AccountID, gObj[iIndex].Name);
	...
	{
		LogAdd("[JewelMix] [%s][%s] iInventoryItemLevel is out of bound : %d",
	...
	{
		LogAdd("[JewelMix] [%s][%s] iItemType is different from request : %d / %d",
			gObj[iIndex].AccountID, gObj[iIndex].Name,
	...
	if ( iMixMoney > gObj[iIndex].Money )
	{
		LogAdd("[JewelMix] [%s][%s] lack of money to unmix : %d / %d",
			gObj[iIndex].AccountID, gObj[iIndex].Name,
			gObj[iIndex].Money, iMixMoney);
	...
	{
		LogAdd("[JewelMix] [%s][%s] lack of empty slot to unmix : %d / %d",
			gObj[iIndex].AccountID, gObj[iIndex].Name,
	...
	LogAdd("[JewelMix] [%s][%s] UnMix - Delete Jewel, Type:%d, Level:%d, Serial:%d",
		gObj[iIndex].AccountID, gObj[iIndex].Name,
	...
	if ( bItemCrtOK == TRUE )
	{
		LogAdd("[JewelMix] [%s][%s] jewel unmix succeed : ItemType:%d, JewelCount:%d",
			gObj[iIndex].AccountID, gObj[iIndex].Name,
			iItemType, iJewelCount);
	...
	{
		LogAdd("[JewelMix] [%s][%s] jewel unmix failed : ItemType:%d, JewelCount:%d",
			gObj[iIndex].AccountID, gObj[iIndex].Name,
			iItemType, iJewelCount);
