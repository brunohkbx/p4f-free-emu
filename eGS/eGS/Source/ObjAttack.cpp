...
//	GS-CS	1.00.18	JPN	0x004B1740	-	Completed
...
BOOL CObjAttack::Attack(LPOBJ lpObj, LPOBJ lpTargetObj, CMagicInf* lpMagic,  int magicsend, unsigned char MSBFlag, int AttackDamage, BOOL bCombo)
{
	...
	if ( lpObj->pInventory[0].m_Type == ITEMGET(0,14) && lpObj->pInventory[0].m_IsValidItem != false )	// Lightning Sword
	{
		if ( !Random(0,4) )
		{
			skill = 3;
			lpMagic = &DefMagicInf[3];
			magicsend = 1;
			LSwordDmg+=15;
		}
	}
	else if ( lpObj->pInventory[1].m_Type == ITEMGET(0,14) && lpObj->pInventory[1].m_IsValidItem != false )	// Lightning Sword
	{
		if ( !Random(0,4) )
		{
			skill = 3;
	...
	if ( AttackDamage == 0 )
	{
		...
		else if ( skill == 76 )	// pluzzmanton
		{
			...
			{
				if ( Random(0,99) < lpObj->m_CriticalDamage )
				{
					MsgDamage = 3;
				}
			}
			...
			{
				if ( Random(0,99) < lpObj->m_ExcelentDamage )
				{
					MsgDamage = 2;
				}
			}
			...
			{
				AttackDamage = (iFenrirAttackDmg + lpMagic->m_DamageMin) + Random(1,lpMagic->m_DamageMax - lpMagic->m_DamageMin); //(lpMagic->m_DamageMax - lpMagic->m_DamageMin + 1)
				AttackDamage -= targetdefense;
			}
		}
		else
		...
		if ( gObjAngelSprite(lpTargetObj) == TRUE )
		{
			if ( AttackDamage > 1 )
			{
				double  damage = (AttackDamage * 8) / 10.0;
		...
		if ( gObjWingSprite(lpTargetObj) == TRUE )
		{
            ...
            if ( Wing->m_Type > ITEMGET(12, 2) && Wing->m_Type < ITEMGET(12, 7) )
                    ...
                    else if ( Wing->m_Type > ITEMGET(12, 35) && Wing->m_Type < ITEMGET(12, 40) ) //Third Wings Damage Absorb
                    {
                            float damage = (float)(AttackDamage * (61 - (Wing->m_Level*2))) / 100.0f;
                            AttackDamage = (int)(damage);   //  #formula
                                                
                    }
                    else if ( Wing->m_Type == ITEMGET(12, 40) ) //Third Wings Damage Absorb
                    {
					...
					{
						double damage = (double)(AttackDamage * (88 - (Wing->m_Level*2))) / 100.0f;
						AttackDamage = (int)(damage);	//  #formula
					}
				}
			}
		...
		if ( lpTargetObj->Live )
		{
			...
			{
				int replacemana = int(lpTargetObj->Mana * 2 / 100);
				...
				if ( lpObj->Type == OBJ_USER && lpTargetObj->Type == OBJ_USER )
				{
					if ( AttackDamage > 0 )
					{
						...
						if ( !lpObj->m_btCsJoinSide || lpObj->m_btCsJoinSide != lpTargetObj->m_btCsJoinSide ) 
						{
							...
							int iEquipmentPos = Random(2,6);	// Select and Armor
							CItem * lpEquipment = &lpTargetObj->pInventory[iEquipmentPos];

							if ( lpEquipment && lpEquipment->IsItem() )
							{
								int iDurEquipment = int(lpEquipment->m_Durability * 50.0 / 100.0);
	...
	if ( AttackDamage > 0 )
	{
		int atd_reflect = (int)((double)AttackDamage * lpTargetObj->DamageReflect / 100.0f);
		...
		if ( Random(0,99) < lpObj->SetOpReflectionDamage )
		{
		...
		if ( Random(0,99) < lpObj->SetOpDoubleDamage )
		{
			if ( skill == 78 )
			{
        ...
        {
                if( Random(0,99) <= g_iWings3ReturnDamageSuccessRate )
                {
                        ...
                        {
                                LogAdd("[%s][%s] [ThirdWings] [Return Damage: %d] Target:[%s] OptionRate:[%d%%]", lpTargetObj->AccountID, lpTargetObj->Name, AttackDamage, lpObj->Name, g_iWings3ReturnDamageSuccessRate);
                        }
		...
		{
			LogAdd("[PvP System] Victim:[%s][%s], Attacker:[%s][%s] - SD[%d] HP[%f] -> SD[%d] HP[%f]",
	...
	if ( lpObj->Life <= 0.0 && lpObj->Type == OBJ_USER )
	{
    ...
    if(lpTargetObj->Type == OBJ_USER &&
        ...
        Random(0,99) <= g_iWings3RecoverFullLifeSuccessRate &&
        lpTargetObj->Life > 0.0)
    {
            //lpTargetObj->Life = lpTargetObj->MaxLife;
            int tLife = 0;
            ...
            if ( lpTargetObj->Life > (lpTargetObj->MaxLife+lpTargetObj->AddLife-1.0) )
                            lpTargetObj->Life = lpTargetObj->MaxLife+lpTargetObj->AddLife;

            GCReFillSend(lpTargetObj->m_Index,(WORD)lpTargetObj->Life,0xFF,FALSE,lpTargetObj->iShield);
            ...
            if ( g_bWings3WriteOptionsLog )
            {
                    LogAdd("[%s][%s] [ThirdWings] [Recovered Life: %d] OptionRate:[%d%%]", lpTargetObj->AccountID, lpTargetObj->Name, tLife, g_iWings3RecoverFullLifeSuccessRate);
            }
            if ( g_bWings3ShowInGameOptions )
            {
    ...
    if(lpTargetObj->Type == OBJ_USER &&
        ...
        Random(0,99) <= g_iWings3RecoverFullManaSuccessRate )
    {
            ...
            if ( lpTargetObj->Mana > (lpTargetObj->MaxMana+lpTargetObj->AddMana-1.0) )
                            lpTargetObj->Mana = lpTargetObj->MaxMana+lpTargetObj->AddMana;

            GCManaSend(lpTargetObj->m_Index,(short)lpTargetObj->Mana,0xFF,FALSE,lpTargetObj->BP);

            //Debug
            if ( g_bWings3WriteOptionsLog )
            {
                    LogAdd("[%s][%s] [ThirdWings] [Recovered Mana: %d] OptionRate:[%d%%]", lpTargetObj->AccountID, lpTargetObj->Name, tMana, g_iWings3RecoverFullManaSuccessRate);
            }
            if ( g_bWings3ShowInGameOptions )
            {
...
int  CObjAttack::GetAttackDamage(LPOBJ lpObj, int targetDefense, BYTE& effect, BOOL bIsOnDuel, CMagicInf* lpMagic)
{
	...
	if ( lpObj->Type == OBJ_MONSTER || lpObj->Type == OBJ_NPC )
	{
		...
		AttackMin = lpObj->m_AttackDamageMin + Random(0,sub);
	...
	else
	{
		...
		__try
		{
			...
			lpObj->m_AttackDamageRight = ( lpObj->m_AttackDamageMinRight + SkillRightMinDamage ) + Random(0,sub);
		}
		__except ( sub=1, 1 )
		{
		...
		__try
		{
			...
			lpObj->m_AttackDamageLeft = ( lpObj->m_AttackDamageMinLeft + SkillLeftMinDamage ) + Random(0,sub);
		}
		__except ( sub=1, 1 )
		{
...
int  CObjAttack::GetAttackDamageWizard(LPOBJ lpObj, int targetDefense, CMagicInf* lpMagic, BYTE& effect, BOOL bIsOnDuel)
{
	...
	__try
	{
		if ( bIsOnDuel )
		{
			ad = ( damagemin + Random(0,subd) ) * 60 / 100 - targetDefense;	// #formula
		}
		else
		{
			ad = ( damagemin + Random(0,subd) ) - targetDefense;
		}
...
int CObjAttack::GetShieldDamage(LPOBJ lpObj, LPOBJ lpTargetObj, int iAttackDamage)
{
	...
	{
		if ( Random(0,99) < lpObj->m_JewelOfHarmonyEffect.HJOpAddIgnoreSDRate )
		{
			iDamageDevideToSDRate = 0;
		}
	}
	...
	{
		LogAdd("[JewelOfHarmony][PvP System] Attacker:[%s][%s]-SD Decrease[%d] SD Ignore[%d] Defender:[%s][%s] SD Increase Option[%d] - SD Rate[%d]",
	...
	iReduceLifeForEffect = int( ( lpTargetObj->MaxLife + lpTargetObj->AddLife ) * 20.0 / 100.0);
