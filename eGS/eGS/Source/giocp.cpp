...
//#endif
	...
	TerminateThread( g_ThreadHandles[0] , 0);
...
int CreateListenSocket()
{
	...
	else
	{
		...
		if ( nRet == -1 )
		{
			MsgBox("bind error (Server can be launched twice. Please terminate game server and restart.");
...
unsigned long __stdcall IocpServerWorker(void * p)
{
	...
	__try
	{
		...
		else
		{
			while ( true )
			{
				...
				{
					LogAdd("error-L1 : %d %d gObjAdd() failed with error %d", Accept, ClientIndex, GetLastError() );
					closesocket(Accept);
					LeaveCriticalSection(&criti);
					continue;
				}
				...
				ZeroMemory(&gObj[ClientIndex].SecurePackets,sizeof(gObj[ClientIndex].SecurePackets));
	...
	__finally
	{
		...
		if ( g_CompletionPort != NULL )
		{
			for ( int i = 0 ; i < (int)g_dwThreadCount ; i++ )
			{
...
#ifndef PUBLIC
	gObj[uIndex].SecurePackets.PacketsCountInOneSec++; //зараховуємо новий пакет
	DWORD CurrentPacketTime = GetTickCount();//зчитуємо поточний час

	if (CurrentPacketTime - gObj[uIndex].SecurePackets.OldPacketTime > 1000)//це такий міні таймер на 1с 
	{
		if (gObj[uIndex].SecurePackets.PacketsCountInOneSec > gMaxPacketsPerSec )//якщо за 1с прийшло більше ніж N пакетів від 1го клієнта 
			return false;//закриваємо клієнт

		if (gObj[uIndex].SecurePackets.PacketsCountInOneSec > gObj[uIndex].SecurePackets.MaxPaketsPerSec)
			gObj[uIndex].SecurePackets.MaxPaketsPerSec = gObj[uIndex].SecurePackets.PacketsCountInOneSec;
		gObj[uIndex].SecurePackets.OldPacketTime = CurrentPacketTime;//зараховуємо час
		gObj[uIndex].SecurePackets.PacketsCountInOneSec = 0;//зкидуємо к-сть пакетів
	}
#endif
			...
			if ( xcode == 0xC3 )
			{
				...
				else
				{
					...
					if ( PacketStream.AddData(&byDec[1], ret +2) == 0 )
					{
						...
						LogAdd( "error-L1 : CStreamPacketEngine Adding Error : ip = %s account:%s name:%s HEAD:%x (%s,%d) State:%d",
							gObj[uIndex].Ip_addr,
					...
					{
						LogAdd( "error-L1 : CStreamPacketEngine ExtractPacket Error : ip = %s account:%s name:%s HEAD:%x (%s,%d) State:%d",
					...
					{
						LogAdd("ѕПИЈЖРЕ¶ : [0x%x]" , headcode); // Wrong Password
					}
...
bool DataSendSocket(SOCKET& sock, unsigned char* lpMsg, DWORD dwSize)
{
	if (send(sock,(char*)lpMsg,dwSize,0) == SOCKET_ERROR)
		return false;
	else
		return true;
}
...
BOOL DataSend(int aIndex, unsigned char* lpMsg, DWORD dwSize)
{
	...
	if ( aIndex < OBJ_STARTUSERINDEX || gObj[aIndex].bot)
	{
		return true;
	}
