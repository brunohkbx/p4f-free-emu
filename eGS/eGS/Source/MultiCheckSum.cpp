...
#include "..\Header\MultiCheckSum.h"
...
{
	strcpy_s(this->m_sFilePath, sizeof(this->m_sFilePath), filepath);
}
...
int CMultiCheckSum::LoadFile()
{
	...
	for (i=0; i<MAX_MULTICHECKSUM; i++)
	{
		...
		{
			this->m_dwLoaded[i]=1;
			LogAdd("Load Checksum file : %s", filenames);
		}
	}
	return 1;
}
...
int CMultiCheckSum::CompareCheckSum(int aIndex, DWORD TableNum, DWORD Key)
{
	...
	for ( i=0;i<MAX_MULTICHECKSUM;i++)
	{
		if(this->m_dwLoaded[i] != 0 )
		{
			...
			{
				LogAdd("CheckSum%d-Exe Compare Success [%s]", i, gObj[aIndex].AccountID );
				return 1;
			}
			else
			{
				LogAdd("CheckSum%d-Exe Compare Fail %d %d %d %d [%s]", i, this->m_dwgCheckSum[i][TableNum] ,Key, TableNum, gObj[aIndex].AccountID );
			}
		}
	}
	return 0;
