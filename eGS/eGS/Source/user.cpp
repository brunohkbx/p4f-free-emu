...
bool gObjLevelUp(LPOBJ lpObj, int addexp, int iMonsterType, int iEventType)
{
	...
	{
		int NExp = 0;//лічильник експи
		while (lpObj->Experience > lpObj->NextExp)//поки к-сть отриманої експи достатньо для лвл апа
		{
			NExp = lpObj->Experience - lpObj->NextExp;// отримуємо залишок експи після лвл апа
			//починаємо брати лвл ап
			...
			if (NExp > 0)//якщо к-сть експи більше від нуля
				lpObj->Experience += NExp;// додаємо її до загальної експи і ще раз берем лвл ап
		}
	}
...
void gObjUserDie(LPOBJ lpObj, LPOBJ lpTargetObj)
{
	...
	if(lpObj->Type == OBJ_USER && lpTargetObj->Type == OBJ_USER)
	{
		...
		//lpObj - кого убили , lpTargetObj
		//KillerName - имя убийцы кто убил персонажа lpObj; при смерти lpObj , записывается lpTargetObj как его убийца
		//KillingName - кого lpTargetObj убил ; при смерти lpObj, записывается lpObj которого lpTargetObj убил
...
BOOL TradeitemInventoryPut(int aIndex)
{
	...
	for(int n = 0; n < TRADE_BOX_SIZE; n++)
	{
		if(gObj[number].Trade[n].IsItem() == 1)
		{
			...
			{
				LogAdd("error : АМ°З Е«АПіЄґВАП!!"); //#error
				return false;
			}
		}
	}
...
bool RefreshMysteryShop()
{
	...
	LPOBJ lpObj;
	//---
	int Number = 1;//порядковий номер гравця в БД
	int ItemsInShop = 0;//скільки добавлено речей в Шоп
	int DBLevel = 0;//рівень гравця який береться з БД
	int old_DBLevel = -1;
	int Rand = 0;//випадкове число з діапазону - 0;gMysteryShop_Type.size()-1
	//---
	char SQLBuf[10]={0};//для переведення в чисела в SQLString
	//---
	std::string Nick;//імя гравця взяте з БД
	std::string old_Nick = "-1";
	std::string SQLString;//запит для БД
	//---
	std::vector<std::string> ListNames;
	while (1)
	{
		Nick = "";
		DBLevel = -1;//занулюємо початкові умови
		lpObj = NULL;
		...
		if ( Nick.empty() || DBLevel == -1 )//якщо не вийшло прочитати записи з БД - виходимо
			break;
		...
		ListNames.push_back(Nick);//добавляємо найденого персонажа в список персонажів для яких потім необхідно буде оновити магазин у грі

		SQLString = "UPDATE MuOnline.dbo.Character SET MysteryShop=('";//починаємо формування запиту для БД

		for (ItemsInShop = 0 ; ItemsInShop != gMysteryShopMaxItems; )//заповнюємо по максимуму речами
		{
			Rand = Random(0,gMysteryShop_Type.size()-1);//вибираємо випадкову річ зі списку
		...
		SQL.Execute((LPTSTR)SQLString.c_str());//складений список речей заносимо в БД. Але наразі у гравця в грі цей список відсутній. 
		//І якщо він вийде з гри то в нього список не оновится. Тому необхідно найти цього гравця у грі і оновити йому магазин
	...
	{
		lpObj = &gObj[i];
		if (lpObj->Connected != PLAYER_PLAYING || lpObj->Type != OBJ_USER)//якщо ігрок не в грі то нема потреби оновлювати йому магазин
			continue;

		for (int a = 0; a < ListNames.size(); a++)//якщо ж на данному слоті присутній гравець тоді починаємо звіряти список з іменем гравця
		{
...
void UserSaveMysteryShopFromDB(LPOBJ lpObj)
{
	...
	std::string SQLString = "UPDATE MuOnline.dbo.Character SET MysteryShop=('";//стрінг тому що повної розмірності шопа не знаємо
