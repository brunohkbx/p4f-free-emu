...
void CCrywolfAltar::SetAltarValidContract(int iAltarObjIndex, int iClass)
{
	...
	{
		MsgOutput(AltarInfo->m_iUserIndex, "Connected to the pedestal [%d] of the Holy wolf", iAltarNumber);
		UTIL.SendCrywolfUserAnyMsg(2, "[%s] connected the Muse to the pedestal [%d]", gObj[AltarInfo->m_iUserIndex].Name, iAltarNumber);

		LogAdd("[ Crywolf ][Altar Op.]  [%s][%s] Set Valid Contract Altar[%d]",
...
BOOL CCrywolfAltar::SetAltarUserIndex(int iAltarObjIndex, int iClass, int iUserIndex)
{
	...
	{
		MsgOutput(iUserIndex, "The pedestal [%d] of Holy wolf has been connected", iAltarNumber);
		return FALSE;
	}

	if ( (dwCurrentTime-AltarInfo->m_dwLastValidContractTime) < 10000 )
	{
		MsgOutput(iUserIndex, "The connecting time of pedestal [%d] has not started yet", iAltarNumber);
		return FALSE;
	}

	if ( AltarInfo->m_iContractCount > MAX_ALTAR_CONTRACTS ) 
	{
		MsgOutput(iUserIndex, "The time for connecting to pedestal [%d] of the holy wolf is over", iAltarNumber);
		return FALSE;
	}

	if ( abs(gObj[iAltarObjIndex].Y - gObj[iUserIndex].Y) > 0 || abs(gObj[iAltarObjIndex].X - gObj[iUserIndex].X) > 0 )
	{
		MsgOutput(iUserIndex, "The position to connect to pedestal [%d] of the holy wolf is incorrect. Try again", iAltarNumber);
		return FALSE;
	}
	...
	UTIL.SendCrywolfUserAnyMsg(2, "%s is trying to contract altar %d!", gObj[AltarInfo->m_iUserIndex].Name, iAltarNumber);

	LogAdd("[ Crywolf ][Altar Op.] [%s][%s] Attempt to contract Altar[%d]",
		gObj[AltarInfo->m_iUserIndex].AccountID, gObj[AltarInfo->m_iUserIndex].Name,
		iAltarNumber);

	MsgOutput(iUserIndex, "You can start connecting in 5 seconds");
	MsgOutput(iUserIndex, "Do not move until the connecting is completed.");
...
void CCrywolfAltar::ResetAltarUserIndex(int iAltarObjIndex, int iClass)
{
	...
	if ( gObjIsConnected(AltarInfo->m_iUserIndex) != FALSE )
	{
		MsgOutput(AltarInfo->m_iUserIndex, "Connecting to the pedestal [%d] is canceled.", iAltarNumber);
		LogAdd("[ Crywolf ][Altar Op.] [%s][%s] Remove contract Altar[%d]",
		gObj[AltarInfo->m_iUserIndex].AccountID, gObj[AltarInfo->m_iUserIndex].Name,
		iAltarNumber);
	}
...
int CCrywolfAltar::GetPriestMaxHPSum()
{
	...
	for ( int i=0;i<MAX_CRYWOLF_ALTAR;i++)
	{
		...
		LPOBJ lpUserObj = &gObj[AltarInfo->m_iUserIndex];
		iPriestHPSum += lpUserObj->AddLife + (int)lpUserObj->MaxLife + lpUserObj->m_SkillAddLife;
	}
