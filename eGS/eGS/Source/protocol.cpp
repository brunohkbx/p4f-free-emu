...
#include "..\Header\KanturuEntranceNPC.h"
...
void ProtocolCore(BYTE protoNum, unsigned char * aRecv, int aLen, int aIndex, BOOL Encrypt, int serial)
{
	...
	else
	{
		...
		switch ( protoNum )
		{
            ...
            case 0xBF:
				{
					...
					{
					case 0x00:
						ILAPI_RegisterMember(aIndex, aRecv[5], aRecv[4]);
						break;

					case 0x02: 
						ILSKILL_SkillUse(aIndex, (aRecv[5] * 256) + aRecv[6], aRecv[4]);
						break;
					...
					//Illusion Temple Prize Case
					//GCIllusionTempleSendReward(aIndex);
					break;
				}
				break;
			case 0xE1:
			...
			default:
				LogAdd( "error-L2 : account:%s name:%s HEAD:%x (%s,%d) State:%d",
...
void CGLiveClient(PMSG_CLIENTTIME * lpClientTime, short aIndex)
{
	...
	if ( gObj[aIndex].CheckSpeedHack != false )
	{
		...
		if ( checktime < -7000 )
		{
			LogAdd("(%s)(%s) Time synchronization poorness caused to block connection %d", gObj[aIndex].AccountID, gObj[aIndex].Name, checktime);
			CloseClient(aIndex);
		}
	...
	if ( gObj[aIndex].Connected == PLAYER_PLAYING )
	{
		...
		{
			LogAdd( "[%s][%s][%s] ClientHack Detected : Editing AttackSpeed [%d][%d] [%d][%d]",
...
void GCCheckMainExeKeySend(int aIndex)
{
	...
	gObj[aIndex].CheckSumTableNum = Random(0,MAX_CHECKSUM_KEY-1);
...
void CGCheckMainRecv(PMSG_CHECK_MAINEXE_RESULT * lpMsg, int aIndex)
{
	...
	if (isCheckSum)
	{
		...
		{
			LogAdd("error-L1 : [%s][%s] CheckSum-Exe error ", lpObj->AccountID, lpObj->Name);
			CloseClient(aIndex);
			return;
		}
		...
		{
			LogAdd("error-L1 : CheckSum-Exe error %d %d %d [%s]", dwgCheckSum[lpObj->CheckSumTableNum],
	...
	{
		Print(aIndex,0,ConnectNotice);

		if (IsGM(aIndex))
		{
			GMOnline.push_back(gObj[aIndex].Name);
			TotalGMOnline++;
		}
		else
			TotalOnline++;
...
/*	char szTempMsg[4096] = {0};
	...
	strcpy_s(pMsg.chatmsg, sizeof(pMsg.chatmsg), szChat);
...
void vChatSend(LPOBJ lpObj, char* szChat, ...)
{
	...
	vsprintf_s(szBuffer, szChat, pArguments);
	va_end(pArguments);
...
/*	char szTempMsg[4096] = {0};
	...
	PHeadSetB((LPBYTE)&pMsg, 0x01, len+6);
	strcpy_s(pMsg.chatmsg, sizeof(pMsg.chatmsg), szBuffer);
	pMsg.NumberH = SET_NUMBERH(lpObj->m_Index);
	pMsg.NumberL = SET_NUMBERL(lpObj->m_Index);
...
void PChatProc(PMSG_CHATDATA * lpChat, short aIndex)
{
	...
	{
		sout<<"[Anti-HACK][PChatProc]["<<lpObj->AccountID<<"]["<<lpObj->Name<<"] Chat Message Len :"<<slen;
		LogAdd(sout.str());
		return;
	}

	if (lpObj->ChatTickCount < (GetTickCount() - 1000))
		return;
	...
	char ChatMsg[80]={0};
	strcpy_s(ChatMsg,lpChat->chatmsg);
#ifndef PUBLIC
	int BanH=0;
	char *tok;
	char* tmp_str=strtok_s(ChatMsg," \0",&tok);
	while (tmp_str!=NULL)
	{
		for (int i=0; i < (int)AntiMat.size(); i++)
		{
			if (_stricmp(tmp_str,AntiMat[i].c_str()) == 0)
			{
				BanH+=BanHours[i];
			}
		}
		tmp_str=strtok_s(NULL," \0",&tok);
	}

	if (BanH>0)
	{
// 		char sql[MAX_PATH]={0};
// 		wsprintf(sql,"UPDATE MuOnline.dbo.Character SET CtlCode=('1') WHERE Name=('%s')",lpObj->Name);
// 		SQL.Execute(sql);
// 		CloseClient(aIndex);
// 		SYSTEMTIME time;
// 		GetLocalTime(&time);
...
// 		}
// 
// 		while (time.wDay > MaxDays(time.wMonth,time.wYear) )
// 		{
// 			time.wMonth++;
// 			time.wDay -= MaxDays(time.wMonth,time.wYear);
// 		}
// 
...
// 
// 		char sql2[30]={0};
// 		sprintf_s(sql2,sizeof(sql2),"%d %d %d %d",time.wHour,time.wDay,time.wMonth,time.wYear);
// 		wsprintf(sql,"UPDATE MuOnline.dbo.Character SET UnBanTime=('%s') WHERE Name=('%s');",sql2, lpObj->Name);
// 		SQL.Execute(sql);
// 		return;
	}
	...
	switch ( lpChat->chatmsg[0] )
	{
		case '!':	// Global Announcement
			if ( slen > 2 )
			{
				...
				{
					if (g_InsertGMNickWhenGlbMsg)
					{
						char tmp[80]={0};
						sprintf_s(tmp,sizeof(tmp),"%s: %s",lpObj->Name,&lpChat->chatmsg[1]);
						memcpy(&lpChat->chatmsg[0],&tmp[0], MAX_CHAT_LEN-1);
						lpChat->chatmsg[MAX_CHAT_LEN]=0;
					}

					//DataSend(aIndex, (LPBYTE)lpChat, lpChat->h.size); - теперь сообщение ГМа не будет дублироватся в общий чат

					AllSendServerMsg(&lpChat->chatmsg[0]);

					sout<<"[Global]["<<lpObj->Name<<"]["<<lpObj->Ip_addr<<"] "<<lpChat->chatmsg;
					
					ChatLogAdd(sout.str());

					sout.str("");

					sout<<"Announcement: "<<gObj[aIndex].AccountID<<" "<<gObj[aIndex].Name<< " "<<lpChat->chatmsg;
					LogAdd(sout.str()); 
					sout.str("");
					return;
				}
		...
		case '/':	// Command
			if ( slen > 2 )
			{
				if ( memcmp(lpChat->chatmsg,"/drop ",strlen("/drop ")) == 0)
				{
					ChatDrop(aIndex,&lpChat->chatmsg[6]);
				}
				else if ( memcmp(lpChat->chatmsg,"/post ",strlen("/post ")) == 0 )
				{
					ChatPost(aIndex,&lpChat->chatmsg[6]);
				}
				else if ( memcmp(lpChat->chatmsg,"/add",strlen("/add")) == 0 )
				{
					ChatAdd(aIndex,&lpChat->chatmsg[4]);
				}
				else if ( memcmp(lpChat->chatmsg,"/online",strlen("/online")) == 0 )
				{
					ChatOnline(aIndex);
				}
				else if ( memcmp(lpChat->chatmsg,"/pkset ",strlen("/pkset ")) == 0 )
				{
					ChatPKSet(aIndex,&lpChat->chatmsg[7]);
				}
				else if ( memcmp(lpChat->chatmsg,"/zenset ",strlen("/zenset ")) == 0 )
				{
					ChatZenSet(aIndex,&lpChat->chatmsg[8]);
				}
				else if ( memcmp(lpChat->chatmsg,"/moveall ",strlen("/moveall ")) == 0 )
				{
					ChatMoveAll(aIndex,&lpChat->chatmsg[9]);
				}
				else if ( memcmp(lpChat->chatmsg,"/report ",strlen("/report ")) == 0 )
				{
					ChatReport(aIndex,&lpChat->chatmsg[8]);
				}
				else if ( memcmp(lpChat->chatmsg,"/reload ",strlen("/reload ")) == 0 )
				{
					ChatReoload(aIndex,&lpChat->chatmsg[8]);
				}
				else if ( memcmp(lpChat->chatmsg,"/disconnect ",strlen("/disconnect ")) == 0 )
				{
					ChatDisconnect(aIndex,&lpChat->chatmsg[12]);
				}
				else if ( memcmp(lpChat->chatmsg,"/skin ",strlen("/skin ")) == 0 )
				{
					ChatSkin(aIndex,&lpChat->chatmsg[6]);
				}
				else if ( memcmp(lpChat->chatmsg,"/banchar ",strlen("/banchar ")) == 0 )
				{
					ChatBanChar(aIndex,&lpChat->chatmsg[9]);
				}
				else if ( memcmp(lpChat->chatmsg,"/unbanchar ",strlen("/unbanchar ")) == 0 )
				{
					ChatUnBanChar(aIndex,&lpChat->chatmsg[11]);
				}
				else if ( memcmp(lpChat->chatmsg,"/banacc ",strlen("/banacc ")) == 0 )
				{
					ChatBanAccount(aIndex,&lpChat->chatmsg[8]);
				}
				else if ( memcmp(lpChat->chatmsg,"/unbanacc ",strlen("/unbanacc ")) == 0 )
				{
					ChatBanChar(aIndex,&lpChat->chatmsg[10]);
				}
	...
	if ( lpChat->chatmsg[0] == '~' || lpChat->chatmsg[0] == ']' )
	{
		if ( lpObj->PartyNumber >= 0 )
		{
			...
			if ( partycount >= 0 )
			{
				...
				sout<<"[Party]["<<lpObj->Name<<"]["<<lpObj->Ip_addr<<"] "<<lpChat->chatmsg;
	...
	else if ( lpChat->chatmsg[0] == '@' )
	{
		if ( lpObj->GuildNumber > 0 )
		{
			...
			if ( lpChat->chatmsg[1] == '>' )
			{
				if ( lpObj->Name[0] == lpObj->lpGuild->Names[0][0] )
				{
					if ( !strcmp(lpObj->Name, lpObj->lpGuild->Names[0] ) )
					{
						...
						sout<<"Guild Announcement: "<<lpObj->lpGuild->Name<<" "<<&lpChat->chatmsg[2];
						LogAdd(sout.str()); 
						sout.str("");
			...
			else if ( lpChat->chatmsg[1] == '@' && lpObj->lpGuild->iGuildUnion )
			{
				...
				sout<<"[Alliance]["<<lpObj->Name<<"]["<<lpObj->Ip_addr<<"] "<<lpChat->chatmsg;
			...
			else
			{
				...
				sout<<"[Guild]["<<lpObj->Name<<"]["<<lpObj->Ip_addr<<"] "<<lpChat->chatmsg;
	...
	{
		sout<<"[Local]["<<lpObj->Name<<"]["<<lpObj->Ip_addr<<"] "<<lpChat->chatmsg;
...
void ChatSkin(int aIndex, char * msg)
{
	int spaces = 0;
	int len=strlen(msg);
	...
	if (!isChatSkin)
	{
		Print(aIndex,0,ChatSkinErrorOff);
		return;
	}

	if (isChatSkinOnlyGM && !IsGM(aIndex))
	{
		Print(aIndex,0,ChatSkinErrorOnlyGM);
		return;
	}

	LPOBJ lpObj = &gObj[aIndex];
	lpObj->m_Change = atoi(msg);
...
void ChatBanChar(int aIndex, char * msg)
{
// 	int spaces = 0;
// 	int len=strlen(msg);
// 
...
// 	if (!isBanChar)
// 	{
// 		Print(aIndex,0,BanCharErrorOff);
// 		return;
// 	}
// 
// 	if (isBanCharOnlyGM && !IsGM(aIndex))
// 	{
// 		Print(aIndex,0,BanCharErrorOnlyGM);
// 		return;
// 	}
// 
// 	std::ostringstream sout;
// 	sout<<"UPDATE MuOnline.dbo.memb_info SET bloc_code = ('1') WHERE memb___id = ('"<<msg<<"')";
// 	SQL.Execute((LPSTR)sout.str().c_str());
// 
// 	int TargetIndex=0;
// 
// 	for (int n = OBJ_STARTUSERINDEX; n < OBJMAX; n++)
// 	{
// 		if (strcmp(msg,gObj[n].Name) == 0)
// 		{
// 			TargetIndex = n;
// 			break;
// 		}
...
void ChatUnBanChar(int aIndex, char * msg)
{
// 	int spaces = 0;
// 	int len=strlen(msg);
// 
...
// 	if (!isUnBanChar)
// 	{
// 		Print(aIndex,0,UnBanCharErrorOff);
// 		return;
// 	}
// 
// 	if (isUnBanCharOnlyGM && !IsGM(aIndex))
// 	{
// 		Print(aIndex,0,UnBanCharErrorOnlyGM);
// 		return;
// 	}
// 
// 	std::ostringstream sout;
// 	sout<<"UPDATE MuOnline.dbo.memb_info SET bloc_code = ('0') WHERE memb___id = ('"<<msg<<"')";
// 	SQL.Execute((LPSTR)sout.str().c_str());
}

//------------------------------------------------------------

void ChatBanAccount(int aIndex, char * msg)
{
// 	int spaces = 0;
// 	int len=strlen(msg);
// 
...
// 	if (!isBanAccount)
// 	{
// 		Print(aIndex,0,BanAccountErrorOff);
// 		return;
// 	}
// 
// 	if (isBanAccountOnlyGM && !IsGM(aIndex))
// 	{
// 		Print(aIndex,0,BanAccountErrorOnlyGM);
// 		return;
// 	}
// 
// 	std::ostringstream sout;
...
void ChatUnBanAccount(int aIndex, char * msg)
{
// 	int spaces = 0;
// 	int len=strlen(msg);
// 
...
// 	if (!isUnBanAccount)
// 	{
// 		Print(aIndex,0,UnBanAccountErrorOff);
// 		return;
// 	}
// 
// 	if (isUnBanAccountOnlyGM && !IsGM(aIndex))
// 	{
// 		Print(aIndex,0,UnBanAccountErrorOnlyGM);
// 		return;
// 	}
// 
// 	std::ostringstream sout;
...
void ChatReport(int aIndex,LPCSTR IpBuffer)
{
	...
	if (!isReport)
	{
		Print(aIndex,1,g_ReportOffError);
		return;
	}
	...
	char time[11]={0};
	wsprintf(time,"%d.%d.%d",systime.wDay,systime.wMonth,systime.wYear);
	char tmp[MAX_PATH]={0};
	wsprintf(tmp,"..\\Reports\\%s",time);
	if (!FolderExists(tmp))
		CreateDirectory(tmp,NULL);
	wsprintf(tmp,"..\\Reports\\%s\\%s",time,gObj[aIndex].Name);
	if (!FolderExists(tmp))
		CreateDirectory(tmp,NULL);
	FILE* f;
	wsprintf(tmp,"..\\Reports\\%s\\%s\\Report.txt",time,gObj[aIndex].Name);
	if ( fopen_s(&f,tmp, "a+") != 0 )
	{
		return;
	}
	char write[MAX_PATH]={0};
	wsprintf(write,"Time: %s\nLogin: %s\nReport: %s\n\n\n",Time().c_str(),gObj[aIndex].AccountID,IpBuffer);
	fputs(write,f);
	fclose(f);
	Print(aIndex,1,g_ReportSend);
}

//------------------------------------------------------------

void ChatDisconnect(int aIndex, char * msg)
{
	int spaces = 0;
	int len=strlen(msg);
	...
	if (!isDisconnect)
	{
		Print(aIndex,0,DisconnectErrorOff);
		return;
	}

	if (isDisconnectOnlyGM && !IsGM(aIndex))
	{
		Print(aIndex,0,DisconnectErrorOnlyGM);
		return;
	}
	...
	if (TargetIndex!=0)//ф-ция gObjByNick возвращает 0xFFFF если не удалось найти такого игрока
	{
		if (gObj[TargetIndex].Connected == PLAYER_PLAYING)//проверяем находится ли данный игрок в сети
		{
			CloseClient(TargetIndex);

			Print(aIndex,1,DisconnectMsg);
		}
		else
		{
			Print(aIndex,1,DisconnectPlayerOFFMsg);
			return;
		}
	}
	else
...
void ChatMoveAll(int aIndex, char * msg)
{
	int spaces = 0;//щитаем количество пробелов в веддоной строке
	int len=strlen(msg);
	...
	if (!isMoveAll)
	{
		Print(aIndex,1,g_MoveAllOffError);
		return;
	}

	if(IsGM(aIndex) == false)//проверяем ето ли ГМ
	{
		Print(aIndex,1,g_MoveAllOnlyGM);
		return;
	}
	...
	if (TargetIndex!=0)//ф-ция gObjByNick возвращает 0xFFFF если не удалось найти такого игрока
	{
		if (gObj[TargetIndex].Connected == PLAYER_PLAYING)//проверяем находится ли данный игрок в сети
		{
			int X=gObj[TargetIndex].X;
			int Y=gObj[TargetIndex].Y;
			int MapID=gObj[TargetIndex].MapNumber;
			for (int n=OBJ_STARTUSERINDEX;n<OBJMAX;n++)
			{
				if (gObj[n].Connected==PLAYER_PLAYING)
				{
					gObjTeleport(n,MapID,X,Y);
				}
			}

			Print(aIndex,1,MoveAllMsg);
		}
		else
		{
			Print(aIndex,1,MoveAllPlayerOFFMsg);
			return;
		}
	}
	else
...
void ChatZenSet(int aIndex, char * msg)
{
	int spaces = 0;//щитаем количество пробелов в веддоной строке
	int len=strlen(msg);
	...
	if (!isZenSet)
	{
		Print(aIndex,1,g_ZenSetOffError);
		return;
	}

	if(IsGM(aIndex) == false)//проверяем ето ли ГМ
	{
		Print(aIndex,1,g_PKSetOnlyGM);
		return;
	}
	...
	if(spaces < 1)//если было меньше 1го пробела (Ник игрока" "количество зен) то щитаем что был не правильный вызов ф-ции
		return;

	char* tok;
	char *Name=strtok_s(msg," ",&tok);//Забираем из входящих данный ник
	char *Zen=strtok_s(NULL," ",&tok);//а потом забираем уровень ПК
	...
	if (TargetIndex!=0)//ф-ция gObjByNick возвращает 0xFFFF если не удалось найти такого игрока
	{
		if (gObj[TargetIndex].Connected == PLAYER_PLAYING)//проверяем находится ли данный игрок в сети
		{
			...
			gObj[TargetIndex].Money=zen;// ставим ПК лвл
			GCMoneySend(aIndex,gObj[TargetIndex].Money);
			Print(aIndex,1,ZenSetMsg);
		}
		else
		{
			Print(aIndex,1,ZenSetPlayerOFFMsg);
			return;
		}
	}
	else
...
void ChatPKSet(int aIndex, char * msg)
{
	if (!isPKSet)
	{
		Print(aIndex,1,g_PKSetOffError);
		return;
	}
	...
	if(spaces < 1)//если было меньше 1го пробела (Ник игрока" "уровень ПК) то щитаем что был не правильный вызов ф-ции
		return;
	...
	char* tok;
	char *Name=strtok_s(msg," ",&tok);//Забираем из входящих данный ник
	char *PKLvl=strtok_s(NULL," ",&tok);//а потом забираем уровень ПК

	int pklvl=atoi(PKLvl);
	int TargetIndex=0;
	...
	if (TargetIndex!=0)//ф-ция gObjByNick возвращает 0xFFFF если не удалось найти такого игрока
	{
		if (gObj[TargetIndex].Connected == PLAYER_PLAYING)//проверяем находится ли данный игрок в сети
		{
			if (pklvl>6)// если задали уровень ПК больше чем 6 (6 - максимум) делаем его равным 6
				pklvl=6;

			if (pklvl<1)
				pklvl=1;

			gObj[TargetIndex].m_PK_Level=pklvl;// ставим ПК лвл
			GCPkLevelSend(aIndex,gObj[TargetIndex].m_PK_Level);
			Print(aIndex,1,PKSetSetPKMsg);
		}
		else
		{
			Print(aIndex,1,PKSetPlayerOFFMsg);
			return;
		}
	}
	else
...
void ChatOnline(int aIndex)
{
	if (!isUsersOnline)
	{
		Print(aIndex,0,UsersOnlineErrorOff);
		return;
	}

	if (isUsersOnlineOnlyGM && !IsGM(aIndex))
	{
		Print(aIndex,0,UsersOnlineErrorOnlyGM);
		return;
	}

	Print(aIndex,0,g_UsersOnline);
	return;

}

void ChatAdd(int aIndex,char* text)
{
	int Type=-1;
	int Points=0;
	...
	if (!isAdd)
	{
		Print(aIndex,1,g_AddOffError);
		return;
	}

	if ( isAddOnlyForGM && !IsGM(aIndex) )
	{
		Print(aIndex,1,g_AddOnlyForGM);
		return;
	}

	if (g_AddNeedLvl > gObj[aIndex].Level)
	{
		Print(aIndex,1,g_AddNotEnoughLvl);
		return;
	}

	if ( memcmp(text,"str",3)==0 )
		Type=0;
	else if ( memcmp(text,"agi",3)==0 )
		Type=1;
	else if ( memcmp(text,"vit",3)==0 )
		Type=2;
	else if ( memcmp(text,"ene",3)==0 )
		Type=3;
	else if ( memcmp(text,"com",3)==0 )
		Type=4;
	else
		Type = -1;
	...
	if (Type==-1)
	{
		Print(aIndex,1,g_AddWorngSyntax);
		return;
	}

	if ( Type == 4 && (gObj[aIndex].DbClass != 64 && gObj[aIndex].DbClass != 66) )
	{
		Print(aIndex,1,g_OnlyDLCanAddCommand);
		return;
	}
	...
	if (g_AddCoast > 0)
	{
		if ( gObj[aIndex].Money < g_AddCoast)
		{
			Print(aIndex,1,g_AddNotEnoughMoney);
			return;
		}

		gObj[aIndex].Money -= g_AddCoast;
...
void ChatPost(int aIndex,char* text)
{
	if (strlen(text) < 1)
		return;
	...
	if (!isPost)
	{
		Print(aIndex,1,g_PostOff);
		return;
	}
	if ( isPostOnlyForGM && !IsGM(aIndex) )
	{
		Print(aIndex,1,g_PostOnlyForGM);
		return;
	}

	if (g_PostNeedLvl > lpObj->Level)
	{
		Print(aIndex,1,g_PostNotEnoughLvl);
		return;
	}

	if (g_PostCoast > 0)
	{
		if ( lpObj->Money < g_PostCoast)
		{
			Print(aIndex,1,g_PostNotEnoughMoney);
			return;
		}

		lpObj->Money -= g_PostCoast;
		GCMoneySend(aIndex,lpObj->Money);

	}
	std::ostringstream sout;
	sout<<"[POST] "<<text;
	GlobalMsg(aIndex,sout.str());

	sout.str("");
	sout<<"[POST]["<<lpObj->Name<<"]["<<lpObj->Ip_addr<<"] "<<text;
	ChatLogAdd(sout.str());
...
void ChatReoload(int aIndex, char* text)
{
	int spaces = 0;
	int len=strlen(text);
	...
	if (isReoloadOnlyForGM && !IsGM(aIndex))
	{
		Print(aIndex,0,g_RoloadOnlyForGMError);
		return;
	}
...
void ChatDrop(int aIndex,char* text)
{
	...
	Item[8]=Item[0] * 512 + Item[1];

	ItemSerialCreateSend ( aIndex , gObj[aIndex].MapNumber , (BYTE)gObj[aIndex].X , (BYTE)gObj[aIndex].Y , Item[8] , Item[2] , 0 , Item[3] , Item[4] , Item[5] , -1 , Item[6] , Item[7] );
...
void CGChatWhisperRecv(PMSG_CHATDATA_WHISPER* lpMsg, int aIndex)
{
	...
	memcpy(pWhisper.id,lpObj->Name,sizeof(pWhisper.id));
	strcpy_s(pWhisper.chatmsg, sizeof(pWhisper.chatmsg), lpMsg->chatmsg);
	...
	if ( len > 0 )
	{
		...
		char text[512]={0};
		sprintf_s(text,sizeof(text),"[Whisper][%s -> %s][%s] %s",lpObj->Name,gObj[index].Name ,lpObj->Ip_addr,lpMsg->chatmsg);
		...
		if ( gWriteChatLog != FALSE )
		{
			...
			pChatMsg.szChatMsg[MAX_CHAT_LEN] = 0;
			pChatMsg.btType = 5;
			strcpy_s(pChatMsg.szTargetName[szTargetNameCount],sizeof(pChatMsg.szTargetName[szTargetNameCount]), gObj[index].Name);
			szTargetNameCount++;
...
void CSPJoinIdPassRequest(PMSG_IDPASS* lpMsg, int aIndex)
{
	...
	{
		LogAdd( "error-L1: Serial error [%s] [%s]", id, serial);
		GCJoinResult(JS_BAD_CLIENT_VERSION, aIndex);
		CloseClient(aIndex);
		return;
	}
	...
	if (PacketCheckTime(lpObj) == FALSE )
	{
		LogAdd("error-L2 : Login time is over. (%d)(%s)", aIndex, id);
		CloseClient(aIndex);
		return;
	}
	...
	if ( gObj[aIndex].Connected != PLAYER_CONNECTED )
	{
		LogAdd("error-L2 : Authentication is allowed after connected. (%d)(%s)", aIndex, id);
		CloseClient(aIndex);
		return;
	}
	...
	if ( gObj[aIndex].LoginMsgSnd != FALSE )
	{
		...
		{
			LogAdd("error : It's after authentication information is sended. (%d)(%s)(%s)", aIndex, id, gObj[aIndex].Ip_addr );
		}
		else
		{
	...
	strcpy_s(spMsg.IpAddress, sizeof(spMsg.IpAddress), gObj[aIndex].Ip_addr);
	...
	LogAdd("join send : (%d)%s", aIndex, gObj[aIndex].AccountID);
...
void CSPJoinIdPassRequestTEST(PMSG_IDPASS * lpMsg, int aIndex)//#error FATAL EERROR
{
	...
	spMsg.Number = aIndex;
	wsprintf(szId, "Ѕёµ№АМ%d", logincounttest);
...
void GCJoinSocketResult(BYTE result, SOCKET Socket)
{
	...
	DataSendSocket(Socket, (unsigned char*)&pResult, pResult.h.size);
}
...
void CGClientCloseMsg(PMSG_CLIENTCLOSE * lpMsg, int aIndex)
{
	switch ( lpMsg->Flag )
	{
		case 0:	// Close Game
			...
			LogAdd("[UserSelClose] Go Exit [%s][%s] Map:[%d][%d/%d]",
		...
		case 2:	// ServerList
			...
			LogAdd("[UserSelClose] Go ServerList [%s][%s] Map:[%d][%d/%d]",
				gObj[aIndex].AccountID, gObj[aIndex].Name,
				gObj[aIndex].MapNumber, gObj[aIndex].X, gObj[aIndex].Y);
			break;
		...
		case 1:	// Select Character
			...
			LogAdd("[UserSelClose] Go CharSelect [%s][%s] Map:[%d][%d/%d]",
				gObj[aIndex].AccountID, gObj[aIndex].Name,
				gObj[aIndex].MapNumber, gObj[aIndex].X, gObj[aIndex].Y);
			break;
	}
...
void CGClientMsg(PMSG_CLIENTMSG* lpMsg, int aIndex)
{
	if ( lpMsg->Flag )
	{
		LogAdd("error-L1 : HackCheck [%s][%s] 0x%x 0x%x",
			gObj[aIndex].AccountID, gObj[aIndex].Name,
...
void CGPCharacterCreate( PMSG_CHARCREATE * lpMsg, int aIndex)
{
	...
	{
		LogAdd("Requested to create character before authentication. (%d) (%s)", aIndex, gObj[aIndex].Ip_addr);
		CloseClient(aIndex);
	...
	if ( gObj[aIndex].Connected == PLAYER_PLAYING )
	{
		LogAdd("[Anti-HACK][CGPCharacterCreate] Attempted Character Create during GamePlay. [%s][%s]",
			gObj[aIndex].AccountID, gObj[aIndex].Name);
		CloseClient(aIndex);
	...
	if ( !gCreateCharacter )
	{
		GCServerMsgStringSend("ј­№цєРЗТ ±в°ЈїЎґВ ДіёЇЕНё¦ »эјєЗТјц ѕшЅАґПґЩ", aIndex, 1); //#error
		JGCharacterCreateFailSend(aIndex, lpMsg->Name);
	...
	if (!GMCreateCharacters)
	{
		for (int i=0 ; i<6 ; i++)
		{
			if (gObj[aIndex].GMHeroes[i])
			{
				GCServerMsgStringSend("Game Master can`t create additional characters", aIndex, 0);
				JGCharacterCreateFailSend(aIndex, lpMsg->Name);
	...
	if ( lpMsg->ClassSkin == 0x30 ) // MG
	{
		...
		{
			LogAdd( "error-L1: Magumsa Character create error [%s]", gObj[aIndex].AccountID);
			JGCharacterCreateFailSend(aIndex, lpMsg->Name);
			return;
		}
	}
	...
	if ( lpMsg->ClassSkin == 0x40 ) // DL
	{
		...
		{
			LogAdd( "error-L1: Darklord Character create error [%s]", gObj[aIndex].AccountID);
			JGCharacterCreateFailSend(aIndex, lpMsg->Name);
			return;
		}
	}
	...
	LogAdd("Character create : %s,%s", szTemp2, szTemp);

	wsDataServerCli.DataSend((char*)&sCreate, sCreate.h.size);
}
...
void CGPCharDel(PMSG_CHARDELETE * lpMsg,int aIndex)
{
	...
	if ( lpObj->Connected == PLAYER_PLAYING )
	{
		LogAdd("[Anti-HACK][CGPCharDel] Attempted Character Delete during GamePlay. [%s][%s]",
	...
	{
		LogAdd("[Anti-HACK][CGPCharDel] Attempted Character Delete during Prevented Time. [%s][%s]",
			lpObj->AccountID, lpObj->Name);
		CloseClient(aIndex);
		return;
	}
	...
	//lpObj->Level=SQL.GetFieldInt("MuOnline.dbo.Character","cLevel","WHERE Name = '%s'",lpMsg->Name);//это дурацкий ГС, и ожидать от него чего то большего не стоит...
	//при заходе на аккаунт с персонажа не считывается никакая информация, собственно и левел = 0... для корректного удаления на чара надо сначала ЗАЙТИ а потом удалять
	//можно и читерить так... например если чар 80 лвл и ему надо быть 200 чтобы удалится то можно просто зайти на чара 200+ лвл а потом удалить того мелкого чара...
	if ( lpObj->Level < gCharacterDeleteMinLevel)
	{
		pResult.result = 2;
		DataSend(aIndex, (LPBYTE)&pResult, pResult.h.size);
		char str[128]={0};
		sprintf_s(str,sizeof(str),"You may delete character after %d level",gCharacterDeleteMinLevel);
		GCServerMsgStringSend(str, aIndex, 1);
		return;
	}
	...
	if ( lpObj->GuildNumber > 0 && lpObj->lpGuild != NULL)
	{
		...
		memcpy(pCDel.GuildName, lpObj->lpGuild->Name, MAX_GUILD_LEN);
	}
	...
	LogAdd("Character delete : %s,%s", szTemp2, szTemp);

	wsDataServerCli.DataSend((char*)&pCDel, pCDel.h.size);
	lpObj->Level = 0;
}
...
void CGPCharacterMapJoinRequest( PMSG_CHARMAPJOIN * lpMsg, int aIndex)
{
	...
	char _name[MAX_ACCOUNT_LEN+1]={0};
	SDHP_DBCHARINFOREQUEST pCRequest;

	PHeadSetB((LPBYTE)&pCRequest, 0x06, sizeof(pCRequest));

	//strncpy_s(_name,sizeof(lpMsg->Name), lpMsg->Name, 10 );
	memcpy(_name,lpMsg->Name,sizeof(lpMsg->Name));
	BuxConvert(_name, MAX_ACCOUNT_LEN);
	_name[10]='\0';
	strcpy_s(pCRequest.Name,sizeof(pCRequest.Name), _name);
	strcpy_s(pCRequest.AccountID, sizeof(pCRequest.AccountID), gObj[aIndex].AccountID);
	pCRequest.Number = aIndex;

	wsDataServerCli.DataSend((char*)&pCRequest, pCRequest.h.size);
}
...
void GCGoalSend(int aIndex, std::string Name1, BYTE score1, std::string Name2, BYTE score2)
{
	PMSG_GOALSEND pMsg;

	PHeadSubSetB((LPBYTE)&pMsg, 0xF3, 0x23, sizeof(pMsg));
	strcpy_s(pMsg.RedTeamName,sizeof(pMsg.RedTeamName),Name1.c_str());
	pMsg.RedTeamScore = score1;
	strcpy_s(pMsg.BlueTeamName,sizeof(pMsg.BlueTeamName), Name2.c_str());
...
void GCSkillKeyRecv(PMSG_SKILLKEY * lpMsg, int aIndex)
{
	...
	LogAdd("Option Recv %d %d %d %d", lpMsg->GameOption,
...
void GCSkillKeySend(int aIndex, LPBYTE keybuffer, BYTE GO, BYTE Qk, BYTE Wk, BYTE Ek, BYTE ChatWnd)
{
	...
	DataSend(aIndex, (UCHAR*)&pMsg, pMsg.h.size);
	LogAdd("Option Send %d %d %d %d", GO, Qk, Wk, Ek);
}
...
void CGItemGetRequest(PMSG_ITEMGETREQUEST * lpMsg, int aIndex)
{
	...
	if ( gObj[aIndex].m_IfState.use != 0 )
	{
		...
		{
			GCServerMsgStringSend("You can't get the item", aIndex, 1);
			pResult.result = -1;
			pResult.h.size -= sizeof(pResult.Data);
			DataSend(aIndex, (UCHAR*)&pResult, pResult.h.size);
	...
	{
		LogAdd("[%s][%s] CGItemGetRequest() Failed : Transaction == 1, IF_TYPE : %d",
	...
	if ( lpItem->IsItem() == TRUE && lpItem->Give == false && lpItem->live == true)
	{
			...
			switch ( lpItem->m_Level )
			{
				case 0:
				{
					...
					if ( iWRCount > 0 )
					{
						...
						DataSend(aIndex, (LPBYTE)&pResult, pResult.h.size);
						GCServerMsgStringSend("You already possess the Wizard's Ring.", aIndex, 1);
		...
		if ( lpItem->m_Type == ITEMGET(13,38) )	// MoonStonePendant
		{
			...
			if ( count > 0 )
			{
				...
				DataSend(aIndex, (LPBYTE)&pResult, pResult.h.size);	
				GCServerMsgStringSend("You already had hang the stone, therefore was unable to pick again.", aIndex, 1);
		...
		if ( lpItem->m_Type == ITEMGET(13,39) )	// ChangeRing
		{
			...
			if ( count > 0 )
			{
				...
				DataSend(aIndex, (LPBYTE)&pResult, pResult.h.size);	
				GCServerMsgStringSend("You may have only one ring.", aIndex, 1);
		...
		strcpy_s(szItemName, sizeof(szItemName), lpItem->GetName());

		if ( lpItem->m_Type == ITEMGET(14,15) ) // Zen
		{
			if ( MapC[map_num].ItemGive(aIndex, item_num, false) == TRUE )
			{
				if ( !gObjCheckMaxZen(aIndex, (int&)lpItem->m_BuyMoney))
				{
					if ( gObj[aIndex].Money < (int)MAX_ZEN )
					{
		...
		else
		{
			...
			if ( lpItem->m_Type == ITEMGET(14,29) ) // Symbol of Kundun
			{
				...
				{
					int pos = ::g_KalimaGate.CheckOverlapKundunMark(aIndex, (BYTE)lpItem->m_Level);
					...
					if ( MAIN_INVENTORY_RANGE(pos) )
					{
						if ( MapC[map_num].ItemGive(aIndex, item_num, true) == TRUE )
						{
							...
							LogAdd("***0477*** [%s][%s] (%d) X: %d/ Y: %d Get serial:%d (%s)(%d)(%d)(%d)(%d)(%d) Dur: (%d) Ex:(%d,%d,%d,%d,%d,%d,%d) Set : (%d)", 
								gObj[aIndex].AccountID, gObj[aIndex].Name, map_num,
							...
							{
								int NewDur = (int)gObj[aIndex].pInventory[pos].m_Durability - 5;
								::gObjInventoryItemSet(aIndex, pos, -1);
								gObj[aIndex].pInventory[pos].Clear();
								::GCInventoryItemDeleteSend(aIndex, pos, 1);
								::ItemSerialCreateSend(aIndex, 235, (BYTE)gObj[aIndex].X, (BYTE)gObj[aIndex].Y, ItemGetNumberMake(14, 28),
									(BYTE)lpItem->m_Level, 0, 0, 0, 0, aIndex, 0, 0);

								LogAdd("[Kalima] [%s][%s] Make Lost Kalima Map (Left Kundun Mark:%d)",
			...
			if ( pResult.result != 0xFF )
			{
				ItemByteConvert((LPBYTE)&pResult.Data[0], lpItem->m_Type, lpItem->m_Option1, lpItem->m_Option2,
					lpItem->m_Option3, (BYTE)lpItem->m_Level, (BYTE)lpItem->m_Durability, lpItem->m_NewOption,
				...
				if ( MapC[map_num].ItemGive(aIndex, item_num, false) == TRUE )
				{
					...
					else
					{
						...
						LogAdd("***0477*** [%s][%s] (%d)%d/%d Get serial:%d (%s)(%d)(%d)(%d)(%d)(%d) Dur (%d) Ex:(%d,%d,%d,%d,%d,%d,%d) Set : (%d)", gObj[aIndex].AccountID, gObj[aIndex].Name, map_num,
							gObj[aIndex].X, gObj[aIndex].Y, lpItem->m_Number, szItemName, type,
							level, lpItem->m_Option1, lpItem->m_Option2, lpItem->m_Option3, (int)lpItem->m_Durability,
							NewOption[0], NewOption[1], NewOption[2], NewOption[3], NewOption[4], NewOption[5],
							NewOption[6], lpItem->m_SetOption/*, lpItem->m_ItemOptionEx>>7,
							...
							if ( g_BloodCastle.CheckQuestItemSerial(map_num-MAP_INDEX_BLOODCASTLE1, lpItem) )
							{
								...
								switch ( lpItem->m_Level )
								{
									case 0:	// Staff of Archangel
										wsprintf(szTempMsg, "%s has acquired the Divine Staff of Archangel.", gObj[aIndex].Name);
										g_BloodCastle.SendNoticeMessage(gObj[aIndex].MapNumber-MAP_INDEX_BLOODCASTLE1, szTempMsg);
										break;

									case 1:	// Sword of Archangel
										wsprintf(szTempMsg, "%s has acquired the Divine Sword of Archangel.", gObj[aIndex].Name);
										g_BloodCastle.SendNoticeMessage(gObj[aIndex].MapNumber-MAP_INDEX_BLOODCASTLE1, szTempMsg);
										break;

									case 2:	// Crossbow of Archangel
										wsprintf(szTempMsg, "%s has acquired the Divine Crossbow of Archangel.", gObj[aIndex].Name);
										g_BloodCastle.SendNoticeMessage(gObj[aIndex].MapNumber-MAP_INDEX_BLOODCASTLE1, szTempMsg);
										break;
								...
								LogAdd("[Blood Castle] (%d) (Account:%s, Name:%s) picked up Angel King's Weapon (%d)",
...
BOOL CGItemDropRequest(PMSG_ITEMTHROW * lpMsg, int aIndex, BOOL drop_type)
{
	...
	{
		LogAdd("[ANTI-HACK][Item Duplication] - Item Drop during MapServer Move [%s][%s]",
			gObj[aIndex].AccountID, gObj[aIndex].Name);
		pResult.Result = false;
	}
	...
	{
		LogAdd("[%s][%s] CGItemDropRequest() Failed : Transaction == 1, IF_TYPE : %d",
			gObj[aIndex].AccountID, gObj[aIndex].Name, gObj[aIndex].m_IfState.type);
		pResult.Result = false;
	...
	if ( (lpObj->pInventory[lpMsg->Ipos].m_Type >= ITEMGET(13,0) && lpObj->pInventory[lpMsg->Ipos].m_Type <= ITEMGET(13,3))
		...
		|| (lpObj->pInventory[lpMsg->Ipos].m_Level > g_MaxItemLvlToDrop 
		&& lpObj->pInventory[lpMsg->Ipos].m_Type < ITEMGET(12,0)) 
	...
	if ( (g_kJewelOfHarmonySystem.IsStrengthenByJewelOfHarmony(&lpObj->pInventory[lpMsg->Ipos]) == TRUE) && (!isJOHItemCanDrop) )
	{
		pResult.Result = false;
		GCServerMsgStringSend("The strengthened goods are unable to fall.", lpObj->m_Index, 1);
	}
	...
	if ( pResult.Result == 1 )
	{
		...
		double dur = lpObj->pInventory[lpMsg->Ipos].m_Durability;
		...
		if ( gObjCheckSerial0ItemList(&lpObj->pInventory[lpMsg->Ipos]) != FALSE )
		{
			...
			LogAdd("[ANTI-HACK][Serial 0 Item] [ItemDrop] (%s)(%s) Item(%s) Pos(%d)",
		...
		if ( bIsItem == TRUE )
		{
			strcpy_s(szItemName, sizeof(szItemName), lpObj->pInventory[lpMsg->Ipos].GetName());
			...
			if ( type == ITEMGET(14,11)) // Box of Luck until Kunduns
			{
				...
				if ( level == 1 )
				{
					...
					LogAdd("[%s][%s][%d]%d/%d Used box of StarOfXMas Serial:%d (%s:%d/level:%d/skill:%d/op2:%d/op3:%d)",
				...
				else if ( level == 2 )
				{
					...
					LogAdd("[%s][%s][%d]%d/%d Used box of FireCracker Serial:%d (%s:%d/level:%d/skill:%d/op2:%d/op3:%d)",
						lpObj->AccountID, lpObj->Name, lpObj->MapNumber, lpObj->X, lpObj->Y,
						serial, szItemName, type, level, Option1, Option2, Option3);
				}
				else if ( level == 3 )
				{
					...
					LogAdd("[%s][%s][%d]%d/%d Used box of HeartOfLove Serial:%d (%s:%d/level:%d/skill:%d/op2:%d/op3:%d)",
						lpObj->AccountID, lpObj->Name, lpObj->MapNumber, lpObj->X, lpObj->Y,
						serial, szItemName, type, level, Option1, Option2, Option3);
				}
				else if ( level == 6 )
				{
					...
					LogAdd("[%s][%s][%d]%d/%d Used box of GoldMedal Serial:%d (%s:%d/level:%d/skill:%d/op2:%d/op3:%d)",
						lpObj->AccountID, lpObj->Name, lpObj->MapNumber, lpObj->X, lpObj->Y,
						serial, szItemName, type, level, Option1, Option2, Option3);
				}
				else if ( level == 5 )
				{
					...
					LogAdd("[%s][%s][%d]%d/%d Used box of SilverMedal Serial:%d (%s:%d/level:%d/skill:%d/op2:%d/op3:%d)",
						lpObj->AccountID, lpObj->Name, lpObj->MapNumber, lpObj->X, lpObj->Y,
						serial, szItemName, type, level, Option1, Option2, Option3);
				}
				else if ( level == 7 )
				{
					...
					LogAdd("[%s][%s][%d]%d/%d Used box of EventChip Serial:%d (%s:%d/level:%d/skill:%d/op2:%d/op3:%d)",
						lpObj->AccountID, lpObj->Name, lpObj->MapNumber, lpObj->X, lpObj->Y,
						serial, szItemName, type, level, Option1, Option2, Option3);
				}
				else if ( level == 8 )
				{
					...
					LogAdd("[%s][%s][%d]%d/%d Used box of ElradoraBox-8 Serial:%d (%s:%d/level:%d/skill:%d/op2:%d/op3:%d)",
						lpObj->AccountID, lpObj->Name, lpObj->MapNumber, lpObj->X, lpObj->Y,
						serial, szItemName, type, level, Option1, Option2, Option3);
				}
				else if ( level == 9 )
				{
					...
					LogAdd("[%s][%s][%d]%d/%d Used box of ElradoraBox-9 Serial:%d (%s:%d/level:%d/skill:%d/op2:%d/op3:%d)",
						lpObj->AccountID, lpObj->Name, lpObj->MapNumber, lpObj->X, lpObj->Y,
						serial, szItemName, type, level, Option1, Option2, Option3);
				}
				else if ( level == 10 )
				{
					...
					LogAdd("[%s][%s][%d]%d/%d Used box of ElradoraBox-10 Serial:%d (%s:%d/level:%d/skill:%d/op2:%d/op3:%d)",
						lpObj->AccountID, lpObj->Name, lpObj->MapNumber, lpObj->X, lpObj->Y,
						serial, szItemName, type, level, Option1, Option2, Option3);
				}
				else if ( level == 11 )
				{
					...
					LogAdd("[%s][%s][%d]%d/%d Used box of ElradoraBox1-11 Serial:%d (%s:%d/level:%d/skill:%d/op2:%d/op3:%d)",
						lpObj->AccountID, lpObj->Name, lpObj->MapNumber, lpObj->X, lpObj->Y,
						serial, szItemName, type, level, Option1, Option2, Option3);
				}
				else if ( level == 12 )
				{
					...
					LogAdd("[%s][%s][%d]%d/%d Used box of ElradoraBox1-12 Serial:%d (%s:%d/level:%d/skill:%d/op2:%d/op3:%d)",
						lpObj->AccountID, lpObj->Name, lpObj->MapNumber, lpObj->X, lpObj->Y,
						serial, szItemName, type, level, Option1, Option2, Option3);
				}
				else if ( level == 13 )
				{
					...
					LogAdd("[%s][%s][%d]%d/%d Used box of DarkLordHeart Serial:%d (%s:%d/level:%d/skill:%d/op2:%d/op3:%d)",
						lpObj->AccountID, lpObj->Name, lpObj->MapNumber, lpObj->X, lpObj->Y,
						serial, szItemName, type, level, Option1, Option2, Option3);
				}
				else if ( level == 0 )
				{
					...
					LogAdd("[%s][%s] (%d)%d/%d Used box of luck Serial:%d (%s:%d/level:%d/skill:%d/op2:%d/op3:%d)",	// [%s][%s][%d]%d/%d Used Box of Luck Serial:%d (%s:%d/level:%d/skill:%d/op2:%d/op3:%d)
						lpObj->AccountID, lpObj->Name, lpObj->MapNumber, lpObj->X, lpObj->Y,
						serial, szItemName, type, level, Option1, Option2, Option3);
				}
				else
				...
				LogAdd("[%s][%s] Used box of luck (level:%d)",
			...
			else if ( type == ITEMGET(12,32) )
			{
				...
				LogAdd("[%s][%s][%d]%d/%d Used Christmas RibbonBox Serial:%d (%s:%d/level:%d/skill:%d/op2:%d/op3:%d)",
					lpObj->AccountID, lpObj->Name, lpObj->MapNumber, lpObj->X, lpObj->Y,
					serial, szItemName, type, level, Option1, Option2, Option3);
			}
			...
			else if ( type == ITEMGET(14,32) && level == 0 )
			{
				...
				LogAdd("[%s][%s][%d]%d/%d Used Valentine's Day ChocolateBox Serial:%d (%s:%d/level:%d/skill:%d/op2:%d/op3:%d)",
					lpObj->AccountID, lpObj->Name, lpObj->MapNumber, lpObj->X, lpObj->Y,
					serial, szItemName, type, level, Option1, Option2, Option3);
			...
			else if ( type == ITEMGET(14,32)  && level == 1 )
			{
				...
				LogAdd("[%s][%s][%d]%d/%d Used WhiteDay CandyBox Serial:%d (%s:%d/level:%d/skill:%d/op2:%d/op3:%d)",
					lpObj->AccountID, lpObj->Name, lpObj->MapNumber, lpObj->X, lpObj->Y,
					serial, szItemName, type, level, Option1, Option2, Option3);
			}
			...
			else if ( type == ITEMGET(14,45) )
			{
				...
				LogAdd("[%s][%s][%d]%d/%d Used Pumpkin of Luck Serial:%d (%s:%d/level:%d/skill:%d/op2:%d/op3:%d)",
					lpObj->AccountID, lpObj->Name, lpObj->MapNumber, lpObj->X, lpObj->Y,
					serial, szItemName, type, level, Option1, Option2, Option3);
			}
            else if ( type == ITEMGET(14,52) )
            {
                    gObjInventoryDeleteItem(aIndex, lpMsg->Ipos);
                    GMBoxOpen(lpObj);
                    LogAdd("[%s][%s][%d]%d/%d Used GM Box Serial:%d (%s:%d/level:%d/skill:%d/op2:%d/op3:%d)",
                            lpObj->AccountID, lpObj->Name, lpObj->MapNumber, lpObj->X, lpObj->Y,
                            serial, szItemName, type, level, Option1, Option2, Option3);
            }
            else if ( type == ITEMGET(14,51) )
            {
                    ...
                    LogAdd("[%s][%s][Used Star Fireworks Effect] : (%d)(X:%d/Y:%d)",
                    lpObj->AccountID, lpObj->Name, lpObj->MapNumber, lpObj->X, lpObj->Y);
            ...
            else if ( type == ITEMGET(14,63) )
            {
                    ...
                    LogAdd("[%s][%s][Used Pounch Fireworks Effect] : (%d)(X:%d/Y:%d)",
                    lpObj->AccountID, lpObj->Name, lpObj->MapNumber, lpObj->X, lpObj->Y);
					...
					case 0:
						{
							...
							ItemSerialCreateSend(aIndex, lpObj->MapNumber, (BYTE)lpObj->X, (BYTE)lpObj->Y,
								ltype, Level, 1, 0, 0, 0, lpObj->m_Index, 0, 0);
							
							LogAdd("[Hidden TreasureBox Event] [%s][%s] Throw Mysterious Bead : Drop (%d) - serial:%d",
					...
					case 1: case 2: case 3:
						{
							...
							LogAdd("[Hidden TreasureBox Event] [%s][%s] Throw Crystal (%d) - serial:%d",
								lpObj->AccountID, lpObj->Name, level, serial);
						}
						break;
					case 4:
						LogAdd("[Hidden TreasureBox Event] [%s][%s] Throw Hidden Treasure Box - serial:%d",
						...
						break;
					case 5:
						LogAdd("[Hidden TreasureBox Event] [%s][%s] Throw Surprise Treasure Box - serial:%d",
							lpObj->AccountID, lpObj->Name,  serial);
						gObjInventoryDeleteItem(aIndex, lpMsg->Ipos);
						pResult.Result = true;
			...
			else if ( type == ITEMGET(13,20) && level == 2 ) // Wizard Ring B
			{
				...
				else
				{
					...
					TNotice::MakeNoticeMsg((TNotice *)&pNotice, 10, "You need to be at least Level 80 to throw the Ring.");
					TNotice::SetNoticeProperty((TNotice *)&pNotice, 10, _ARGB(255, 128, 149, 196), 1, 0, 20);
					TNotice::SendNoticeToUser(aIndex, (TNotice *)&pNotice);
					pResult.Result = false;
					DataSend(aIndex, (LPBYTE)&pResult, pResult.h.size);
			...
			else if ( type == ITEMGET(14,28) ) // Lost Map
			{
				...
				{
					LogAdd("[Kalima] [%s][%s] Success to Make Kalima Gate (Lost Kalima Map Serial:%d)",
			...
			else
			{
					...
					NOption, SOption, serial2, OwnerIndex, PetLevel, PetExp, btItemEffectEx) == TRUE )
				{
					...
					if ( drop_type == FALSE )
					{
						LogAdd("***0479*** [%s][%s] (%d)%d/%d Throw away Serial:%d (%s)(%d)(%d)(%d)(%d)(%d) Dur:(%d) Ex:(%d,%d,%d,%d,%d,%d,%d) Set : (%d)", 
					...
					{
						LogAdd("***0480*** [%s][%s] (%d)%d/%d Throw away (Drop) Serial:%d (%s)(%d)(%d)(%d)(%d)(%d) Dur: (%d) Ex:(%d,%d,%d,%d,%d,%d,%d) Set : (%d)", 
							lpObj->AccountID, lpObj->Name, lpObj->MapNumber,
							lpObj->X, lpObj->Y, serial, szItemName, type, level, Option1, Option2,
							Option3, (int)dur, NewOption[0], NewOption[1], NewOption[2], NewOption[3], NewOption[4],
		...
		{
			LogAdd("Doesn't have item to throw away");
			pResult.Result = false;
		}
	}
...
void CGInventoryItemMove(PMSG_INVENTORYITEMMOVE * lpMsg, int aIndex)
{
	...
	{
		LogAdd("[%s][%d] error-L3", __FILE__, __LINE__);
		::GCItemMoveResultSend(aIndex, -1, 0, ItemInfo);
		return;
	}
	...
	if ( lpMsg->sFlag == 3 || lpMsg->tFlag == 3 )
	{
		...
		{
			LogAdd("[%s][%s] error-L3 : CBMixing", gObj[aIndex].AccountID, gObj[aIndex].Name);
			::GCItemMoveResultSend(aIndex, -1, 0, (LPBYTE)&ItemInfo);
	...
	if ( lpMsg->sFlag == 5 || lpMsg->tFlag == 5 )
	{
		...
		if ( lpObj->ChaosLock == TRUE )
		{
			LogAdd("[%s][%s] error-L3 : DarkTrainerBoxMixing", gObj[aIndex].AccountID, gObj[aIndex].Name);
			::GCItemMoveResultSend(aIndex, -1, 0, (LPBYTE)&ItemInfo);
		 ...
		 lpMsg->tFlag == 6 || lpMsg->tFlag == 7 || lpMsg->tFlag == 8 )
	{
		...
		if ( lpObj->ChaosLock == TRUE )
		{
			LogAdd("[%s][%s] error-L3 : JewelOfHarmony Mixing",
				gObj[aIndex].AccountID, gObj[aIndex].Name);
			GCItemMoveResultSend(aIndex, -1, 0, (LPBYTE)&ItemInfo);
	...
	if ( lpMsg->sFlag == 2 && lpMsg->tFlag == 0 )
	{
		...
		if ( (lpObj->Money - money ) < 1 && (lpObj->WarehouseMoney - money) < 1)
		{
			::GCItemMoveResultSend(aIndex, -1, 0, (LPBYTE)&ItemInfo);
			char szTemp[256];
			wsprintf(szTemp, "You need %d Zen to use your vault.", money);
		...
		if ( ::bCanWarehouseLock == TRUE )
		{
			...
			{
				::GCItemMoveResultSend(aIndex, -1, 0, (LPBYTE)&ItemInfo);
				::GCServerMsgStringSend("The vault is locked.", lpObj->m_Index, 1);
...
bool CheckMysteryShop(LPOBJ lpNpcObj, LPOBJ lpUserObj)
{
	if (gMysteryShopOnOff && ( gMysteryShopClass == -1 ? TRUE : (lpNpcObj->Class == gMysteryShopClass? TRUE: FALSE) ) && lpNpcObj->X == gMysteryShopX && lpNpcObj->Y == gMysteryShopY && lpNpcObj->MapNumber == gMysteryShopMap)
	{	
		if ( lpUserObj->MysteryShop.ItemCount < 1 )
		{
			if ( Random(0,1) != 0 )
				ChatTargetSend(&gObj[lpNpcObj->m_Index], "We don't sell Ale here!", lpUserObj->m_Index);//#info - ChatTargetSend - может что то интересненькое
			else
				ChatTargetSend(&gObj[lpNpcObj->m_Index], "Tough times are putting us out of business.", lpUserObj->m_Index);

			return true;
		}

		if ( ::gObjFixInventoryPointer(lpUserObj->m_Index) == false )
		{
			LogAdd("[Fix Inv.Ptr] False Location - %s, %d", __FILE__, __LINE__);
		}

		PMSG_TALKRESULT pResult;
		lpUserObj->TargetShopNumber = 14;
		lpUserObj->m_IfState.use = 1;
		lpUserObj->m_IfState.type = 3;
		lpUserObj->m_ShopTime = 0;
		pResult.h.c = 0xC3;
		pResult.h.headcode = 0x30;
		pResult.h.size = sizeof(pResult);
		pResult.result = 0;
		DataSend(lpUserObj->m_Index, (LPBYTE)&pResult, pResult.h.size);
		...
		int size = lOfs + lpUserObj->MysteryShop.SendItemDataLen;
		PHeadSetW((LPBYTE)&pShopItemCount, 0x31, size);
		pShopItemCount.Type = 0;
		pShopItemCount.count = lpUserObj->MysteryShop.ItemCount;
		memcpy(SendByte, &pShopItemCount, sizeof(pShopItemCount));
		memcpy(&SendByte[lOfs], lpUserObj->MysteryShop.SendItemData, lpUserObj->MysteryShop.SendItemDataLen);

		DataSend(lpUserObj->m_Index, SendByte, size);
		GCAnsCsMapSvrTaxInfo(lpUserObj->m_Index,2,  ::g_CastleSiegeSync.GetTaxRateStore(lpUserObj->m_Index));
		return true;
	}
	else
	{
...
void CGTalkRequestRecv(PMSG_TALKREQUEST * lpMsg, int aIndex)
{
	...
	if ( !gObjIsConnectedGP(aIndex) )
	{
		LogAdd("[ANTI-HACK][NPC Talk] Couldn't talk.. (%s)(%s)",
			gObj[aIndex].AccountID, gObj[aIndex].Name);
...
// 	if (CheckMysteryShop(&gObj[DealerNumber], lpObj))
// 		return;
	...
	if ( ShopNum < MAX_SHOP )
	{
		...
		{
			if ( Random(0,1) != 0 )
				ChatTargetSend(&gObj[DealerNumber], "We don't sell Ale here!", aIndex);//#info - ChatTargetSend - может что то интересненькое
			else
				ChatTargetSend(&gObj[DealerNumber], "Tough times are putting us out of business.", aIndex);
		...
		if ( gPkLimitFree == FALSE )
		{
			...
			{
				if ( Random(0,1) != 0 )
					ChatTargetSend(&gObj[DealerNumber], "I'm not selling anything to the likes of you.", aIndex);
				else
					ChatTargetSend(&gObj[DealerNumber], "I will not aid criminals like you! Look elsewhere.", aIndex);
	...
	if ( gMerryXMasNpcEvent == TRUE )
	{
		if ( Random(0,5) == 0 )
		{
			ChatTargetSend(&gObj[DealerNumber], "Merry Christmas!!", aIndex);
		}
	}
	...
	if ( ::gHappyNewYearNpcEvent == TRUE )
	{
		if ( Random(0,5) == 0 )
		{
			ChatTargetSend(&gObj[DealerNumber], "Happy New Year!", aIndex);
		}
	}
		...
		if ( bCanChaosBox == TRUE )
		{
			if ( lpObj->m_bPShopOpen == true )
			{
				LogAdd("[%s][%s] is Already Opening PShop, ChaosBox Failed",
					lpObj->AccountID, lpObj->Name);
				GCServerMsgStringSend("You cannot use the Chaos Machine while your Personal Shop is open.", lpObj->m_Index, 1);
			...
			LogAdd("[%s][%s] Open Chaos Box", lpObj->AccountID, lpObj->Name);
...
bool BuyMysteryShopItem(PMSG_BUYREQUEST * lpMsg, LPOBJ lpObj)
{
	/*if (lpObj->MysteryShop.BuyShopItemAndDelete(lpMsg->Pos,lpObj->m_Index,&lpObj->MysteryShop))
		return true;
	else
		return false;*/
/*	if ( lpObj->MysteryShop.m_item[lpMsg->Pos].IsItem() == TRUE )
	{
		...
		int iStoreTaxMoney = lpObj->MysteryShop.m_item[lpMsg->Pos].m_BuyMoney + (int)((__int64)lpObj->MysteryShop.m_item[lpMsg->Pos].m_BuyMoney * (__int64)g_CastleSiegeSync.GetTaxRateStore(lpObj->m_Index) / (__int64)100);

		if ( iStoreTaxMoney < 0 )
			iStoreTaxMoney  = 0;

		int iStoreTaxMoney2 = (int)((__int64)lpObj->MysteryShop.m_item[lpMsg->Pos].m_BuyMoney * (__int64)g_CastleSiegeSync.GetTaxRateStore(lpObj->m_Index) / (__int64)100);
		...
		else
		{
			...
			if ( (lpObj->MysteryShop.m_item[lpMsg->Pos].m_Type >= ITEMGET(14,0) && lpObj->MysteryShop.m_item[lpMsg->Pos].m_Type <= ITEMGET(14,8)) ||
				(lpObj->MysteryShop.m_item[lpMsg->Pos].m_Type >= ITEMGET(14,35) && lpObj->MysteryShop.m_item[lpMsg->Pos].m_Type <= ITEMGET(14,40)))
			{
				int dur = (int)lpObj->MysteryShop.m_item[lpMsg->Pos].m_Durability;

				if ( dur == 0 )
					dur = 1;

				if ( ::gObjSearchItem(lpObj, lpObj->MysteryShop.m_item[lpMsg->Pos].m_Type,dur, lpObj->MysteryShop.m_item[lpMsg->Pos].m_Level) == TRUE )
				{
					bNoItem = FALSE;
			...
			if ( bNoItem != FALSE )
			{
				pResult.Result = gObjShopBuyInventoryInsertItem(lpObj->m_Index, lpObj->MysteryShop.m_item[lpMsg->Pos]);

				if ( pResult.Result != 0xFF )
				{
					ItemByteConvert((LPBYTE)&pResult.ItemInfo, lpObj->MysteryShop.m_item[lpMsg->Pos]);
					int lc64 = lpObj->MysteryShop.m_item[lpMsg->Pos].m_BuyMoney;
					lpObj->Money -= iStoreTaxMoney;
					::g_CastleSiegeSync.AddTributeMoney(iStoreTaxMoney2);
					...
					lpObj->MysteryShop.m_item[lpMsg->Pos].Clear();

					lpObj->MysteryShop.SendItemDataLen -= 8;
					lpObj->MysteryShop.ItemCount -- ;
					lpObj->MysteryShop.RefreshShopItems();
					DataSend(lpObj->m_Index, (LPBYTE)&pResult, pResult.h.size);

					LogAdd("[%s][%s] (%d) Shop buy [%d][%d][%d][%s] LEV:%d, DUR:%d, OP:[%d][%d][%d]",
						lpObj->AccountID, lpObj->Name, 14, lc64, iTaxRate, lc64+iStoreTaxMoney2, lpObj->MysteryShop.m_item[lpMsg->Pos].GetName(),
						lpObj->MysteryShop.m_item[lpMsg->Pos].m_Level, lpObj->MysteryShop.m_item[lpMsg->Pos].m_Durability,
						lpObj->MysteryShop.m_item[lpMsg->Pos].m_Option1, lpObj->MysteryShop.m_item[lpMsg->Pos].m_Option2,
						lpObj->MysteryShop.m_item[lpMsg->Pos].m_Option3);


					if ( lpObj->MysteryShop.ItemCount < 1 )
					{
						PMSG_TALKRESULT pResult;
					...
					else
					{
						...
						int size = lOfs + lpObj->MysteryShop.SendItemDataLen;
						PHeadSetW((LPBYTE)&pShopItemCount, 0x31, size);
						pShopItemCount.Type = 0;
						pShopItemCount.count = lpObj->MysteryShop.ItemCount;
						memcpy(SendByte, &pShopItemCount, sizeof(pShopItemCount));
						memcpy(&SendByte[lOfs], lpObj->MysteryShop.SendItemData, lpObj->MysteryShop.SendItemDataLen);
...
void CGBuyRequestRecv(PMSG_BUYREQUEST * lpMsg, int aIndex)
{
	...
	/*if (tShop == 14)
		if (BuyMysteryShopItem(lpMsg,lpObj) )
			return;*/
...
void CGSellRequestRecv(PMSG_SELLREQUEST * lpMsg, int aIndex)
{
	...
	if ( gObj[aIndex].pTransaction == 1 )
	{
		LogAdd("[%s][%s] CGSellRequestRecv() Failed : Transaction == 1, IF_TYPE : %d",
			gObj[aIndex].AccountID, gObj[aIndex].Name, gObj[aIndex].m_IfState.type);
	...
	{
		DataSend(aIndex, (LPBYTE)&pResult, pResult.h.size);
		LogAdd("Item location value exceeds : %d", lpMsg->Pos);
		return;
	}
	...
	if ( gObjCheckSerial0ItemList(&lpObj->pInventory[lpMsg->Pos]) != FALSE )
	{
		...
		LogAdd("[ANTI-HACK][Serial 0 Item] [Sell Item] (%s)(%s) Item(%s) Pos(%d)",
	...
	{
		GCServerMsgStringSend("The strengthened goods are unable to sell.", lpObj->m_Index, 1);
		DataSend(aIndex, (LPBYTE)&pResult, pResult.h.size);
		return;
	}
	...
	if ( lpObj->pInventory[lpMsg->Pos].IsItem() == FALSE )
	{
		pResult.Result = false;
		pResult.Money = lpObj->Money;
		LogAdd("Invalid item index. %s %d (%s)Pos:%d", __FILE__, __LINE__, lpObj->Name, lpMsg->Pos);
	...
	else
	{
		...
		LogAdd("***0485*** [%s][%s] Shop(%d) Item Sell Money:%d %s:%d/%d/%d/%d/%d Dur:%d Ex:[%d,%d,%d,%d,%d,%d,%d] Set : (%d)", 
			lpObj->AccountID, lpObj->Name, tShop,
...
int GetNeedMoneyItemDurRepaire(CItem *  DurItem, BOOL RequestPos)
{
	...
	double ItemDurability = DurItem->m_Durability;
	...
	if ( ::HasItemDurability(type ) != FALSE )
	{
		...
		double lc6 = 1.0f - ItemDurability / lc4;
		int lc7;
		...
		double lc8 = sqrt( double(lc7) );
		double lc9 = sqrt(sqrt(double(lc7))); 
		lc5 = 3.0f * lc8 * lc9;
		lc5 *= lc6;
		lc5 += 1.0f;
...
void ItemDurRepaire(LPOBJ lpObj, CItem * DurItem, int pos, int RequestPos)
{
	...
	else
	{
		...
		pResult.Money = lpObj->Money;
		DurItem->m_Durability = (double)((int)DurItem->m_BaseDurability);
...
void CGTradeRequestSend(PMSG_TRADE_REQUEST * lpMsg, int aIndex)
{
	...
	if (bCanTrade == FALSE )
	{
		GCServerMsgStringSend("The Trade function will be disabled for a while.", aIndex, 1);
		GCServerMsgStringSend("Please be patient. Trading will be enabled soon.", aIndex, 1);
		return;
	}
	...
	{
		::GCServerMsgStringSend("Trading is not allowed in Devil Square.", aIndex, 1);
		return;
	}
	...
	{
		::GCServerMsgStringSend("You cannot trade inside Chaos Castle.", aIndex, 1);
		return;
	}
	...
	if ( BC_MAP_RANGE(gObj[aIndex].MapNumber) != FALSE )
	{
		...
		{
			GCServerMsgStringSend("You cannot trade while inside Blood Castle.", aIndex, 1);
			return;
		}	
	}
	...
	if ( gObj[aIndex].pTransaction == 1 )
	{
		LogAdd("[%s][%s] CGTradeRequestSend() Failed : Transaction == 1, IF_TYPE : %d",
			gObj[aIndex].AccountID, gObj[aIndex].Name, gObj[aIndex].m_IfState.type);
	...
	LogAdd("Request to trade [%s][%s][%s]  to [%s][%s][%s]", gObj[aIndex].AccountID, gObj[aIndex].Name, gObj[aIndex].Ip_addr,
...
void CGTradeResponseRecv(PMSG_TRADE_RESPONSE * lpMsg, int aIndex)
{
	...
	{
		LogAdd("(%s)(%s) Trade Interface State Error : use:%d type:%d", gObj[aIndex].AccountID, gObj[aIndex].Name, 
	...
	{
		LogAdd("(%s)(%s) Trade Interface State Error : use:%d type:%d", gObj[number].AccountID, gObj[number].Name, 
	...
	if ( gObj[aIndex].pTransaction == 1 )
	{
		LogAdd("[%s][%s] CGTradeResponseRecv() Failed : Transaction == 1, IF_TYPE : %d",
			gObj[aIndex].AccountID, gObj[aIndex].Name, gObj[aIndex].m_IfState.type);
	...
	else
	{
		...
		if ( Result != TRUE )
		{
			...
			if ( Result == 2 )
			{
				LogAdd("[%s][%s] Made Trade Error (in Transaction) [%s][%s]",
					gObj[aIndex].AccountID, gObj[aIndex].Name,
	...
	if ( Result == FALSE )
	{
		if ( gObj[aIndex].m_IfState.use != FALSE && gObj[aIndex].m_IfState.type == 1 )
		{
			gObj[aIndex].m_IfState.use = FALSE;
			gObj[aIndex].TargetNumber = -1;
			LogAdd("Interface State : %d", gObj[aIndex].m_IfState.use);
		}
		...
		if ( gObj[number].TargetNumber == aIndex )
		{
			if ( gObj[number].m_IfState.use != FALSE && gObj[number].m_IfState.type == 1 )
			{
				...
				LogAdd("Interface State : %d", gObj[number].m_IfState.use);
			}
		}
	}
}
...
void CGTradeMoneyRecv(PMSG_TRADE_GOLD * lpMsg, int aIndex)
{
	...
	if ( gObj[aIndex].CloseType != -1 )
	{
		LogAdd("error : [%s][%s] Trade broken by disconnect", gObj[aIndex].AccountID, gObj[aIndex].Name);
		return;
	}
	...
	{
		LogAdd("error : Trade failed because the opposite doesn't exist [%s][%s] (%d)", gObj[aIndex].AccountID, gObj[aIndex].Name, number);
		return;
	}

	if ( gObj[number].CloseType != -1 )
	{
		LogAdd("error : Trade failed because the opposite is disconnecting [%s][%s] [%s][%s].", gObj[aIndex].AccountID, gObj[aIndex].Name, gObj[number].AccountID, gObj[number].Name);
		return;
	}
...
void CGTradeOkButtonRecv(PMSG_TRADE_OKBUTTON * lpMsg, int aIndex)
{
	...
	if ( lpMsg->Flag == 1 )
	{
		if ( gObj[aIndex].TradeOk == false )
		{
			...
			LogAdd("[%s][%s] Trade Accept", gObj[aIndex].AccountID, gObj[aIndex].Name);
...
void CGPShopReqSetItemPrice(PMSG_REQ_PSHOP_SETITEMPRICE * lpMsg, int aIndex)
{
	...
	if ( !gObjIsConnected(aIndex))
	{
		LogAdd("[PShop] ERROR : Index is not CONNECTED : %d", aIndex);
		return;
	}

	if ( gObj[aIndex].Type != OBJ_USER )
	{
		LogAdd("[PShop] ERROR : Index is not CHARACTER : %d", aIndex);
		return;
	}

	if ( gObj[aIndex].Level <= 5 )
	{
		LogAdd("[PShop] [%s][%s] ERROR : Level is Under 6 : %d",
	...
	{
		LogAdd("[PShop] [%s][%s] ERROR : Item Position Out of Bound : %d",
	...
	{
		LogAdd("[PShop] [%s][%s] ERROR : Item Does Not Exist : %d",
			gObj[aIndex].AccountID, gObj[aIndex].Name, lpMsg->btItemPos);
	...
	if ( gObjCheckSerial0ItemList(sitem) != FALSE )
	{
		...
		LogAdd("[ANTI-HACK][Serial 0 Item] [PShop] (%s)(%s) Item(%s) Pos(%d)",
	...
	if ( iItemPrice <= 0 )
	{
		LogAdd("[PShop] [%s][%s] ERROR : Personal Shop Item Price <= 0 : %d",
	...
	{
		LogAdd("[PShop] [%s][%s] ERROR : Item Price Change - Item Block",gObj[aIndex].AccountID, gObj[aIndex].Name,sitem->m_iPShopValue);	// Deathway fix Here - Last parameter can cause crash
	...
	LogAdd("[PShop] [%s][%s] Changing Item Price : %d (Type:%d (%s), Lev:%d, Serial:%d, Dur:%d",
...
void CGPShopReqOpen(PMSG_REQ_PSHOP_OPEN * lpMsg, int aIndex)
{
	...
	if ( (gObj[aIndex].Penalty&4) == 4 || (gObj[aIndex].Penalty&8) == 8 )
	{
		LogAdd("[PShop] [%s][%s] ERROR : Cant't Open Shop - Item Block" ,gObj[aIndex].AccountID, gObj[aIndex].Name);
	...
	{
		::GCServerMsgStringSend("You cannot open your Personal Shop within Chaos Castle.", aIndex, 1);
	...
	else
	{
		if ( gObj[aIndex].m_IfState.use == TRUE )
		{
			...
			if ( !bEnablePShopOpen  )
			{
				LogAdd("[PShop] [%s][%s] ERROR : m_IfState.type is Using : %d", gObj[aIndex].AccountID, gObj[aIndex].Name, gObj[aIndex].m_IfState.type);
			}
		}
		else
		{
			LogAdd("[PShop] [%s][%s] ERROR : Unknown m_IfState.type : %d", gObj[aIndex].AccountID, gObj[aIndex].Name, gObj[aIndex].m_IfState.type);
			return;
		}
	}
	...
	if ( gObj[aIndex].pTransaction == 1 )
	{
		LogAdd("[PShop] [%s][%s] ERROR : Transaction == 1, IF_TYPE : %d", gObj[aIndex].AccountID, gObj[aIndex].Name, gObj[aIndex].m_IfState.type);
	...
	if ( bEnablePShopOpen )
	{
		if ( gObj[aIndex].m_bPShopOpen == false )
		{
			...
			LogAdd("[PShop] [%s][%s] Personal Shop Opened", gObj[aIndex].AccountID, gObj[aIndex].Name);
		}
		else
		{
			memcpy(gObj[aIndex].m_szPShopText, lpMsg->szPShopText, sizeof(lpMsg->szPShopText));
			...
			LogAdd("[PShop] [%s][%s] Personal Shop Already Opened - Changing PShop Name",
				gObj[aIndex].AccountID, gObj[aIndex].Name);
		}
	}
	else
...
void CGPShopReqClose(int aIndex)
{
	...
	LogAdd("[PShop] [%s][%s] Requested to Close PShop", gObj[aIndex].AccountID, gObj[aIndex].Name);
...
{
	LogAdd("[PShop] [%s][%s] Close PShop", gObj[aIndex].AccountID, gObj[aIndex].Name);
...
void CGPShopReqBuyList(PMSG_REQ_BUYLIST_FROM_PSHOP * lpMsg, int aSourceIndex)
{
	...
	{
		LogAdd("[PShop] [%s][%s] PShop List Request Failed : Seller did Not CONNECTED (%d)",
	...
	{
		LogAdd("[PShop] [%s][%s] PShop List Request Failed : Seller is Not CHARACTER (%d)"
			,gObj[aSourceIndex].AccountID, gObj[aSourceIndex].Name,  MAKE_NUMBERW(lpMsg->NumberH, lpMsg->NumberL));
		::CGPShopAnsBuyList(aSourceIndex, -1, 2, 0);
		return;
	}
	...
	{
		LogAdd("[PShop] [%s][%s] PShop Item Buy Request Failed : Seller is Closing (%d)"
			,gObj[aSourceIndex].AccountID, gObj[aSourceIndex].Name,  MAKE_NUMBERW(lpMsg->NumberH, lpMsg->NumberL) );
	...
	{
		LogAdd("[PShop] [%s][%s] PShop Item Buy Request Failed : Buyer is Closing (%d)",
			gObj[aSourceIndex].AccountID, gObj[aSourceIndex].Name,  MAKE_NUMBERW(lpMsg->NumberH, lpMsg->NumberL));
		CGPShopAnsBuyItem(aSourceIndex, -1, 0, 2);
		return;
	}
	...
	{
		LogAdd("[PShop] [%s][%s] PShop List Requested to Him(/Her)Self",
	...
	{
		LogAdd("[PShop] [%s][%s] PShop List Request Failed : [%s][%s] Did not Open PShop",
			gObj[aSourceIndex].AccountID, gObj[aSourceIndex].Name,
			lpObj->AccountID, lpObj->Name);
	...
	{
		LogAdd("[PShop] [%s][%s] PShop List Request Failed : [%s][%s] is in Item Block",
			gObj[aSourceIndex].AccountID, gObj[aSourceIndex].Name,
			lpObj->AccountID, lpObj->Name);
	...
	{
		LogAdd("[PShop] [%s][%s] PShop List Request Failed : Requester Transaction == 1, IF_TYPE : %d",
			gObj[aSourceIndex].AccountID, gObj[aSourceIndex].Name, gObj[aSourceIndex].m_IfState.type);
		return;
	}

	LogAdd("[PShop] [%s][%s] is Receiving PShop List From [%s][%s]",
			gObj[aSourceIndex].AccountID, gObj[aSourceIndex].Name,
			lpObj->AccountID, lpObj->Name);
	...
	else
	{
		...
		for ( int n=MAIN_INVENTORY_SIZE;n<INVENTORY_SIZE;n++)
		{
			if ( lpObjTarget->Inventory1[n].IsItem() == TRUE )
			{
				...
				LogAdd("ITEM TYPE:%d", lpObjTarget->Inventory1[n].m_Type);
			}
		}
...
void CGPShopReqBuyItem(PMSG_REQ_BUYITEM_FROM_PSHOP * lpMsg, int aSourceIndex)
{
	...
	if ( gObjIsConnected( MAKE_NUMBERW(lpMsg->NumberH, lpMsg->NumberL)) == FALSE )
	{
		LogAdd("[PShop] [%s][%s] PShop Item Buy Request Failed : Seller did Not CONNECTED (%d)",
			gObj[aSourceIndex].AccountID, gObj[aSourceIndex].Name, MAKE_NUMBERW(lpMsg->NumberH, lpMsg->NumberL));
		::CGPShopAnsBuyItem(aSourceIndex, -1, 0, 2);
		return;
	}
	...
	if ( lpObj->Type != OBJ_USER )
	{
		LogAdd("[PShop] [%s][%s] PShop Item Buy Request Failed : Seller is Not CHARACTER (%d)",
			gObj[aSourceIndex].AccountID, gObj[aSourceIndex].Name,  MAKE_NUMBERW(lpMsg->NumberH, lpMsg->NumberL));
		::CGPShopAnsBuyItem(aSourceIndex, -1, 0, 2);
		return;
	}
	...
	if ( lpObj->m_bPShopOpen == false )
	{
		LogAdd("[PShop] [%s][%s] PShop Item Buy Request Failed : [%s][%s] Did not Open PShop",
			gObj[aSourceIndex].AccountID, gObj[aSourceIndex].Name,
			lpObj->AccountID, lpObj->Name);
	...
	{
		LogAdd("[PShop] [%s][%s] PShop Item Buy Request Failed : Name Mismatch [%s] - [%s]",
			gObj[aSourceIndex].AccountID, gObj[aSourceIndex].Name,
			szName, lpObj->Name);
	...
	{
		LogAdd("[PShop] [%s][%s] PShop Item Buy Request Failed : [%s][%s] is Closing Connection",
			gObj[aSourceIndex].AccountID, gObj[aSourceIndex].Name,
			lpObj->AccountID, lpObj->Name);
	...
	if ( (lpObj->Penalty &4) == 4 || (lpObj->Penalty &8) == 8 )
	{
		LogAdd("[PShop] [%s][%s] PShop Item Buy Request Failed : [%s][%s] is in Item Block",
			gObj[aSourceIndex].AccountID, gObj[aSourceIndex].Name,
			lpObj->AccountID, lpObj->Name);
	...
	{
		LogAdd("[PShop] [%s][%s] PShop Item Buy Request Failed : [%s][%s] Already Trade With Other",
			gObj[aSourceIndex].AccountID, gObj[aSourceIndex].Name,
			lpObj->AccountID, lpObj->Name);
	...
	else
	{
		...
		{
			MsgOutput(lpObj->m_Index, "Unable to use");
			GCReFillSend(aSourceIndex, (WORD)gObj[aSourceIndex].Life, 0xFD, 1, gObj[aSourceIndex].iShield);

			LogAdd("[ANTI-HACK][Serial 0 Item] [PShop Buy] (%s)(%s) Item(%s) Pos(%d)",
		...
		{
			::GCReFillSend(aSourceIndex, (WORD)gObj[aSourceIndex].Life, -3, 1, gObj[aSourceIndex].iShield);
		...
		if ( gObj[aSourceIndex].pTransaction == 1 )
		{
			LogAdd("[PShop] [%s][%s] PShop Item Buy Request Failed : Requester Transaction == 1, IF_TYPE : %d",
				gObj[aSourceIndex].AccountID, gObj[aSourceIndex].Name, gObj[aSourceIndex].m_IfState.type);
			return;
		}
		...
		__try
		{
			...
			if ( lpObj->Inventory1[lpMsg->btItemPos].IsItem() == TRUE )
			{
				...
				{
					LogAdd("[PShop] [%s][%s] PShop Item Buy Request Failed : [%s][%s] Did Not Choose Item Price",
						gObj[aSourceIndex].AccountID, gObj[aSourceIndex].Name,
						lpObj->AccountID, lpObj->Name);
					::CGPShopAnsBuyItem(aSourceIndex, lpObj->m_Index, 0, 6);
				...
				{
					LogAdd("[PShop] [%s][%s] PShop Item Buy Request Failed : [%s][%s] Lack of Zen",
						gObj[aSourceIndex].AccountID, gObj[aSourceIndex].Name,
						lpObj->AccountID, lpObj->Name);
				...
				if ( gObjCheckMaxZen(lpObj->m_Index, (int&)dwCost) == FALSE )
				{
					LogAdd("[PShop] [%s][%s] PShop Item Buy Request Failed : [%s][%s] Exceeding Zen of the Host",
						gObj[aSourceIndex].AccountID, gObj[aSourceIndex].Name,
						lpObj->AccountID, lpObj->Name);
				...
				{
					LogAdd("[PShop] [%s][%s] PShop Item Buy Request Failed : [%s][%s] No Room to Buy Item",
						gObj[aSourceIndex].AccountID, gObj[aSourceIndex].Name,
						lpObj->AccountID, lpObj->Name);
					::CGPShopAnsBuyItem(aSourceIndex, lpObj->m_Index, 0, 8);
				...
				LogAdd("[PShop] [%s][%s][%s] PShop Item Buy Request Succeed : [%s][%s][%s] (Price=%d, ItemType:%d (%s), ItemLevel:%d, ItemDur:%d, Serial:%d",
				...
				{
					LogAdd("[PShop] [%s][%s] Sold All Items - Auto Closing PShop",
						lpObj->AccountID, lpObj->Name);
					GCServerMsgStringSend("All items in your Personal Shop have been sold!", lpObj->m_Index, 1);
...
void CGPartyRequestRecv(PMSG_PARTYREQUEST * lpMsg, int aIndex)
{
	...
	LogAdd("Party result : %d %d %d %d", gObj[aIndex].CloseCount, gObj[number].CloseCount, gObj[aIndex].Connected, gObj[number].Connected);
	LogAdd("%s %s", gObj[aIndex].Name, gObj[number].Name);
	...
	if ( result == 1 )
	{
		...
		if ( gObj[number].PartyNumber >= 0  )
		{
			...
			if ( iPartyPos >= 0 )
			{
				...
				result = true;
				wsprintf(szTemp, "You are going to party with %s", gObj[aIndex].Name);
				::GCServerMsgStringSend(szTemp, number, 1);
				wsprintf(szTemp, "You are going to party with %s", gObj[number].Name);
			...
			{
				result = 2;
				wsprintf(szTemp, "%s is already in a party.", gObj[aIndex].Name);
				GCServerMsgStringSend(szTemp, aIndex, 1);
			}
		}
	}
...
void CGGuildRequestRecv(PMSG_GUILDJOINQ * lpMsg, int aIndex)
{
	...
	{
		MsgOutput(aIndex, "Loren Deep event 01");
		return;
	}
	...
	LogAdd("[%s][%s] Guild Request - Join : %s",
		gObj[aIndex].AccountID, gObj[aIndex].Name,
...
void CGGuildDelUser(PMSG_GUILDDELUSER * lpMsg, int aIndex)
{
	...
	if ( g_CastleSiegeSync.GetCastleState() == CASTLESIEGE_STATE_STARTSIEGE )
	{
		MsgOutput(aIndex, "Loren Deep event 02");
		return;
	}
	...
	strcpy_s(guildname, sizeof(guildname), gObj[aIndex].lpGuild->Name);
	...
	if ( !strcmp(gObj[aIndex].lpGuild->Names[0], gObj[aIndex].Name ))
	{
		...
		LogAdd("[%s][%s] Guild Request - Dismiss All : %s",
	...
	else
	{
		...
		LogAdd("[%s][%s] Guild Request - Leave : %s",
			gObj[aIndex].AccountID, gObj[aIndex].Name, guildname);
	}
}
...
void GCGuildWarRequestResult(LPSTR GuildName, int aIndex, int type)
{
    ...
    {
            LogAdd("[ANTI-HACK][Trade & War] (%s)(%s)",
                    gObj[aIndex].AccountID, gObj[aIndex].Name);
            return;
    }
	...
	if ( lpMyGuild->WarState == 1 || lpMyGuild->WarDeclareState == 1 )
	{
		...
		LogAdd("Error on declaring war : %d %d %s", lpMyGuild->WarState,
	...
	if ( strcmp(lpMyGuild->Names[0], gObj[aIndex].Name ) )
	{
		...
		LogAdd("Error on declaring war : 0x05 %s %s",
			lpMyGuild->Names[0], gObj[aIndex].Name);
		DataSend(aIndex, (LPBYTE)&pMsg, pMsg.h.size);
	...
	if ( lpNode )
	{
		if ( lpNode->WarState == 1 || lpNode->WarDeclareState == 1 )
		{
			...
			LogAdd("Error on declaring war : %d %d %s", lpNode->WarState,
		...
		{
			LogAdd("[U.System][Guildwar] Can't declare guildwar between Same Union (%s / %d) <-> (%s / %d)",
		...
		for (int n = OBJ_STARTUSERINDEX; n < OBJMAX - 1; n++)
		{
			if ( gObj[n].Connected == PLAYER_PLAYING )
			{
				if ( gObj[n].Name[0] == lpNode->Names[0][0] )
		...
		if ( warmaster >= 1 )
		{
			...
			{
				GCServerMsgStringSend("Guild War cannot be waged within Chaos Castle.", aIndex, 1);
				return;
			}
			...
			// 
			LogAdd("Declared guild war [%s][%s] sGuild:(%s) TargetGuild:(%s)", gObj[aIndex].AccountID, gObj[aIndex].Name,
				lpMyGuild->Name, lpNode->Name);

			strcpy_s(lpMyGuild->TargetGuildName, sizeof(lpMyGuild->TargetGuildName), lpNode->Name);
			strcpy_s(lpNode->TargetGuildName, sizeof(lpNode->TargetGuildName), lpMyGuild->Name);
...
void GCGuildWarRequestSend(LPSTR GuildName, int aIndex, int type)
{
	...
	LogAdd("(%s) guild declared guild war", GuildName);
...
void GCGuildWarRequestSendRecv(PMSG_GUILDWARSEND_RESULT * lpMsg, int aIndex)
{
	...
	if ( lpMyNode->lpTargetGuildNode && lpMsg->Result )
	{
			 ...
			 DS_MAP_RANGE(gObj[iTarGetIndex].MapNumber)  )
		{
			...
			TNotice::MakeNoticeMsgEx(&pNotice, 1, "Guild War cannot be waged within Chaos Castle.");
			...
			if ( lpMyNode->lpTargetGuildNode )
			{
				if (lpMyNode->lpTargetGuildNode->WarDeclareState == 1 )
				{
						...
						if ( lpMyNode->Use[n] )
						{
							if ( lpMyNode->Index[n] >= 0 )
							{
								...
								if ( g_call )
								{
									...
									LogAdd("Declared guild war against (%s)", lpMyNode->Name);
						...
						if ( lpMyNode->lpTargetGuildNode->Use[n] )
						{
							if ( lpMyNode->lpTargetGuildNode->Index[n] >= 0 )
							{
								...
								if ( g_call )
								{
									...
									LogAdd("Declared guild war against (%s)", lpMyNode->lpTargetGuildNode->Name);
					...
					if ( lpMyNode->WarType == 1 )
					{
						...
						GCServerMsgStringSendGuild(lpMyNode->lpTargetGuildNode,"The match starts automatically in 10 Seconds", 1);
						GCServerMsgStringSendGuild(lpMyNode,"The match starts automatically in 10 Seconds", 1);
					}
				}
			}
		}
...
void CGWarehouseMoneyInOut(int aIndex, PMSG_WAREHOUSEMONEYINOUT* lpMsg)
{
	...
	{
		case 0x00:
			if ( money <= 0 || money > (int)MAX_WAREHOUSE_ZEN )
			{
				GCWarehouseInventoryMoneySend(aIndex, 0, 0, 0);
				return;
			}

			if ( (lpObj->WarehouseMoney+money) > (int)MAX_WAREHOUSE_ZEN )
			{
				GCWarehouseInventoryMoneySend(aIndex, 0, 0, 0);
				return;
			}
		...
		case 0x01:
			{
				...
				else
				{
					...
					wsprintf(msg, "You need %d Zen to use your vault.", rZen);
...
void CGChaosBoxItemMixButtonClick(PMSG_CHAOSMIX* aRecv, int aIndex)
{
	...
	if ( lpObj->ChaosLock == TRUE )
	{
		...
		GCServerMsgStringSend("You cannot open your Personal Shop within Chaos Castle.", lpObj->m_Index, 1);
		return;
	}
	...
	switch ( iMixType )	// TEST - Good
	{
        ...
        case CHAOS_TYPE_CONDOR_FEATHER:
                ThirdWingLevel1ChaosMix(lpObj);
                break;
        case CHAOS_TYPE_THIRD_WING:     
                ThirdWingLevel2ChaosMix(lpObj);
                break;
		default:
			LogAdd("[%s][%s] Undefine chaosmix type detect %d",
...
void PMoveProc(PMSG_MOVE* lpMove, int aIndex)
{
	...
	if ( (GetTickCount()-lpObj->m_LastMoveTime) < 100 )
	{
		if (!IsGM(aIndex))
		{
			lpObj->SecurePackets.MoveHackCount++;
			if (lpObj->SecurePackets.MoveHackCount > gMaxMovePacketsPerSec)
			{
				CloseClient(aIndex);
				return;
			}
...
#endif
	DWORD CurrentPacketTime = GetTickCount();//зчитуємо поточний час
	if (CurrentPacketTime - lpObj->SecurePackets.OldMovePacketTime > 1000)
	{
		lpObj->SecurePackets.OldMovePacketTime = CurrentPacketTime;
		lpObj->SecurePackets.MoveHackCount = 0;
	}
	...
	lpObj->m_LastMoveTime = CurrentPacketTime;
	lpObj->m_Rest = 0;
	lpObj->PathCur = 0;
	...
	PHeadSetB((LPBYTE)&pMove, P_WALK, sizeof(pMove));
...
void RecvPositionSetProc(PMSG_POSISTION_SET * lpMove, int aIndex)
{
	...
	PHeadSetB((LPBYTE)&pMove, P_DAMAGE, sizeof(pMove));//P_DAMAGE
	pMove.NumberH = SET_NUMBERH(aIndex);
	pMove.NumberL = SET_NUMBERL(aIndex);
	pMove.X = lpMove->X;
	pMove.Y = lpMove->Y;
	...
	{
		LogAdd("[HACK] Position check filed. Account %s , Nick %s try to move higher then 15 squares",lpObj->AccountID,lpObj->Name);
		CloseClient(aIndex);
		return;
	}
...
void CGAttack(PMSG_ATTACK* lpMsg, int aIndex)
{
	...
	if ( iTimeCalc < lpObj->m_DetectSpeedHackTime && iTimeCalc && lpObj->HaveWeaponInHand )
	{
		...
		if ( lpObj->m_DetectCount > gHackCheckCount )
		{
			...
			if ( gIsKickDetecHackCountLimit )
			{
				...
				{
					LogAdd("[%s][%s] Kick DetecHackCountLimit Over User (%d)",
						lpObj->AccountID, lpObj->Name,
						lpObj->m_DetectedHackKickCount);
			...
			LogAdd("[%s][%s] Attack Speed Is Wrong Normal (%d)(%d) Penalty %d",
				lpObj->AccountID, lpObj->Name, 
	...
	if ( bIsIgnorePacketSpeedHackDetect )
	{
		if ( lpObj->m_SpeedHackPenalty > 0 )
		{
			...
			LogAdd("[%s][%s] Apply Attack Speed Penalty (%d left)",
				lpObj->AccountID, lpObj->Name, 
				lpObj->m_SpeedHackPenalty);
...
void GCDamageSend(int aIndex, int TargetIndex, int AttackDamage, int MSBFlag, int MSBDamage, int iShieldDamage)
{
	...
	PHeadSetB((LPBYTE)&pResult, P_ATTACK, sizeof(pResult));
...
void CGMagicAttack(PMSG_MAGICATTACK* lpMsg, int aIndex)
{
	...
	if ( attackret )
	{
		LogAdd("[%s][%s] Try Attack In Not Attack Area [Protocol] (MapNum: %d, X: %d, Y: %d) errortype = %d",
			lpObj->AccountID, lpObj->Name, lpObj->MapNumber, lpObj->X, lpObj->Y,
			attackret);
	...
	if ( lpObj->Type == OBJ_USER )
	{
		...
		{
			LogAdd( "[0x19] CGMagicAttack() - Invalid Status");
			return;
		}

		if ( MagicDamageC.CheckKillCount(lpMagic->m_Skill, lpObj->m_btKillCount) < 0 )
		{
			LogAdd( "[0x19] CGMagicAttack() - Invalid KillCount");
			return;
		}
	}
	else
	...
	{
		int dwTick = GetTickCount() - lpObj->m_dwSkillDistanceErrorTick;
		...
		if ( lpObj->m_iSkillDistanceErrorCount > g_iSkillDistanceKickCount )
		{
			...
			{
				LogAdd("[SKILL DISTANCE CHECK] [%s][%s] Kick Invalid Skill Area User. count(%d)",
...
void CGTeleportRecv(PMSG_TELEPORT* lpMsg, int aIndex)
{
	...
	pMsg.NumberL = SET_NUMBERL(aIndex);
	char name[]="Good";
	memcpy(pMsg.btPShopText, name, sizeof(pMsg.btPShopText));
	memcpy(pMsg.btName, gObj[aIndex].Name, sizeof(pMsg.btName));
	...
	if ( lpMsg->MoveNumber == 0 )
	{
		...
		{
			LogAdd( "[%s][%s] Try Teleport Not Move Area [%d,%d]",
				gObj[aIndex].AccountID, gObj[aIndex].Name,
				x, y);
		...
		PMSG_TELEPORT_RESULT pTeleportResult;
			...
			pTeleportResult.MapX = (BYTE)gObj[aIndex].X;
			pTeleportResult.MapY = (BYTE)gObj[aIndex].Y;
		...
		if ( lpMagic )
		{
			...
			if ( usemana >= 0 )
			{
				...
				if ( usebp >= 0 )
				{
					...
					GCManaSend(gObj[aIndex].m_Index, (short)gObj[aIndex].Mana, -1, 0, gObj[aIndex].BP);
				}
			}
		}
		return;
	...
	GCTeleportSend(&gObj[aIndex], lpMsg->MoveNumber, 
		gObj[aIndex].MapNumber, (BYTE)gObj[aIndex].X, (BYTE)gObj[aIndex].Y, gObj[aIndex].Dir);
...
void CGTargetTeleportRecv(PMSG_TARGET_TELEPORT * lpMsg, int aIndex)
{
	...
	pAttack.SourceNumberH = SET_NUMBERH(aIndex);
	pAttack.SourceNumberL = SET_NUMBERL(aIndex);
	int iTargetIndex = MAKE_NUMBERW(lpMsg->NumberL,lpMsg->NumberH ); // Not error! Only security	
	...
	{
		LogAdd( "[%s][%s] Try Target Teleport Not Move Area [%d,%d]",
...
void CGBeattackRecv(unsigned char* lpRecv, int aIndex, int magic_send)
{
	...
	// Check the Protocol
	if ( lpCount->h.headcode != P_SKILL )
	{
		LogAdd("error-L3 HEADCODE:%d , %s %d",lpCount->h.headcode, __FILE__, __LINE__);
		return;
	}
	...
	if ( gObj[aIndex].Type == OBJ_USER )
	{
		...
		{
			// 
			LogAdd( "It's not his magic [%s][%s] Magic: %d.",
		...
		{
			LogAdd( "Too long time passed after casting magic. [%s][%s] (%d)(%d)",
				gObj[aIndex].AccountID,
				gObj[aIndex].Name,
				lpMagic->m_Skill,
	...
	// Avoid Attack out of the predetermination
		...
		if ( NSAttackSerial > gObj[aIndex].OSAttackSerial )
		{
			...
			if ( iTimeCalc < lpObj->m_DetectSpeedHackTime && iTimeCalc )
			{
				...
				if ( lpObj->m_DetectCount > gHackCheckCount )
				{
					...
					LogAdd("[%s][%s] Attack Speed Is Wrong Magic3 (%d)(%d) Penalty %d",
						lpObj->AccountID, lpObj->Name, 
	...
	for (int n=0;n<lpCount->Count;n++)
	{
		...
		if ( lpMsg->MagicKey  && lpMagic->m_Skill == 9 )	// "Evil Spirit"
		{
			...
			{
				LogAdd( "InValid DurationTime Key = %d ( Time : %d) [%s][%s]",	
			...
			{
				LogAdd( "InValid VailidCount = %d ( Count : %d) [%s][%s]",	
		...
		if ( gWriteSkillLog )
		{
			LogAdd("Magic Attack3 : %d, serial = %d, Tgt =  %d, cnt = %d",
		...
		for (int i=0;i<lpCount->Count;i++)
		{
			if ( n != i)
			{
				...
				{
					LogAdd("[%s][%s] Detect Hack : Multi Attack",
						gObj[aIndex].AccountID, gObj[aIndex].Name);
...
void CGDurationMagicRecv(PMSG_DURATION_MAGIC_RECV* lpMsg, int aIndex)
{
	...
	if ( lpObj->Type == OBJ_USER )
	{
		...
		{
			LogAdd( "[0x1E] CGDurationMagicRecv() - Invalid Status");	// #warning maybe protocol here change
			return;
		}

		if ( MagicDamageC.CheckKillCount(lpMagic->m_Skill, lpObj->m_btKillCount) < 0 )
		{
			LogAdd( "[0x1E] CGDurationMagicRecv() - Invalid KillCount");
			return;
		}
	}
...
void CGUseItemRecv(PMSG_USEITEM* lpMsg, int aIndex)
{
	...
	if ( gObj[aIndex].pTransaction == 1 )
	{
		LogAdd("[%s][%s] CGUseItemRecv() Failed : Transaction == 1, IF_TYPE : %d",
			gObj[aIndex].AccountID,
			gObj[aIndex].Name,
	...
	{
		LogAdd("error : Location is over the range. %s/%d", __FILE__, __LINE__);
	...
	if ( citem->IsItem() )
	{
		...
		if ( gObjCheckSerial0ItemList(citem) )
		{
			...
			LogAdd("[ANTI-HACK][Serial 0 Item] [UseItem] (%s)(%s) Item(%s) Pos(%d)",
				gObj[aIndex].AccountID,
				gObj[aIndex].Name,
				citem->GetName(),
				pos);
				 ...
				 citem->m_Type == ITEMGET(14,6)  )
		{
			...
			GCManaSend(aIndex, (short)gObj[aIndex].Mana, 0xFF, 0, gObj[aIndex].BP);
		...
		else if ( citem->m_Type == ITEMGET(14,38) )	// Small Compound Potion 
		{
			...
			LogAdd("[%s][%s]Use Compound Potion Lv1 - SD[%d] HP[%f] -> SD[%d] HP[%f]",
				gObj[aIndex].AccountID, gObj[aIndex].Name, 
				gObj[aIndex].iShield, gObj[aIndex].Life,
				gObj[aIndex].iShield+iShieldGage, (double)(gObj[aIndex].Life+(double)iHPGage));
		}
		else if ( citem->m_Type == ITEMGET(14,39) )	// Compound Potion 
		{
			...
			LogAdd("[%s][%s]Use Compound Potion Lv2 - SD[%d] HP[%f] -> SD[%d] HP[%f]",
				gObj[aIndex].AccountID, gObj[aIndex].Name, 
				gObj[aIndex].iShield, gObj[aIndex].Life,
				gObj[aIndex].iShield+iShieldGage, (double)(gObj[aIndex].Life+(double)iHPGage));
		}
		else if ( citem->m_Type == ITEMGET(14,40) )	// Large Compound Potion 
		{
			...
			LogAdd("[%s][%s]Use Compound Potion Lv3 - SD[%d] HP[%f] -> SD[%d] HP[%f]",
				gObj[aIndex].AccountID, gObj[aIndex].Name, 
				gObj[aIndex].iShield, gObj[aIndex].Life,
				gObj[aIndex].iShield+iShieldGage, (double)(gObj[aIndex].Life+(double)iHPGage));
		}
				  ...
				  citem->m_Type == ITEMGET(12,35) )	// Scroll of Fire Scream
		{
			...
			if ( citem->IsClass((char)gObj[aIndex].Class, gObj[aIndex].ChangeUP) == FALSE )
			{	
				GCInventoryItemDeleteSend(aIndex, -1, 1);
			}
			else
			{
				int addskill = gObjMagicAdd(&gObj[aIndex], BYTE(citem->m_Type>>9), (BYTE)citem->m_Type %  MAX_SUBTYPE_ITEMS, (BYTE)citem->m_Level, skillnumber);

				if ( addskill >= 0 )
				{
					gObjInventoryItemSet(aIndex, pos, -1);
					GCMagicListOneSend(aIndex, addskill, skillnumber, (BYTE)citem->m_Level, 0, 0);
					gObj[aIndex].pInventory[pos].Clear();
					GCInventoryItemDeleteSend(aIndex, pos, 1);
				}
				else
		...
		{
			LogAdd("[Using Item] [Return Scroll] [%s][%s] - Current Map:[%d]",
		...
		else if ( citem->m_Type == ITEMGET(14,9) ) // Ale
		{
			...
			gObj[aIndex].pInventory[pos].Clear();
			GCInventoryItemDeleteSend(aIndex, pos, 1);
			gObjUseDrink(&gObj[aIndex], citem->m_Level);
		}
        ...
        else if ( citem->m_Type == ITEMGET(14,78) ) // CSHOP Arcanum Forзa
        {
                //g_CashItemPeriodSystem.GDReqPeriodItemInsert(&gObj[aIndex], citem->m_Type, 1);
                g_ItemAddOption.SetItemEffect(&gObj[aIndex], ITEMGET(14,78), 0);
                if ( !gObjSearchItemMinus(&gObj[aIndex], pos, 1) )
                {
                        gObjInventoryItemSet(aIndex, pos, -1);
                        gObj[aIndex].pInventory[pos].Clear();
        ...
        {
                //g_CashItemPeriodSystem.GDReqPeriodItemInsert(&gObj[aIndex], citem->m_Type, 1);
                g_ItemAddOption.SetItemEffect(&gObj[aIndex], ITEMGET(14,79), 0);
                if ( !gObjSearchItemMinus(&gObj[aIndex], pos, 1) )
                {
                        gObjInventoryItemSet(aIndex, pos, -1);
                        gObj[aIndex].pInventory[pos].Clear();
        ...
        {
                //g_CashItemPeriodSystem.GDReqPeriodItemInsert(&gObj[aIndex], citem->m_Type, 1);
                g_ItemAddOption.SetItemEffect(&gObj[aIndex], ITEMGET(14,80), 0);
                if ( !gObjSearchItemMinus(&gObj[aIndex], pos, 1) )
                {
                        gObjInventoryItemSet(aIndex, pos, -1);
                        gObj[aIndex].pInventory[pos].Clear();
        ...
        {
                //g_CashItemPeriodSystem.GDReqPeriodItemInsert(&gObj[aIndex], citem->m_Type, 1);
                g_ItemAddOption.SetItemEffect(&gObj[aIndex], ITEMGET(14,81), 0);
                if ( !gObjSearchItemMinus(&gObj[aIndex], pos, 1) )
                {
                        gObjInventoryItemSet(aIndex, pos, -1);
                        gObj[aIndex].pInventory[pos].Clear();
        ...
        {
                //g_CashItemPeriodSystem.GDReqPeriodItemInsert(&gObj[aIndex], citem->m_Type, 1);
                g_ItemAddOption.SetItemEffect(&gObj[aIndex], ITEMGET(14,82), 0);
                if ( !gObjSearchItemMinus(&gObj[aIndex], pos, 1) )
                {
                        gObjInventoryItemSet(aIndex, pos, -1);
                        gObj[aIndex].pInventory[pos].Clear();
        ...
        else if ( citem->m_Type == ITEMGET(13,60) ) // CSHOP PK Free Ticket
        {
                if ( gObj[aIndex].m_PK_Level == 6 )
                        ...
                        LogAdd("[PCS] [%s][%s] Use PK Clear Item", gObj[aIndex].AccountID, gObj[aIndex].Name);
        }
		else if ( citem->m_Type == ITEMGET(14,7) )	// Siege Potion
		{
			...
			{
				LogAdd("[CastleSiege] Using CastleSiege Potion Lv[%d]/Si[%d]/Dur[%d] - [%s][%s] - (Guild : %s)",
					iLevel, iSerial, iDur, gObj[aIndex].AccountID, gObj[aIndex].Name, gObj[aIndex].lpGuild->Name);
			}
			else
			{
				LogAdd("[CastleSiege] Using CastleSiege Potion Lv[%d]/Si[%d]/Dur[%d] - [%s][%s]",
		...
		else if ( citem->m_Type == ITEMGET(13,48) )
		{
			if ( (gObj[aIndex].MapNumber < MAP_INDEX_KALIMA1 || gObj[aIndex].MapNumber > MAP_INDEX_KALIMA6 ) && gObj[aIndex].MapNumber != MAP_INDEX_KALIMA7 )
			{
				...
				iLevel++;
				iKalimaGateX = gObj[aIndex].X + Random(0,5) - 2;
				iKalimaGateY = gObj[aIndex].Y + Random(0,5) - 2;
				...
				if ( bKalimaGateCreateResult == TRUE )
				{
					...
					LogAdd("[PCS] Use Free Kalima Ticket Success");
				}
				else
				{
					LogAdd("[PCS] Use Free Kalima Ticket Failed");
				}
			}
		}
		else
...
void GCReFillSend(int aIndex, WORD Life, BYTE Ipos, unsigned char flag,  WORD wShield)
{
	...
	pMsg.Flag = 1;	// #error Flag is Disabled=0
...
void GCReqmoveDevilSquare(PMSG_REQ_MOVEDEVILSQUARE* lpMsg, int aIndex)
{
	...
	LogAdd("[DevilSquare] [%s][%s] Request Move DevilSquare [%d][%d][%d]",
	...
	if ( MAIN_INVENTORY_RANGE(cInvitationItemPos) == FALSE )
	{
		...
		LogAdd("[DevilSquare] [%s][%s] Move DevilSquare Fail [%d][%d][%d]",
			lpObj->AccountID, lpObj->Name, lpObj->Class, lpObj->Level, cSquareNumber+1);
	...
	if ( ItemLevel != 0 )
	{
		if ( ItemLevel != 10 )
		{
			if ( ItemLevel != (cSquareNumber+1) )
			{
				...
				LogAdd("[DevilSquare] [%s][%s] Move DevilSquare Invitation LevelError [%d][%d][%d][%d]",
	...
	{
		LogAdd("[DevilSquare] [%s][%s] GetUserLevelToEnter() failed",
			lpObj->AccountID, lpObj->Name);
	...
	if ( bEnterCheck == 1 )
	{
		...
		LogAdd("[DevilSquare] [%s][%s] Move DevilSquare Fail [%d][%d][%d][%d]",
	...
	LogAdd("[DevilSquare] [%s][%s] Move DevilSquare success [%d][%d] RemoveItem[%s][%d][%d][%d]",
	...
	switch ( lpMsg->hEventType )
	{
		case 1:
			if ( gObj[aIndex].m_PK_Level >= 4 )
			{
				...
				LogAdd("[PK User][DevilSquare] [%s][%s] Move Fail [PK Level:%d]",
		...
		case 2:
			if ( gObj[aIndex].m_PK_Level >= 4 )
			{
				...
				LogAdd("[PK User][BloodCastle] [%s][%s] Move Fail [PK Level:%d]",
					gObj[aIndex].AccountID, gObj[aIndex].Name, gObj[aIndex].m_PK_Level);
		...
		case 4:
			if ( gObj[aIndex].m_PK_Level >= 4 )
			{
				...
				LogAdd("[PK User][ChaosCastle] [%s][%s] Move Fail [PK Level:%d]",
					gObj[aIndex].AccountID, gObj[aIndex].Name, gObj[aIndex].m_PK_Level);
...
void GCRegEventChipRecv(PMSG_REGEVENTCHIP* lpMsg, int aIndex)
{
	...
	{
		LogAdd("[EventChip] [%s][%s] Attempted ItemCopy using Trade Window",
			lpObj->AccountID, lpObj->Name);
	...
	if ( lpObj->m_IfState.type == 7 )
	{
		LogAdd("[EventChip] [%s][%s] Attempted ItemCopy using ChaosBox Window",
			lpObj->AccountID, lpObj->Name);
	...
	if ( lpObj->UseEventServer != FALSE )
	{
		...
		LogAdd("[EventChip] [%s][%s] Not Found EventChip (RegEventchip) #1 %d",
	...
	if ( !sitem->IsItem() )
	{
		...
		LogAdd("[EventChip] [%s][%s] Not Found EventChip (RegEventchip) #2 %d",
			lpObj->AccountID, lpObj->Name, lpMsg->ChipPos);
	...
	switch ( lpMsg->Type )
	{
		case 0x00:
			...
			else
			{
				...
				LogAdd("[EventChip] [%s][%s] Not Found EventChip (RegEventchip) #3 %d",
					lpObj->AccountID, lpObj->Name, lpMsg->ChipPos);
		...
		case 0x01:
			...
			else
			{
				...
				LogAdd("[Stone] [%s][%s] Not Found EventChip (Stone Pos: %d)",
					lpObj->AccountID, lpObj->Name, lpMsg->ChipPos);
...
void GCGetMutoNumRecv(PMSG_GETMUTONUMBER* lpMsg, int aIndex)
{
	...
	{
		char msg[255];
		wsprintf(msg, "АМ№М ·з°ЎµеАЗ јэАЪ°Ў АЦЅАґПґЩ");
...
void CGCloseWindow(int aIndex)
{
	...
	if ( lpObj->m_IfState.use && lpObj->m_IfState.type )
	{
		...
		if ( lpObj->m_IfState.type == 10 && lpObj->TargetShopNumber == 229 )	// Marlon
		{
...
void CGRequestEnterBloodCastle(PMSG_REQ_MOVEBLOODCASTLE* lpMsg, int iIndex)
{
	...
	char szTemp[256];
	wsprintf(szTemp, "You are entering a Blood Castle No.%d", iITEM_LEVEL);
	...
	LogAdd("[Blood Castle] (%d) (Account:%s, Name:%s) Entered Blood Castle (Invisible Cloak Serial:%d)",
...
void CGRequestEnterChaosCastle(PMSG_REQ_MOVECHAOSCASTLE* lpMsg, int iIndex)
{
	...
	{
		PMSG_NOTICE pNotice;
		TNotice::MakeNoticeMsgEx(&pNotice, 1, "You cannot enter Chaos Castle from here.");
	...
	if ( gPkLimitFree == FALSE )
	{
		...
		{
			PMSG_NOTICE pNotice;
			TNotice::MakeNoticeMsgEx(&pNotice, 1, "Outlaws are not permitted to enter Chaos Castle.");
			DataSend(iIndex, (LPBYTE)&pNotice, pNotice.h.size);
			return;
		}
	}
	...
	{
		PMSG_NOTICE pNotice;
		TNotice::MakeNoticeMsgEx(&pNotice, 1, "You cannot enter Chaos Castle while in a Duel.");
		DataSend(iIndex, (LPBYTE)&pNotice, pNotice.h.size);
		return;
	}
	...
	if ( gObj[iIndex].lpGuild )
	{
		...
		{
			PMSG_NOTICE pNotice;
			TNotice::MakeNoticeMsgEx(&pNotice, 1, "You cannot enter Chaos Castle while in a Guild War.");
			DataSend(iIndex, (LPBYTE)&pNotice, pNotice.h.size);
			return;
		}
	}
	...
	{
		PMSG_NOTICE pNotice;
		TNotice::MakeNoticeMsgEx(&pNotice, 1, "You cannot enter Chaos Castle while riding a beast.");
		DataSend(iIndex, (LPBYTE)&pNotice, pNotice.h.size);
		return;
	}
	...
	{
		GCServerMsgStringSend("You cannot enter Chaos Castle while wearing a transformation ring.", iIndex, 1);
		return;
	}
	...
	char szTemp[128];
	wsprintf(szTemp, "You have entered Chaos Castle %d.", iENTER_LEVEL+1);
	GCServerMsgStringSend(szTemp, iIndex, 1);
	...
	LogAdd("[Chaos Castle] (%d) (Account:%s, Name:%s) Entered Chaos Castle (Guard Mail Set Serial:%d)",
...
void CGReqMoveOtherServer(PMSG_REQ_MOVE_OTHERSERVER * lpMsg, int aIndex)
{
	...
	wsDataServerCli.DataSend((char*)&pCharTransfer, sizeof(pCharTransfer));

	LogAdd("[CharTrasfer] Request Character Trasfer [%s][%s]",
		lpObj->AccountID, lpObj->Name);
}
...
void CGDuelStartRequestRecv(PMSG_REQ_START_DUEL * lpMsg, int aIndex)
{
	...
	if ( gNonPK )
	{
		GCServerMsgStringSend("Dueling is not allowed in NON-PK servers.", aIndex, 1);
		return;
	}
	...
	if ( !gPkLimitFree )
	{
		if ( gObj[aIndex].m_PK_Level >= 6 )
		{
			GCServerMsgStringSend("An outlaw can't issue a Duel challenge.", aIndex, 1);
			return;
		}
	}
	...
	{
		GCServerMsgStringSend("You have already challenged someone to a Duel.", aIndex, 1);
		GCServerMsgStringSend("In order to issue a Duel, cancel the previous request.", aIndex, 1);
		return;
	}

	if ( OBJMAX_RANGE(gObj[aIndex].m_iDuelUser ) )
	{
		GCServerMsgStringSend("You are already in a Duel.", aIndex, 1);
		GCServerMsgStringSend("In order to issue a Duel, cancel the previous request.", aIndex, 1);
		return;
	}
	...
	if ( CC_MAP_RANGE(gObj[aIndex].MapNumber) )
	{
		GCServerMsgStringSend("Duels are not allowed within Chaos Castle.", aIndex, 1);
		return;
	}
	...
	if ( !gPkLimitFree )
	{
		...
		{
			GCServerMsgStringSend("You cannot Duel with an Outlaw.", aIndex, 1);
			return;
		}
	}
	...
	{
		GCServerMsgStringSend("The name of the character you have challenged is incorrect.", aIndex, 1);
		return;
	}

	if ( IsDuelEnable(iDuelIndex) == FALSE )
	{
		GCServerMsgStringSend("Your challenge to Duel has been refused.", aIndex, 1);
		return;
	}

	if ( lpObj->lpGuild && lpObj->lpGuild->WarState == 1 )
	{
		GCServerMsgStringSend("You cannot challenge someone to a duel while in a guild war.", aIndex, 1);
		return;
	}
	...
	if ( bRetVal )
	{
		GCServerMsgStringSend("Self-defense is active. You cannot Duel.", aIndex, 1);
		return;
	}
	...
	{
		GCServerMsgStringSend("A duel is not allowed in Devil Square.", aIndex, 1);
		return;
	}

	if ( BC_MAP_RANGE(gObj[aIndex].MapNumber) )
	{
		GCServerMsgStringSend("You cannot Duel inside Blood Castle.", aIndex, 1);
		return;
	}

	if ( lpObj->m_IfState.use > 0 )
	{
		GCServerMsgStringSend("You cannot Duel while trading.", aIndex, 1);
		return;
	}

	if ( gObj[iDuelIndex].m_IfState.use > 0 )
	{
		wsprintf(szTempText, "%s is trading an item.", gObj[iDuelIndex].Name);
	...
	{
		wsprintf(szTempText, "%s has already accepted a challenge to Duel.", gObj[iDuelIndex].Name);
		GCServerMsgStringSend(szTempText, aIndex, 1);
		return;
	}
	...
	{
		wsprintf(szTempText, "%s issues a Duel challenge.", gObj[iDuelIndex].Name);
		GCServerMsgStringSend(szTempText, aIndex, 1);
		return;
	}
	...
	{
		wsprintf(szTempText, "%s is in a Duel.", gObj[iDuelIndex].Name);
		GCServerMsgStringSend(szTempText, aIndex, 1);
		return;
	}
	...
	wsprintf(szTempText, "Challenged %s to a Duel.", gObj[iDuelIndex].Name);
	GCServerMsgStringSend(szTempText, aIndex, 1);

	LogAdd("[Duel] [%s][%s] Requested to Start Duel to [%s][%s]",
...
void CGDuelEndRequestRecv(PMSG_REQ_END_DUEL * lpMsg, int aIndex)
{
	...
	if ( OBJMAX_RANGE(gObj[iDuelIndex].m_iDuelUser) )
	{
		if ( gObj[iDuelIndex].m_iDuelUser == aIndex )
		{
			...
			LogAdd("[Duel] [%s][%s] Duel Ended [%s][%s]",
				gObj[aIndex].AccountID, gObj[aIndex].Name,
...
void CGDuelOkRequestRecv(PMSG_ANS_DUEL_OK * lpMsg, int aIndex)
{
	...
	if ( OBJMAX_RANGE(iDuelIndex) )
	{
		...
		if ( OBJMAX_RANGE(gObj[aIndex].m_iDuelUserReserved) )
		{
			GCServerMsgStringSend("You are already issuing a Duel challenge.", aIndex, 1);
			gObj[aIndex].m_iDuelUserRequested = -1;
			gObj[iDuelIndex].m_iDuelUserReserved = -1;
			memcpy(pMsgSend.szName, gObj[aIndex].Name, sizeof(pMsgSend.szName));
			pMsgSend.bDuelStart = false;
		...
		if ( gObj[iDuelIndex].m_iDuelUserReserved == aIndex )
		{
			...
			if ( !strcmp(szDuelName, szDuelName2))
			{
				...
				else if ( lpMsg->bDuelOK == true )
				{
					...
					LogAdd("[Duel] [%s][%s] Duel Started [%s][%s]",
						gObj[aIndex].AccountID, gObj[aIndex].Name,
						gObj[iDuelIndex].AccountID, gObj[iDuelIndex].Name);
				}
			}
...
void CGRequestPetItemInfo(PMSG_REQUEST_PET_ITEMINFO * lpMsg, int aIndex)
{
	...
	if ( lpMsg->InvenType == 0 )	// Inventory
	{
		...
		{
			LogAdd("[PetItem] [%s][%s] RequestPetItem Invalid Iventory nPos %d",
	...
	else if ( lpMsg->InvenType == 1 )	// Warehouse
	{
		if ( lpObj->LoadWareHouseInfo != false )
		{
			...
			{
				LogAdd("[PetItem] [%s][%s] RequestPetItem Invalid WareHouse nPos %d",
					lpObj->AccountID, lpObj->Name, lpMsg->nPos);
	...
	else if ( lpMsg->InvenType == 2 )	// Trade
	{
		...
		{
			LogAdd("[PetItem] [%s][%s] RequestPetItem Invalid Trade nPos %d",
				lpObj->AccountID, lpObj->Name, lpMsg->nPos);
	...
	else if ( lpMsg->InvenType == 3 )	// Target Trade
	{
		...
		if ( !TRADE_BOX_RANGE(lpMsg->nPos))
		{
			LogAdd("[PetItem] [%s][%s] RequestPetItem Invalid TargetTrade nPos %d",
				lpObj->AccountID, lpObj->Name, lpMsg->nPos);
	...
	else if ( lpMsg->InvenType == 4 )	// Chaos
	{
		...
		{
			LogAdd("[PetItem] [%s][%s] RequestPetItem Invalid Chaos nPos %d",
				lpObj->AccountID, lpObj->Name, lpMsg->nPos);
...
void GCGuildViewportInfo(PMSG_REQ_GUILDVIEWPORT * aRecv, int aIndex)
{
	...
	if ( lpGuildInfo )
	{
		...
		//strcpy_s(pMsg.GuildName, sizeof(pMsg.GuildName), lpGuildInfo->Name);
	...
	{
		LogAdd("ЎЪЎЪЎЩ ±жµе Б¤єё ГЈА»јц ѕшАЅ. АМё§ : [%s] №шИЈ : %d",
...
void CGRelationShipReqJoinBreakOff(PMSG_RELATIONSHIP_JOIN_BREAKOFF_REQ * aRecv, int aIndex)
{
	...
	{
		MsgOutput(aIndex, "Loren Deep event 03");
		return;
	}
	...
	if ( aRecv->btRequestType == 1 )
	{
		if ( aRecv->btRelationShipType == 1 )
		{
			...
			std::string szCastleOwnerGuildName = g_CastleSiegeSync.GetCastleOwnerGuild();

			if ( szCastleOwnerGuildName.compare(lpGuildInfo->Name) != 0 )
			{
...
void CGRelationShipReqKickOutUnionMember(PMSG_KICKOUT_UNIONMEMBER_REQ* aRecv, int aIndex)
{
	...
	{
		GCResultSend(aIndex, 0x51, 3);
		MsgOutput(aIndex, "ЎЪ Terminated User.");
		return;
	}
	...
	{
		GCResultSend(aIndex, 0x51, 3);
		MsgOutput(aIndex, "ЎЩ Terminated Guild.");
		return;
	}
...
void CGReqMapSvrAuth(PMSG_REQ_MAPSERVERAUTH * lpMsg, int iIndex)
{
	...
	{
		LogAdd( "error-L1: Serial error [%s] [%s]", id, btSerial);
		GCAnsMapSvrAuth(iIndex, 6);
		CloseClient(iIndex);
	...
	if ( PacketCheckTime(lpObj) == FALSE )
	{
		LogAdd("error-L2 : Login time is over. (%d)(%s)", iIndex, id);
		CloseClient(iIndex);
	...
	{
		LogAdd("error-L2 : Authentication is allowed after connected. (%d)(%s)", iIndex, id);
		CloseClient(iIndex);
	...
	if ( gObj[iIndex].LoginMsgSnd != 0 )
	{
		...
		{
			LogAdd("error : It's after authentication information is sended. (%d)(%s)(%s)", iIndex, id, gObj[iIndex].Ip_addr);
		}
		else
		{
	...
	LogAdd("[MapServerMng] Map Server Join Send : [%s][%s](%d)",
...
void GCAnsMapSvrAuth(int iIndex, int iResult)
{
	if ( !OBJMAX_RANGE(iIndex) )
	{
		LogAdd("[MapServerMng] Packet Error JG [0x7B] - Index out of bound : %d",iIndex);
		return;
	}
...
void GCAnsRegCastleSiege(int iIndex, int iResult, LPSTR lpszGuildName)
{
	...
	{
		LogAdd("[CastleSiege] [%s][%s] Registered Castle Siege (GUILD:%s) - Result:%d",
...
void GCAnsGiveUpCastleSiege(int iIndex, int iResult, BOOL bGiveUp, int iMarkCount, LPSTR lpszGuildName)
{
	...
	if ( gObjIsConnected(iIndex) )
	{
		LogAdd("[CastleSiege] [%s][%s] GiveUp Castle Siege (GUILD:%s) - Result:%d",
			gObj[iIndex].AccountID, gObj[iIndex].Name, gObj[iIndex].GuildName, iResult);
	}
...
void CGReqNpcBuy(PMSG_REQ_NPCBUY * lpMsg, int iIndex)
{
	...
	{
		LogAdd( "[CastleSiege] CGReqNpcBuy() ERROR - Authority Fail [%s][%s], Guild:(%s)(%d), Npc:(CLS:%d, IDX:%d)",
	...
	if ( !bRET_VAL )
	{
		...
		LogAdd( "[CastleSiege] CGReqNpcBuy() ERROR - CCastleSiege::CheckAddDbNPC() [%s][%s], Guild:(%s)(%d), Npc:(CLS:%d, IDX:%d)",
			gObj[iIndex].AccountID, gObj[iIndex].Name, gObj[iIndex].GuildName,
			gObj[iIndex].GuildStatus, lpMsg->iNpcNumber, lpMsg->iNpcIndex);
		return;
	}

	LogAdd( "[CastleSiege] CGReqNpcBuy() OK - [%s][%s], Guild:(%s)(%d), Npc:(CLS:%d, IDX:%d)",
		gObj[iIndex].AccountID, gObj[iIndex].Name, gObj[iIndex].GuildName,
		gObj[iIndex].GuildStatus, lpMsg->iNpcNumber, lpMsg->iNpcIndex);
...
void CGReqNpcRepair(PMSG_REQ_NPCREPAIR * lpMsg, int iIndex)
{
	...
	if ( g_CastleSiege.CheckGuildOwnCastle(gObj[iIndex].GuildName) == FALSE ||
		 (gObj[iIndex].GuildStatus != 0x80 && gObj[iIndex].GuildStatus != 0x40 ) )
	{
		LogAdd( "[CastleSiege] CGReqNpcRepair() ERROR - Authority Fail [%s][%s], Guild:(%s)(%d)",
			gObj[iIndex].AccountID, gObj[iIndex].Name, gObj[iIndex].GuildName,
	...
	if ( !bRETVAL )
	{
		...
		LogAdd( "[CastleSiege] CGReqNpcRepair() ERROR - CL Request Fail [%s][%s], Guild:(%s)(%d), Npc:(CLS:%d, IDX:%d)",
			gObj[iIndex].AccountID, gObj[iIndex].Name, gObj[iIndex].GuildName,
			gObj[iIndex].GuildStatus, lpMsg->iNpcNumber, lpMsg->iNpcIndex);
	...
	if ( !gObjIsConnected(iNpcIndex) )
	{
		...
		LogAdd( "[CastleSiege] CGReqNpcRepair() ERROR - Npc Alive Data Mismatch [%s][%s], Guild:(%s)(%d), Npc:(CLS:%d, IDX:%d)",
			gObj[iIndex].AccountID, gObj[iIndex].Name, gObj[iIndex].GuildName,
	...
	{
		case 277:	iRepairCost = int( (gObj[iNpcIndex].MaxLife - gObj[iNpcIndex].Life) * 5.0 + (gObj[iNpcIndex].m_btCsNpcDfLevel * 1000000) );	
			break;
		case 283:	iRepairCost = int( (gObj[iNpcIndex].MaxLife - gObj[iNpcIndex].Life) * 3.0 + ((gObj[iNpcIndex].m_btCsNpcDfLevel + gObj[iNpcIndex].m_btCsNpcRgLevel) * 1000000) );	
			break;
		default:
			GCAnsNpcRepair(iIndex, 0, lpMsg->iNpcNumber, lpMsg->iNpcIndex, 0, 0);
			return;
	...
	if ( gObj[iIndex].Money < iRepairCost )
	{
		...
		LogAdd( "[CastleSiege] CGReqNpcRepair() ERROR - Money isn't enough [%s][%s], Guild:(%s)(%d), Npc:(CLS:%d, IDX:%d)",
			gObj[iIndex].AccountID, gObj[iIndex].Name, gObj[iIndex].GuildName,
			gObj[iIndex].GuildStatus, pNpcData.m_iNPC_NUM, pNpcData.m_iNPC_INDEX);
	...
	LogAdd( "[CastleSiege] CGReqNpcRepair() OK - [%s][%s], Guild:(%s)(%d), Npc:(CLS:%d, IDX:%d)",
		gObj[iIndex].AccountID, gObj[iIndex].Name, gObj[iIndex].GuildName,
		gObj[iIndex].GuildStatus, pNpcData.m_iNPC_NUM, pNpcData.m_iNPC_INDEX);
...
void CGReqNpcUpgrade(PMSG_REQ_NPCUPGRADE * lpMsg, int iIndex)
{
	...
	{
		LogAdd("[CastleSiege] CGReqNpcUpgrade() ERROR - Authority Fail [%s][%s], Guild:(%s)(%d)",
			gObj[iIndex].AccountID, gObj[iIndex].Name, gObj[iIndex].GuildName,
			gObj[iIndex].GuildStatus);
		return;
	}
		...
		case 283:
			{
				...
				if ( bRETVAL )
				{
					switch ( lpMsg->iNpcUpType )
					{
						...
						default:
							GCAnsNpcUpgrade(iIndex, 0x05, lpMsg->iNpcNumber, lpMsg->iNpcIndex, lpMsg->iNpcUpType, lpMsg->iNpcUpValue);
							LogAdd("[CastleSiege] CGReqNpcUpgrade() ERROR - UpType doesn't Exist [%s][%s], Guild:(%s)(%d), NPC CLS:%d, IDX:%d, UPTYPE:%d, UPVAL:%d",
								gObj[iIndex].AccountID, gObj[iIndex].Name, gObj[iIndex].GuildName,
								gObj[iIndex].GuildStatus, lpMsg->iNpcNumber, lpMsg->iNpcIndex,
	...
	if ( gObj[iIndex].Money < iNEED_MONEY )
	{
		...
		LogAdd("[CastleSiege] CGReqNpcUpgrade() ERROR - Money is Low [%s][%s], Guild:(%s)(%d), NPC CLS:%d, IDX:%d, UPTYPE:%d, UPVAL:%d, NXTLV:%d, Money:%d, Need Money:%d",
			gObj[iIndex].AccountID, gObj[iIndex].Name, gObj[iIndex].GuildName,
	...
	if ( iGEMOFDEFEND_COUNT < iNEED_GEMOFDEFEND )
	{
		...
		LogAdd("[CastleSiege] CGReqNpcUpgrade() ERROR - Gem is Low [%s][%s], Guild:(%s)(%d), NPC CLS:%d, IDX:%d, UPTYPE:%d, UPVAL:%d, NXTLV:%d, Gem:%d, Need Gem:%d",
			gObj[iIndex].AccountID, gObj[iIndex].Name, gObj[iIndex].GuildName,
	...
	LogAdd("[CastleSiege] [0xB2][0x07] CGReqNpcUpgrade() - Pay For Npc Upgrade (CLS:%d, IDX:%d, UPTYPE:%d, UPVAL:%d)",
	...
	LogAdd("[CastleSiege] CGReqNpcUpgrade() REQ OK - [%s][%s], Guild:(%s)(%d), NPC CLS:%d, IDX:%d, UPTYPE:%d, UPVAL:%d, NXTLV:%d",
		gObj[iIndex].AccountID, gObj[iIndex].Name, gObj[iIndex].GuildName,
...
void CGReqTaxMoneyInfo(PMSG_REQ_TAXMONEYINFO * lpMsg, int iIndex)
{
	...
	if ( g_CastleSiege.CheckGuildOwnCastle(gObj[iIndex].GuildName) == FALSE ||
		 (gObj[iIndex].GuildStatus != 0x80 ) )
	{
		LogAdd( "[CastleSiege] CGReqTaxMoneyInfo() ERROR - Authority Fail [%s][%s], Guild:(%s)(%d)",
			gObj[iIndex].AccountID, gObj[iIndex].Name, gObj[iIndex].GuildName,
			gObj[iIndex].GuildStatus);
		return;
	}
	...
	LogAdd("[CastleSiege] CGReqTaxMoneyInfo() REQ OK - [%s][%s], Guild:(%s)(%d)",
		gObj[iIndex].AccountID, gObj[iIndex].Name, gObj[iIndex].GuildName,
		gObj[iIndex].GuildStatus);
...
void CGReqTaxRateChange(PMSG_REQ_TAXRATECHANGE * lpMsg, int iIndex)
{
	...
	if ( g_CastleSiege.CheckGuildOwnCastle(gObj[iIndex].GuildName) == FALSE ||
		 (gObj[iIndex].GuildStatus != 0x80 ) )
	{
		LogAdd( "[CastleSiege] CGReqTaxRateChange() ERROR - Authority Fail [%s][%s], Guild:(%s)(%d)",
			gObj[iIndex].AccountID, gObj[iIndex].Name, gObj[iIndex].GuildName,
			gObj[iIndex].GuildStatus);
		return;
	}
	...
	if ( iTaxRate < 0 || iTaxRate > iMaxTaxRate ) 
	{
		...
		LogAdd("[CastleSiege] CGReqTaxRateChange() ERROR - Tax Rate Out of Range [%s][%s], Guild:(%s)(%d), TaxType:%d, TaxRate:%d",
			gObj[iIndex].AccountID, gObj[iIndex].Name, gObj[iIndex].GuildName,
	...
	LogAdd("[CastleSiege] CGReqTaxRateChange() REQ OK - [%s][%s], Guild:(%s)(%d), TaxType:%d, TaxRate:%d",
		gObj[iIndex].AccountID, gObj[iIndex].Name, gObj[iIndex].GuildName,
		gObj[iIndex].GuildStatus, lpMsg->btTaxType, iTaxRate);
...
void CGReqMoneyDrawOut(PMSG_REQ_MONEYDRAWOUT * lpMsg, int iIndex)
{
	...
	if ( g_CastleSiege.CheckGuildOwnCastle(gObj[iIndex].GuildName) == FALSE ||
		 (gObj[iIndex].GuildStatus != 0x80 ) )
	{
		LogAdd( "[CastleSiege] CGReqMoneyDrawOut() ERROR - Authority Fail [%s][%s], Guild:(%s)(%d)",
			gObj[iIndex].AccountID, gObj[iIndex].Name, gObj[iIndex].GuildName,
			gObj[iIndex].GuildStatus);
		return;
	}
	...
	if ( iMoneyChange <= 0 )
	{
		...
		LogAdd("[CastleSiege] CGReqMoneyDrawOut() ERROR - Req Money < 0 [%s][%s], Guild:(%s)(%d), Money:%d, TotMoney:%d",
			gObj[iIndex].AccountID, gObj[iIndex].Name, gObj[iIndex].GuildName,
	...
	if ( bRETVAL == FALSE )
	{
		...
		LogAdd("[CastleSiege] CGReqMoneyDrawOut() ERROR - Castle Money is Low [%s][%s], Guild:(%s)(%d), Money:%d, TotMoney:%d",
			gObj[iIndex].AccountID, gObj[iIndex].Name, gObj[iIndex].GuildName,
			gObj[iIndex].GuildStatus, iMoneyChange, gObj[iIndex].Money);
	...
	LogAdd("[CastleSiege] CGReqMoneyDrawOut() REQ OK - [%s][%s], Guild:(%s)(%d), Money:%d, TotMoney:%d",
		gObj[iIndex].AccountID, gObj[iIndex].Name, gObj[iIndex].GuildName,
		gObj[iIndex].GuildStatus, iMoneyChange, gObj[iIndex].Money);
...
void CGReqCsGateOperate(PMSG_REQ_CSGATEOPERATE * lpMsg, int iIndex)
{
	...
	if ( g_CastleSiege.GetCastleState() == CASTLESIEGE_STATE_STARTSIEGE )
	{
		...
		{
			LogAdd("[CastleSiege] CGReqCsGateOperate() ERROR - Authority Fail [%s][%s], Guild:(%s)(%d)",
				gObj[iIndex].AccountID, gObj[iIndex].Name, gObj[iIndex].GuildName,
				gObj[iIndex].GuildStatus);
			return;
		}
	...
	if ( g_CastleSiege.CheckCsGateAlive(iGateIndex) == TRUE )
	{
		...
		if ( !bRETVAL )
		{
			...
			LogAdd("[CastleSiege] CGReqCsGateOperate() ERROR - Operate Gate Failed [%s][%s], Guild:(%s)(%d)",
				gObj[iIndex].AccountID, gObj[iIndex].Name, gObj[iIndex].GuildName,
				gObj[iIndex].GuildStatus);
		...
		else
		{
			...
			LogAdd("[CastleSiege] CGReqCsGateOperate() OK - [%s][%s], Guild:(%s)(%d), DOOR:(%d)(X:%d,Y:%d)(STATUE:%d)",
				gObj[iIndex].AccountID, gObj[iIndex].Name, gObj[iIndex].GuildName,
	...
	else
	{
		...
		LogAdd("[CastleSiege] CGReqCsGateOperate() ERROR - Gate Doesn't Exist [%s][%s], Guild:(%s)(%d)",
			gObj[iIndex].AccountID, gObj[iIndex].Name, gObj[iIndex].GuildName,
			gObj[iIndex].GuildStatus);
	}
...
void GCAnsCsAccessCrownState(int iIndex, BYTE btCrownState)
{
	...
	if ( btCrownState == 0 )
	{
		...
		LogAdd("[CastleSiege] [Reg. Attempt] Accumulated Crown AccessTime : %d [%s](%s)(%s)",
	...
	if ( btCrownState == 1 )
	{
		LogAdd("[CastleSiege] [Reg. Success] Accumulated Crown AccessTime : %d [%s](%s)(%s)",
			gObj[iIndex].m_iAccumulatedCrownAccessTime, gObj[iIndex].GuildName, gObj[iIndex].AccountID,
			gObj[iIndex].Name);
		gObj[iIndex].m_iAccumulatedCrownAccessTime = 0;
	}
	...
	if ( btCrownState == 2 )
	{
		...
		LogAdd("[CastleSiege] [Reg. Fail] Accumulated Crown AccessTime : %d [%s](%s)(%s)",
			gObj[iIndex].m_iAccumulatedCrownAccessTime, gObj[iIndex].GuildName, gObj[iIndex].AccountID,
			gObj[iIndex].Name);
	}
...
void CGReqNpcDbList(PMSG_REQ_NPCDBLIST * lpMsg, int iIndex)
{
	...
	if ( g_CastleSiege.CheckGuildOwnCastle(gObj[iIndex].GuildName) == FALSE ||
		 ( gObj[iIndex].GuildStatus != 0x80 && gObj[iIndex].GuildStatus != 0x40 ) )
	{
		LogAdd( "[CastleSiege] CGReqNpcDbList() ERROR - Authority Fail [%s][%s], Guild:(%s)(%d)",
			gObj[iIndex].AccountID, gObj[iIndex].Name, gObj[iIndex].GuildName,
			gObj[iIndex].GuildStatus);
...
void CGReqWeaponDamageValue(PMSG_REQ_WEAPON_DAMAGE_VALUE * aRecv, int iIndex)
{
	...
	{
		MsgOutput(iIndex, "The damage information of the weapon can't be found.");
	}
}
...
void CGReqGuildMarkOfCastleOwner(PMSG_REQ_GUILDMARK_OF_CASTLEOWNER * aRecv, int iIndex)
{
	...
	LPOBJ lpObj = &gObj[iIndex];
	std::string  szGuildName =  g_CastleSiegeSync.GetCastleOwnerGuild();
		...
		if ( lpObj->Money > iEnterTaxMoney )
		{
			if ( lpObj->MapNumber == MAP_INDEX_CASTLESIEGE )
			{
				if ( gObjMoveGate(lpObj->m_Index, 95) == TRUE )
				{
					...
					LogAdd("[Castle HuntZone] [%s][%s] - Entrance TAX : %d - %d = %d",
...
void CGReqAlatrContract(PMSG_REQ_CRYWOLF_ALTAR_CONTRACT* lpMsg, int iIndex)
{
	...
	{
		MsgOutput(iIndex, "Connection requires a Muse level 260 or higher");	
	}
...
{
	LPOBJ lpObj=&gObj[aIndex];
	if (lpObj->Authority == 32 || lpObj->Authority==12 || lpObj->Authority==10 || lpObj->Authority==8 )
	{
		return true;
	}
	else
...
void WereWolfQuarrelCheck(int aIndex)
{
        ...
        if (gObj[aIndex].m_Quest[1] == 0xF6 || gObj[aIndex].m_Quest[1] == 0xDA || gObj[aIndex].m_Quest[1] == 0xEA) //Anti-Hack
        {
                ...
                {
                        LogAdd("[Anti-Hack][WereWolf Quarrel] [%s][%s] User out of range",gObj[aIndex].AccountID, gObj[aIndex].Name);
                }
        }
        else
        {
                LogAdd("[Anti-Hack][WereWolf Quarrel] [%s][%s] User try to Enter Barracks without Season3 Quest State",gObj[aIndex].AccountID, gObj[aIndex].Name);
        }
...
void GateKeeperCheck(int aIndex)
{
        ...
        if (gObj[aIndex].m_Quest[1] == 0xDA) //Anti-Hack
        {
                ...
                {
                        LogAdd("[Anti-Hack][GateKeeper] [%s][%s] User out of range",gObj[aIndex].AccountID, gObj[aIndex].Name);
                }
        }
        else
        {
                LogAdd("[Anti-Hack][GateKeeper] [%s][%s] User try to Enter Refugee without Season3 Quest State",gObj[aIndex].AccountID, gObj[aIndex].Name);
        }
}

void GCIllusionTempleSendReward(int aIndex) //Drop 1 Jewel of Chaos for any Player
{
        if ( !gObjIsConnected(aIndex))
                return;

        if ( !(gObj[aIndex].MapNumber >= 45 && gObj[aIndex].MapNumber <= 49) )
        {
                return;
        }

        int Item = ItemGetNumberMake( 12, 15); //Jewel of Chaos
        ItemSerialCreateSend(aIndex, gObj[aIndex].MapNumber, (BYTE)gObj[aIndex].X, (BYTE)gObj[aIndex].Y, Item,0,0,0,0,0,-1,0,0);            
}