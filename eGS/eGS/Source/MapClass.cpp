...
MapClass::MapClass()
{
	...
	this->m_NextWeatherTimer=Random(0,10000)+10000;
	this->init();
...
void MapClass::GetLevelPos( short level, short& ox, short& oy)
{
	...
	while ( loopcount-- != 0 )
	{
		...
		__try
		{
			x = (short)(this->gRegenRect[levelpos].left + Random(0,w-1));
			y = (short)(this->gRegenRect[levelpos].top  + Random(0,h-1));
		}
		__except(w=1, h=1,1)
		{
		}
	...
	LogAdd( "error : Can't find the location. %s %d" , __FILE__, __LINE__);
...
void MapClass::GetRandomLengthPos(short& x, short& y, int length)
{
	...
	while ( lc-- != 0 )
	{
		px = (x-length)+Random(0,maxlen-1);
		py = (y-length)+Random(0,maxlen-1);
...
void MapClass::GetMapPos(short Map, short& ox, short& oy)
{
	if ( Map != 8 )
	{
		if ( Map == 33 || Map == 34 || Map == 41 || Map == 42)
		{
...
void MapClass::LoadMapAttr(char * filename, int MapNumber)
{
	...
	{
		MsgBox( "Map attribute reading error" );
	}
...
void MapClass::WeatherVariationProcess()
{
	...
	if ( (GetTickCount() - this->m_WeatherTimer) > this->m_NextWeatherTimer)
	{
		this->m_WeatherTimer = GetTickCount();
		this->m_NextWeatherTimer = Random(0,10000) + 10000;
		this->m_WeatherVariation = Random(0,9);
		this->m_Weather = Random(0,2);
		...
		for (int n=OBJ_STARTUSERINDEX ; n<OBJMAX ; n++ )
		{
			...
			if ( lpObj->Connected > PLAYER_CONNECTED && lpObj->Live != 0 && lpObj->MapNumber == this->thisMapNumber && !lpObj->bot )
			{
				CGWeatherSend(n, weather);
			}
		}
...
int MapClass::MonsterItemDrop(int type, int level, double dur,  int x, int y, BYTE Option1, BYTE Option2, BYTE Option3, BYTE NOption, BYTE SOption,  int aIndex, DWORD number, BYTE ItemEffectEx)
{
	int count;
	int counttot=0;
	...
	LogAdd( "Error : Can't throw away the item. %s %d" , __FILE__, __LINE__ );
...
BOOL MapClass::ItemDrop(int type, int level, double dur,  int x, int y, BYTE Option1, BYTE Option2, BYTE Option3, BYTE NOption, BYTE SOption, DWORD number, int aIndex, int PetLevel, int PetExp, BYTE ItemEffectEx)
{
	int count;
	int counttot = 0;
	BYTE attr = this->GetAttr(x, y);
					...
					if ( ((level<0)?FALSE:(level>2)?FALSE:TRUE) != FALSE )
					{
						if ( g_BloodCastle.m_BridgeData[this->thisMapNumber-MAP_INDEX_BLOODCASTLE1].m_nBC_QUESTITEM_SERIAL != -1 )
						{
							if ( number == g_BloodCastle.m_BridgeData[this->thisMapNumber-MAP_INDEX_BLOODCASTLE1].m_nBC_QUESTITEM_SERIAL )
							{
								...
								{
									LogAdd("[Blood Castle] (%d) (Account:%s, Name:%s) Dropped Angel King's Weapon (%d)",
...
BOOL MapClass::ItemGive(int aIndex, int item_num, bool bFailNotSend)
{
	...
	{
		LogAdd( "error : Item number error %s %d", __FILE__, __LINE__);
		return FALSE;
	}

	if ( gObj[aIndex].MapNumber != this->thisMapNumber )
	{
		LogAdd( "Map Number error %s %d (%d) %s" , __FILE__, __LINE__, gObj[aIndex].MapNumber, gObj[aIndex].Name);
		return FALSE;
	}
	...
	if ( lootresult == 0 )
	{
		if ( bFailNotSend == false )
		{
			...
			wsprintf(szTemp, "This item does not belong to you, %s", gObj[aIndex].Name );
	...
	for (int n=0;n<MAX_MAPITEM;n++)
	{
		...
		if ( this->m_cItem[n].IsItem() != FALSE )
		{
			if ( this->m_cItem[n].m_State != 8 )
			{
				...
				{
					LogAdd("Item has disappeared(%s:%d/level:%d/skill:%d/serial:%d)"
						, this->m_cItem[n].GetName(), this->m_cItem[n].m_Type, this->m_cItem[n].m_Level, this->m_cItem[n].m_Special[0] , this->m_cItem[n].m_Number);
...
BOOL MapClass::AttrLoad(char* filename)
{
	...
	{
		MsgBox("Attribute file exceeds maximum size");
		fclose(fp);
		return false;
	}
...
{
	//карта соответствует параметрам 255 на 255
	if ( x < 0 || y < 0 || (x > this->m_width -1) || (y > this->m_height-1) || this->m_attrbuf==NULL )
	{
		return 4;
	}
	else
