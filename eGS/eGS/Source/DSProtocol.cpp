		...
		case 0xFF:	// Test Packet
			{
				...
				LogAdd( "error-L2DataServer RECV : %d (%d)", aLen, pMsg->count);
			}
			break;
	}
}
...
void TestDSSend()
{
	...
	wsDataServerCli.DataSend((char *)&pMsg, size);
	iCount++;
...
void DataServerLogin(int server)
{
	...
	strcpy_s( pInfo.ServerName, sizeof(pInfo.ServerName), szServerName );

	wsDataServerCli.DataSend((char*)&pInfo , pInfo.h.size  );	
}
...
void DataServerLoginResult(SDHP_RESULT * lpMsg)
{
	...
	{
		MsgBox("Authentication error from authentication server");	// Authentication error from authentication server
		return;
	}
...
void JGPGetCharList(unsigned char * lpRecv)
{
	...
	{
		LogAdd( "Requested character list doesn't match the user. (%s)%s", gObj[aIndex].AccountID, szId);
		CloseClient(aIndex);
	...
	ZeroMemory(gObj[aIndex].GMHeroes,sizeof(gObj[aIndex].GMHeroes));
	...
	if ( pCLCount.Count > 0 )
	{
		for ( int n=0;n<pCLCount.Count ;n++)
		{
			...
			//---
// 			std::string UnBanTime=SQL.GetFieldChar("MuOnline.dbo.Character","UnBanTime","WHERE Name = '%s'",pCList.Name);
// 
// 			if (UnBanTime!="" && !UnBanTime.empty() && UnBanTime!="0")
// 			{
// 				SYSTEMTIME time={0};
// 				char t_time[30]={0};
// 				sprintf_s(t_time,sizeof(t_time),"%s",UnBanTime.c_str());
// 
// 				char *tok;
// 				char *T_T=strtok_s(t_time," \0",&tok);
// 				time.wHour=atoi(T_T);
// 				T_T=strtok_s(NULL," \0",&tok);
// 				time.wDay=atoi(T_T);
// 				T_T=strtok_s(NULL," \0",&tok);
// 				time.wMonth=atoi(T_T);
// 				T_T=strtok_s(NULL," \0",&tok);
// 				time.wYear=atoi(T_T);
// 
// 				SYSTEMTIME CurrTime;
...
// 
// 				if (DataEcval(time,CurrTime) == 2)
// 				{
// 					pCList.CtlCode = 0;
// 					char sql[MAX_PATH]={0};
// 					wsprintf(sql,"UPDATE MuOnline.dbo.Character SET UnBanTime=('') WHERE Name=('%s')", pCList.Name);
// 					SQL.Execute(sql);
// 					wsprintf(sql,"UPDATE MuOnline.dbo.Character SET CtlCode=('0') WHERE Name=('%s')", pCList.Name);
// 					SQL.Execute(sql);
// 				}
// 			}
			//---

			if ( pCList.CtlCode==8 || pCList.CtlCode==10 || pCList.CtlCode==12 ||  pCList.CtlCode==32 )
				gObj[aIndex].GMHeroes[n]=true;
			else
				gObj[aIndex].GMHeroes[n]=false;
...
/*
			...
			pCList.CharSet[CS_CLASS] = gObjTakeDbClass(lpCL->Class);

			int changeup = CS_GET_CHANGEUP(lpCL->Class);
			pCList.CharSet[CS_CLASS] |= CS_SET_CHANGEUP(changeup);
			//pCList.CharSet[CS_CLASS] = gObjTakeClass(lpCL->Class,changeup);
			pCList.CharSet[9] = 0;
			*/
			...
			else	// Case DB 3
			{
                ...
                if( TempInventory[7] >= 36 && TempInventory[7] <= 40 ) //Third Wings
                {
                        pCList.CharSet[16] |= (TempInventory[7] - 35) * 4;
                }
...
void DataServerGetCharListRequest(short aIndex)
{
	...
	if ( AccountId == 0 )
	{
		LogAdd("error-L2 : User who is not authenticated requested character list?? %s %d", __FILE__, __LINE__);
		CloseClient(aIndex);
		return;
	}
	if ( strlen(AccountId) < 1 )
	{
		LogAdd("error-L2 : ID doesn't exist. %s %d", __FILE__, __LINE__);
		CloseClient(aIndex);
		return;
	}

	strcpy_s(TempId, sizeof(TempId), AccountId);
...
void JGCharacterCreateRequest(SDHP_CREATECHARRESULT* lpMsg)
{
	...
	{
		LogAdd( "Request to create character doesn't match the user. %s", szAccountID);
		CloseClient(aIndex);
		return;
	}
...
void JGCharDelRequest(SDHP_CHARDELETERESULT * lpMsg)
{
	...
	if ( ::gObjIsAccontConnect(aIndex, szAccountId) == FALSE )
	{
		LogAdd( "Request to delete character doesn't match the user. %s", szAccountId);
		CloseClient(aIndex);
...
void JGGetCharacterInfo( SDHP_DBCHAR_INFORESULT * lpMsg)
{
	...
	if ( gObjIsAccontConnect(aIndex, szAccountId) == FALSE )
	{
		LogAdd( "Request to receive character information doesn't match the user. %s", szAccountId);
		CloseClient(aIndex);
	...
	if ( lpMsg->result == false )
	{
		LogAdd( "error : %s Character data setting fail %s", szName, gObj[aIndex].AccountID);
		CloseClient(aIndex);
	...
	{
		LogAdd( "error-L1 : Load Block Character infomation; Name: %s , AccoundID: %s", szName, gObj[aIndex].AccountID);	
		CloseClient(aIndex);
	...
	for (int i=OBJ_STARTUSERINDEX;i<OBJMAX;i++)
	{
		if ( gObj[i].Connected == PLAYER_PLAYING )
		{
			if ( gObj[i].Type == OBJ_USER )
			{
				...
				{
					LogAdd("[Anti-HACK][JGGetCharacterInfo] Attempted Character-Copy by double logging [%s][%s]",
	...
	if ( gObjSetCharacter( lpMsg, aIndex) == FALSE )
	{
		LogAdd( "error : %s Character data setting fail", szName);
		CloseClient(aIndex);
	...
	{
		LogAdd( "Index number error %s %d", __FILE__, __LINE__);
		CloseClient(aIndex);
	...
	if ( lpObj->Money < 0 )
	{
		LogAdd( "¡Ú¡Ù ZEN MODIFY ¡Ù¡Ú [%s][%s] %d -> 0", szName, gObj[aIndex].AccountID, lpObj->Money);
		lpObj->Money = 0;
	}
	...
	if ( lpObj->m_bMapSvrMoveReq == false )
	{
		...
		if ( wGameServerCode != gGameServerCode )
		{
			...
			{
				LogAdd("[MapServerMng] Map Server Move Fail : CheckMoveMapSvr() == -1 [%s][%s] (%d)",
			...
			GJReqMapSvrMove(lpObj->m_Index, wGameServerCode, lpObj->MapNumber, (BYTE)lpObj->X, (BYTE)lpObj->Y);
			LogAdd("[MapServerMng] Request to Move Map Server : (%d) - [%s][%s] (%d)",
	...
	LogAdd("[FRUIT System] [%s][%s] (MinusPoint:%d/PlusPoint:%d) (MaxMinus:%d/MaxPlus:%d)",
	...
	{
		LogAdd("[FRUIT System] Character Stat Error [%s][%s] (MinusPoint:%d/PlusPoint:%d) (MaxMinus:%d/MaxPlus:%d)",
			lpObj->AccountID, lpObj->Name, MinusPoint, AddPoint, MaxMinusPoint, MaxAddPoint);
	}
...
// 
// 	gObj[aIndex].Resets=SQL.GetFieldInt("MuOnline.dbo.Character","Resets","WHERE Name = '%s'",gObj[aIndex].Name);
// 	UserLoadMysteryShopFromDB(&gObj[aIndex]);
// 	LoadUserBuff(szName,aIndex);
...
void GJSetCharacterInfo(LPOBJ lpObj, int aIndex, BOOL bMapServerMove)//ô-öèÿ äëÿ ñîõðàíåíèÿ ïàðàìåòðîâ ïåðñîíàæà (çàïèñè óðîâíÿ, îïûòà, èíâåíòàðÿ è ò.ä.)
{
	if ( lpObj->m_bMapSvrMoveQuit == true )
	{
		LogAdd("[MapServerMng] GJSetCharacterInfo() - Inventory Already Saved [%s][%s] (%d)",
			lpObj->AccountID, lpObj->Name, lpObj->m_Index);
	...
	if ( bMapServerMove == TRUE )
	{
		...
		LogAdd( "[MapServerMng][%s][%s] GJSetCharacterInfo() - MapServerMove Character Save Start (%d)",
			lpObj->AccountID, lpObj->Name, lpObj->m_Index);
	}
	...
	pCSave.Life = WORD(lpObj->Life*10.0);
	pCSave.MaxLife = WORD(lpObj->MaxLife*10.0);
	pCSave.Mana = WORD(lpObj->Mana*10.0);
	pCSave.MaxMana = WORD(lpObj->MaxMana*10.0);
	...
	wsDataServerCli.DataSend((char*)&pCSave, sizeof(SDHP_DBCHAR_INFOSAVE));
...
void DGGetWarehouseList(SDHP_GETWAREHOUSEDB_SAVE * lpMsg)
{
	...
	{
		LogAdd( "error-L1 : Request to receive Warehouse information doesn't match the user. (%s)(%d)", szId, aIndex);
		return;
	}
	...
	for ( int n=0;n<WAREHOUSE_SIZE;n++)
	{
		...
		if ( _type == ITEMGET(14,17) || _type == ITEMGET(14,18) )
		{
			...
			{
				itype = -1;	// #error maybe this is itype
			}
		}
			...
			if ( dbsize >= 16 )	// DB Version 3
			{
				...
				if ( item.m_ItemOptionEx != 0 )
				{
					...
					if ( g_kItemSystemFor380.Is380Item(&item)== FALSE )
					{
						...
						LogAdd("[380Item][%s][%s] Invalid 380 Item Option in Warehouse pos[%d]",
...
void GDGetWarehouseNoItem( SDHP_GETWAREHOUSEDB_RESULT * lpMsg)
{
	...
	if ( lpObj->m_IfState.type == 6 )
	{
		if ( lpObj->m_IfState.state == 0 )
		{
			...
			DataSend(lpObj->m_Index, (LPBYTE)&pResult, pResult.h.size);
			LogAdd( "warehouse item not found (%s)", lpObj->Name);
		}
	}
...
void GDSetWarehouseList(int aIndex)
{
	...
	{
		LogAdd("[%s][%s] WareHouse Save Fail : Not Open",
	...
	wsDataServerCli.DataSend((PCHAR)&pMsg, sizeof(pMsg));
...
void ItemSerialCreateSend(int aIndex, BYTE MapNumber, BYTE x, BYTE y, int type, BYTE level, BYTE dur, BYTE Op1, BYTE Op2, BYTE Op3, int LootIndex, BYTE NewOption, BYTE SetOption)
{
	...
	if ( OBJMAX_RANGE(aIndex) != FALSE  && gObj[aIndex].Type != OBJ_USER )
	{
		...
		LogAdd("Monster Item Drop Request [%s] [%d][%d][%d] : [%s][%d][%d][%d][%d] Ex:[%d,%d,%d,%d,%d,%d,%d] Set:[%d]",
...
/*	PBMSG_HEAD h;
	...
	if ( lpMsg->MapNumber == (BYTE)-1 || lpMsg->MapNumber == (BYTE)-2 )
	{
		...
		LogAdd("[%s][%s] CBMix Item Create Item:%s [%d][%d] [%d][%d][%d][%d] Ex:[%d,%d,%d,%d,%d,%d,%d] Set:[%d]",
	...
	if ( lpMsg->MapNumber == 235 )
	{
		if ( gObj[lpMsg->aIndex].Connected > PLAYER_CONNECTED )
		{
			...
			if ( iItemPos == (BYTE)-1 )
			{
				LogAdd("[Mu_2Anv_Event] Error : Failed To Gift Item Type:%d, TypeIndex:%d to [%s][%s]",
	...
	else if ( lpMsg->MapNumber == 236 )
	{
		if ( gObj[lpMsg->aIndex].Connected > PLAYER_CONNECTED )
		{
			...
			if ( btItemPos == (BYTE)-1 )
			{
				LogAdd("[CashShop] Error : Failed To Insert Item Type:%d, TypeIndex:%d to [%s][%s]",
	...
	else
	{
		...
		LogAdd("Monster Item Drop [%d][%d][%d] : serial:%d [%s][%d][%d][%d][%d] Ex:[%d,%d,%d,%d,%d,%d,%d] Set:[%d]",
...
void ItemMovePathSave(char* ActID, char* Name, BYTE level, BYTE mapnumber, BYTE x, BYTE y, char* Item, BYTE op1, BYTE op2, BYTE op3, DWORD serial)
{
	...
	strcpy_s(pMsg.ItemName, sizeof(pMsg.ItemName), Item);
	memcpy(pMsg.Name, Name, sizeof(pMsg.Name));
	strcpy_s(pMsg.ServerName, sizeof(pMsg.ServerName), szServerName);
...
void DGMoveOtherServer(SDHP_CHARACTER_TRANSFER_RESULT * lpMsg)
{
	...
	LogAdd("[CharTrasfer] Success [%s][%s] (%d)",
		lpObj->AccountID, lpObj->Name, lpMsg->Result);
...
void gObjRequestPetItemInfo(int aIndex, int inventype)
{
	...
	if ( founditemcount )
	{
		...
		wsDataServerCli.DataSend((PCHAR)pbuffer, lofs+sizeof(pMsg));
	}
}
...
void DGRecvPetItemInfo(LPBYTE lpData)
{
	...
	{
		LogAdd( "Request to receive petitem infomation doesn't match the user %s", szAccountId);
		CloseClient(aIndex);
			...
			if ( lpObj->pInventory[n].IsItem() )
			{
				if ( lpObj->pInventory[n].m_Type == ITEMGET(13,4) || lpObj->pInventory[n].m_Type == ITEMGET(13,5) )
				{
					if ( lpObj->pInventory[n].m_IsLoadPetItemInfo != FALSE )
					{
						...
						LogAdd("[%s][%s][PI][%d,%s]serial:[%d] Level:%d Exp:%d",
			...
			if ( lpObj->pWarehouse[n].IsItem() )
			{
				if ( lpObj->pWarehouse[n].m_Type == ITEMGET(13,4) || lpObj->pWarehouse[n].m_Type == ITEMGET(13,5) )
				{
					if ( lpObj->pWarehouse[n].m_IsLoadPetItemInfo != FALSE )
					{
						...
						LogAdd("[%s][%s][PW][%d,%s]serial:[%d] Level:%d Exp:%d",
...
void GS_GDReqMapSvrMsgMultiCast( int iMapSvrGroup,char * lpszMsgText)
{
	...
	pMsg.wMapSvrNum = iMapSvrGroup;
	strcpy_s(pMsg.szMsgText, sizeof(pMsg.szMsgText), lpszMsgText);
...
void GS_DGAnsCastleTotalInfo(LPBYTE lpRecv)
{
	...
	if ( lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup() )
		LogAdd( "[CastleSiege] PACKET-ERROR [0x80][0x00] GS_DGAnsCastleTotalInfo() - lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()");
...
void GS_DGAnsOwnerGuildMaster(LPBYTE lpRecv)
{
	...
	if ( lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup() )
	{
		LogAdd( "[CastleSiege] PACKET-ERROR [0x80][0x00] GS_DGAnsCastleTotalInfo() - lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()");	// #error on Msg
		return;
	}
...
void GS_DGAnsCastleNpcBuy(unsigned char * lpRecv )
{
	...
	if ( lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup() )
	{
		LogAdd( "[CastleSiege] PACKET-ERROR [0x80][0x03] GS_DGAnsCastleNpcBuy() - lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()");
		return;
	}
	...
	if ( lpMsg->iResult == 1 )
	{
		...
		if ( bRET == TRUE )
		{
			...
			LogAdd("[CastleSiege] GS_DGAnsCastleNpcBuy() - CCastleSiege::AddDbNPC() OK - Npc:(CLS:%d, IDX:%d)",
				lpMsg->iNpcNumber, lpMsg->iNpcIndex);
		}
		else
		{
			LogAdd("[CastleSiege] GS_DGAnsCastleNpcBuy() - CCastleSiege::AddDbNPC() FAILED - Npc:(CLS:%d, IDX:%d)",
				lpMsg->iNpcNumber, lpMsg->iNpcIndex);
		}
	}
	else
	{
		LogAdd("[CastleSiege] GS_DGAnsCastleNpcBuy() FAILED - Result:(%d), Npc:(CLS:%d, IDX:%d)",
...
void GS_DGAnsCastleNpcRepair(LPBYTE lpRecv)
{
	...
	if ( lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup() )
	{
		LogAdd( "[CastleSiege] PACKET-ERROR [0x80][0x04] GS_DGAnsCastleNpcRepair() - lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()");
		return;
	}
	...
	if ( lpMsg->iResult == 1 )
	{
		...
		if ( bRET == TRUE )
		{
			...
			LogAdd("[CastleSiege] GS_DGAnsCastleNpcRepair() - CCastleSiege::RepairDbNPC() OK - Npc:(CLS:%d, IDX:%d)",
				lpMsg->iNpcNumber, lpMsg->iNpcIndex);
		}
		else
		{
			LogAdd("[CastleSiege] GS_DGAnsCastleNpcRepair() - CCastleSiege::RepairDbNPC() FAILED - Npc:(CLS:%d, IDX:%d)",
				lpMsg->iNpcNumber, lpMsg->iNpcIndex);
		}
	}
	else
	{
		LogAdd("[CastleSiege] GS_DGAnsCastleNpcRepair() FAILED - Result:(%d), Npc:(CLS:%d, IDX:%d)",
			lpMsg->iResult, lpMsg->iNpcNumber, lpMsg->iNpcIndex);
	}
...
void GS_DGAnsCastleNpcUpgrade(LPBYTE lpRecv)
{
	...
	if ( lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup() )
	{
		LogAdd( "[CastleSiege] PACKET-ERROR [0x80][0x05] GS_DGAnsCastleNpcUpgrade() - lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()");
		return;
	}

	if ( lpMsg->iResult == 0 )
	{
		LogAdd( "[CastleSiege] ERROR - Castle NPC Upgrade Fail() (CLS:%d, IDX:%d, UPTYPE:%d, UPVAL:%d)",
	...
	else
	{
		...
		LogAdd("[CastleSiege] [0x80][0x06] GS_DGAnsTaxInfo() - Npc Upgrade OK (CLS:%d, IDX:%d, UPTYPE:%d, UPVAL:%d)",
			lpMsg->iNpcNumber, lpMsg->iNpcIndex, lpMsg->iNpcUpType, lpMsg->iNpcUpValue);
	}
...
void GS_DGAnsTaxInfo(LPBYTE lpRecv)
{
	...
	if ( lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup() )
	{
		LogAdd( "[CastleSiege] PACKET-ERROR [0x80][0x06] GS_DGAnsTaxInfo() - lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()");
		return;
	}
...
void GS_DGAnsTaxRateChange(LPBYTE lpRecv)
{
	...
	if ( lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup() )
	{
		LogAdd( "[CastleSiege] PACKET-ERROR [0x80][0x07] GS_DGAnsTaxRateChange() - lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()");
		return;
	}
...
void GS_DGAnsCastleMoneyChange(LPBYTE lpRecv)
{
	...
	if ( lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup() )
	{
		LogAdd( "[CastleSiege] PACKET-ERROR [0x80][0x08] GS_DGAnsCastleMoneyChange() - lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()");
		return;
	}
	...
	if ( lpMsg->iResult == 1 )
	{
		if ( gObjIsConnected(lpMsg->iIndex) )
		{
			if ( lpMsg->iMoneyChanged < 0 )
			{
				...
				LogAdd("[CastleSiege] [0x80][0x08] GS_DGAnsCastleMoneyChange() - Withdraw Request OK [%s][%s] (ReqMoney:%d, TotMoney:%I64d)",
...
void GS_DGAnsSiegeDateChange(LPBYTE lpRecv)
{
	...
	if ( lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup() )
	{
		LogAdd( "[CastleSiege] PACKET-ERROR [0x80][0x09] GS_DGAnsSiegeDateChange() - lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()");
		return;
	}
...
void GS_DGAnsGuildMarkRegInfo(LPBYTE lpRecv)
{
	...
	if ( lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup() )
	{
		LogAdd( "[CastleSiege] PACKET-ERROR [0x80][0x0A] GS_DGAnsGuildMarkRegInfo() - lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()");
		return;
	}
...
void GS_DGAnsSiegeEndedChange(LPBYTE lpRecv)
{
	...
	if ( lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup() )
	{
		LogAdd( "[CastleSiege] PACKET-ERROR [0x80][0x0B] GS_DGAnsSiegeEndedChange() - lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()");
		return;
	}
...
void GS_DGAnsCastleOwnerChange(LPBYTE lpRecv)
{
	...
	if ( lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup() )
	{
		LogAdd( "[CastleSiege] PACKET-ERROR [0x80][0x0C] GS_DGAnsCastleOwnerChange() - lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()");
		return;
	}
...
void GS_DGAnsRegAttackGuild(LPBYTE lpRecv)
{
	...
	if ( lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup() )
	{
		LogAdd( "[CastleSiege] PACKET-ERROR [0x80][0x0D] GS_DGAnsRegAttackGuild() - lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()");
		return;
	}
...
void GS_DGAnsRestartCastleState(LPBYTE lpRecv)
{
	...
	if ( lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup() )
	{
		LogAdd( "[CastleSiege] PACKET-ERROR [0x80][0x0E] GS_DGAnsRestartCastleState() - lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()");
		return;
	}
...
void GS_DGAnsMapSvrMsgMultiCast(LPBYTE lpRecv)
{
	...
	if ( lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup() )
	{
		LogAdd( "[CastleSiege] PACKET-ERROR [0x80][0x0F] GS_DGAnsMapSvrMsgMultiCast() - lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()");
		return;
	}
	char szNotice[144] = {0};
...
void GS_DGAnsRegGuildMark(LPBYTE lpRecv)
{
	...
	if ( lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup() )
	{
		LogAdd( "[CastleSiege] PACKET-ERROR [0x80][0x10] GS_DGAnsRegGuildMark() - lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()");
		return;
	}
...
void GS_DGAnsGuildMarkReset(LPBYTE lpRecv)
{
	...
	if ( lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup() )
	{
		LogAdd( "[CastleSiege] PACKET-ERROR [0x80][0x11] GS_DGAnsGuildMarkReset() - lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()");
		return;
	}
...
void GS_DGAnsGuildSetGiveUp(LPBYTE lpRecv)
{
	...
	if ( lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup() )
	{
		LogAdd( "[CastleSiege] PACKET-ERROR [0x80][0x12] GS_DGAnsGuildSetGiveUp() - lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()");
		return;
	}
...
void GS_DGAnsNpcRemove(LPBYTE lpRecv)
{
	...
	if ( lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup() )
	{
		LogAdd( "[CastleSiege] PACKET-ERROR [0x80][0x16] GS_DGAnsNpcRemove() - lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()");
		return;
	}
...
void GS_DGAnsCastleTributeMoney(LPBYTE lpRecv)
{
	...
	if ( lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup() )
	{
		LogAdd( "[CastleSiege] PACKET-ERROR [0x80][0x18] GS_DGAnsCastleTributeMoney() - lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()");
		return;
	}
	
	LogAdd("[CastleSiege] [0x80][0x18] GS_DGAnsCastleTributeMoney() - Money Tribute OK (%d)",
...
void GS_DGAnsResetCastleTaxInfo(LPBYTE lpRecv)
{
	...
	if ( lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup() )
	{
		LogAdd( "[CastleSiege] PACKET-ERROR [0x80][0x19] GS_DGAnsResetCastleTaxInfo() - lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()");
		return;
	}
...
void GS_DGAnsResetSiegeGuildInfo(LPBYTE lpRecv)
{
	...
	if ( lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup() )
	{
		LogAdd( "[CastleSiege] PACKET-ERROR [0x80][0x1A] GS_DGAnsResetSiegeGuildInfo() - lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()");
		return;
	}
...
void GS_DGAnsResetRegSiegeInfo(LPBYTE lpRecv)
{
	...
	if ( lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup() )
	{
		LogAdd( "[CastleSiege] PACKET-ERROR [0x80][0x1B] GS_DGAnsResetRegSiegeInfo() - lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()");
		return;
	}
...
void GS_DGAnsCastleInitData(LPBYTE lpRecv)
{
	...
	if ( lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup() )
	{
		LogAdd( "[CastleSiege] PACKET-ERROR [0x81] GS_DGAnsCastleInitData() - lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()");
		return;
	}

	if ( g_CastleSiege.GetDataLoadState() != CASTLESIEGE_DATALOAD_2 )
	{
		LogAdd( "[CastleSiege] CASTLE SIEGE DATA SETTING FAILED [0x81] - m_iCastleDataLoadState != CASTLESIEGE_DATALOAD_2 (%d)",
	...
	if ( lpMsg->iResult == 0 )
	{
		LogAdd( "[CastleSiege] CASTLE SIEGE DATA SETTING FAILED [0x81] - lpMsg->iResult == 0");
	...
	if ( bRET_VAL == FALSE )
	{
		LogAdd( "[CastleSiege] CASTLE SIEGE DATA SETTING FAILED [0x81] - g_CastleSiege.SetCastleInitData() == FALSE");
		return;
	}
	...
	if ( bRET_VAL == FALSE )
	{
		LogAdd( "[CastleSiege] CASTLE SIEGE DATA SETTING FAILED [0x81] - g_CastleSiege.SetCastleNpcData() == FALSE");
		return;
	}
...
void GS_DGAnsCastleNpcInfo(LPBYTE lpRecv)
{
	...
	if ( lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup() )
	{
		LogAdd( "[CastleSiege] PACKET-ERROR [0x82] GS_DGAnsCastleNpcInfo() - lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()");
		return;
	}
...
void GS_DGAnsAllGuildMarkRegInfo(LPBYTE lpRecv)
{
	...
	if ( lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup() )
	{
		LogAdd( "[CastleSiege] PACKET-ERROR [0x83] GS_DGAnsAllGuildMarkRegInfo() - lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()");
		return;
	}
...
void GS_DGAnsFirstCreateNPC(LPBYTE lpRecv)
{
	...
	if ( lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup() )
	{
		LogAdd( "[CastleSiege] PACKET-ERROR [0x84] GS_DGAnsFirstCreateNPC() - lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()");
		return;
	}
...
void GS_DGAnsCalcREgGuildList(LPBYTE lpRecv)
{
	...
	if ( lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup() )
	{
		LogAdd( "[CastleSiege] PACKET-ERROR [0x85] GS_DGAnsCalcREgGuildList() - lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()");
		return;
	}
	...
	{
		LogAdd( "[CastleSiege] PACKET-ERROR [0x85] GS_DGAnsCalcREgGuildList() - lpMsg->iResult != 1 (%d)", lpMsg->iResult);
	}
...
void GS_DGAnsCsGulidUnionInfo(LPBYTE lpRecv)
{
	...
	if ( lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup() )
	{
		LogAdd( "[CastleSiege] PACKET-ERROR [0x86] GS_DGAnsCsGulidUnionInfo() - lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()");
		return;
	}
...
void GS_DGAnsCsSaveTotalGuildInfo(LPBYTE lpRecv)
{
	...
	if ( lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup() )
	{
		LogAdd( "[CastleSiege] PACKET-ERROR [0x87] GS_DGAnsCsSaveTotalGuildInfo() - lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()");
		return;
	}
...
void GS_DGAnsCsLoadTotalGuildInfo(LPBYTE lpRecv)
{
	...
	if ( lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup() )
	{
		LogAdd( "[CastleSiege] PACKET-ERROR [0x88] GS_DGAnsCsLoadTotalGuildInfo() - lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()");
		return;
	}
...
void GS_DGAnsCastleNpcUpdate(LPBYTE lpRecv)
{
	...
	if ( lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup() )
	{
		LogAdd( "[CastleSiege] PACKET-ERROR [0x89] GS_DGAnsCastleNpcUpdate() - lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()");
		return;
	}

	LogAdd("[CastleSiege] [0x89] GS_DGAnsCastleNpcUpdate() - Npc Data Update Result : '%s'", (lpMsg->iResult) ? "SUCCEED" : "FAIL");
...
void DGAnsCrywolfSync(LPBYTE lpRecv)
{
	...
	{
		LogAdd( "[ CRYWOLF ] SYNC Occupation: %d, State: %d", lpMsg->iOccupationState, lpMsg->iCrywolfState);
...
void DGAnsCrywolfInfoLoad(LPBYTE lpRecv)
{
	...
	{
		LogAdd("[ Crywolf ] Incorrect DB OccupationState!!!");
	}
